...
typedef void (*att_handler_t)(struct net_buf *buf);
static att_handler_t att_handlers[LAST_SUPPORTED_ATT_OPCODE];
...
static void send_write_rsp(void)
{
	...
	UNSET_FLAG(flag_req_in_progress);

	net_buf_add_u8(buf, BT_ATT_OP_WRITE_RSP);
	send_l2cap_packet(buf, BT_L2CAP_CID_ATT);
}

static void handle_att_write_0(struct net_buf *buf)
{
	uint16_t handle = net_buf_pull_le16(buf);
...
static void handle_att_write_1(struct net_buf *buf)
{
	uint16_t handle = net_buf_pull_le16(buf);
	...
	/* For this test procedure, the response is sent from main */
}
...
static void handle_att(struct net_buf *buf)
{
	...
	if (ARRAY_SIZE(att_handlers) > op && att_handlers[op]) {
		LOG_DBG("executing custom ATT handler for op %x", op);
		att_handlers[op](buf);
		return;
	}
...
{
	att_handlers[BT_ATT_OP_WRITE_REQ] = handle_att_write_0;

	init_tinyhost();

	/* wait until the good peer has connected */
	k_msleep(1000);
	...
	while (k_uptime_get_32() - start_time < timeout_ms) {
		gatt_write(BT_ATT_OP_WRITE_REQ);
	}
...
{
	att_handlers[BT_ATT_OP_WRITE_REQ] = handle_att_write_1;
