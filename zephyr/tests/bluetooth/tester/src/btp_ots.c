...
/* Modifications:
2025-09-15 by Airoha - Adding the parameter `mode` to the `obj_write` callback function indicates whether to truncate the content of object.
*/
			       ...
			       void *rsp, uint16_t *rsp_len)
{
	...
	BT_OTS_OBJ_SET_PROP_TRUNCATE(supported_props);
...
static ssize_t ots_obj_write(struct bt_ots *ots, struct bt_conn *conn,
			     ...
			     off_t offset, size_t rem, enum bt_ots_oacp_write_op_mode mode)
{
	uint32_t obj_index = OTS_OBJ_ID_TO_OBJ_IDX(id);
...
static int ots_init(void)
{
    ...
    BT_OTS_OACP_SET_FEAT_TRUNCATE(ots_init.features.oacp);
