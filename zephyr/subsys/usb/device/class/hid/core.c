			  ...
			  const uint8_t *param)
{
	...
	if (common == NULL) {
		LOG_WRN("hid_status_cb - Device data not found for cfg %p", cfg);
		return;
	}
				...
				int32_t *len, uint8_t **data)
{
	...
	if (common == NULL) {
		LOG_WRN("hid_class_handle_req - Device data not found for interface %u",
			setup->wIndex);
		return -ENODEV;
	}
				 ...
				 int32_t *len, uint8_t **data)
{
	...
	if (usb_reqtype_is_to_host(setup) &&
		...
		if (common == NULL) {
			LOG_WRN("hid_custom_handle_req - Device data not found for interface %u",
				iface_num);
			return -EINVAL;
		}
...
static void hid_int_in(uint8_t ep, enum usb_dc_ep_cb_status_code ep_status)
{
	...
	if (common == NULL) {
		LOG_WRN("hid_int_in - Device data not found for endpoint %u", ep);
		return;
	}
...
static void hid_int_out(uint8_t ep, enum usb_dc_ep_cb_status_code ep_status)
{
	...
	common = usb_get_dev_data_by_ep(&usb_hid_devlist, ep);
	if (common == NULL) {
		LOG_WRN("hid_int_out - Device data not found for endpoint %u", ep);
		return;
	}
...
/* Secondary HID Descriptors */
#ifdef CONFIG_ENABLE_HID_INT_OUT_EP
#define DEFINE_HID_SEC_DESCR(x, _) \
    USBD_CLASS_SEC_DESCR_DEFINE(primary, x) \
    struct usb_hid_config hid_sec_cfg_##x = { \
        .if0 = INITIALIZER_IF, \
        .if0_hid = INITIALIZER_IF_HID, \
...
#define DEFINE_HID_SEC_CFG_DATA(x, _) \
    USBD_DEFINE_SEC_CFG_DATA(hid_sec_config_##x) = { \
        .usb_device_description = NULL, \
        .interface_config = hid_interface_config, \
        .interface_descriptor = &hid_sec_cfg_##x.if0, \
        .cb_usb_status = hid_status_cb, \
        .interface = { \
            .class_handler = hid_class_handle_req, \
            .custom_handler = hid_custom_handle_req, \
...
/* Define Secondary HID Device Data and Device */
#define DEFINE_HID_SEC_DEV_DATA(x, _) \
    struct hid_device_info usb_hid_sec_dev_data_##x

#define DEFINE_HID_SEC_DEVICE(x, _) \
    DEVICE_DEFINE(usb_hid_sec_device_##x, \
                "HID_SEC_" #x, \
                &usb_hid_device_init, \
                NULL, \
                &usb_hid_sec_dev_data_##x, \
                &hid_sec_config_##x, \
                POST_KERNEL, \
                CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, \
                &hid_api)

/* Because a gamepad is already in use in Xinput mode, one HID device (gamepad) is removed. */
#if defined(CONFIG_AIR_USB_XINPUT_ENABLE)
LISTIFY(CONFIG_USB_XINPUT_HID_DEVICE_COUNT, DEFINE_HID_SEC_DESCR, (;), _);
LISTIFY(CONFIG_USB_XINPUT_HID_DEVICE_COUNT, DEFINE_HID_SEC_CFG_DATA, (;), _);
LISTIFY(CONFIG_USB_XINPUT_HID_DEVICE_COUNT, DEFINE_HID_SEC_DEV_DATA, (;), _);
LISTIFY(CONFIG_USB_XINPUT_HID_DEVICE_COUNT, DEFINE_HID_SEC_DEVICE, (;), _);
#else
LISTIFY(CONFIG_USB_HID_DEVICE_COUNT, DEFINE_HID_SEC_DESCR, (;), _);
LISTIFY(CONFIG_USB_HID_DEVICE_COUNT, DEFINE_HID_SEC_CFG_DATA, (;), _);
LISTIFY(CONFIG_USB_HID_DEVICE_COUNT, DEFINE_HID_SEC_DEV_DATA, (;), _);
LISTIFY(CONFIG_USB_HID_DEVICE_COUNT, DEFINE_HID_SEC_DEVICE, (;), _);
#endif
