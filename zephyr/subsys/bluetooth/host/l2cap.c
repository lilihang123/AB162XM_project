			     ...
			     bt_l2cap_chan_state_t state)
{
	...
	LOG_INF("[L2CAP][STATE] chan_set_state, psm:0x%04X, state:%d", BT_L2CAP_LE_CHAN(chan)->psm, state);
}
...
{
	if (le_chan->chan.conn == NULL) {
		LOG_ERR("[L2CAP] debug log, raise data check conn is NULL, return");
		return;
	}
...
{
	LOG_INF("[L2CAP][INIT] server_register, accept_cb:0x%08X, psm:0x%04X", server->accept, server->psm);
	if (!server->accept) {
		return -EINVAL;
	}
...
static int l2cap_recv(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
	...
	LOG_INF("[L2CAP][RX] recv, signaling_code:0x%02X", hdr->code);
	switch (hdr->code) {
				...
				struct bt_l2cap_chan **chan, uint16_t psm)
{
	...
	LOG_INF("[L2CAP][CONN] ecred_chan_connect, conn:0x%08X, chan:0x%08X, psm:0x%04X", conn, chan, psm);
	if (!conn || !chan) {
		return -EINVAL;
	}
...
int bt_l2cap_ecred_chan_reconfigure(struct bt_l2cap_chan **chans, uint16_t mtu)
{
	...
	LOG_INF("[L2CAP][TX] chan_reconfigure, chans:0x%08X, mtu:%d", chans, mtu);
	if (!chans) {
		return -EINVAL;
	}
			  ...
			  uint16_t psm)
{
	...
	LOG_DBG("conn %p chan %p psm 0x%04x", conn, chan, psm);
	LOG_INF("[L2CAP][CONN] chan_connect, conn:0x%08X, chan:0x%08X, psm:0x%4X", conn, chan, psm);
...
int bt_l2cap_chan_disconnect(struct bt_l2cap_chan *chan)
{
	...
	struct bt_l2cap_le_chan *le_chan;
	LOG_INF("[L2CAP][CONN] chan_disconnect, conn:0x%08X, chan:0x%08X", conn, chan);
	if (!conn) {
		return -ENOTCONN;
	}
...
{
	if (!buf || !chan) {
		LOG_INF("[L2CAP][TX] chan_send, invalid parameters, chan:0x%08X, buf:0x%08X", chan, buf);
		return -EINVAL;
	}
