...
/* Modifications:
2026-03-05 by Airoha - Add modification to support Airoha hardware P256 algorithm.
*/
...
static uint8_t generate_keys(void)
{
    hal_p256_make_ecdh_key(ecc.private_key_be, ecc.public_key_be);
	if (IS_ENABLED(CONFIG_BT_LOG_SNIFFER_INFO)) {
		LOG_INF("SC private key 0x%s", bt_hex(ecc.private_key_be, BT_PRIV_KEY_LEN));
	}
	return 0;
...
static void emulate_le_p256_public_key_cmd(void)
{
    ...
    #else
        memcpy(evt->key, ecc.public_key_be, BT_PUB_KEY_COORD_LEN);
		memcpy(&evt->key[BT_PUB_KEY_COORD_LEN],
             &ecc.public_key_be[BT_PUB_KEY_COORD_LEN], BT_PUB_KEY_COORD_LEN);
    #endif
	}
...
static void emulate_le_generate_dhkey(void)
{
	...
	hal_p256_gen_dhkey(
        use_debug ? debug_private_key_be : ecc.private_key_be,
        ecc.public_key_be, ecc.dhkey_be);
	ret = 0;
#endif /* CONFIG_BT_USE_PSA_API */
...
exit:
    ...
    #else
        memcpy(evt->dhkey, ecc.dhkey_be, sizeof(ecc.dhkey_be));
    #endif
	}
...
#else
    memcpy(ecc.public_key_be, key, BT_PUB_KEY_COORD_LEN);
    memcpy(&ecc.public_key_be[BT_PUB_KEY_COORD_LEN], &key[BT_PUB_KEY_COORD_LEN],
			BT_PUB_KEY_COORD_LEN);
#endif
