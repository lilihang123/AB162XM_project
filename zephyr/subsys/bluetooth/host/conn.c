...
/* Modifications:
2026-01-21 by Airoha - Add conn_ref count judgement because get_conn_ready() maybe don't call bt_conn_ref().
2026-01-29 by Airoha - Add spinlock protection when use sys_slist related api.
*/
...
struct k_spinlock conn_list_spin;
		    ...
		    size_t len, void *cb, void *ud)
{
    ...
    lock = k_spin_lock(&conn_list_spin);
...
exit:
    if (atomic_get(&conn->ref) == 0) {
        /* BTD-5746:Not need to call bt_conn_unref() due to 'get_conn_ready' maybe don't gave us */
        LOG_ERR("Zephyr bug");
    } else {
...
found:
	if (conn) {
		if (type & conn->type) {
            LOG_INF("[GAP] [CONN] lookup_handle handle is 0x%02x", conn->handle);
			return conn;
		}
...
struct bt_conn *bt_conn_ref(struct bt_conn *conn)
{
    ...
    LOG_INF("[GAP] [CONN] Conn ref handle 0x%02x ref %ld -> %ld", conn->handle, old, old + 1);
...
void bt_conn_unref(struct bt_conn *conn)
{
    ...
    LOG_INF("[GAP] [CONN] Conn unref handle 0x%02x ref %ld -> %ld", conn_handle, old, (old - 1));
...
static void notify_recycled_conn_slot(void)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&conn_cbs, callback, _node) {
		if (NULL != callback && callback->recycled) {
			callback->recycled();
		}
	}

	STRUCT_SECTION_FOREACH(bt_conn_cb, cb) {
		if (NULL != cb && cb->recycled) {
			cb->recycled();
		}
	}
#endif
...
int bt_conn_disconnect(struct bt_conn *conn, uint8_t reason)
{
	 ...
	 * starts advertising.
	 */
    LOG_INF("[GAP] [CONN] le_dis_conn handle is 0x%02x, state is %d", conn->handle, conn->state);
...
struct bt_conn *bt_conn_lookup_addr_le(uint8_t id, const bt_addr_le_t *peer)
{
	...
	for (i = 0; i < ARRAY_SIZE(acl_conns); i++) {
        ...
        LOG_INF("[GAP] [CONN] lookup_addr_le handle is 0x%02x", conn->handle);
		return conn;
	}
					...
					const bt_conn_state_t state)
{
	...
	for (i = 0; i < ARRAY_SIZE(acl_conns); i++) {
        ...
        LOG_INF("[GAP] [CONN] lookup_state_le handle is 0x%02x", conn->handle);
		return conn;
	}
		      ...
		      const struct bt_le_conn_param *conn_param, struct bt_conn **ret_conn)
{
	...
	int err;
	if (NULL != peer) {
        LOG_INF("[GAP] [CONN] le_create peer address is 0x%02x:%02x:%02x:%02x:%02x:%02x",peer->a.val[5],peer->a.val[4],peer->a.val[3],peer->a.val[2],peer->a.val[1],peer->a.val[0]);
	}
...
struct bt_conn *bt_conn_lookup_index(uint8_t index)
{
    ...
    LOG_INF("[GAP] [CONN] lookup_index index is %d", index);
