...
2026-03-05 by Airoha - Modify PDU header to fit Airoha chip.
*/
...
// #define PDU_DC_PAYLOAD_SIZE_MIN       251

#define PDU_DC_PAYLOAD_TIME_MIN       328
// #define PDU_DC_PAYLOAD_TIME_MIN       2120
...
#define PDU_ADV_DATA_TYPE_PAWR_PROPRIETARY_INFO  0xFE
/*
...
struct pdu_adv_sync_pawr_pro_info_ind {
	uint8_t  slot_offset;//unit:1.25ms
	uint16_t sse_interval;//unit:1.25ms
	uint8_t  total_sse_count;//total sse count: eg:50
	uint8_t  timing[1];//CR
} __packed;
#endif
...
enum pdu_data_llctrl_type {
    ...
    PDU_DATA_LLCTRL_TYPE_PERIODIC_SYNC_WR_IND = 0x2A,
	PDU_DATA_LLCTRL_TYPE_PERIODIC_SYNC_POLL = 0xF0, //AUX_SYNC_POLL
	PDU_DATA_LLCTRL_TYPE_PERIODIC_SYNC_NOTIFCATION = 0xF1, //AUX_SYNC_NOTIFICATION
...
struct pdu_data_llctrl_periodic_sync_wr_ind {
...
#if defined(CONFIG_BT_CTLR_SYNC_PERIODIC_RSP_PRO)
#if 0
struct pdu_aux_sync_data_train_ind {
#ifdef CONFIG_LITTLE_ENDIAN
	uint8_t ll_id:2;
	uint8_t nesn:1;
	uint8_t sn:1;
...
#endif
	uint8_t len;
	uint8_t ext_seq_num:5;
	uint8_t ext_rfu:3;
	
	struct pdu_data_vnd_octet3 octet3;//What's this?
	uint8_t payload[0];
}__packed;


struct pdu_aux_sync_ctrl {
#ifdef CONFIG_LITTLE_ENDIAN
	uint8_t ll_id:2;
	uint8_t nesn:1;
	uint8_t sn:1;
	...
	uint8_t opcode;
	union {
		struct pdu_aux_sync_ctrl_poll aux_sync_poll;
		struct pdu_aux_sync_ctrl_notification aux_sync_notification;
	} __packed;
}__packed;
#endif

struct pdu_aux_data_train_ext_hdr {
	uint8_t ext_seq_num:5;
	uint8_t rfu:3;
} __packed;

struct pdu_aux_sync_ctrl_poll {
	/*no members*/
} __packed;

struct pdu_aux_sync_ctrl_notification {
	uint8_t id;
	uint32_t rx_map;
} __packed;
...
struct pdu_data_llctrl {
	...
	union {
		...
		struct pdu_data_llctrl_periodic_sync_ind periodic_sync_ind;
        struct pdu_data_llctrl_periodic_sync_wr_ind periodic_sync_wr_ind;
	#if defined(CONFIG_BT_CTLR_SYNC_PERIODIC_RSP_PRO)
		struct pdu_aux_sync_ctrl_poll aux_sync_poll;
		struct pdu_aux_sync_ctrl_notification aux_sync_notification;
	#endif 
