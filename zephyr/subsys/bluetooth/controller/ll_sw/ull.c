...
2026-03-05 by Airoha - Add ticker node for controller debug.
2026-03-05 by Airoha - Set critical sectioin to RAM code.
*/
...
#define BT_SCAN_SYNC_WR_TICKER_NODES 2
#define BT_SCAN_SYNC_WR_RSP_TICKER_NODES 1
#else
#define BT_SCAN_SYNC_WR_TICKER_NODES 0
#define BT_SCAN_SYNC_WR_RSP_TICKER_NODES 0
#endif
#if defined(CONFIG_BT_CTLR_SYNC_PERIODIC_RSP_PRO)
#define BT_SCAN_SYNC_WR_PRO_NODES 1
#else
#define BT_SCAN_SYNC_WR_PRO_NODES 0
#endif
...
#define TICKER_NODES              (TICKER_ID_ULL_BASE + \
				   ...
				   BT_SCAN_SYNC_WR_TICKER_NODES + \
				   BT_SCAN_SYNC_WR_RSP_TICKER_NODES + \
				   BT_SCAN_SYNC_WR_PRO_NODES + \
...
#define PDU_RX_CNT    (5 + BT_CTLR_ADV_EXT_RX_CNT + CONFIG_BT_CTLR_RX_BUFFERS)
...
uint16_t rx_mem_block_size = PDU_RX_NODE_POOL_ELEMENT_SIZE - NODE_RX_HEADER_SIZE;
	...
	while (rx) {
		...
		case NODE_RX_TYPE_SYNC:
		{
			...
			if ((status == BT_HCI_ERR_SUCCESS) ||
				...
				struct ll_scan_set *scan;
				if (rx_free->hdr.type == NODE_RX_TYPE_SYNC) {
					//PA Sync by scan
					...
					ull_sync_setup_reset(scan);///* Remove the sync context from being associated with scan contexts */
				} else {
					//BTD-5036: PAST should not bonded with scan, PA Sync is not by scan
					...
					ll_rx_release(rx_free);
					//=> sync context not being associated with scan context, so not need remove sync context
...
ATTR_TEXT_IN_TCM void *ull_pdu_rx_alloc_peek(uint8_t count)
{
...
uint8_t ull_rx_buf_mfifo_avail_count_get(void)
{
	return MFIFO_AVAIL_COUNT_GET(pdu_rx_free);
}
...
ATTR_TEXT_IN_TCM void *ull_pdu_rx_alloc(void)
{
	return MFIFO_DEQUEUE(pdu_rx_free);
}

ATTR_TEXT_IN_TCM void ull_rx_put(memq_link_t *link, void *rx)
{
#if defined(CONFIG_BT_CONN)
	struct node_rx_hdr *rx_hdr = rx;
...
ATTR_TEXT_IN_TCM void ull_rx_sched(void)
{
	static memq_link_t link;
...
ATTR_TEXT_IN_TCM void ull_rx_put_sched(memq_link_t *link, void *rx)
{
	ull_rx_put(link, rx);
	ull_rx_sched();
}
...
ATTR_TEXT_IN_TCM  void *ull_prepare_dequeue_get(void)
{
...
ATTR_TEXT_IN_TCM void ull_prepare_dequeue(uint8_t caller_id)
{
...
ATTR_TEXT_IN_TCM struct event_done_extra *ull_event_done_extra_get(void)
{
...
ATTR_TEXT_IN_TCM void *ull_event_done(void *param)
{
	struct node_rx_event_done *evdone;
	memq_link_t *link;
