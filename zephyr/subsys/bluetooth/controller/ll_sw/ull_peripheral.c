...
2026-03-05 by Airoha - Add PAwR connection implmentation.
*/
		     ...
		     struct lll_conn *lll)
{
	...
	uint8_t chan_sel;
	void *node;
    bool connection_from_pawr;
    struct ll_sync_set *sync;
    //BT_ULL_LOG_MSGID_I("ull_periph_setup, user_meta: %d", 1, rx->hdr.user_meta);
    connection_from_pawr = (rx->hdr.user_meta == LE_PERIPHERAL_CONNECTION_FROM_PAWR) ? true : false;

    if (connection_from_pawr == false) {
    	...
    	lll->periph.window_widening_max_us = (conn_interval_us >> 1) - EVENT_IFS_US - 
					(EVENT_OVERHEAD_PREEMPT_MIN_US + EVENT_OVERHEAD_START_US + EVENT_JITTER_US + EVENT_TICKER_RES_MARGIN_US);
        ...
        BT_ULL_LOG_MSGID_I("peripheral setup, intvl:%d, tk_anchor:%d, conn_offset:%d", 4, conn_interval_us, ftr->ticks_anchor, conn_offset_us);
    	/* Start Peripheral */
...
#if defined(CONFIG_BT_CTLR_SYNC_PERIODIC_RSP) && defined(CONFIG_BT_CTLR_ADV_EXT)
	else {
        //PAwR Connection
        sync = ((struct lll_sync *)ftr->param)->hdr.parent;
        conn = lll->hdr.parent;

        /* Populate the peripheral context */
        pdu_adv = (void *)rx->pdu;

        /* Record peer address type and address*/
        peer_addr_type = pdu_adv->tx_addr;
        memcpy(peer_addr, pdu_adv->connect_ind.init_addr, BDADDR_SIZE);
        ...
        //TODO: Consider do not connect twice to the same peer
    	...
    	lll->data_chan_hop = pdu_adv->connect_ind.hop;
    	lll->interval = sys_le16_to_cpu(pdu_adv->connect_ind.interval);
		BT_ULL_LOG_MSGID_I("[PAWR_CONN]param, hop: %d, interval: %d, chan_count: %d, time_out: %d", 4, pdu_adv->connect_ind.hop, pdu_adv->connect_ind.interval, lll->data_chan_count,pdu_adv->connect_ind.timeout);
		if ((lll->data_chan_count < CHM_USED_COUNT_MIN) ||
			(lll->data_chan_hop < CHM_HOP_COUNT_MIN) ||
			(lll->data_chan_hop > CHM_HOP_COUNT_MAX) ||
			!lll->interval) {
			invalid_release(&sync->ull, lll, link, rx);
			BT_ULL_LOG_MSGID_I("[PAWR_CONN] param error!", 0);
			return;
		}

		((struct lll_sync *)ftr->param)->conn = NULL;
    	...
    	*/
    	conn->appto_reload = (conn->apto_reload > (lll->latency + 6)) ?
    					 ...
    					 conn->apto_reload;
    #endif /* CONFIG_BT_CTLR_LE_PING */
		pdu_adv->chan_sel = 1; // Default Use CSA2 for PAwR Connection
        chan_sel = pdu_adv->chan_sel;
		...
		cc->timeout = conn->supervision_timeout;
		cc->sca = conn->periph.sca;
		cc->is_from_pawr = true;
        cc->sync_handle = ull_sync_handle_get(sync);
		cc->adv_handle = 0xFF;
		lll->handle = ll_conn_handle_get(conn);
		rx->hdr.handle = lll->handle;
    #if defined(CONFIG_BT_CTLR_TX_PWR_DYNAMIC_CONTROL)
	...
	if (IS_ENABLED(CONFIG_BT_CTLR_CHAN_SEL_2)) {
		...
		/* Enqueue the connection event */
		ll_rx_put(link, rx); //HCI Event: LE Enhanced Connection Complete V2
		...
		rx->hdr.handle = lll->handle;
		rx->hdr.type = NODE_RX_TYPE_CHAN_SEL_ALGO; //HCI Event: LE Channel Selection Algorithm
    	...
    	/* Start Peripheral */
    	ticker_id_conn = TICKER_ID_CONN_BASE + ll_conn_handle_get(conn);
		BT_ULL_LOG_MSGID_I("[PAWR_CONN] start ticker, id: %d, conn_offset_us: %d, conn_interval_us: %d", 3, ticker_id_conn, conn_offset_us, conn_interval_us);
    	ticker_status = ticker_start(TICKER_INSTANCE_ID_CTLR,
    				     TICKER_USER_ID_ULL_HIGH,
    				     ticker_id_conn,
    				     ftr->ticks_anchor - ticks_slot_offset,
