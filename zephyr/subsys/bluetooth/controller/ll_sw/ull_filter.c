...
#if defined(CONFIG_BT_CTLR_FILTER_ACCEPT_LIST)
inline struct lll_filter *ull_filter_lll_get(bool filter)
{
#if defined(CONFIG_BT_CTLR_PRIVACY)
	if (filter) {
		return &fal_filter;
...
inline uint8_t ull_filter_lll_fal_match(const struct lll_filter *const filter,
...
ATTR_TEXT_IN_TCM uint8_t ull_filter_rl_find(uint8_t id_addr_type, uint8_t const *const id_addr,
			   uint8_t *const free_idx)
{
	uint8_t i;
...
inline bt_addr_t *ull_filter_lll_lrpa_get(uint8_t rl_idx)
{
	if ((rl_idx >= ARRAY_SIZE(rl)) || !rl[rl_idx].lirk ||
...
inline uint8_t *ull_filter_lll_irks_get(uint8_t *count)
{
	*count = peer_irk_count;
	return (uint8_t *)peer_irks;
}

inline uint8_t ull_filter_lll_rl_idx(bool filter, uint8_t devmatch_id)
{
	uint8_t i;
...
inline uint8_t ull_filter_lll_rl_irk_idx(uint8_t irkmatch_id)
{
	uint8_t i;
...
inline bool ull_filter_lll_irk_in_fal(uint8_t rl_idx)
{
	if (rl_idx >= ARRAY_SIZE(rl)) {
		return false;
	}
...
inline struct lll_resolve_list *ull_filter_lll_resolve_list_get(void)
{
	return rl;
}

inline bool ull_filter_lll_rl_idx_allowed(uint8_t irkmatch_ok, uint8_t rl_idx)
{
...
ATTR_TEXT_IN_TCM bool ull_filter_lll_rl_addr_allowed(uint8_t id_addr_type,
...
ATTR_TEXT_IN_TCM bool ull_filter_lll_rl_addr_resolve(uint8_t id_addr_type,
...
inline bool ull_filter_lll_rl_enabled(void)
{
	return rl_enable;
}
...
ATTR_TEXT_IN_TCM static uint32_t filter_find(const struct lll_filter *const filter,
			    uint8_t addr_type, const uint8_t *const bdaddr)
{
	int index;
