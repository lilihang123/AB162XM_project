				       ...
				       uint16_t sync_conn_event_count, uint8_t sender_sca)
{
    ...
    //BT_ULL_LOG_MSGID_I("[DEBUG_11] %d",  1, lll->window_widening_periodic_us);
...
uint8_t ll_sync_terminate(uint16_t handle)
{
    ...
    if (!sync->is_pawr) {
		...
		err = ull_sync_rsp_terminate(handle);
    #endif  
    }
...
uint8_t ll_sync_transfer(uint16_t conn_handle, uint16_t service_data, uint16_t sync_handle)
{
	...
	return (sync->is_pawr) ? (ull_cp_periodic_sync(conn, sync, NULL, service_data, true)) : (ull_cp_periodic_sync(conn, sync, NULL, service_data, false));
}
...
{
	struct ll_conn *conn;
    BT_ULL_LOG_MSGID_I("timeout: %d", 1, timeout);
	conn = ll_connected_get(conn_handle);
	if (!conn) {
		return BT_HCI_ERR_UNKNOWN_CONN_ID;
	}
...
/* Local helper: parse EXT_IND from sync-establish PDU and do CHM update */
static void ull_sync_pawr_chm_update_from_sync_rx(struct node_rx_pdu *rx)
{
	struct pdu_adv *pdu;
	...
	/* Only handle Extended ADV Indications */
	if (pdu->type != PDU_ADV_TYPE_EXT_IND) {
		BT_ULL_LOG_MSGID_I("[PAWR_CHM] pdu_type=%d, skip", 1, pdu->type);
		return;
	}
	...
	if (!pdu->len || !p->ext_hdr_len) {
		BT_ULL_LOG_MSGID_I("[PAWR_CHM] no ext_hdr, len=%d ext_hdr_len=%d", 2,
				   pdu->len, p->ext_hdr_len);
		return;
	}
	...
	if (h->rfu) {
		BT_ULL_LOG_MSGID_I("[PAWR_CHM] RFU set, clear", 0);
		h->rfu = 0;
	}
	...
	if (hdr_len > hdr_buf_len) {
		BT_ULL_LOG_MSGID_I("[PAWR_CHM] invalid hdr_len=%d buf_len=%d", 2,
				   hdr_len, hdr_buf_len);
		return;
	}

	acad_len = hdr_buf_len - hdr_len;
	BT_ULL_LOG_MSGID_I("[PAWR_CHM] acad_len=%d", 1, acad_len);

	if (!acad_len) {
		return;
	}

	BT_ULL_LOG_MSGID_I("[PAWR_CHM] CHM_UPDATE handle=%d", 1, rx->hdr.handle);
...
//#if defined(CONFIG_BT_CTLR_SYNC_PERIODIC_CTE_TYPE_FILTERING)
	...
	if (1) {
		...
		BT_ULL_LOG_MSGID_I("ull_sync_established_report,  type: %d, sync_status = %d", 2, rx_establ->hdr.type, sync_status);
        if (rx_establ->hdr.type != NODE_RX_TYPE_SYNC_TRANSFER_RECEIVED && rx_establ->hdr.type != NODE_RX_TYPE_SYNC_TRANSFER_RECEIVED_V2) {
		    rx_establ->hdr.type = NODE_RX_TYPE_SYNC;
        }
	#if defined(CONFIG_BT_CTLR_SYNC_PERIODIC_RSP)
        ull_sync_rsp_record_anchor(sync, rx->rx_ftr.ticks_anchor);
	#endif
		...
		ll_rx_put_sched(rx_establ->hdr.link, rx_establ);// report PAST Received Event
	}
...
//#if defined(CONFIG_BT_CTLR_SYNC_PERIODIC_CTE_TYPE_FILTERING)
	...
	if (sync_status == SYNC_STAT_ALLOWED || sync_status == SYNC_STAT_READY) {
        ...
        BT_ULL_LOG_MSGID_I("switch to lll_sync_prepare", 0);
		sync->lll_sync_prepare = lll_sync_prepare; //for subevent #0
	#if defined(CONFIG_BT_CTLR_SYNC_PERIODIC_RSP)
		ull_sync_rsp_switch_prepare_cb_func(sync);
	#endif
		/* Change node type to appropriately handle periodic
		 * advertising PDU report.
		 */
		if (rx_establ->hdr.type != NODE_RX_TYPE_SYNC_TRANSFER_RECEIVED_V2) {
			rx->hdr.type = NODE_RX_TYPE_SYNC_REPORT;
			ull_scan_aux_setup(link, rx);
		} else {
			struct node_rx_pdu *extra =rx->rx_ftr.extra;
            if (extra) {
			    extra->hdr.type = NODE_RX_TYPE_RELEASE;
			    ll_rx_put(extra->hdr.link, extra);
            }
    		ull_sync_pawr_chm_update_from_sync_rx(rx);		/* check CHM info in first sync packet*/ 
...
#endif /* CONFIG_BT_CTLR_SYNC_PERIODIC_CTE_TYPE_FILTERING */
	{
		...
		if (ticks_drift_plus || ticks_drift_minus || lazy || force) {
			...
			if (!sync->is_pawr) {
				...
				ull_sync_rsp_update(sync, ticks_drift_minus, ticks_drift_plus, lazy, force);
			}
		#endif
		}
	}
...
void ull_sync_chm_update(uint8_t sync_handle, uint8_t *acad, uint8_t acad_len)
{
	...
	BT_ULL_LOG_MSGID_I("ull_sync_chm_update, chm: %2x%2x%2x%2x%2x, chm_instant: %d", 6, chm_upd_ind->chm[0], chm_upd_ind->chm[1], chm_upd_ind->chm[2], chm_upd_ind->chm[3], chm_upd_ind->chm[4], lll->chm_instant);
					   ...
					   uint8_t cte_type, uint8_t rx_enable, uint8_t nodups)
{
	...
	link_sync_estab = ll_rx_link_alloc();
	if (!link_sync_estab) {
		BT_ULL_LOG_MSGID_E(" ull_sync_create: link_sync_estab alloc failed\n", 0);
		return NULL;
	}
	...
	if (!link_sync_lost) {
		ll_rx_link_release(link_sync_estab);
		BT_ULL_LOG_MSGID_E(" ull_sync_create: link_sync_lost alloc failed\n", 0);
		return NULL;
	}
	...
	if (!node_rx) {
		ll_rx_link_release(link_sync_lost);
		ll_rx_link_release(link_sync_estab);
		BT_ULL_LOG_MSGID_E(" ull_sync_create: node_rx alloc failed\n", 0);
		return NULL;
	}
	...
	if (!sync) {
		...
		ll_rx_link_release(link_sync_lost);
		ll_rx_link_release(link_sync_estab);
		BT_ULL_LOG_MSGID_E(" ull_sync_create: sync_acquire failed\n", 0);
		return NULL;
	}
...
#if defined(CONFIG_BT_CTLR_SYNC_PERIODIC_RSP)
    ull_sync_init_subevent_ctx(lll);
#endif
...
static void sync_ticker_cleanup(struct ll_sync_set *sync, ticker_op_func stop_op_cb)
{
    if (!sync->is_pawr) {
		...
		ull_sync_rsp_clean_up(sync, stop_op_cb);
    }
#endif
}
				...
				uint8_t addr_resolved)
{
    ...
    //printk("[DEBUG_4]ull_sync_create success", 0);
