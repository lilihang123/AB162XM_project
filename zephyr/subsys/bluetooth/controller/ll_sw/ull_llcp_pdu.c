					 ...
					 const struct data_pdu_length *dle)
{
	...
	// p->max_rx_octets = sys_cpu_to_le16(251);
	// p->max_tx_octets = sys_cpu_to_le16(251);
	// p->max_rx_time = sys_cpu_to_le16(2120);
	// p->max_tx_time = sys_cpu_to_le16(2120);    
}
...
void llcp_pdu_encode_periodic_sync_ind(struct proc_ctx *ctx, struct pdu_data *pdu)
{
	...
	pdu->ll_id = PDU_DATA_LLID_CTRL;
    //BT_ULL_LOG_MSGID_I("Size of periodic_sync_ind: %zu", 1, sizeof(struct pdu_data_llctrl_periodic_sync_ind));

	pdu->len = PDU_DATA_LLCTRL_LEN(periodic_sync_ind);
    //BT_ULL_LOG_MSGID_I("pdu->len: %zu", pdu->len);    
...
#if 0
void llcp_pdu_encode_periodic_sync_wr_ind(struct proc_ctx *ctx, struct pdu_data *pdu)
{
	struct pdu_data_llctrl_periodic_sync_wr_ind *p = &pdu->llctrl.periodic_sync_wr_ind;

	pdu->ll_id = PDU_DATA_LLID_CTRL;
	pdu->len = PDU_DATA_LLCTRL_LEN(periodic_sync_wr_ind);
	pdu->llctrl.opcode = PDU_DATA_LLCTRL_TYPE_PERIODIC_SYNC_WR_IND;

    p->periodic_sync_ind.id = sys_cpu_to_le16(ctx->data.periodic_sync.id);

	//memcpy(&p->sync_info, &ctx->data.periodic_sync.sync_info, sizeof(struct pdu_adv_sync_info));
    memcpy(&(p->periodic_sync_ind.sync_info), &ctx->data.periodic_sync.sync_info, sizeof(struct pdu_adv_sync_info));
	p->periodic_sync_ind.sync_info.evt_cntr = sys_cpu_to_le16(ctx->data.periodic_sync.sync_info.evt_cntr);

	p->periodic_sync_ind.conn_event_count = sys_cpu_to_le16(ctx->data.periodic_sync.conn_event_count);
	p->periodic_sync_ind.last_pa_event_counter = sys_cpu_to_le16(ctx->data.periodic_sync.last_pa_event_counter);
	p->periodic_sync_ind.sid = ctx->data.periodic_sync.sid;
	p->periodic_sync_ind.addr_type = ctx->data.periodic_sync.addr_type;
	p->periodic_sync_ind.sca = ctx->data.periodic_sync.sca;
	p->periodic_sync_ind.phy = ctx->data.periodic_sync.phy;

	memcpy(&(p->periodic_sync_ind.adv_addr), &ctx->data.periodic_sync.adv_addr, sizeof(bt_addr_t));

	p->periodic_sync_ind.sync_conn_event_count = sys_cpu_to_le16(ctx->data.periodic_sync.sync_conn_event_count);

	//PAWR
    p->rsp_aa = sys_cpu_to_le32(ctx->data.periodic_sync.response_aa);
    p->num_subevents = ctx->data.periodic_sync.num_subevents;
    p->subevent_interval = ctx->data.periodic_sync.subevent_interval;
    p->response_slot_delay = ctx->data.periodic_sync.response_slot_delay;
    p->response_slot_spacing = ctx->data.periodic_sync.response_slot_spacing;
...
void llcp_pdu_encode_periodic_sync_wr_ind(struct proc_ctx *ctx, struct pdu_data *pdu)
{
	...
	pdu->ll_id = PDU_DATA_LLID_CTRL;
    BT_ULL_LOG_MSGID_I("[PAWR]llcp_pdu_encode_periodic_sync_wr_ind", 0);
	pdu->len = PDU_DATA_LLCTRL_LEN(periodic_sync_wr_ind);
	pdu->llctrl.opcode = PDU_DATA_LLCTRL_TYPE_PERIODIC_SYNC_WR_IND;
	...
	//PAWR
    //p->rsp_aa = sys_cpu_to_le32(ctx->data.periodic_sync.response_aa);
    memcpy(p->rsp_aa, ctx->data.periodic_sync.response_aa, 4);
    p->num_subevents = ctx->data.periodic_sync.num_subevents;
    p->subevent_interval = ctx->data.periodic_sync.subevent_interval;
    p->response_slot_delay = ctx->data.periodic_sync.response_slot_delay;
    p->response_slot_spacing = ctx->data.periodic_sync.response_slot_spacing;
...
void llcp_pdu_decode_periodic_sync_ind(struct proc_ctx *ctx, struct pdu_data *pdu)
{
    ...
    ctx->data.periodic_sync.is_pawr = false;
    //BT_ULL_LOG_MSGID_I("[DEBUG_3]llcp_pdu_decode_periodic_sync_ind", 0);

}

void llcp_pdu_decode_periodic_sync_wr_ind(struct proc_ctx *ctx, struct pdu_data *pdu)
{
	struct pdu_data_llctrl_periodic_sync_wr_ind *p = &pdu->llctrl.periodic_sync_wr_ind;
    ctx->data.periodic_sync.id = sys_le16_to_cpu(p->id);
    memcpy(&ctx->data.periodic_sync.sync_info, &p->sync_info, sizeof(struct pdu_adv_sync_info));
    ...
    //ctx->data.periodic_sync.response_aa = sys_le32_to_cpu(p->rsp_aa);
    memcpy(&ctx->data.periodic_sync.response_aa[0], &p->rsp_aa[0], 4);
    ctx->data.periodic_sync.num_subevents = p->num_subevents;
    ctx->data.periodic_sync.subevent_interval = p->subevent_interval;
    ctx->data.periodic_sync.response_slot_delay = p->response_slot_delay;
    ctx->data.periodic_sync.response_slot_spacing = p->response_slot_spacing;
    ctx->data.periodic_sync.is_pawr = true;
    BT_ULL_LOG_MSGID_I("[DEBUG_8]llcp_pdu_decode_periodic_sync_wr_ind", 0);
