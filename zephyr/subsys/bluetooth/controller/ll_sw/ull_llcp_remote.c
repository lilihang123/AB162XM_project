		...
		struct node_rx_pdu *rx)
{
    ...
    BT_ULL_LOG_MSGID_I("[PROC_LLCP] rr_rx, proc id: %d, state: %d", 2, ctx->proc, ctx->state);
	switch (ctx->proc) {
	case PROC_UNKNOWN:
		/* Do nothing */
		break;
...
static void rr_act_run(struct ll_conn *conn)
{
    ...
    //BT_ULL_LOG_MSGID_I("[PROC_LLCP] act_run, proc id: %d, state: %d", 2, ctx->proc, ctx->state);
	switch (ctx->proc) {
#if defined(CONFIG_BT_CTLR_LE_PING)
	case PROC_LE_PING:
...
static const struct proc_role new_proc_lut[] = {
    ...
    [PDU_DATA_LLCTRL_TYPE_PERIODIC_SYNC_WR_IND] = { PROC_PERIODIC_WR_SYNC, ACCEPT_ROLE_BOTH },
...
void llcp_rr_new(struct ll_conn *conn, memq_link_t *link, struct node_rx_pdu *rx, bool valid_pdu)
{
	...
	BT_ULL_LOG_MSGID_I("[PROC_LLCP] rr new, op: %d, proc id: %d, proc_ctx:0x%x", 3, pdu->llctrl.opcode, proc, ctx);
	if (!ctx) {
		return;
	}
