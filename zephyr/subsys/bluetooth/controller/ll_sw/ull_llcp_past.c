			       ...
			       void *param)
{
	...
	switch (evt) {
	case RP_PAST_EVT_RX:
        if (pdu->llctrl.opcode == PDU_DATA_LLCTRL_TYPE_PERIODIC_SYNC_IND) {
		    llcp_pdu_decode_periodic_sync_ind(ctx, pdu);
        } else if (pdu->llctrl.opcode == PDU_DATA_LLCTRL_TYPE_PERIODIC_SYNC_WR_IND) {
            llcp_pdu_decode_periodic_sync_wr_ind(ctx, pdu);
        } else {
            LL_ASSERT(0);
        }
        ...
        if (pdu->llctrl.opcode == PDU_DATA_LLCTRL_TYPE_PERIODIC_SYNC_IND) {
    		...
    		/* Hand over to ULL */
    		ull_sync_rsp_transfer_received(conn,
    					   ctx->data.periodic_sync.id,
    					   &ctx->data.periodic_sync.sync_info,
    					   ctx->data.periodic_sync.conn_event_count,
    					   ctx->data.periodic_sync.last_pa_event_counter,
    					   ...
    					   ctx->data.periodic_sync.sync_conn_event_count,
    					   ctx->data.periodic_sync.addr_resolved,
    					   ctx->data.periodic_sync.response_aa,
    					   ctx->data.periodic_sync.num_subevents,
    					   ctx->data.periodic_sync.subevent_interval,
    					   ctx->data.periodic_sync.response_slot_delay,
    					   ctx->data.periodic_sync.response_slot_spacing);        
        }
	#endif
		rp_past_complete(conn, ctx);
		break;
...
static void lp_past_tx(struct ll_conn *conn, struct proc_ctx *ctx)
{
	...
	struct pdu_data *pdu;
#if defined (CONFIG_BT_PERIPHERAL)
	if (/*IS_ENABLED(CONFIG_BT_PERIPHERAL)*/1) {
		if (conn->lll.role == BT_HCI_ROLE_PERIPHERAL) {
			uint32_t offset_us;
    ...
    if (ctx->data.periodic_sync.is_pawr) {
        llcp_pdu_encode_periodic_sync_wr_ind(ctx, pdu);
    } else {
