...
/* Modifications:
2025-09-15 by Airoha - Add truncation of objects feature of OACP.
2026-01-14 by Airoha - Add a feature to disconnect the COC channel when there is no data
transfer for 30 seconds during the write/read object operation.
*/
			...
			struct bt_conn *conn)
{
    ...
    k_work_cancel_delayable(&ots->object_transfer_work);
	...
	struct bt_gatt_ots_oacp_proc *proc)
{
	...
	k_work_reschedule(&ots->object_transfer_work, OBJECT_TRANSFER_TIMEOUT);
			...
			struct bt_conn *conn, struct net_buf *buf)
{
	...
	rc = ots->cb->obj_write(ots, conn, ots->cur_obj->id, buf->data, len,
				  offset, rem, write_op->oacp_params.mode);
    ...
    else if(BT_OTS_OACP_WRITE_OP_MODE_TRUNCATE == write_op->oacp_params.mode) {
		ots->cur_obj->metadata.size.cur = offset + len;
    }
    k_work_reschedule(&ots->object_transfer_work, OBJECT_TRANSFER_TIMEOUT);
	return rc;
