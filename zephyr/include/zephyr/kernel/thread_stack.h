...
/* Modifications:
2026-03-13 by Airoha - Stack region specified section reassigned
*/
...
#endif /* CONFIG_AIR_RAM_BLOCKS_CONTROL_ENABLE */
...
#define Z_KERNEL_STACK_ARRAY_DEFINE_IN_NOINIT(sym, nmemb, size, lsect) \
	struct z_thread_stack_element lsect \
		__aligned(Z_KERNEL_STACK_OBJ_ALIGN) \
		sym[nmemb][K_KERNEL_STACK_LEN(size)]
#else
...
#define K_KERNEL_PINNED_STACK_ARRAY_DEFINE(sym, nmemb, size) \
	Z_KERNEL_STACK_ARRAY_DEFINE_IN(sym, nmemb, size, __kstackmem)
#if defined(CONFIG_AIR_RAM_BLOCKS_CONTROL_ENABLE)
#define K_KERNEL_PINNED_STACK_ARRAY_DEFINE_RETENTION(sym, nmemb, size) \
	Z_KERNEL_STACK_ARRAY_DEFINE_IN_NOINIT(sym, nmemb, size, __kstackmem)
#endif /* CONFIG_AIR_RAM_BLOCKS_CONTROL_ENABLE */
#endif /* CONFIG_LINKER_USE_PINNED_SECTION */
