 ...
 */
 ...
 /* Modifications:
2026-03-13 by Airoha - re-define logging macro.
*/
...
#define LOG_LEVEL_ERR  3U
#define LOG_LEVEL_WRN  2U
#define LOG_LEVEL_INF  1U
#define LOG_LEVEL_DBG  0U
...
/*****************************************************************************/
#if defined(AIR_LOGGING_ENABLE)
#define Z_LOG_TO_AIR_LOG(_level, fmt, ...) do { \
    ATTR_LOG_STRING msg_id_string[] = fmt; \
    print_module_msgid_log( NULL, \
                            (print_level_t)_level, \
                            (msg_id_string), \
...
#define Z_LOG2(_level, _inst, _source, _dsource, ...) do { \
	if (IS_ENABLED(CONFIG_AIR_LOGGING_ENABLE)) { \
		Z_LOG_TO_AIR_LOG(_level, __VA_ARGS__); \
		break; \
	} \
} while (false)
#else
