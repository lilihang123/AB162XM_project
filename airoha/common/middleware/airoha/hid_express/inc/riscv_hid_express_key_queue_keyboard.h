...
#ifndef _RISCV_HID_EXPRESS_KEY_QUEUE_KEYBOARD_H_
#define _RISCV_HID_EXPRESS_KEY_QUEUE_KEYBOARD_H_
...
#define KEY_BUFFER_SIZE                              (1 << 5)                /** The size of the buffer used to store the mouse key info, it should be a power of 2 */
...
#define DEFAULT_VALID_KEYBOARD_KEY_MASK              0xFFFFFFFF              /** The default valid keyboard key mask, 8 bits stand for std key, 8 bits stand for n_key_0, 8 bits stand for n_key_1, 8 bits stand for CONSUMER key*/
#define PUSH_KEY 0
#define PUSH_Z 1

#define KB_NKEY_SEND_GROUP                            (0)
#define KB_NKEY_SEND_GROUP_0                          (1)
#define KB_NKEY_SEND_GROUP_1                          (1 << 1)
#define KB_NKEY_SEND_GROUP_2                          (1 << 2)

typedef struct {
    uint32_t type;
    uint32_t items[4];                      /** The nkey value */
    uint32_t item_consumer;                 /** The consumer key value */
    uint32_t item_ms;                       /** The ms key value */
    uint32_t send_group;                    /** The nkey group for send */
...
typedef struct {
    T_KEY_INFO_S    buffer[KEY_BUFFER_SIZE];        /** The queue buffer of mouse keys */
    uint32_t        valid_key_mask;                 /** The mask of valid keys */
    uint32_t        last_pushed_nkey[4];            /** The delivered key (combo + 15 byte nkey data) are used to determine whether the outgoing key has changed */
    uint32_t        old_pushed_nkey[4];             /** mouse key status is changed, This flag is used to get key latency info after HID report is sent to avoid any delay */
    
    uint32_t        outgoing_item_idx;
    uint32_t        last_pushed_kb_consumer;
    uint32_t        old_pushed_kb_consumer;
