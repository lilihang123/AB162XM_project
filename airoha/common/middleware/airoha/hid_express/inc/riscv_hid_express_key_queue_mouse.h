...
#define KEY_BUFFER_SIZE                              (1<<3)                 /** The size of the buffer used to store the mouse key info, it should be a power of 2 */
...
#define DEFAULT_VALID_MOUSE_KEY_MASK                 0x1F                   /** The default valid mouse key mask, 5 bits stand for left, right, middle, page next, and page previous keys  5 bits stand for left, right, middle, page next, and page previous keys*/
#define PUSH_KEY 0
#define PUSH_Z 1
...
typedef struct {
    ...
    uint8_t nack_cnt;                      /** The number of nack received after the key was send */
    uint8_t key_delivery_status;           /** The delivery status of the key in this item*/
    uint8_t type;
}T_KEY_INFO_S;
...
typedef struct {
    ...
    uint32_t        last_pushed_ms;                 /** The delivered key and z value are used to determine whether the outgoing key has changed and whether a z value is present*/
    uint32_t        old_pushed_ms;                  /** mouse key status is changed, This flag is used to get key latency info after HID report is sent to avoid any delay */
