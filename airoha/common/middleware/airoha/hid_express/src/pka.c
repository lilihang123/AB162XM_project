...
#define RBB_REG_INTERVAL_PHASE_ADDR     0xA00107D0
...
void riscv_pka_rt_tx_post_prepare_callback(uint8_t tx_cnt)
{
    ...
    if (pka_share_hwmt_get_curidx() == 0) {
        ...
        int32_t ref_offset = 0;
        uint32_t interval = *(volatile uint32_t *)RBB_REG_INTERVAL_PHASE_ADDR;
        ref_offset = (interval == 250) ? anchor_diff[0] : anchor_diff[1];
#endif
...
#if 0
            LOG_MSGID_I(riscv_pka, "[RISCV][HWM_SYNC] offset %d, ref_offset = %d, sync_time = 0x%x %d, last_anchor = 0x%x %d, pico_offset 0x%x %d %x %d", 10,
                    offset,
                    ref_offset,
                    sync_time.period,
                    sync_time.phase,
