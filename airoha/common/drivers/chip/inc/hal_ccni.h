 ...
 *      (B) There are events with a message which uses reserved memory.\n
 *
 * - How to define share memory buffer \n
 *   Please use the ATTR_SHARE_RWDATA attribute for RW data or the ATTR_SHARE_ZIDATA attribute for Zero Initialize (ZI) data.
 *
 * - How to define an event \n
 *   The format of the software event is as follows:
 *    |   bit 31~24   |      bit 23~16     |        bit 15~8           |      bit 7         |       bit 6~0      |
 *    |---------------|--------------------|---------------------------|------------------ -|--------------------|
 *    |  source core  |  destination core  |  message length(optional) |  High/low priority |  event index 0~31  |
 *
 ...
 *      (A) Check the event usage status in \\mcu\\driver\\chip\\xxx\\inc\\hal_ccni_config.h. The event in default status can be used.\n
 *      (B) Check the ccni_*_to_*_function_table[] in hal_ccni_config.c; The event is not used if the isr is default_isr.\n
 *           For example, if you want to apply an event for MCU to DSP0, please check ccni_mcu_to_dsp0_function_table[] to find an unused event.\n
 ...
 *
 *  - Step 3: Replace the event name in \\mcu\\driver\\chip\\xxx\\inc\\hal_ccni_config.h.\n
 *    We strongly advise that you use a name which implies the event function.
 *    @code
 *    //define EVENT 8 for exception from DSP0 to MCU
      #define IRQGEN_DSP0_TO_MCU_EXCEPTION (HAL_CCNI_EVENT8|CCNI_EVENT_SRC_DSP0|CCNI_EVENT_DST_MCU)
 *    @endcode
 ...
 *            //Receiver should do memory address remap by hal_memview_infrasys_to_mcu(), otherwise it can not get right data.
 *            hal_memview_infrasys_to_mcu(pMsg[0]);
 ...
 *    @endcode
 *
 *  - Step 5: Register the isr handler to ccni_*_to_*_function_table[] in \\mcu\\driver\\chip\\xxx\\src\\hal_ccni_config.c. \n
 ...
 *  - Sample code:
 *    @code
 *       //MCU send an event IRQGEN_MCU_TO_DSP0_EVENT9 to DSP0.
 *       hal_ccni_status_t status;
 *       hal_ccni_event_t event = IRQGEN_MCU_TO_DSP0_EVENT9;
 *       // the second parameter should be NULL for event without message.
 *       status = hal_ccni_set_event(event, NULL);
 *       if (HAL_CCNI_STATUS_OK != status) {
 *           // Error handler;
 *       }
 *    @endcode
 *
 ...
 *    @code
 *       //MCU send an event CCNI_MCU_TO_DSP0_EVENT1 to DSP0 with const value in message.
 *       hal_ccni_status_t status;
         //MCU send event CCNI_MCU_TO_DSP0_EVENT1 to DSP0
 *       hal_ccni_event_t event = CCNI_MCU_TO_DSP0_EVENT1;
 *       uint32_t msg_array[2];
 *       //the destination core will receive the next 8-byte message.
 *       msg_array[0] = 0x80;
 *       msg_array[1] = 0xa5;
 *       //the second parameter should not be NULL.
 *       status = hal_ccni_set_event(event, msg_array);
 *       if (HAL_CCNI_STATUS_OK != status) {
 *           //Error handler;
 *       }
 *
 *       //MCU send an event CCNI_MCU_TO_DSP0_EVENT2 to DSP0 and notify send 128 Bytes data with sharing memory.
 *       hal_ccni_status_t status;
 *       uint32_t sharing_memory_address;
 *       //MCU send event CCNI_MCU_TO_DSP0_EVENT2 to DSP0
 *       hal_ccni_event_t event = CCNI_MCU_TO_DSP0_EVENT2;
 *       uint32_t msg_array[2];
 *       //the destination core will receive the next 8-byte message.
 *       sharing_memory_address = 0xabc;  // the memory 0xabc should be defined with #ATTR_SHARE_RWDATA.
 *       //Please use hal_memview_mcu_to_infrasys() to do address remap.
 *       msg_array[0] = hal_memview_mcu_to_infrasys(sharing_memory_address);
 ...
 *       uint32_t count;
 *       do{
 *           status = hal_ccni_set_event(CCNI_MCU_TO_DSP0_EVENT11, NULL);
 ...
 * @addtogroup CCNI
 * @{
 * - \b Using \b variable \b message \b size \b API \n
 *  Send message with 3 words data, The steps are shown below:
 *  - Step 1. Call #hal_ccni_set_event() to trigger an event.
 *  - Step 2. Should check the return value, and do error handling if return value is not #HAL_CCNI_STATUS_OK.
 *  - Sample code:
 *    //MCU send an event CCNI_MCU_TO_DSP0_HP_EVENT0 to DSP0 with 3 words data.
 *    @code
 *       //MCU send an event CCNI_MCU_TO_DSP0_HP_EVENT1 to DSP0 with 3 words message.
 *       hal_ccni_status_t status;
 *       hal_ccni_event_t event = CCNI_MCU_TO_DSP0_HP_EVENT0;
 *       uint32_t msg_array[3] = {0x5A5A5A5A,0xA5A5A5A5, 0x5555AAAA};
 *       //the destination core will receive the 12 bytes message.
 *       hal_ccni_message_t msg;
 *       msg.message_length = sizeof(msg_array);
 *       msg.ccni_message = msg_array;
 *       //the second parameter should not be NULL.
 *       status = hal_ccni_set_event(event, &msg);
 *       if (HAL_CCNI_STATUS_OK != status) {
 *           //Error handler;
 *       }
 *    @endcode
 *
 *  Send event with over 3 words data, The steps are shown below:
 *  - Step 1. at default the user can utilize a message buffer of 12 bytes. we can adjust the event's message length within hal_ccni_donfig.h.
 *  - Sample code:
 *    //define an event with 4 words message data. on the next example it defines the event MCU_TO_DSP0_HP_EVENT1 with 4 words message.
 *    @code
 *    #define HP_EVENT_MSG_LEN = 20;  //event data size is 16, keep 4 bytes for internal used.
 *    typedef enum {
 *        MCU_TO_DSP0_HP_EVENT0_MSG_LEN = DEFAULT_MSG_LENGTH,
 *        MCU_TO_DSP0_HP_EVENT1_MSG_LEN = HP_EVENT_MSG_LEN,   //define MCU_TO_DSP0_HP_EVENT1 with 4 words message
 *        MCU_TO_DSP0_HP_EVENT2_MSG_LEN = DEFAULT_MSG_LENGTH,
 *    } MSG_LEN_TYPE_T;
 *    @endcode
 *
 *  - Step 2. Call #hal_ccni_set_event() to trigger an event.
 *  - Step 3. Should check the return value, and do error handling if return value is not #HAL_CCNI_STATUS_OK. \n
 *  - Sample code:
 *    //MCU send an event CCNI_MCU_TO_DSP0_HP_EVENT1 to DSP0 with 4 words data.
 *    @code
 *       //MCU send an event CCNI_MCU_TO_DSP0_HP_EVENT1 to DSP0 with 4 words message.
 *       hal_ccni_status_t status;
 *       hal_ccni_event_t event = CCNI_MCU_TO_DSP0_HP_EVENT1;
 *       uint32_t msg_array[4] = {0x5A5A5A5A,0xA5A5A5A5, 0x5555AAAA, 0xAAAA5555};
 *       //the destination core will receive the 32-bytes message.
 *       hal_ccni_message_t msg;
 *       msg.message_length = sizeof(msg_array);
 *       msg.ccni_message = msg_array;
 *       //the second parameter should not be NULL.
 ...
 *           // the msg points to the message address if the event is with message. Otherwise, the msg is equal to NULL.
 *           if( pMsg != NULL) {
 *               //The variable 'msg' refers to the received message, and its length is equal to the length of the data that was sent.
 *               //The event contains the length of data sent by the user, which can be calculated as ((event & 0xFF00 >> 8) -4).
 *               log_hal_info("pMsg[0]=0x%x, pMsg[1]=0x%x, pMsg[0]=0x%x, pMsg[1]=0x%x", pMsg[0], pMsg[1], pMsg[2], pMsg[3]);
 *           }
 *
 *           //user deal with the event!
 *
 *           //If the event is managed here, please clear and unmask the event when it is completed.
 *           //or the event will be managed at a thread level. Please clear and unmask the event when it is completed.
 *           //Otherwise, it cannot receive the event anymore.
 *           hal_ccni_clear_event(event);  // clear the event.
...
/**@brief CCNI event type definition */
typedef uint32_t hal_ccni_event_t;

/**@brief CCNI message format with variable message size. The default message size is 4 words.
   The first word is allocated for the message length, while the remaining 3 words are reserved for user data.
   The user can modify the length of the user data in #MSG_LEN_TYPE_T within hal_ccni_config.h.*/
typedef union {
    struct {
        void *ptr;                  /**< point to the user's message data. */
        uint32_t len;               /**< how many bytes of the user's message */
    } variable_len_msg;
    uint32_t data[2];               /**< 8 byte fixed message data */
} hal_ccni_message_t;
_Static_assert(sizeof(hal_ccni_message_t) == 8, "size of hal_ccni_message_t is not 8 bytes");

/**@brief CCNI interrupt handling route type definition */
/**< Define the type of call back function. */
 ...
 * @param[in] message is point which point to a message struct when event is event 0 to 7.
