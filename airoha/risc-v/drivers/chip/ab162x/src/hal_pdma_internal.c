...
// #include "hal_log.h"
#define log_hal_msgid_info(_str, ...)

#if !defined(CORE_MCU)
// #define hal_nvic_irq_t                      uint32_t
#define UART_DMA0_IRQn                      21
#define UART_DMA1_IRQn                      21
#define UART_DMA2_IRQn                      21
#define I3C_DMA0_IRQn                       26
#define I3C_DMA1_IRQn                       26
#endif
...
static void internal_dma_enable_clock(dma_channel_t dma_channel)
{
    ...
    if((dma_channel >= PDMA_DMA_CHANNEL_I3C_BEGIN) && (dma_channel <= PDMA_DMA_CHANNEL_I3C_END)){
        /* I3C DMA Register has been modified */
...
static void internal_dma_enable_irq(dma_channel_t dma_channel)
{
    ...
    if((dma_channel >= PDMA_DMA_CHANNEL_I3C_BEGIN) && (dma_channel <= PDMA_DMA_CHANNEL_I3C_END)){
        /* I3C DMA Register has been modified */
        ((I3C_DMA_REGISTER_T*)(g_dma_reg_table[dma_channel/2]))->dma_glb_cpu1_set.word = DMA_GLB_IRQ_CFG_BIT_MASK(domain_offset);
    } else {
        g_dma_reg_table[dma_channel/2]->RG_GLB_CPU1_INT_SET = DMA_GLB_IRQ_CFG_BIT_MASK(domain_offset);
    }
}
...
static void internal_dma_disable_irq(dma_channel_t dma_channel)
{
    ...
    if((dma_channel >= PDMA_DMA_CHANNEL_I3C_BEGIN) && (dma_channel <= PDMA_DMA_CHANNEL_I3C_END)){
        /* I3C DMA Register has been modified */
        ((I3C_DMA_REGISTER_T*)(g_dma_reg_table[dma_channel/2]))->dma_glb_cpu1_clr.word = DMA_GLB_IRQ_CFG_BIT_MASK(domain_offset);
    } else {
        g_dma_reg_table[dma_channel/2]->RG_GLB_CPU1_INT_CLR = DMA_GLB_IRQ_CFG_BIT_MASK(domain_offset);
    }
}
...
pdma_status_t pdma_dump_reg_list(pdma_channel_t channel)
{
    ...
    (void)pdma_cfg;      /* avoid build error */
