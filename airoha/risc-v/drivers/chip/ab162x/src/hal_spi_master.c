...
//#define HAL_SPIM_CLOCK_ENABLED
#endif
#define SPI_MST0_IRQn  27
#define SPI_MST1_IRQn  18
static const IRQn_Type g_spi_master_irq_code[HAL_SPI_MASTER_MAX] = {SPI_MST0_IRQn,SPI_MST1_IRQn};
#ifdef HAL_SPIM_CLOCK_ENABLED
static const hal_clock_cg_id g_spi_master_cg_code[HAL_SPI_MASTER_MAX] = {HAL_CLOCK_CG_SPIMST0,HAL_CLOCK_CG_SPIMST1};
static const hal_clock_cg_id g_spi_master_bus_cg_code[HAL_SPI_MASTER_MAX] = {HAL_CLOCK_CG_SPIMST0_BUS,HAL_CLOCK_CG_SPIMST1_BUS};
...
// sleep_management_backup_restore_module_t g_spi_master_sleep_backup_restore_handle[HAL_SPI_MASTER_MAX] = {SLEEP_BACKUP_RESTORE_SPI_MST0,SLEEP_BACKUP_RESTORE_SPI_MST1};
#endif
...
static volatile uint32_t spi_sw_fifo_status[HAL_SPI_MASTER_MAX] = {NB_STATUS_OF_IDLE,NB_STATUS_OF_IDLE};



static uint8_t  s_spi_sw_fifo_init_flag[HAL_SPI_MASTER_MAX] = {0,0};
static hal_spi_master_callback_t  g_spi_m_nb_current_user_cb[HAL_SPI_MASTER_MAX] = {NULL,NULL};
typedef struct {
                                                ...
                                                uint32_t size)
{
    ...
    // spi_master_set_rwaddr(master_port, SPI_MASTER_TX, (uint8_t *) hal_memview_dsp0_to_infrasys((uint32_t)data));
    ...
    if (status != HAL_SPI_MASTER_STATUS_OK) {
        ...
        spi_master_start_transfer_dma(master_port, false, true, true);
    }
                                                            ...
                                                            hal_spi_master_send_and_receive_config_t *spi_send_and_receive_config,bool start)
{
    ...
    if (status != HAL_SPI_MASTER_STATUS_OK) {
        ...
        spi_master_start_transfer_dma(master_port, false, false,start);
    }
                                                            ...
                                                            hal_spi_master_send_and_receive_config_t *spi_send_and_receive_config)
{
    ...
    if (spi_send_and_receive_config->send_data != NULL) {
        spi_master_set_rwaddr(master_port, SPI_MASTER_TX, (uint8_t *) ((uint32_t)spi_send_and_receive_config->send_data));
        // spi_master_set_rwaddr(master_port, SPI_MASTER_TX, (uint8_t *) hal_memview_dsp0_to_infrasys((uint32_t)spi_send_and_receive_config->send_data));
    } else {
        spi_master_set_rwaddr(master_port, SPI_MASTER_TX, 0);
        spi_master_clear_fifo(master_port);
    }
    spi_master_set_rwaddr(master_port, SPI_MASTER_RX, (uint8_t *) ((uint32_t)spi_send_and_receive_config->receive_buffer));
    // spi_master_set_rwaddr(master_port, SPI_MASTER_RX, (uint8_t *) hal_memview_dsp0_to_infrasys((uint32_t)spi_send_and_receive_config->receive_buffer));
    spi_master_set_mode(master_port, SPI_MASTER_RX, SPI_MASTER_MODE_DMA);
    spi_master_set_mode(master_port, SPI_MASTER_TX, SPI_MASTER_MODE_DMA);
    spi_master_set_interrupt(master_port, true);
    status = spi_master_analyse_transfer_size(master_port, spi_send_and_receive_config->receive_length);
...
#endif
        g_spi_master_direction[master_port] = SPI_MASTER_RX;
        spi_master_start_transfer_dma(master_port, false, false,true);
    }
...
hal_spi_master_status_t hal_spi_master_fifo_access(hal_spi_master_port_t master_port, bool is_write, uint32_t *data, uint32_t size)
{
    if (master_port >= HAL_SPI_MASTER_MAX) {
        return HAL_SPI_MASTER_STATUS_ERROR_PORT;
    }
    if (data == NULL || size == 0 || size > HAL_SPI_MAXIMUM_POLLING_TRANSACTION_SIZE || ((((uint32_t)data) % 4) != 0)) {
        return HAL_SPI_MASTER_STATUS_INVALID_PARAMETER;
    }
