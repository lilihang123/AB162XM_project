...
//#define SPIS_NVIC_ENABLED
//#define SPIS_SLEEP_MANAGER_ENABLED
#ifdef SPIS_NVIC_ENABLED
static const IRQn_Type g_spi_slave_irq_code[HAL_SPI_SLAVE_MAX] = {0xFF};
static const uint16_t g_spi_slave_pri_code[HAL_SPI_SLAVE_MAX] = {0xFF};
...
hal_spi_slave_status_t hal_spi_slave_init(hal_spi_slave_port_t spi_port, hal_spi_slave_config_t *spi_configure)
{
    ...
    if (HAL_CLOCK_STATUS_OK != clock_mux_sel(g_spi_slave_cg_sel[spi_port], 0)) {
    ...
    sleep_management_register_infra_pd_suspend_callback(INFRA_PD_BACKUP_RESTORE_SPI_SLV, (sleep_management_suspend_callback_t)spi_slave_backup_register_callback, NULL);
    sleep_management_register_infra_pd_resume_callback(INFRA_PD_BACKUP_RESTORE_SPI_SLV, (sleep_management_resume_callback_t)spi_slave_restore_register_callback, NULL);
#endif
