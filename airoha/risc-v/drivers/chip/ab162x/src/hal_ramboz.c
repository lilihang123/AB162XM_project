...
#define RAMBOZ_RISCV_IRQ_NUM 11
...
hal_ramboz_status_t hal_ramboz_init(hal_ramboz_config_t *ramboz_cfg)
{
    ...
    // nvic_status = hal_nvic_register_isr_handler(RAMBOZ_IRQn, (void *)hw_ramboz_Isr);
    hal_nvic_register_isr_handler(RAMBOZ_RISCV_IRQ_NUM, (void *)hw_ramboz_Isr_zephyr);
    // if (nvic_status != HAL_NVIC_STATUS_OK)
    // {
    //     return HAL_RAMBOZ_STATUS_ERROR;
    // }
    hal_nvic_enable_irq(RAMBOZ_RISCV_IRQ_NUM);
