...
void uart_receive_handler(hal_uart_port_t uart_port, bool is_timeout)
{
    ...
    // vdma_get_available_receive_bytes(channel, &rcv_bytes);
    if (status != VDMA_OK) {
        // UART_ASSERT();
        return;
    }
...
{
    sleep_management_register_suspend_callback(SLEEP_BACKUP_RESTORE_UART, (sleep_management_resume_callback_t)uart_restore_all_registers, NULL);
}
#if 0
...
void uart_restore_all_registers(void)
{
    ...
    // internal_vdma_clock_control(VDMA_UART0TX, true); // UART0 TX VDMA_CLOCK enable
    // internal_vdma_clock_control(VDMA_UART0RX, true); // UART0 RX VDMA_CLOCK enable
    // internal_vdma_clock_control(VDMA_UART1TX, true); // UART1 TX VDMA_CLOCK enable
    // internal_vdma_clock_control(VDMA_UART1RX, true); // UART1 RX VDMA_CLOCK enable
    // internal_vdma_clock_control(VDMA_UART2TX, true); // UART2 TX VDMA_CLOCK enable
    // internal_vdma_clock_control(VDMA_UART2RX, true); // UART2 RX VDMA_CLOCK enable

    // internal_vdma_irq_control(VDMA_UART0TX, true);  // UART0 TX VDMA IRQ enable
    // internal_vdma_irq_control(VDMA_UART0RX, true);  // UART0 RX VDMA IRQ enable
    // internal_vdma_irq_control(VDMA_UART1TX, true);  // UART1 TX VDMA IRQ enable
    // internal_vdma_irq_control(VDMA_UART1RX, true);  // UART1 RX VDMA IRQ enable
    // internal_vdma_irq_control(VDMA_UART2TX, true);  // UART2 TX VDMA IRQ enable
    // internal_vdma_irq_control(VDMA_UART2RX, true);  // UART2 RX VDMA IRQ enable
