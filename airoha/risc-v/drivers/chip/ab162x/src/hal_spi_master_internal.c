...
volatile SPIM_REGISTER_T *const g_spi_master_register[HAL_SPI_MASTER_MAX] = {(SPIM_REGISTER_T *)SPI_MASTER_0_BASE, (SPIM_REGISTER_T *)SPI_MASTER_1_BASE};
...
void spi_master_isr_handler(hal_spi_master_port_t master_port, hal_spi_master_callback_t user_callback, void *user_data)
{
    ...
    if (interrupt_status & SPIM_INT_FINISH_INT_MASK) {
        ...
        //spi_master_start_transfer_dma(master_port, true, false,true);
    }
}
...
void spi_master_init(hal_spi_master_port_t master_port, const hal_spi_master_config_t *spi_config)
{
    ...
    switch (spi_config->bit_order) {
        case HAL_SPI_MASTER_LSB_FIRST:
            g_spi_master_register[master_port]->spimst_ctrl0.field.SPIM_CTRL0 &= (~(SPIM_CTRL0_TXMSBF_MASK | SPIM_CTRL0_RXMSBF_MASK));
            break;
    ...
    switch (spi_config->polarity) {
        case HAL_SPI_MASTER_CLOCK_POLARITY0:
            g_spi_master_register[master_port]->spimst_ctrl0.field.SPIM_CTRL0 &= (~SPIM_CTRL0_CPOL_MASK);
            break;
    ...
    switch (spi_config->phase) {
        case HAL_SPI_MASTER_CLOCK_PHASE0:
            g_spi_master_register[master_port]->spimst_ctrl0.field.SPIM_CTRL0 &= (~SPIM_CTRL0_CPHA_MASK);
            break;
    ...
    g_spi_master_register[master_port]->spimst_ctrl0.field.SPIM_CTRL0 |= HAL_SPI_MASTER_SINGLE_MODE << SPIM_CTRL0_TYPE_OFFSET;
}
...
void spi_master_set_advanced_config(hal_spi_master_port_t master_port, const hal_spi_master_advanced_config_t *advanced_config)
{
    switch (advanced_config->byte_order) {
        case HAL_SPI_MASTER_LITTLE_ENDIAN:
            g_spi_master_register[master_port]->spimst_ctrl1.field.SPIM_CTRL1 &= ~(SPIM_CTRL1_ENDIAN_MASK);
            break;
    ...
    switch (advanced_config->chip_polarity) {
        case HAL_SPI_MASTER_CHIP_SELECT_LOW:
            g_spi_master_register[master_port]->spimst_ctrl1.field.SPIM_CTRL1 &= (~SPIM_CTRL1_CS_POL_MASK);
            break;
    ...
    switch (advanced_config->sample_select) {
        case HAL_SPI_MASTER_SAMPLE_POSITIVE:
            g_spi_master_register[master_port]->spimst_ctrl1.field.SPIM_CTRL1 &= (~SPIM_CTRL1_SAMPLE_SEL_MASK);
            break;
...
hal_spi_master_status_t spi_master_push_data(hal_spi_master_port_t master_port, const uint8_t *data, uint32_t size, uint32_t total_size)
{
    ...
    if ((g_spi_master_register[master_port]->spimst_ctrl1.field.SPIM_CTRL1) & SPIM_CTRL1_TX_ENDIAN_MASK) {
...
hal_spi_master_status_t spi_master_pop_data(hal_spi_master_port_t master_port, uint8_t *buffer, uint32_t size)
{
    ...
    /* if byte_order setting is big_endian, return error */
    if ((g_spi_master_register[master_port]->spimst_ctrl1.field.SPIM_CTRL1) & SPIM_CTRL1_RX_ENDIAN_MASK) {
...
void spi_master_set_interrupt(hal_spi_master_port_t master_port, bool status)
{
    switch (status) {
        case false:
            g_spi_master_register[master_port]->spimst_cfg4.word = (~(SPIM_IE_FINISH_IE_MASK | SPIM_IE_PAUSE_IE_MASK));
            break;
        case true:
            g_spi_master_register[master_port]->spimst_cfg4.word = (SPIM_IE_FINISH_IE_MASK | SPIM_IE_PAUSE_IE_MASK);
            break;
...
void spi_master_start_transfer_fifo(hal_spi_master_port_t master_port, bool is_write)
{
    ...
    if (is_write == true) {
        g_spi_master_register[master_port]->spimst_ctrl0.field.SPIM_CTRL0 |= (1 << SPIM_CTRL0_RW_MODE_OFFSET);
    } else {
        g_spi_master_register[master_port]->spimst_ctrl0.field.SPIM_CTRL0 &= ~(1 << SPIM_CTRL0_RW_MODE_OFFSET);
    }
...
void spi_master_start_transfer_dma(hal_spi_master_port_t master_port, bool is_continue, bool is_write,bool start)
{
    ...
    if (g_full_packet_count[master_port] > 0) {
        g_spi_master_register[master_port]->spimst_cfg2.field.SPIM_PACKET_LOOP_CNT = (uint8_t)SPI_MASTER_MAX_PACKET_COUNT_MASK;
        g_spi_master_register[master_port]->spimst_cfg2.field.SPIM_PACKET_LENGTH_CNT = SPI_MASTER_MAX_PACKET_LENGTH_MASK;
...
void spi_master_start_transfer_dma_blocking(hal_spi_master_port_t master_port, bool is_write)
{
    ...
    do {
        ...
        while ((g_spi_master_register[master_port]->spimst_sta.field.SPIM_BUSY) == SPIM_STATUS_BUSY_MASK);
        do {
...
void spi_master_set_delay_cnt(hal_spi_master_port_t master_port, uint16_t delay_cnt,uint8_t one_location_cnt,bool is_multy){
...
hal_spi_master_status_t spi_master_set_clear_data(hal_spi_master_port_t master_port, uint8_t cmd_cnt, bool is_high, bool enable){
...
void spi_master_set_bt_enable(hal_spi_master_port_t master_port, bool bt_enable){

    g_spi_master_register[master_port]->spimst_cfg3.field.SPIM_BT_HW_EN = bt_enable;
}

void spi_master_trigger_start(hal_spi_master_port_t master_port,bool start){
...
{
    g_spi_master_register[master_port]->spimst_ctrl0.field.SPIM_CTRL0 &= ~(SPIM_CTRL0_TYPE_MASK);
    g_spi_master_register[master_port]->spimst_ctrl0.field.SPIM_CTRL0 |= mode << SPIM_CTRL0_TYPE_OFFSET;
}
...
//         g_spi_master_ctrl_0_reg[master_port] = g_spi_master_register[master_port]->spimst_ctrl0.word;
//         g_spi_master_ctrl_1_reg[master_port] = g_spi_master_register[master_port]->spimst_ctrl1.word;
//         g_spi_master_cfg_0_reg[master_port] = g_spi_master_register[master_port]->spimst_cfg0.word;
//         g_spi_master_cfg_1_reg[master_port] = g_spi_master_register[master_port]->spimst_cfg1.word;
//         g_spi_master_cfg_2_reg[master_port] = g_spi_master_register[master_port]->spimst_cfg2.word;
//         g_spi_master_cfg_3_reg[master_port] = g_spi_master_register[master_port]->spimst_cfg3.word;
//         for (i = 0; i <= 9; i++) {
//             g_spi_master_register[master_port]->spimst_cfg4.field.SPIM_SEL_ADDR = i;
//             g_spi_master_cfg_4_reg[master_port][i] = g_spi_master_register[master_port]->spimst_cfg4.field.SPIM_SEL_RDATA;
//         }
//     }
// }
...
//         g_spi_master_register[master_port]->spimst_ctrl0.word = g_spi_master_ctrl_0_reg[master_port];
//         g_spi_master_register[master_port]->spimst_ctrl1.word = g_spi_master_ctrl_1_reg[master_port];
//         g_spi_master_register[master_port]->spimst_cfg0.word = g_spi_master_cfg_0_reg[master_port];
//         g_spi_master_register[master_port]->spimst_cfg1.word = g_spi_master_cfg_1_reg[master_port];
//         g_spi_master_register[master_port]->spimst_cfg2.word = g_spi_master_cfg_2_reg[master_port];
//         g_spi_master_register[master_port]->spimst_cfg3.word = g_spi_master_cfg_3_reg[master_port];
//         for (i = 0; i <= 9; i++) {
//             g_spi_master_register[master_port]->spimst_cfg4.field.SPIM_SEL_ADDR = i;
//             g_spi_master_register[master_port]->spimst_cfg4.field.SPIM_SEL_WDATA = g_spi_master_cfg_4_reg[master_port][i];
//         }
//     }
// }
#endif
...
//#include "syslog.h"
/* user should gurant the buffer is 4byte aligned */
void spi_master_config_fifo(uint8_t master_port, bool is_tx, uint32_t *buff, uint32_t size)
{
    int i = 0;
    int loop_cnt = 0;
    uint32_t temp = 0;
    ...
    if (is_tx) {
        ...
        for (i = 0; i < loop_cnt; i++) {
            //LOG_MSGID_I(common,"[hal][spim] risc-v write data: 0x%08x\r\n", 1, (unsigned int)buff[i]);
            g_spi_master_register[master_port]->spimst_tx_data.word = buff[i];
        }
    } else {
        for (i = 0; i < loop_cnt; i++) {
            buff[i] = g_spi_master_register[master_port]->spimst_rx_data.word;
            //LOG_MSGID_I(common,"[hal][spim] risc-v read data: 0x%08x\r\n", 1, (unsigned int)buff[i]);
        }
    }
}
