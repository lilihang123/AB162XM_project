...
#define HAL_HW_KEYSCAN_MODULE_DUMMY_IRQ_ENABLED
...
#define KEYSCAN_RISCV_INT_NUM 16
...
uint32_t  key_data;
#ifdef HAL_HW_KEYSCAN_MODULE_DUMMY_IRQ_ENABLED
static void hw_keyscan_dummy_Isr(void)
{
    keyscan_register->SFR_IRQ_CLEAR = 0x01;//clearing this irq takes time and should not be palced at the end of the isr,as this may cause irq errors to recur
...
static void hw_keyscan_Isr(void)
{
    ...
    LOG_MSGID_I(common, "hw_keyscan_Isr", 0);
#endif
    while (!(keyscan_register->AFIFO_STATUS_UNION.AFIFO_STATUS_CELLS.AFIFO_EMPTY))
    {
        volatile uint8_t available_data_num = keyscan_register->AFIFO_STATUS_UNION.AFIFO_STATUS_CELLS.AFIFO_DNUM;
#ifdef KEYSCAN_DEBUG_LOG
    LOG_MSGID_I(common, "available_data_num, %d", 1, available_data_num);
    LOG_MSGID_I(common, "one_packet_valid_num, %d", 1, one_packet_valid_num);
#endif
        ...
        else
        {
            ...
            if (scan_stop_flag)
            {
                ...
                for (i = 0; i < available_data_num; i++)
                {
                    ...
                    key_fifo_invalid_data += key_fifo_invalid_data;
                    key_fifo_invalid_data = 0;
                }
                break;
            }
            else
...
#ifdef KEYSCAN_DEBUG_LOG
    LOG_MSGID_I(common, "hw_keyscan_Isr-out", 0);
#endif   
...
#ifdef HAL_HW_KEYSCAN_MODULE_DUMMY_IRQ_ENABLED
    hal_nvic_register_isr_handler(KEYSCAN_RISCV_INT_NUM, (void *)hw_keyscan_dummy_Isr);
#else
    hal_nvic_register_isr_handler(KEYSCAN_RISCV_INT_NUM, (void *)hw_keyscan_Isr);
#endif
...
//     }
    ...
    keyscan_register->SFR_IRQ_CLEAR = 0x01;
    hal_nvic_enable_irq(KEYSCAN_RISCV_INT_NUM);
...
#ifdef KEYSCAN_DEBUG_LOG
    LOG_MSGID_I(common, "hal_hw_keyscan_enable success", 0);
#endif
...
hal_hw_keyscan_status_t hal_hw_keyscan_disable(void)
{
    ...
    hw_keyscan_status = KEYSCAN_HWSTATUS_INITIALIZED;
#ifdef KEYSCAN_DEBUG_LOG
    LOG_MSGID_I(common, "hal_hw_keyscan_disable success", 0);
#endif
    return HAL_HW_KEYSCAN_STATUS_OK;
}
...
#ifdef KEYSCAN_DEBUG_LOG
    LOG_MSGID_I(common, "hal_hw_keyscan_register_callback success", 0);
#endif
    return HAL_HW_KEYSCAN_STATUS_OK;
}
