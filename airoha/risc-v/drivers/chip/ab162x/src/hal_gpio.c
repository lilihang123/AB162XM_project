...
hal_gpio_status_t hal_gpio_set_rsel_register(hal_gpio_pin_t gpio_pin, uint8_t gpio_rsel0, uint8_t gpio_rsel1)
{
    ...
    io_cfg_base = (gpio_pin <= 22) ? IO_CFG_0_BASE : IO_CFG_1_BASE;
    rsel_shift = ((gpio_pin == 4) || (gpio_pin == 38)) ? 0 : 2;     //gpio4/5/38/39

    GPIO_REG32(io_cfg_base + 0x80 + shift_addr[gpio_rsel0 ? 1 : 0])  = rsel_shift;
    GPIO_REG32(io_cfg_base + 0x80 +  shift_addr[gpio_rsel1 ? 1 : 0]) = rsel_shift << 1;
