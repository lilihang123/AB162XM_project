...
hal_gpt_status_t hal_gpt_init(hal_gpt_port_t gpt_port)
{
    ...
    GPTGLB->GPT_IRQMSK1    &= ~(1 << gpt_port); //risc-v IRQ enable
    GPTGLB->GPT_WAKEUPMSK1 &= ~(1 << gpt_port); //risc-v wakeup enable
...
#ifdef FPGA_ENV
            gpt_start_free_run_timer(GPT(HAL_GPT_US_PORT), GPT_CLOCK_16MHZ, GPT_DIVIDE_16);
#else
...
#ifdef FPGA_ENV
    GPT(gpt_port)->GPT_CLK       = GPT_CLOCK_16MHZ | (uint32_t)GPT_DIVIDE_16;
    GPT(gpt_port)->GPT_COMPARE   = (timeout_time_ms * 1000);
#else

    /*note:default clock source is 16M,clear GPT_CLR will take two cycles of 16M,if clock source is 32K,it will
     *take two cycles of 32K,so we need to switch clock to 16M to save more time.
     */
    GPT(gpt_port)->GPT_CLK = GPT_CLOCK_16MHZ | (uint32_t)GPT_DIVIDE_1;
...
hal_gpt_status_t hal_gpt_start_timer_us(hal_gpt_port_t gpt_port, uint32_t timeout_time_us, hal_gpt_timer_type_t timer_type)
{
    ...
    //set to 16MHz clock and 16 division, clear counter,1 us per tick
    GPT(gpt_port)->GPT_CLK       = GPT_CLOCK_16MHZ | (uint32_t)GPT_DIVIDE_16;
...
uint8_t g_sw_gpt_running_note[SW_GPT_MAX_TYPE] = {HAL_SW_GPT_MAX_USERS, HAL_SW_GPT_MAX_USERS};
