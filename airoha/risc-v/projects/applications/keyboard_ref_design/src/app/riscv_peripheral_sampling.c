...
void peripheral_sampling_inactivity_timeout(void* user_data)
{
    ...
    riscv_app_ccni_send((uint8_t*)msg, 2);
...
void peripheral_sampling_handler(T_PS_CB_TYPE_E ps_cb_type, T_OUTGOING_VALUES_S* outgoing_data)
{
    switch(ps_cb_type)
    {
        ...
        {
#if 0
            if ((riscv_app.state == APP_STATE_2_4G_CONNECTED_ACTIVE ) \
...
#ifdef AIR_KEYBOARD_M607
            /* trigger MCU sampling data */
            // hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M , &riscv_app.timing_sync_active_timestamp);
            hal_gpio_set_output(TIMING_SYNC_IO, HAL_GPIO_DATA_LOW);
#endif
        }
        break;
        ...
        case HID_EXP_2ND_CB:
        {
            ...
            {
                /* calc timing, wait signal to trigger SPI communication */
            }
        }
        break;
        ...
        case HID_EXP_3RD_CB_GET_KEY__TIMING_CRITICAL:
        {
            ...
            else
            {
                ...
                riscv_keyboard_key_get_key_status(&(outgoing_data->kb_nkey_map_addr));
                /* push the new key status */
                // extern uint32_t timestamp_spi_cb_start;
                // extern T_KEY_QUEUE_CTRL_S kCtrl_k;
                // riscv_key_push(&kCtrl_k, outgoing_data, timestamp_spi_cb_start);
                // /* pop the oldest key in queue temperory, if ok this key is confired in the begining of the next process time */
                // if (riscv_key_pop_temperory(&kCtrl_k, outgoing_data, timestamp_spi_cb_start) == STATUS_SUCCESS)
                // {
...
uint32_t peripheral_init_count[2] = {0, 0};
void peripheral_sampling_init_bt_trigger(void)
{
#if defined(AIR_KEYBOARD_M607)
    peripheral_init_count[0]++;
    /* set hw keyscan align bt trigger, dummy keyscan */
    ...
    if(ret != HAL_HW_KEYSCAN_STATUS_OK) {
        HID_DBG_LOG(riscv_ps, "hal_hw_keyscan_init failed-%d, %d, %d", 3, ret,
            peripheral_init_count[0],
            peripheral_init_count[1]
        );
    }
    ret = hal_hw_keyscan_enable();
    if(ret != HAL_HW_KEYSCAN_STATUS_OK) {
        HID_DBG_LOG(riscv_ps, "hal_hw_keyscan_init failed-%d", 1, ret);
    }
#else
...
void peripheral_sampling_deinit_bt_trigger(void)
{
#if defined(AIR_KEYBOARD_M607)
...
int peripheral_sampling_init()
{
    ...
    peripheral_sampling_init_bt_trigger();

    /*register critical path callback for data processing */
...
void peripheral_sampling_deinit()
{
    ...
    /* peripherals deinit */
    peripheral_sampling_deinit_bt_trigger();
}
...
{
    riscv_keyboard_key_debug_print();
}
...
bool peripheral_sampling__check_condition(uint32_t check_type)
{
    switch(check_type)
    {
