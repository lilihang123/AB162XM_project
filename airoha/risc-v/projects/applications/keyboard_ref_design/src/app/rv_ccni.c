...
void rv_sleep_lock_request(uint32_t lock)
{
    ...
    if (lock  && (risv_app_sleep_lock_status == false)) {
        ...
        LOG_MSGID_I(riscv_ps, "rv_sleep_lock_request hal_sleep_manager_lock_sleep = %d", 1, ret);
    }
        ...
        LOG_MSGID_I(riscv_ps, "rv_sleep_lock_request hal_sleep_manager_unlock_sleep", 1, ret);
    }
    else {
        LOG_MSGID_I(riscv_ps, "rv_sleep_lock_request  Ignored request, lock req = %d, risv_app_sleep_lock_status = %d"
            , 2
            , lock
...
{
    g_riscv_ccni_irq_occur = true;

    uint8_t * raw_msg = (uint8_t *)msg;

    LOG_MSGID_I(riscv_ps, "riscv_app_ccni_event_handler msg_id = 0x%02X"
        , 1
        , raw_msg[0]
        );
    ...
    switch(raw_msg[0])
    {
        case HID_CCNI_MSG_APP_STATE: // app_state
        {
            ...
            LOG_MSGID_I(riscv_ps, "======== app_state = 0x%x", 1, app_state);
            switch(app_state)
            {
        ...
        {
            LOG_MSGID_I(riscv_ps, "======== fake report request  enable = %d, pattern = %d, rounds = %d \n"
                , 3
                , raw_msg[1]
                , raw_msg[2]
                , raw_msg[3]
        ...
        {
            hid_experss_bt_set_attr_handle(raw_msg[1], raw_msg[2], raw_msg[3], raw_msg[4]);
        }
        break;
        #endif
...
void riscv_app_ccni_init()
{
    hal_ccni_unmask_event(APP_CCNI_EVENT_TO_RISCV);
}

hal_ccni_status_t riscv_app_ccni_send(uint8_t* payload, uint8_t len)
{
    ...
    status = hal_ccni_set_event(APP_CCNI_EVENT_TO_CM33, &msg);
    ...
    if(status == HAL_CCNI_STATUS_BUSY)
    {
        ...
        while((data == HAL_CCNI_EVENT_STATUS_BUSY) && (retry_cnt < MAX_CCNI_STATUS_RETRY_CNT)){
            retry_cnt++;
            hal_ccni_query_event_status(APP_CCNI_EVENT_TO_CM33, &data);
        }
        assert( (retry_cnt < MAX_CCNI_STATUS_RETRY_CNT) && "Error : RISC-V busy for too long");
        HID_DBG_LOG(riscv_ps,"riscv_app_ccni_send busy retry count = %d", 1, retry_cnt);
...
void riscv_app_ccin_fake_data_disable()
{
    ...
    riscv_app_ccni_send(msg, sizeof(msg));
}
...
void riscv_app_ccni_send_log_data_ptr()
{
    ...
    riscv_app_ccni_send((uint8_t*)&msg, sizeof(T_LOG_REQ_S));
...
void riscv_app_ccni_key_remap_send(uint32_t status, uint32_t attribute)
{
    ...
    // HID_DBG_LOG(key_remap, "riscv_app_ccni_key_remap_send status = %d, attribute = 0x%04X", status, attribute);
}
#endif
