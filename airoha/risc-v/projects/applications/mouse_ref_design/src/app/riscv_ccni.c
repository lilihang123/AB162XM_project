...
void rv_sleep_lock_request(uint32_t lock)
{
    ...
    LOG_MSGID_I(riscv_ps, "rv_sleep_lock_request, lock req = %d, risv_app_sleep_lock_status = %d, ret = %d"
        , 3
        , lock
...
void CCNI_DEF_HANDLER(3)(hal_ccni_event_t event, hal_ccni_message_t *msg)
{
    ...
    switch(raw_msg[0])
    {
        ...
        {
            // HID_DBG_LOG(rv_exp, "HID_CCNI_MSG_KEY, value_type = %d, value = 0x%X" , 2, (raw_msg[1]), (*(uint32_t*)&raw_msg[2]));
