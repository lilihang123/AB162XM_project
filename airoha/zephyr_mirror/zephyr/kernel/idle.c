 ...
 */
 ...
 /* Modifications:
  * 2025-12-19 by Airoha - Add Airoha pure WFI
  */
...
#define  MAX_NVS_WAIT_TIME_MS  100
...
void idle(void *unused1, void *unused2, void *unused3)
{
	...
	while (true) {
#if defined(CONFIG_AIR_NVKEY_GC_IN_IDLE_ENABLE) && defined(CONFIG_AIR_NVKEY_ENABLE)
		int32_t sleep_time_ms  = 0;

		sleep_time_ms = z_get_next_timeout_expiry();
		if (sleep_time_ms > MAX_NVS_WAIT_TIME_MS) {
			int free_sector = nvkey_get_free_sector();
			if(free_sector < 3) {
				printk("[idle] system triggered NVS_GC, free sector %d, sleep %dms", free_sector, sleep_time_ms);
				nvkey_trigger_gc(4);
			}
		}
#endif
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
		/* Airoha Pure WFI Mode */
		if(hal_sleep_manager_is_idle_locked())
		{
			z_arm_on_enter_cpu_idle_prepare();
			/* enter pure wfi and do not disable irq for better wakeup performance */
			__asm volatile("dsb");
			__asm volatile("wfi");
			__asm volatile("isb");
			continue;
