...
2026-03-26 by Airoha - Fix PA event counter report.
*/
...
#define DEFAULT_EVENT_MASK           0xffffffffffffffff

#define DEFAULT_EVENT_MASK_PAGE_2    0x0
//#define DEFAULT_LE_EVENT_MASK 0x1f
#define DEFAULT_LE_EVENT_MASK        0xffffffffffffffff
...
static void le_ltk_req_reply(struct net_buf *buf, struct net_buf **evt)
{
	...
	// printk LTK
	 BT_LOG_MSGID_I("LTK: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n", 16,
			cmd->ltk[0], cmd->ltk[1], cmd->ltk[2], cmd->ltk[3],
			cmd->ltk[4], cmd->ltk[5], cmd->ltk[6], cmd->ltk[7],
			cmd->ltk[8], cmd->ltk[9], cmd->ltk[10], cmd->ltk[11],
			cmd->ltk[12], cmd->ltk[13], cmd->ltk[14], cmd->ltk[15]);
...
static void le_set_periodic_sync_subevent(struct net_buf *buf, struct net_buf **evt)
{
    ...
    if ((cmd->sync_handle > 0x0EFF) || (cmd->num_subevents == 0) || (cmd->num_subevents > 0x80)) {
        *evt = cmd_status(BT_HCI_ERR_INVALID_PARAM);
    }
    handle = sys_le16_to_cpu(cmd->sync_handle);
    sync = ull_sync_is_enabled_get(handle);
    if (!sync) {
        status = BT_HCI_ERR_UNKNOWN_ADV_IDENTIFIER;
	    *evt = cmd_complete_status(status);
        return;
    }

    lll_sync = &sync->lll;
    /*err = lll_sync_wr_response_data_init(&lll_sync->rsp_data);
    if (err) {
        status = BT_HCI_ERR_MEM_CAPACITY_EXCEEDED;
        *evt = cmd_complete_status(status);
        return;
    }
    lll_sync_wr_response_data_reset(&lll_sync->rsp_data);*/
    //Copy param
    //sync->pawr_sync_subevent.DataState = PAWR_DATA_USED;
	//cmd->num_subevents = 1;
	//cmd->subevents[0] = 1;
    sync->pawr_sync_subevent.sync_handle = sys_le16_to_cpu(cmd->sync_handle);
    sync->pawr_sync_subevent.properties = cmd->periodic_adv_properties;

	for (i = 0; i < cmd->num_subevents; i++) {
		if (cmd->subevents[i] != 0) {
			sync->pawr_sync_subevent.subevents[i + 1] = cmd->subevents[i]; //subevents[0] is alwyas asigned for subevent#0
			sync->pawr_sync_subevent.num_subevents++;
			BT_LOG_MSGID_I("set sync subevent: %d, num_subevents: %d\n", 2, cmd->subevents[i], sync->pawr_sync_subevent.num_subevents);
		}
	}

	if (sync->pawr_sync_subevent.num_subevents > 1) {
		//start ticker by sync subevent
		ull_sync_rsp_start(sync, sync->pawr_timing.persync_ticks_anchor);
	}

	*evt = cmd_status(BT_HCI_ERR_SUCCESS);
}

static void le_set_periodic_sync_response_data(struct net_buf *buf, struct net_buf **evt)
{
    ...
    if (cmd->response_data_length > MAX_PAWR_SUBEVENT_RSP_DATA_LEN) {
        status = BT_HCI_ERR_PACKET_TOO_LONG;
	    *evt = cmd_complete_status(status);
        return;        
    }
    //set rsp param
    //cmd->response_slot = 8;
    sync->pawr_sync_rsp_param.data_state = PAWR_DATA_USED;
    sync->pawr_sync_rsp_param.sync_handle = handle;
    sync->pawr_sync_rsp_param.req_event = cmd->request_event;
    sync->pawr_sync_rsp_param.req_subevent = cmd->request_subevent;
    sync->pawr_sync_rsp_param.rsp_subevent = cmd->response_subevent;
    sync->pawr_sync_rsp_param.rsp_slot = cmd->response_slot;
    sync->pawr_sync_rsp_param.rsp_data_length = cmd->response_data_length;
    //memcpy(sync->pawr_sync_rsp_param.rsp_data+0, cmd->response_data+0, sync->pawr_sync_rsp_param.rsp_data_length);//TODO: need store rsp_data?
    BT_LOG_MSGID_I("set rsp data, req_event: %d, req_subevent: %d, rsp_subevent: %d, rsp_slot: %d, rsp_data_length: %d", 5, cmd->request_event, cmd->request_subevent, cmd->response_subevent, cmd->response_slot, cmd->response_data_length);
	//BT_LOG_MSGID_I("data: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x", 6, cmd->response_data[0], cmd->response_data[1], cmd->response_data[2], cmd->response_data[3], cmd->response_data[4], cmd->response_data[5]);
	sync->lll.rsp_data_len = cmd->response_data_length;
    memcpy(sync->lll.rsp_data+0, cmd->response_data+0, cmd->response_data_length);//TODO: need store rsp_data?    
    //set rsp data
	status = ull_sync_rsp_data_set(handle, cmd->response_data_length, cmd->response_data);
...
static void le_per_adv_sync_transfer_received(struct pdu_data *pdu_data_rx,
					      struct node_rx_pdu *node_rx, struct net_buf *buf, bool is_pawr)
{
	struct bt_hci_evt_le_past_received *sep;
    struct bt_hci_evt_le_past_received_v2 *sep_v2;
	struct node_rx_past_received *se;
	struct ll_sync_set *sync;
	void *node;
	BT_LOG_MSGID_I("is_pawr: %d", 1, is_pawr);
    if (!is_pawr) {//PA
        //BT_LOG_MSGID_I("le_per_adv_sync_transfer_received", 0);
    	if (!(event_mask & BT_EVT_MASK_LE_META_EVENT) ||
        ...
        //////PAwR
    	/*if (!(event_mask & BT_EVT_MASK_LE_META_EVENT) ||
    	    !(le_event_mask & BT_EVT_MASK_LE_PAST_RECEIVED_V2)) {
    		return;
    	} */
        LOG_DBG("le_per_adv_rsp_sync_transfer_received v2\n");
    	sep_v2 = meta_evt(buf, BT_HCI_EVT_LE_PAST_RECEIVED_V2, sizeof(*sep_v2));
    	...
    	se = node;
    	sep_v2->status = se->rx_sync.status;
    
    	sync = node_rx->rx_ftr.param;
    
    	/* Resolved address, if private, has been populated in ULL */
    	sep_v2->addr.type = sync->peer_id_addr_type;
    	if (sync->peer_addr_resolved) {
    		/* Mark it as identity address from RPA (0x02, 0x03) */
    		MARK_AS_IDENTITY_ADDR(sep_v2->addr.type);
    	}
    	(void)memcpy(sep_v2->addr.a.val, sync->peer_id_addr, BDADDR_SIZE);
    
    	sep_v2->adv_sid = sync->sid;
    	sep_v2->phy = find_lsb_set(se->rx_sync.phy);
    	sep_v2->interval = sys_cpu_to_le16(se->rx_sync.interval);
    	sep_v2->clock_accuracy = se->rx_sync.sca;
    	sep_v2->conn_handle = sys_cpu_to_le16(se->conn_handle);
    	sep_v2->service_data = sys_cpu_to_le16(se->service_data);
    	sep_v2->sync_handle = sys_cpu_to_le16(node_rx->hdr.handle);
        sep_v2->num_subevents = se->rx_sync.num_subevent;
        sep_v2->subevent_interval = se->rx_sync.subevent_interval;
        sep_v2->response_slot_delay = se->rx_sync.rsp_slot_delay;
        sep_v2->response_slot_spacing = se->rx_sync.rsp_slot_spacing;
    }
}

#if defined(CONFIG_BT_CTLR_SYNC_PERIODIC_RSP_PRO)
static void le_per_adv_sync_tailord_data_received(struct pdu_data *pdu_data_rx,
					      struct node_rx_pdu *node_rx, struct net_buf *buf)
{
	struct bt_hci_evt_le_air_pa_tailored_data_received *data_rcvd;
	struct node_rx_pa_tailored_data_received *se;
	struct ll_sync_set *sync;
	void *node;
	LOG_DBG("le_per_adv_sync_tailord_data_received\n");
	data_rcvd = vs_event(buf, BT_HCI_EVT_LE_AIR_PA_TAILORED_DATA_RECEIVED_COMPLETE, sizeof(*data_rcvd));
    node = pdu_data_rx;
    LL_ASSERT(IS_PTR_ALIGNED(node, struct node_rx_past_received));
	se = node;

	data_rcvd->status = se->status;
	data_rcvd->id = se->id;
	data_rcvd->packet_num = se->packet_num;
	data_rcvd->subevent = se->subevent;
}
#endif
				 ...
				 struct net_buf **evt, void **node_rx)
{
    ...
    case BT_OCF(BT_HCI_OP_LE_SET_PER_ADV_SYNC_SUBEVENT):
        le_set_periodic_sync_subevent(cmd, evt);
        break;
    case BT_OCF(BT_HCI_OP_LE_SET_PER_ADV_RESPONSE_DATA):
        le_set_periodic_sync_response_data(cmd, evt);
        break;
#endif
	default:
		return -EINVAL;
...
#if defined(CONFIG_BT_CTLR_SYNC_PERIODIC_RSP)
static void le_per_adv_sync_wr_report(struct pdu_data *pdu_data,
				   struct node_rx_pdu *node_rx,
				   struct net_buf *buf)
{
		...
		int8_t rssi;
		bool accept;
		//struct bt_hci_evt_le_per_advertising_report_v2 *sep_v2;

		if (!(event_mask & BT_EVT_MASK_LE_META_EVENT) ||
			(!(le_event_mask & BT_EVT_MASK_LE_PER_ADVERTISING_REPORT_V2))) {
			return;
		}
        ...
        /*Error Handling*/
		if ((le_event_mask & BT_EVT_MASK_LE_PER_ADVERTISING_REPORT_V2) &&
			...
			sep_v2->handle = sys_cpu_to_le16(node_rx->hdr.handle);
			sep_v2->tx_power = BT_HCI_LE_ADV_TX_POWER_NO_PREF;
			sep_v2->rssi = BT_HCI_LE_RSSI_NOT_AVAILABLE;
			sep_v2->cte_type = BT_HCI_LE_NO_CTE;
            sep_v2->periodic_event_counter = 0;
            sep_v2->subevent = 0;
			sep_v2->data_status = BT_HCI_LE_ADV_EVT_TYPE_DATA_STATUS_INCOMPLETE;
			sep_v2->length = 0;
	...
	no_ext_hdr:
		...
		*/
		...
		if ((le_event_mask & BT_EVT_MASK_LE_PER_ADVERTISING_REPORT_V2) && accept) {
			...
			do {
            	...
            	sep_v2 = meta_evt(evt_buf,
            			   BT_HCI_EVT_LE_PER_ADVERTISING_REPORT_V2,
            			   sizeof(*sep_v2) + data_len_frag);
            	
            	sep_v2->handle = sys_cpu_to_le16(node_rx->hdr.handle);
            	sep_v2->tx_power = tx_pwr;
            	sep_v2->rssi = rssi;
            	sep_v2->cte_type = cte_type;
            	sep_v2->periodic_event_counter = sync->lll.event_counter - 1;//PA event counter
            	sep_v2->subevent = sync->pawr_timing.subevent;//PAwR subevent
            	sep_v2->length = data_len_frag;
            	memcpy(&sep_v2->data[0], data, data_len_frag);
				...
				sep_v2->data_status = data_status;
			} while (data_len > 0);
	
			evt_buf = NULL;
		}
		//hal_gpio_set_output(HAL_GPIO_23, 0);//IDS_DEBUG
}
#endif
				   ...
				   struct net_buf *buf)
{
	...
	int8_t rssi;
	bool accept;
    LOG_DBG("le_per_adv_sync_report, aux_failed: %d", ftr->aux_failed);
	if (!(event_mask & BT_EVT_MASK_LE_META_EVENT) ||
			     ...
			     struct net_buf *buf)
{
	...
	BT_LOG_MSGID_I("le_conn_complete, conn_handle: %d, is_from_pawr: %d", 2, handle, cc->is_from_pawr);

    if (cc->is_from_pawr) {
    		struct bt_hci_evt_le_enh_conn_complete_v2 *leecc;
    
    		leecc = meta_evt(buf, BT_HCI_EVT_LE_ENH_CONN_COMPLETE_V2,
    				 sizeof(*leecc));
    		...
    		leecc->supv_timeout = sys_cpu_to_le16(cc->timeout);
    		leecc->clock_accuracy = cc->sca;
			leecc->sync_handle = sys_cpu_to_le16(cc->sync_handle);
			leecc->adv_handle = 0xFF;
    		return;		
    } else {
    #if defined(CONFIG_BT_CTLR_PRIVACY) || defined(CONFIG_BT_CTLR_ADV_EXT)
    	if (le_event_mask & BT_EVT_MASK_LE_ENH_CONN_COMPLETE) {
			   ...
			   struct pdu_data *pdu_data, struct net_buf *buf)
{
	...
	case NODE_RX_TYPE_SYNC_REPORT:
		//le_per_adv_sync_report(pdu_data, node_rx, buf, false);
		le_per_adv_sync_report(pdu_data, node_rx, buf);
		break;
#if defined(CONFIG_BT_CTLR_SYNC_PERIODIC_RSP)
	case NODE_RX_TYPE_SYNC_REPORT_V2:
		//le_per_adv_sync_report(pdu_data, node_rx, buf, true);
		le_per_adv_sync_wr_report(pdu_data, node_rx, buf);
		break;
#endif
	...
	case NODE_RX_TYPE_SYNC_TRANSFER_RECEIVED:
		le_per_adv_sync_transfer_received(pdu_data, node_rx, buf, false);
		return;
    case NODE_RX_TYPE_SYNC_TRANSFER_RECEIVED_V2:
        le_per_adv_sync_transfer_received(pdu_data, node_rx, buf, true);
        return;
#if defined(CONFIG_BT_CTLR_SYNC_PERIODIC_RSP_PRO)
	case NODE_RX_TYPE_AIR_PA_TAILORED_DATA_RECEIVED:
		le_per_adv_sync_tailord_data_received(pdu_data, node_rx, buf);
		return;
#endif
#endif /* CONFIG_BT_CTLR_SYNC_TRANSFER_RECEIVER */
