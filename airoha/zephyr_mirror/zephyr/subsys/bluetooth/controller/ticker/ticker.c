...
2026-03-05 by Airoha - Add Airoha chip tick overflow handling.
*/
...
void ticker_worker(void *param)
{
	...
	*/
		...
		if (ticker->timeout_func) {
        ...
        #if CONFIG_AIR_BT_CLOCK_SCHEDULE_ENABLE
...
#endif /* CONFIG_BT_TICKER_EXT_EXPIRE_INFO */
			{
				DEBUG_TICKER_TASK(1);
					...
					if ((ticker->ticker_id >= TICKER_ID_SCAN_SYNC_WR_BASE) && (ticker->ticker_id <= TICKER_ID_SCAN_SYNC_WR_LAST)) {
						struct ll_sync_set *sync = ticker->context;
						sync->lll.current_subevent = sync->pawr_sync_subevent.subevents[ticker->ticker_id - TICKER_ID_SCAN_SYNC_WR_BASE];
                        sync->pawr_timing.subevent = sync->pawr_sync_subevent.subevents[ticker->ticker_id - TICKER_ID_SCAN_SYNC_WR_BASE];
					}
            #endif
...
//#if defined(CONFIG_BT_CTLR_ADV_PERIODIC_SYNC_RSP)
#if (1)
	ticker->ticker_id = user_op->id;
    //printk("ticker_job_op_start, ticker_id = %d\n", ticker->ticker_id);
#endif
...
void ticker_job(void *param)
{
	...
	if (instance->ticks_elapsed_first != instance->ticks_elapsed_last) {
		...
		instance->ticks_current += ticks_elapsed;
    #if CONFIG_AIR_BT_CLOCK_SCHEDULE_ENABLE
        instance->ticks_current = instance->ticks_current % HAL_TICKER_CNTR_MASK;
    #else
...
uint32_t ticker_ticks_diff_get(uint32_t ticks_now, uint32_t ticks_old)
{
    ...
    if (ticks_now >= ticks_old) {
        ...
        return HAL_TICKER_CNTR_MASK - ticks_old + 1 + ticks_now; 
    }
#else
