...
static void conn_setup_from_pawr_sync(void *param);
...
void ull_conn_setup(memq_link_t *rx_link, struct node_rx_pdu *rx)
{
	...
	//BT_ULL_LOG_MSGID_I("[PAWR_CONN]ull_conn_setup, user_meta: %d",  1, rx_hdr->user_meta);
	if (rx_hdr->user_meta == LE_PERIPHERAL_CONNECTION_FROM_ADVERTISING) {
		if (ull_ref_get(hdr)) {
			/*Connetion from ADV*/
		...
		/*Connection from PAwR Sync*/
		conn_setup_from_pawr_sync(rx);
	}
}
...
void ull_conn_rx(memq_link_t *link, struct node_rx_pdu **rx)
{
	...
	if (!conn) {
        ...
        /* BRX will report test data as acl data with one not exist conn handle. Disable this so that hci can receive BRX data. */
#ifndef CONFIG_BT_CTLR_DTM
...
ATTR_TEXT_IN_TCM void ull_conn_lll_ack_enqueue(uint16_t handle, struct node_tx *tx)
{
	struct lll_tx *lll_tx;
	uint8_t idx;
...
ATTR_TEXT_IN_TCM uint16_t ull_conn_lll_max_tx_octets_get(struct lll_conn *lll)
{
...
static int init_reset(void)
{
	...
	default_tx_octets = PDU_DC_PAYLOAD_SIZE_MAX;
	default_tx_time = PDU_DC_MAX_US(PDU_DC_PAYLOAD_SIZE_MAX, PHY_1M);
#endif /* CONFIG_BT_CTLR_DATA_LENGTH */
...
static void conn_setup_from_pawr_sync(void *param)
{
	...
	lll_sync = (struct lll_sync *)ftr->param;

	lll_conn = lll_sync->conn;
	//BT_ULL_LOG_MSGID_I("[PAWR_CONN]conn_setup_from_pawr_sync, role: %d", 1, lll_conn->role);

	switch (lll_conn->role) {
#if defined(CONFIG_BT_PERIPHERAL)
	case 1:
		ull_periph_setup(rx, ftr, lll_conn);
		break;
#endif /* CONFIG_BT_PERIPHERAL */
...
void ull_dle_init(struct ll_conn *conn, uint8_t phy)
{
    ...
    // conn->lll.dle.local.max_rx_octets = PDU_DC_PAYLOAD_SIZE_MAX;
#if defined(CONFIG_BT_CTLR_PHY)
    ...
    // conn->lll.dle.remote.max_tx_octets = PDU_DC_PAYLOAD_SIZE_MAX;
    // conn->lll.dle.remote.max_rx_octets = PDU_DC_PAYLOAD_SIZE_MAX;
...
static void mfy_past_sender_offset_get(void *param)
{
	...
	if (adv_sync_handle != BT_HCI_ADV_HANDLE_INVALID) {//PAST from advertiser
        ...
        ull_lp_past_offset_calc_reply(conn, ticker_offset_us, pa_event_counter,
				      last_pa_event_counter, adv_sync->is_pawr);
	} 
	else 
#endif
        {//PAST from scanner
        ...
        ull_lp_past_offset_calc_reply(conn, ticker_offset_us, pa_event_counter,
		    last_pa_event_counter, sync->is_pawr);
	}
}
