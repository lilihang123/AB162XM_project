...
void ull_scan_aux_setup(memq_link_t *link, struct node_rx_pdu *rx)
{
    ...
    case NODE_RX_TYPE_SYNC_REPORT_V2:
        {
			...
			/*Check ACAD For PAWR Proprietary*/
			ull_sync_pro_check_acad(rx);//CR
		#endif
			ull_sync_rsp_scan_aux_update_timing(rx, &rx_incomplete, &ticker_yield_handle);
			lll = NULL;
			lll_aux = NULL;
			aux = NULL;
			scan = NULL;
	...
	if (IS_ENABLED(CONFIG_BT_CTLR_SYNC_PERIODIC) &&
	    (rx->hdr.type == NODE_RX_TYPE_SYNC_REPORT || rx->hdr.type == NODE_RX_TYPE_SYNC_REPORT_V2) &&
	    acad_len) {
...
ull_scan_aux_rx_flush:
    ...
    if (rx->hdr.type == NODE_RX_TYPE_SYNC_REPORT_V2 && ftr->aux_data_len == 0) {
        struct node_rx_pdu *rx_back = rx;
        ll_rx_link_release(link);
        rx->hdr.next = NULL;
        ll_rx_mem_release((void **)&rx_back);
        if (rx_incomplete) {
            rx_incomplete->hdr.type = NODE_RX_TYPE_SYNC_REPORT_V2;
            ll_rx_link_release(rx_incomplete->hdr.link);
            rx_incomplete->hdr.next = NULL;
            ll_rx_mem_release((void **)&rx_incomplete);
	    }
        return;
    }
	ll_rx_put(link, rx);
...
ATTR_TEXT_IN_TCM void *ull_scan_aux_lll_parent_get(struct lll_scan_aux *lll,
				  uint8_t *is_lll_scan)
{
	struct ll_scan_aux_set *aux;
