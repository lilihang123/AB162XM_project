...
uint8_t ull_cp_periodic_sync(struct ll_conn *conn, struct ll_sync_set *sync,
			     struct ll_adv_sync_set *adv_sync, uint16_t service_data, bool is_pawr)
{
    ...
    //ctx = is_pawr ? (llcp_create_local_procedure(PROC_PERIODIC_WR_SYNC)) : (llcp_create_local_procedure(PROC_PERIODIC_SYNC));
    BT_ULL_LOG_MSGID_I("[DEBUG_PAWR] create PAST Procedure", 0);
    ctx = llcp_create_local_procedure(PROC_PERIODIC_SYNC);
	if (!ctx) {
		return BT_HCI_ERR_CMD_DISALLOWED;
	}
    /*if (is_pawr) {
        BT_ULL_LOG_MSGID_I("[DEBUG_PAWR] create PROC_PERIODIC_WR_SYNC", 0);
        ctx = llcp_create_local_procedure(PROC_PERIODIC_WR_SYNC);
    } else {
	    ctx = llcp_create_local_procedure(PROC_PERIODIC_SYNC);
    }*/
	if (sync) {
...
#if (0)
        if (is_pawr) {
            ctx->data.periodic_sync.rsp_aa = sync.rsp_aa;
            ctx->data.periodic_sync.num_subevents = sync.num_subevent;
            ctx->data.periodic_sync.subevent_interval = sync.subevent_interval;
            ctx->data.periodic_sync.response_slot_delay = sync.rsp_slot_delay;
            ctx->data.periodic_sync.response_slot_spacing = sync.rsp_slot_spacing;
        }
#endif
        //Demo code
        //PAWR FEATURE OPTION
        is_pawr = true;
        ctx->data.periodic_sync.is_pawr = is_pawr;
        if (is_pawr && sync) {
            ctx->data.periodic_sync.response_aa = 0x12345678;
            ctx->data.periodic_sync.num_subevents = 0x02;
            ctx->data.periodic_sync.subevent_interval = 0x04;
            ctx->data.periodic_sync.response_slot_delay = 0x08;
            ctx->data.periodic_sync.response_slot_spacing = 0x02;
        }
...
void ull_lp_past_offset_calc_reply(struct ll_conn *conn, uint32_t offset_us,
				   uint16_t pa_event_counter, uint16_t last_pa_event_counter, bool is_pawr)
{
	struct proc_ctx *ctx;
	...
	if (ctx) {
		...
		ctx->data.periodic_sync.sync_conn_event_count = ull_conn_event_counter(conn);//CEs
		ctx->data.periodic_sync.conn_event_count = ull_conn_event_counter(conn) +
							   conn_event_offset;//CEref

		ctx->data.periodic_sync.sync_info.evt_cntr = pa_event_counter;//PEa

		ctx->data.periodic_sync.last_pa_event_counter = last_pa_event_counter;//PEb
        //is_pawr = true; //for UT
        //ctx->data.periodic_sync.is_pawr = is_pawr;
...
static bool pdu_validate_periodic_sync_wr_ind(struct pdu_data *pdu)
{
	return VALIDATE_PDU_LEN(pdu, periodic_sync_wr_ind);
}
...
static const struct pdu_validate pdu_validate[] = {
	...
	[PDU_DATA_LLCTRL_TYPE_PERIODIC_SYNC_WR_IND] = { pdu_validate_periodic_sync_wr_ind },
...
void ull_cp_rx(struct ll_conn *conn, memq_link_t *link, struct node_rx_pdu *rx)
{
    ...
    BT_ULL_LOG_MSGID_I("[PROC LLCP] cp rx, opcode: %d, val: %d", 2, pdu->llctrl.opcode, pdu_valid);
