...
/* Modifications:
2026-03-05 by Airoha - Fix bug that disconnect fail due to remote device not ack termninate llcp.
*/
...
static void lp_comm_tx(struct ll_conn *conn, struct proc_ctx *ctx)
{
	...
	if (ctx->proc != PROC_TERMINATE) {
		...
		/* Use a shorter timeout for the ACL Termination procedure */
		const uint16_t terminate_timeout_events = 20U;
		llcp_lr_prt_restart_with_value(conn, terminate_timeout_events);
	}
}
