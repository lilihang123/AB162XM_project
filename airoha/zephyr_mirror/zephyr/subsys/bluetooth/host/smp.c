...
static struct net_buf *smp_create_pdu(struct bt_smp *smp, uint8_t op, size_t len)
{
	...
	LOG_DBG("[SMP] create_pdu, buf = 0x%08X", buf);
	if (!buf) {
...
static void smp_pairing_complete(struct bt_smp *smp, uint8_t status)
{
	...
	LOG_INF("[SMP] pairing_complete, conn = 0x%08X status = 0x%02X", conn, status);
...
static uint8_t smp_pairing_req(struct bt_smp *smp, struct net_buf *buf)
{
	...
	LOG_INF("[SMP] pairing_req, io_capability 0x%02X, oob_flag 0x%02X, auth_req 0x%02X, "
		"max_key_size 0x%02X, init_key_dist 0x%02X, resp_key_dist 0x%02X",
		req->io_capability, req->oob_flag, req->auth_req,
		req->max_key_size, req->init_key_dist, req->resp_key_dist);
...
static int smp_send_pairing_req(struct bt_conn *conn)
{
	...
	LOG_INF("[SMP] send_pairing_req, flag = 0x%08X", smp->flags);
...
static uint8_t smp_pairing_rsp(struct bt_smp *smp, struct net_buf *buf)
{
	...
	LOG_INF("[SMP] pairing_rsp, io_capability 0x%02X, oob_flag 0x%02X, auth_req 0x%02X, "
		"max_key_size 0x%02X, init_key_dist 0x%02X, resp_key_dist 0x%02X",
		rsp->io_capability, rsp->oob_flag, rsp->auth_req,
		rsp->max_key_size, rsp->init_key_dist, rsp->resp_key_dist);
...
static uint8_t compute_and_send_central_dhcheck(struct bt_smp *smp)
{
	...
	if (bt_crypto_f6(smp->mackey, smp->prnd, smp->rrnd, r, &smp->preq[1],
		...
		bt_addr_le_t *init_address_p = &smp->chan.chan.conn->le.init_addr;
		bt_addr_le_t *resp_address_p = &smp->chan.chan.conn->le.init_addr;
		uint8_t *init_address_val_p = (uint8_t *)init_address_p->a.val;
		uint8_t *resp_address_val_p = (uint8_t *)resp_address_p->a.val;
		LOG_INF("[SMP] send_central_dhcheck, local address type = 0x%02X, address = 0x%02X:%02X:%02X:%02X:%02X:%02X, remote address type = 0x%02X, address = 0x%02X:%02X:%02X:%02X:%02X:%02X,",
		init_address_p->type,
		init_address_val_p[0],
		init_address_val_p[1],
		init_address_val_p[2],
		init_address_val_p[3],
		init_address_val_p[4],
		init_address_val_p[5],
		resp_address_p->type,
		resp_address_val_p[0],
		resp_address_val_p[1],
		resp_address_val_p[2],
		resp_address_val_p[3],
		resp_address_val_p[4],
		resp_address_val_p[5]);
		return BT_SMP_ERR_UNSPECIFIED;
	}
...
static uint8_t compute_and_check_and_send_periph_dhcheck(struct bt_smp *smp)
{
	...
	if (bt_crypto_f6(smp->mackey, smp->prnd, smp->rrnd, r, &smp->prsp[1],
		...
		uint8_t *init_address_val_p = (uint8_t *)init_address_p->a.val;
		uint8_t *resp_address_val_p = (uint8_t *)resp_address_p->a.val;
		LOG_INF("[SMP] send_periph_dhcheck, local address type = 0x%02X, address = 0x%02X:%02X:%02X:%02X:%02X:%02X, remote address type = 0x%02X, address = 0x%02X:%02X:%02X:%02X:%02X:%02X,",
		init_address_p->type,
		init_address_val_p[0],
		init_address_val_p[1],
		init_address_val_p[2],
...
{
	LOG_INF("[SMP] start_security, conn = 0x%08X role = 0x%02X", conn, conn->role);
	switch (conn->role) {
#if defined(CONFIG_BT_CENTRAL)
