...
/* Modifications:
2025-12-08 by Airoha - Add spinlock protection when use sys_slist related api.
*/
...
static void hci_num_completed_packets(struct net_buf *buf)
{
	...
	for (i = 0; i < evt->num_handles; i++) {
		...
		while (count--) {
			 ...
			 * the `complete` list.
			 */
            k_spinlock_key_t lock = k_spin_lock(&list_spin);
			node = sys_slist_get(&conn->tx_pending);
            k_spin_unlock(&list_spin, lock);
			if (!node) {
				LOG_ERR("packets count mismatch");
				__ASSERT_NO_MSG(0);
				break;
			}
            lock = k_spin_lock(&list_spin);
...
static struct bt_conn *find_pending_connect(uint8_t role, bt_addr_le_t *peer_addr)
{
    ...
    conn = bt_conn_lookup_state_le(0, BT_ADDR_LE_NONE, BT_CONN_ADV_CONNECTABLE);
    if (conn) {
    	return conn;
    }
	return NULL;
...
void bt_hci_le_enh_conn_complete(struct bt_hci_evt_le_enh_conn_complete *evt)
{
	...
	if(conn->role == BT_HCI_ROLE_CENTRAL || conn->role == BT_HCI_ROLE_PERIPHERAL)
	{
...
static void le_ltk_request(struct net_buf *buf)
{
	...
	if (bt_smp_request_ltk(conn, evt->rand, evt->ediv, ltk)) {
        ...
        LOG_INF("ltk:0x%02x%02x%02x%02x", ltk[0], ltk[1], ltk[2], ltk[3]);
        LOG_INF("ltk:0x%02x%02x%02x%02x", ltk[4], ltk[5], ltk[6], ltk[7]);
        LOG_INF("ltk:0x%02x%02x%02x%02x", ltk[8], ltk[9], ltk[10], ltk[11]);
        LOG_INF("ltk:0x%02x%02x%02x%02x", ltk[12], ltk[13], ltk[14], ltk[15]);
	} else {
