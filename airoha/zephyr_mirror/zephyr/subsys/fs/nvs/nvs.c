...
/* Modifications:
2026-03-13 by Airoha - add feature CONFIG_NVS_LOOKUP_CACHE_FOR_SETTINGS for speed up read
*/
...
static int nvs_gc(struct nvs_fs *fs)
{
	...
	do {
		...
		if (!nvs_ate_valid(fs, &gc_ate) || (gc_ate.id == 0xFFFF)) {
			continue;
		}
        ...
        /* if cache miss, change iterate addr to fs->ate_wra*/
        uint32_t tmp_addr = wlk_addr;
        rc = nvs_prev_ate(fs, &tmp_addr, &wlk_ate);
        if (rc) {
            return rc;
        }
...
int nvs_get_free_sector_cnt(struct nvs_fs *fs)
{
    struct nvs_ate close_ate;
    size_t ate_size;
    uint32_t sec_addr, wlk_addr;
    int rc, empty_sect_cnt = 0;

    ate_size = nvs_al_size(fs, sizeof(struct nvs_ate));
    wlk_addr = fs->ate_wra;

    while (1) {
        nvs_sector_advance(fs, &wlk_addr);
        sec_addr = (wlk_addr & ADDR_SECT_MASK);
        wlk_addr = sec_addr + fs->sector_size - ate_size;

        rc = nvs_flash_ate_rd(fs, wlk_addr, &close_ate);
        if (rc < 0) {
            return rc;/* flash error */
        }
        rc = nvs_ate_cmp_const(&close_ate, fs->flash_parameters->erase_value);
        if (rc == 0) {
            rc = nvs_flash_cmp_const(fs, (wlk_addr & ADDR_SECT_MASK), fs->flash_parameters->erase_value, fs->sector_size);
            if (rc == 0) {
                empty_sect_cnt += 1;
                continue;
            } else {
 ...
 * sector.
 */
int nvs_trigger_gc(struct nvs_fs *fs, uint32_t gc_cnt)
{
    int rc = 0;
    struct nvs_ate close_ate, gc_ate, wlk_ate;
    uint32_t sec_addr, gc_addr, gc_prev_addr, wlk_addr, wlk_prev_addr,
             data_addr, stop_addr, free_sector_cnt = 0;
    size_t ate_size;

    ate_size = nvs_al_size(fs, sizeof(struct nvs_ate));
    sec_addr = (fs->ate_wra & ADDR_SECT_MASK);

    LOG_INF("[nvs] GC start......");

    k_mutex_lock(&fs->nvs_lock, K_FOREVER);
    while (free_sector_cnt < gc_cnt) {
        /*switch to next sector and check is empty or not*/
        nvs_sector_advance(fs, &sec_addr);
        gc_addr = sec_addr + fs->sector_size - ate_size;

        /* if the sector is not closed only erase, else do garbage collection. */
        rc = nvs_flash_ate_rd(fs, gc_addr, &close_ate);
        if (rc < 0) {
            goto EXIT;
        }
        ...
        if (rc == 0) {
            free_sector_cnt++;
            /* check sector is empty or not for reduce erase time*/
            rc = nvs_flash_cmp_const(fs, (gc_addr & ADDR_SECT_MASK), fs->flash_parameters->erase_value, fs->sector_size);
            if (rc) {
                rc = nvs_flash_erase_sector(fs, sec_addr);
                if (rc) {
                    goto EXIT;
            ...
            /* if not close ate, then start gc collection*/
            stop_addr = gc_addr - ate_size;
            rc = nvs_recover_last_ate(fs, &gc_addr);
            if (rc) {
                goto EXIT;
        ...
        do {
             ...
             */
            if ((wlk_prev_addr == gc_prev_addr) && gc_ate.len) {
                uint32_t require_sz = 0;
                /* copy needed */
                LOG_DBG("Moving id %x, len %d", gc_ate.id, gc_ate.len);

                data_addr = (gc_prev_addr & ADDR_SECT_MASK);
                data_addr += gc_ate.offset;
                require_sz = gc_ate.len + ate_size + NVS_DATA_CRC_SIZE;

                if (fs->ate_wra <= (fs->data_wra + require_sz)) {
                    rc = nvs_sector_close(fs);
                    if (rc) {
                        goto EXIT;
                    }
...
EXIT:
    ...
    LOG_INF("[nvs] gc done, free sector %d", nvs_get_free_sector_cnt(fs));
    return rc;
}
