...
/* Modifications:
2026-03-12 by Airoha - Add CSL ahead time control API
2026-03-12 by Airoha - Set Tx power from nvkey
2026-03-12 by Airoha - Modify default sensitivity value
*/
...
void platformRadioInit(void)
{
	...
	uint16_t TxGcOffset;
	uint32_t nvkey_size = sizeof(uint16_t);
	dataInit();
	nvkey_status_t status = nvkey_read_data(NVID_RF_802154_0V96_TXGC, (uint8_t *)(&TxGcOffset), (uint32_t*)(&nvkey_size));
	if(status == NVKEY_STATUS_OK) {
		int8_t TxPower = IEEE802154_Controller_ConvertTxGC2TxPower(TxGcOffset);
		LOG_ERR("TX Power: %d, TXGC: %d", TxPower, TxGcOffset);
		tx_power = TxPower;
    }
    else {
		LOG_ERR("TXGC NVKEY status fail: %d! ", status); 
    }
...
uint32_t otPlatRadioGetMacCslReqAhead(otInstance *aInstance)
{
	ARG_UNUSED(aInstance);
	return 4000;//OPENTHREAD_CONFIG_MAC_CSL_REQUEST_AHEAD_US
}
