...
int mbedtls_mpi_grow(mbedtls_mpi *X, size_t nblimbs)
{
    ...
    if (X->n < nblimbs) {
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
        if ((p = (mbedtls_mpi_uint*)mbedtls_air_malloc(nblimbs * ciL)) == NULL) {
            return MBEDTLS_ERR_MPI_ALLOC_FAILED;
        }
        air_mbedtls_word_memset(p, 0, nblimbs * ciL);

        if (X->p != NULL) {
            air_mbedtls_word_memcpy(p, X->p, X->n * ciL);
            mbedtls_mpi_zeroize_and_free(X->p, X->n);
        }
#else
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
    if ((p = (mbedtls_mpi_uint*)mbedtls_air_malloc(i * ciL)) == NULL) {
        return MBEDTLS_ERR_MPI_ALLOC_FAILED;
    }
    
    if (X->p != NULL) {
        air_mbedtls_word_memcpy(p, X->p, i * ciL);
        mbedtls_mpi_zeroize_and_free(X->p, X->n);
    }
#else
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
        air_mbedtls_word_memset(X->p, 0, limbs * ciL);
#else
...
int mbedtls_mpi_copy(mbedtls_mpi *X, const mbedtls_mpi *Y)
{
    ...
    if (Y->n == 0) {
        if (X->n != 0) {
            X->s = 1;
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
            air_mbedtls_word_memset(X->p, 0, X->n * ciL);
#else
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
        air_mbedtls_word_memset(X->p + i, 0, (X->n - i) * ciL);
#else
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
    air_mbedtls_word_memcpy(X->p, Y->p, i * ciL);
#else
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
    air_mbedtls_word_memcpy(&T, X, sizeof(mbedtls_mpi));
    air_mbedtls_word_memcpy(X,  Y, sizeof(mbedtls_mpi));
    air_mbedtls_word_memcpy(Y, &T, sizeof(mbedtls_mpi));
#else
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
    air_mbedtls_word_memset(s, 0, sizeof(s));
#else
    memset(s, 0, sizeof(s));
#endif
...
int mbedtls_mpi_cmp_mpi(const mbedtls_mpi *X, const mbedtls_mpi *Y)
{
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE  
    mbedtls_mpi_uint* p_x = (mbedtls_mpi_uint*)(X->p + X->n - 1);
    for (i = X->n; i != 0; i--) {
        if (*p_x-- != 0) {
            break;
        }
    ...
    mbedtls_mpi_uint* p_y = (mbedtls_mpi_uint*)(Y->p + Y->n - 1);
    for (j = Y->n; j != 0; j--) {
        if (*p_y-- != 0) {
            break;
        }
    ...
    p_x = (mbedtls_mpi_uint*)(X->p + i - 1);
    p_y = (mbedtls_mpi_uint*)(Y->p + i - 1);
    for (; i != 0; i--) {
        mbedtls_mpi_uint x_d = *p_x--;
        mbedtls_mpi_uint y_d = *p_y--;
        if (x_d != y_d) {
            return (x_d > y_d) ? X->s : -X->s;
        }
    }
#else
    for (i = X->n; i > 0; i--) {
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
static inline mbedtls_mpi_uint inline_mbedtls_mpi_core_sub(mbedtls_mpi_uint* X,
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
    if (A->n > n && A != X) {
        air_mbedtls_word_memcpy(X->p + n, A->p + n, (A->n - n) * ciL);
    }
    if (X->n > A->n) {
        air_mbedtls_word_memset(X->p + A->n, 0, (X->n - A->n) * ciL);
    }

    carry = inline_mbedtls_mpi_core_sub(X->p, A->p, B->p, n);
#else
    if (A->n > n && A != X) {
