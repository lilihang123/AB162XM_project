...
int main(void)
{
    hal_pinmux_set_function(1,HAL_GPIO_1_GPIO1);
    hal_pinmux_set_function(28,HAL_GPIO_28_AUXADC3_SARADC2_AIODIFF1_SADIFF1_AMIC);
    hal_gpio_set_direction(1,1);

    uint32_t channel_mask=0b0100;
    hal_saradc_data_t saradc_data={
        .channel_mask=channel_mask,
        .channel_data={0}
    };
    hal_adc_init();
    hal_saradc_set_gpio_mode(channel_mask,HAL_SARADC_GPIO_ANALOG_MODE);
    for(;;)
    {
        hal_gpio_set_output(1,1);
        hal_saradc_get_data_immediately(&saradc_data);
        hal_saradc_get_calibration_voltage_mV(HAL_SARADC_MEASURE_DIV_MODE,saradc_data.channel_data[2],&cal_voltage_mV);
        printk("votalge0:%d",cal_voltage_mV);

        hal_gpio_set_output(1,0);
        hal_saradc_get_data_immediately(&saradc_data);
        hal_saradc_get_calibration_voltage_mV(HAL_SARADC_MEASURE_DIV_MODE,saradc_data.channel_data[2],&cal_voltage_mV);
        printk("votalge1:%d",cal_voltage_mV);
