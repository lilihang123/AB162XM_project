...
static void run_pwm_test(const struct device *pwm)
{
    uint32_t pwm_period_ns, pwm_pulse_ns;
    uint32_t pwm_duty_percent = 80U;
    uint32_t pwm_frequency = 1000U;
    pwm_period_ns = NSEC_PER_SEC / pwm_frequency;
    pwm_pulse_ns = (pwm_duty_percent * pwm_period_ns) / 100;

    pwm_set(pwm, 8, pwm_period_ns, pwm_pulse_ns, 0);
}
...
int main(void)
{
    ...
    pwm = DEVICE_DT_GET(DT_NODELABEL(pwm0));
            if (!device_is_ready(pwm)) {
                printk("Device %s is not ready", pwm->name);
                return 0;
            }
