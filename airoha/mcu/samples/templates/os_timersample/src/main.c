...
#define OS_TIME (1000 * 10)

void os_isr(hal_nvic_irq_t irq_number)
{
    printk("------------this is os_timer irq--------------\n");
    uint32_t count = hal_os_timer_stop();
    printk("count = %d\n", count);
}

void os_timer_test(void)
{
    printk("----------os_timer_test start----------\n");
    hal_nvic_disable_irq(OS_GPT_IRQn);
    hal_nvic_clear_pending_irq(OS_GPT_IRQn);
    hal_nvic_register_isr_handler(OS_GPT_IRQn, os_isr);
    hal_os_timer_start(OS_TIME, OS_GPT_TIMER_TYPE_ONE_SHOT, OS_GPT_CLOCK_SOURCE_1M, 32.768f);
}
