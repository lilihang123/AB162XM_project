...
#define ERASE_START_ADDRESS (START_ADDRESS + SHIFT)
#define DATA_LENGTH 4096

uint8_t write_data_buffer[] = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE};
...
void sfc_test(void)
{
    ...
    if (ret != HAL_FLASH_STATUS_OK) {
        printk("hal_flash_init failed\n");
    } else {
        printk("hal_flash_init success\n");
    }

    ret = hal_flash_erase(ERASE_START_ADDRESS, HAL_FLASH_BLOCK_4K);
    if (ret != HAL_FLASH_STATUS_OK) {
    ...
    for (uint32_t i = ERASE_START_ADDRESS; i < ERASE_START_ADDRESS + DATA_LENGTH; i++) {
        if (*(volatile uint8_t *)i != 0xff) {
            printk("data at 0x%x is not 0xff\n", i);
            return;
        }
    }
    printk("all data is 0xff");

    ret = hal_flash_write(ERASE_START_ADDRESS, write_data_buffer, 5);
    if (ret != HAL_FLASH_STATUS_OK) {
    ...
    ret = hal_flash_read(ERASE_START_ADDRESS, read_data_buffer, 5);
    if (ret != HAL_FLASH_STATUS_OK) {
    ...
    for (uint32_t i = 0; i < 5; i++) {
        if (read_data_buffer[i] != write_data_buffer[i]) {
            printk("flash write data error\n");
            return;
        }
    }
    printk("after write and compare, data write succsee\n");
}
