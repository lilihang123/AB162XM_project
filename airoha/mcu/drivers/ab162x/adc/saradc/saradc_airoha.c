...
static int saradc_airoha_channel_setup(const struct device *dev,
				  const struct adc_channel_cfg *channel_cfg)
{
    const struct saradc_airoha_cfg *config = (struct saradc_airoha_cfg*)(dev->config);
...
static int saradc_airoha_read(const struct device *dev,
...
static int saradc_airoha_read_async(const struct device *dev,
			       const struct adc_sequence *sequence,
...
static int saradc_airoha_init(const struct device *dev)
{
    // const struct saradc_airoha_cfg *config = dev->config;
    hal_adc_init();
    return 0;
}

static const struct adc_driver_api saradc_airoha_api = {
	.channel_setup = saradc_airoha_channel_setup,
	.read = saradc_airoha_read,
#ifdef CONFIG_ADC_ASYNC
	.read_async = saradc_airoha_read_async,
#endif
	.ref_internal = 1800,
};

#define SAR_ADC_AIROHA_DEFINE(id)                                      \
	static struct saradc_airoha_cfg saradc_airoha_##id##_cfg = {       \
    ...
    static struct saradc_airoha_data saradc_airoha_##id##_data;        \
                                                                       \
	DEVICE_DT_INST_DEFINE(  id,                                        \
                            saradc_airoha_init,                        \
                            NULL,                                      \
                            &saradc_airoha_##id##_data,                \
			                &saradc_airoha_##id##_cfg,                 \
                            CONFIG_SAR_ADC_INIT_LEVEL,                 \
                            CONFIG_SAR_ADC_INIT_PRIORITY,              \
			                &saradc_airoha_api);                       \

DT_INST_FOREACH_STATUS_OKAY(SAR_ADC_AIROHA_DEFINE)