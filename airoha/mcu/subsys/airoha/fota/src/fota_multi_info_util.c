 ...
 *
 * Description: This file defines and implements the general interfaces to read and write any multiple information stored in the upgrade info partition
 *
 *
 */
...
#ifdef FOTA_SUPPORT_OTHER_MUTLI_INFO
...
#define FOTA_MULTI_INFO_ROLLBACK_FLAG_ADDR_OFFSET  (0x300)
...
#define FOTA_MULTI_INFO_NULL_ADDR_OFFSET  (0x380) /* The address offset of the NULL type. It is an invalid offset and represents the end of the in-use part of the multiple information sector. */
#else
#define FOTA_MULTI_INFO_BL_DFU_FLAG_ADDR_OFFSET  (0x280) /* The address offset of the BL DFU flag with the range of 512th-639th bytes of the sector. */
#define FOTA_MULTI_INFO_ROLLBACK_FLAG_ADDR_OFFSET  (0x300)
/* Add the address offset of a new multiple information type before this line and update the offset value of the NULL type below if needed. */
#define FOTA_MULTI_INFO_NULL_ADDR_OFFSET  (0x380) /* The address offset of the NULL type. It is an invalid offset and represents the end of the in-use part of the multiple information sector. */
#endif
...
FOTA_ERRCODE fota_multi_info_sector_start_addr_get(uint32_t *addr)
{
    ...
    p_status = partition_get_begin_address(PARTITION_UPGRADE_INFO, &fota_address);
...
static FOTA_ERRCODE fota_multi_info_sector_erase(void)
{
    ...
    ret = fota_multi_info_sector_start_addr_get(&start_addr);
    ...
    printk("Before erase start_addr:%x", start_addr);
    result = fota_flash_erase(start_addr,
                            HAL_FLASH_BLOCK_4K);
    if (result == false) {
...
/* static void fota_multi_info_update_ret_val(FOTA_ERRCODE *ret_val, int new_ret) {
    if (FOTA_ERRCODE_SUCCESS != new_ret) {
        *ret_val = new_ret;
    }
} */
...
FOTA_ERRCODE fota_multi_info_sector_clean(uint16_t not_wb_info_types, bool *erase_done)
{
    ...
    uint16_t state = 0;
    bool upgrade_flag_set = false, rollback_flag_set = false;
    uint8_t version[FOTA_VERSION_MAX_SIZE] = {0};
    bool write_back_state = false, write_back_upgrade_flag = false, write_back_version = false, write_back_rollback_flag = false;
    bool bl_dfu_flag_set = false;
    bool write_back_bl_dfu_flag = false;
    ...
    if (!(FOTA_MULTI_INFO_TYPE_STATE & not_wb_info_types)) {
        ...
        if (FOTA_ERRCODE_SUCCESS == ret && 0xFFFF != state) {
            write_back_state = true;
        }
        //fota_multi_info_update_ret_val(&ret_val, ret);
    }
    ...
    if (!(FOTA_MULTI_INFO_TYPE_ROLLBACK_FLAG & not_wb_info_types)) {
        rollback_flag_set = fota_rollback_flag_is_set();
        write_back_rollback_flag = true;
    }


    /* Read the bl dfu flag. */
    if (!(FOTA_MULTI_INFO_TYPE_BL_DFU_FLAG & not_wb_info_types)) {
        bl_dfu_flag_set = fota_bl_dfu_flag_is_set();
        write_back_bl_dfu_flag = true;
    }
    ...
    if (!(FOTA_MULTI_INFO_TYPE_DL_INTEGRITY_RES & not_wb_info_types)) {
        ...
        if (FOTA_ERRCODE_SUCCESS == ret && FOTA_DL_INTEGRITY_RES_VAL_EMPTY != dl_integrity_res) {
    ...
    if (FOTA_ERRCODE_SUCCESS != ret) {
        printk("Failed to erase FOTA info partition. ret:%d", ret);
        return ret;
    }
    ...
    if (write_back_upgrade_flag && upgrade_flag_set) {
        ret = fota_upgrade_flag_set();
        //fota_multi_info_update_ret_val(&ret_val, ret);
    }

    if (write_back_rollback_flag && rollback_flag_set) {
        ret = fota_rollback_flag_set();
        //fota_multi_info_update_ret_val(&ret_val, ret);
    }

    /* Write back the bl DFU flag. */
    if (write_back_bl_dfu_flag && bl_dfu_flag_set) {
        ret = fota_bl_dfu_flag_set();
        //fota_multi_info_update_ret_val(&ret_val, ret);
    }
    ...
    if (write_back_nvdm_incompatible_flag && nvdm_incompatible_flag_set) {
...
fota_multi_info_info_struct *fota_multi_info_get_info(fota_multi_info_type_enum info_type)
{
    ...
    switch (info_type) {
        ...
        case FOTA_MULTI_INFO_TYPE_BL_DFU_FLAG: {
            return &(g_fota_multi_info_ctx_ptr->bl_dfu_flag);
        }
...
#endif
        case FOTA_MULTI_INFO_TYPE_ROLLBACK_FLAG: {
            return &(g_fota_multi_info_ctx_ptr->rollback_flag);
        }
                                             ...
                                             uint16_t *offset)
{
    ...
    switch (info_type) {
        ...
        case FOTA_MULTI_INFO_TYPE_BL_DFU_FLAG: {
            *offset = FOTA_MULTI_INFO_BL_DFU_FLAG_ADDR_OFFSET;
            break;
        }
...
#endif
        case FOTA_MULTI_INFO_TYPE_ROLLBACK_FLAG: {
            *offset = FOTA_MULTI_INFO_ROLLBACK_FLAG_ADDR_OFFSET;
            break;
        }
                                            ...
                                            fota_multi_info_type_enum info_type)
{
    ...
    ret = fota_multi_info_sector_start_addr_get(addr);
    if (FOTA_ERRCODE_SUCCESS != ret) {
        return ret;
    }
...
fota_multi_info_type_enum fota_multi_info_next_info_type_get(fota_multi_info_type_enum info_type)
{
    switch (info_type) {
        ...
        case FOTA_MULTI_INFO_TYPE_VERSION: {
            return FOTA_MULTI_INFO_TYPE_ROLLBACK_FLAG;
        }

        case FOTA_MULTI_INFO_TYPE_ROLLBACK_FLAG: {
#ifndef FOTA_SUPPORT_OTHER_MUTLI_INFO
        ...
        case FOTA_MULTI_INFO_TYPE_NVDM_INCOMPATIBLE_FLAG: {
            return FOTA_MULTI_INFO_TYPE_BL_DFU_FLAG;
        }
                                            ...
                                            uint8_t *record_len)
{
    ...
    switch (info_type) {
        ...
        case FOTA_MULTI_INFO_TYPE_BL_DFU_FLAG: {
            *record_len = FOTA_MULTI_INFO_BL_DFU_FLAG_RECORD_LEN;
            break;
        }
...
FOTA_ERRCODE fota_multi_info_init(fota_multi_info_type_enum info_type)
{
    ...
    printk("info_type:%x, info->is_init:%d",
                 info_type,
                 info->is_init);
    ret = fota_multi_info_start_addr_get(&start_addr, info_type);
    if (FOTA_ERRCODE_SUCCESS != ret) {
        return ret;
    }

    ret = fota_multi_info_end_addr_get(&end_addr, info_type);
    if (FOTA_ERRCODE_SUCCESS != ret) {
        return ret;
    }
    ...
    while (unread_len) {
        ...
        result = fota_flash_read(read_addr, buffer, FOTA_MULTI_INFO_RECORD_LEN_LCM);
        if (true != result) {
            ret = FOTA_ERRCODE_READ_FOTA_DATA_FAIL;
            return ret;
        }
                                  ...
                                  uint8_t record_len)
{
    ...
    result = fota_flash_read(info->last_record_addr,
                          record_data,
                          record_len);
    if (result == true) {
        ret = FOTA_ERRCODE_SUCCESS;
                                   ...
                                   uint8_t record_len)
{
    ...
    printk("buffer:%p, record_data:%p, record_len:%d",
    ...
    if (FOTA_ERRCODE_SUCCESS == ret_val) {
        result = fota_flash_write(address,
                               record_data,
                               record_len);
        if (result == false) {
            ret = FOTA_ERRCODE_FAIL;
        ...
        if (FOTA_ERRCODE_SUCCESS != ret) {
            ...
            result = fota_flash_read(address,
                                  record_data_read,
                                  record_len);
