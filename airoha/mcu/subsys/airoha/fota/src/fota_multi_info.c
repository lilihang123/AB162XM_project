 ...
 *
 * Description: This file defines and imeplements the specific interfaces to read and write the multiple informations stored in the upgrade info partition
 *
 *
 */
...
#define FOTA_BL_DFU_FLAG_MARK   (FOTA_UPGRADE_INFO_FLAG_MARK) /* The mark of the BL DFU flag. */
#define FOTA_BL_DFU_FLAG_SET    (FOTA_UPGRADE_INFO_FLAG_SET) /* One of the BL DFU flag values which represents the BL DFU flag is set. */
#define FOTA_BL_DFU_FLAG_CLEAR  (FOTA_UPGRADE_INFO_FLAG_CLEAR) /* One of the BL DFU flag values which represents the BL DFU flag is cleared. */
#define FOTA_ROLLBACK_INFO_FLAG_MARK        (FOTA_UPGRADE_INFO_FLAG_MARK) /* The mark of the FOTA upgrade flag. */
#define FOTA_ROLLBACK_INFO_FLAG_SET         (FOTA_UPGRADE_INFO_FLAG_SET) /* One of the FOTA upgrade flag values which represents the FOTA upgrade flag is set. */
#define FOTA_ROLLBACK_INFO_FLAG_CLEAR       (FOTA_UPGRADE_INFO_FLAG_CLEAR) /* One of the FOTA upgrade flag values which represents the FOTA upgrade flag is cleared. */
...
{
    printk("fota_upgrade_flag_clear()");
...
{
    printk("fota_upgrade_flag_set()");
    return fota_upgrade_flag_write(FOTA_UPGRADE_INFO_FLAG_SET);
}

FOTA_ERRCODE fota_rollback_flag_read(uint8_t *rollback_flag)
{
    FOTA_ERRCODE ret = FOTA_ERRCODE_FAIL;
    uint8_t buffer[FOTA_MULTI_INFO_ROLLBACK_FLAG_RECORD_LEN] = {0};
    ...
    *rollback_flag = 0xFF;
    ...
    ret = fota_multi_info_read(FOTA_MULTI_INFO_TYPE_ROLLBACK_FLAG,
                               buffer,
                               FOTA_MULTI_INFO_ROLLBACK_FLAG_RECORD_LEN);
    if (FOTA_ERRCODE_SUCCESS != ret) {
        return ret;
    }

    /* The format of the FOTA upgrade flag is [mark, value]. */
    if (FOTA_ROLLBACK_INFO_FLAG_MARK != buffer[0] ||
        (FOTA_ROLLBACK_INFO_FLAG_CLEAR != buffer[1] &&
         FOTA_ROLLBACK_INFO_FLAG_SET != buffer[1])) {
        return FOTA_ERRCODE_FAIL;
    }

    *rollback_flag = buffer[1];
    return FOTA_ERRCODE_SUCCESS;
}


bool fota_rollback_flag_is_set(void)
{
    uint8_t rollback_flag = 0;

    if (FOTA_ERRCODE_SUCCESS == fota_rollback_flag_read(&rollback_flag)) {
        if (FOTA_ROLLBACK_INFO_FLAG_SET == rollback_flag) {
            return true;
        }
    }
...
FOTA_ERRCODE fota_rollback_flag_write(uint8_t rollback_flag)
{
    FOTA_ERRCODE ret = FOTA_ERRCODE_SUCCESS;
    uint8_t buffer[FOTA_MULTI_INFO_ROLLBACK_FLAG_RECORD_LEN] = {FOTA_ROLLBACK_INFO_FLAG_MARK, rollback_flag};

    ret = fota_multi_info_write(FOTA_MULTI_INFO_TYPE_ROLLBACK_FLAG,
                                buffer,
                                FOTA_MULTI_INFO_ROLLBACK_FLAG_RECORD_LEN);
    if (FOTA_ERRCODE_OUT_OF_RANGE == ret) {
        ret = fota_multi_info_sector_clean(FOTA_MULTI_INFO_TYPE_ROLLBACK_FLAG, NULL);
        if (FOTA_ERRCODE_SUCCESS == ret) {
            ret = fota_multi_info_write(FOTA_MULTI_INFO_TYPE_ROLLBACK_FLAG,
                                        buffer,
                                        FOTA_MULTI_INFO_ROLLBACK_FLAG_RECORD_LEN);
...
FOTA_ERRCODE fota_rollback_flag_clear(void)
{
    printk("fota_rollback_flag_clear()");
    return fota_rollback_flag_write(FOTA_ROLLBACK_INFO_FLAG_CLEAR);
}


FOTA_ERRCODE fota_rollback_flag_set(void)
{
    printk("fota_rollback_flag_set()");
    return fota_rollback_flag_write(FOTA_ROLLBACK_INFO_FLAG_SET);
}

FOTA_ERRCODE fota_bl_dfu_flag_read(uint8_t *ugprade_flag)
{
    FOTA_ERRCODE ret = FOTA_ERRCODE_FAIL;
    uint8_t buffer[FOTA_MULTI_INFO_BL_DFU_FLAG_RECORD_LEN] = {0};
    ...
    *ugprade_flag = 0xFF;
    ...
    ret = fota_multi_info_read(FOTA_MULTI_INFO_TYPE_BL_DFU_FLAG,
                               buffer,
                               FOTA_MULTI_INFO_BL_DFU_FLAG_RECORD_LEN);
    if (FOTA_ERRCODE_SUCCESS != ret) {
        return ret;
    }

    /* The format of the FOTA upgrade flag is [mark, value]. */
    if (FOTA_BL_DFU_FLAG_MARK != buffer[0] ||
        (FOTA_BL_DFU_FLAG_CLEAR != buffer[1] &&
         FOTA_BL_DFU_FLAG_SET != buffer[1])) {
        return FOTA_ERRCODE_FAIL;
    }
...
bool fota_bl_dfu_flag_is_set(void)
{
    uint8_t ugprade_flag = 0;
    ...
    if (FOTA_ERRCODE_SUCCESS == fota_bl_dfu_flag_read(&ugprade_flag)) {
        if (FOTA_BL_DFU_FLAG_SET == ugprade_flag) {
            return true;
        }
    }
...
FOTA_ERRCODE fota_bl_dfu_flag_write(uint8_t ugprade_flag)
{
    FOTA_ERRCODE ret = FOTA_ERRCODE_SUCCESS;
    uint8_t buffer[FOTA_MULTI_INFO_BL_DFU_FLAG_RECORD_LEN] = {FOTA_BL_DFU_FLAG_MARK, ugprade_flag};

    ret = fota_multi_info_write(FOTA_MULTI_INFO_TYPE_BL_DFU_FLAG,
                                buffer,
                                FOTA_MULTI_INFO_BL_DFU_FLAG_RECORD_LEN);
    if (FOTA_ERRCODE_OUT_OF_RANGE == ret) {
        ret = fota_multi_info_sector_clean(FOTA_MULTI_INFO_TYPE_BL_DFU_FLAG, NULL);
        if (FOTA_ERRCODE_SUCCESS == ret) {
            ret = fota_multi_info_write(FOTA_MULTI_INFO_TYPE_BL_DFU_FLAG,
                                        buffer,
                                        FOTA_MULTI_INFO_BL_DFU_FLAG_RECORD_LEN);
...
FOTA_ERRCODE fota_bl_dfu_flag_clear(void)
{
    printk("fota_bl_dfu_flag_clear()");
    return fota_bl_dfu_flag_write(FOTA_BL_DFU_FLAG_CLEAR);
}


FOTA_ERRCODE fota_bl_dfu_flag_set(void)
{
    printk("fota_bl_dfu_flag_set()");
    return fota_bl_dfu_flag_write(FOTA_BL_DFU_FLAG_SET);
}
...
FOTA_ERRCODE fota_version_read(uint8_t *buffer, uint8_t buf_size)
{
    ...
    if (FOTA_ERRCODE_SUCCESS != ret) {
        printk("[FOTA] fota_multi_info_read read version failed, ret:%x", ret);
        return ret;
    }

    if (0xFF == version_record[0] || version_record[FOTA_VERSION_MAX_SIZE - 1]) {
        const char* default_version = "v1.0.0";
        memcpy(buffer, default_version, strlen(default_version));
        buffer[strlen(default_version)] = '\0';
        return FOTA_ERRCODE_SUCCESS;
    }
...
FOTA_ERRCODE fota_version_write(uint8_t *version, uint8_t version_len)
{
    ...
    if (FOTA_ERRCODE_SUCCESS == ret) {
#if defined(MTK_NVDM_ENABLE) && !defined(AIR_FOTA_UPGRADE_ENABLE)
#ifdef AIR_REPLACE_NVDM_WITH_NVKEY
...
SYS_INIT(fota_init, CONFIG_RACE_CMD_INIT_LEVEL, CONFIG_RACE_CMD_INIT_PRIORITY);
