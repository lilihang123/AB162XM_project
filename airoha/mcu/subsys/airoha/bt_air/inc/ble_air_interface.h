...
#define BT_CONNECTION_MAX    CONFIG_BT_MAX_CONN
#endif
...
#define BLE_AIR_RECEIVE_BUFFER_SIZE       (600)//(1250)
#else
#define BLE_AIR_RECEIVE_BUFFER_SIZE       (1250)
#endif
/**
...
#define AIR_SERVICE_UUID BT_UUID_DECLARE_128(0x45, 0x4C, 0x42, 0x61, 0x68, 0x6F, 0x72, 0x69,               \
     0x41, 0x03, 0xAB, 0x2D, 0x4D, 0x49, 0x52, 0x50)
#define AIR_RX_CHAR_UUID BT_UUID_DECLARE_128(0x45, 0x4C, 0x42, 0x61, 0x68, 0x6F, 0x72, 0x69,               \
     0x41, 0x32, 0xAB, 0x2D, 0x52, 0x41, 0x48, 0x43)
#define AIR_TX_CHAR_UUID BT_UUID_DECLARE_128(0x45, 0x4C, 0x42, 0x61, 0x68, 0x6F, 0x72, 0x69,               \
     0x41, 0x31, 0xAB, 0x2D, 0x52, 0x41, 0x48, 0x43)

#define CONNECTION_HANDLE_IS_VALIAD(var) ((var) != 0xFFFF)
...
#define BLE_AIR_DISCONNECT_REASON_NORMAL           0x00 /**< The disconnect reason is normal . */
#define BLE_AIR_DISCONNECT_REASON_RHO_DONE         0x01 /**< The disconnect reason is RHO done . */
typedef uint8_t ble_air_disconnect_reason_t; /**< The disconnect reason of BLE AIR Service. */
...
typedef struct {
    struct bt_conn                  *conn;
    ble_air_disconnect_reason_t  disconnect_reason;   /**< The disconnect reason of the current connection. */
...
typedef struct {
    bt_addr_t    bdaddr;         /**< The BLE address of the remote device. */
 ...
 * @return     int.
 */
//int ble_air_main(const struct device *dev);
...
#define BLEAIR_MUTEX_LOCK() ble_air_mutex_lock()

#define BLEAIR_MUTEX_UNLOCK() ble_air_mutex_unlock()
