...
static struct k_timer g_race_timer;

static void race_timer_hdlr(struct k_timer *timer)
{
    if (NULL == timer) {
        LOG_ERR("race timer is NULL");
        return;
    }
    race_general_msg_t msg_queue_item = {0};
    RACE_ERRCODE ret = RACE_ERRCODE_FAIL;
    if (FALSE == g_race_timer_exp_ind_handled) {
        // LOG_WRN("g_race_timer_exp_ind_handled is FALSE, not send msg");
        return;
    }
    /*
...
{
    static uint32_t init_timer_flag = true;
    LOG_INF("period: %d ms", delay_msec);

    if(init_timer_flag){
        k_timer_init(&g_race_timer, race_timer_hdlr, NULL);
        init_timer_flag = false;
    }
    k_timer_start(&g_race_timer, K_MSEC(delay_msec), K_MSEC(delay_msec));
...
{
    LOG_INF("race_timer_stop");
    k_timer_stop(&g_race_timer);
    return TRUE;
}
