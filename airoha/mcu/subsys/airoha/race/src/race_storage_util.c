...
// #ifdef RACE_STORAGE_UTIL_ENABLE_DEBUG_LOG
    LOG_INF("[FOTA] race_storage_block_erase ret:%x", ret);
// #endif
    ...
    if (!msg) {
        LOG_WRN("Lost a ERASE_PARTITION_CONTINUE message!");
    ...
    LOG_WRN("[FOTA] send ERASE_PARTITION_CONTINUE to race!");
...
// #endif
    ...
    if (continue_msg->callback) {
        LOG_INF("[FOTA] erase done, callback:%x", ret);
                                          ...
                                          uint8_t storage_type)
{
    ...
    if (HAL_DVFS_STATUS_OK != ret) {
        ...
        LOG_ERR("Failed to lock alone cpu frequency.");
        return RACE_ERRCODE_FAIL;
    }
#endif
#endif
                                        ...
                                        uint8_t storage_type)
{
    ...
    *crc8 = 0;
    ...
    while (data_length > 0) {
        ...
        *crc8 = (uint8_t)CRC8_Generate((uint8_t *)buffer, (uint16_t)read_length, (uint8_t)*crc8);
                                                     ...
                                                     uint8_t storage_type)
{
    ...
    uint8_t *buffer = NULL;
#ifdef RACE_STORAGE_UTIL_ENABLE_DEBUG_LOG
    LOG_INF("erase_status:%x,erase_status_size:%x,start_address:%x,length:%x,storage_type:%d",
                     erase_status,
                     erase_status_size,
                     start_address,
                     length,
    ...
    *erase_status_size = ((bit_size >> 3) + (bit_size % 8 ? 1 : 0));
...
#ifdef RACE_STORAGE_UTIL_ENABLE_DEBUG_LOG
    LOG_INF("bit_size:%d, *erase_status_size:%d", bit_size, *erase_status_size);
#endif
    if (!erase_status) {
...
#ifdef RACE_STORAGE_UTIL_ENABLE_DEBUG_LOG
    LOG_INF("erase_status_size_old:%d, *erase_status_size:%d",
    ...
    for (int k = 0; k < *erase_status_size; k++) {
        LOG_INF("erase_status[%d]:%x", k, erase_status[k]);
    }
#endif
