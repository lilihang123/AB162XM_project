...
* Define
**************************************************************************************************/
#define RACE_MUX_SPP_TX_BUFFER_SIZE                     (1024 + 128 + 512 + 384)
...
#define RACE_MUX_BLE_TX_BUFFER_SIZE                     (600)
#define RACE_MUX_BLE_RX_BUFFER_SIZE                     (600)
#else
...
#define RACE_MUX_GATT_OVER_BREDR_RX_BUFFER_SIZE         (1024 * 3 - 512 - 384)
...
static bt_status_t race_rho_srv_allowed_callback(const bt_addr_t *addr);
static uint8_t race_rho_get_data_len_callback(const bt_addr_t *addr);
static bt_status_t race_rho_get_data_callback(const bt_addr_t *addr, void *data);
static bt_status_t race_rho_update_data_callback(bt_role_handover_update_info_t *info);
static void race_rho_srv_status_callback(const bt_addr_t *addr,
...
static void race_set_bt_connection_addr(mux_port_t dev, bt_addr_t *addr);
...
static void race_set_bt_connection_addr(mux_port_t dev, bt_addr_t *addr)
{
    ...
    if (info == NULL) {
        LOG_ERR("race_set_bt_connection_addr, dev %d not support", dev);
        return;
    }
    if (addr) {
        ...
        memcpy(&(info->addr.val), addr->val, sizeof(bt_addr_t));
    } else {
        //RACE_LOG_MSGID_I("race_set_bt_connection_addr, dev %d, set 0", 1, dev);
        memset(&(info->addr.val), 0, sizeof(bt_addr_t));
    }
}
#if 0
static uint32_t race_iap2_tx(mux_handle_t mux_handle, uint8_t *buf, uint32_t buf_len)
{
    ...
    if (iap2_session_id != 0) {
        ...
        p_malloc_buff_addr = race_mem_alloc(buf_len + 2);
        ...
        race_mem_free(p_malloc_buff_addr);
        return data_len;
    }
    return 0;
}
...
static uint32_t race_spp_tx(mux_handle_t mux_handle, uint8_t *buf, uint32_t buf_len)
{
    ...
    LOG_INF("race_serial_port_send_data spp, state[%d], retry[%d]", tx_state, retry_cnt);
    return data_len;
}
#endif
static race_event_type_enum race_bt_get_notify_event(race_port_t port, mux_event_t event)
...
static void race_mux_event_handler(mux_handle_t handle, mux_event_t event, uint32_t data_info, void *user_data)
{
    ...
    switch(event) {
...
#endif
#ifdef RACE_GATT_RELAY_ENABLE
                race_gatt_event_callback(RACE_GET_PORT_BY_MUX_HANDLE(handle), event, ind);
#endif
            }
            break;
        }
        ...
        case MUX_EVENT_DISCONNECTION: {
            if (0 != data_info) {
                mux_bt_disconnect_ind_t *dis_ind= (mux_bt_disconnect_ind_t *)data_info;
                LOG_INF("race_mux_event, dis_ind->disconnect_reason = %02x", dis_ind->disconnect_reason);
                if (MUX_BT_DISCONNECT_REASON_RHO_DONE != dis_ind->disconnect_reason) {
                    if (RACE_EVENT_TYPE_NONE != noti_event) {
                        race_send_event_notify_msg(noti_event, NULL);
                    }
                }
...
#endif
#ifdef RACE_GATT_RELAY_ENABLE
            race_gatt_event_callback(RACE_GET_PORT_BY_MUX_HANDLE(handle), event, NULL);
#endif
            break;
        }
...
{
#if 1
    LOG_INF("race_port_bt_open_all");
#if defined(AIR_BT_AIR_SERVICE_ENABLE)
    //race_port_bt_open(MUX_BT_SPP);
    race_port_bt_open(MUX_BT_BLE);
/*dongle support 3 ble link, others support 1 ble link*/
...
{
    LOG_INF("race_port_bt_close_all");
#if defined(AIR_BT_AIR_SERVICE_ENABLE)
...
bt_addr_t *race_get_bt_connection_addr(uint8_t channel_id)
{
...
uint8_t race_get_channel_id_by_conn_address(bt_addr_t *addr)
{
    uint8_t i = 0;
    if (!addr) {
    ...
    for (i = 0; i < RACE_BT_CONNECTION_NUM; i ++) {
        if (memcmp(&g_race_bt_connection[i].addr , addr , sizeof(bt_addr_t)) == 0) {
...
{
    bt_addr_t bdaddr = {{0, 0, 0, 0, 0, 0}};
    race_bt_conn_info_t *pinfo = race_find_bt_conn_info_by_port(dev);
    if (NULL != pinfo) {
        if (0 == memcmp(&bdaddr, &pinfo->addr, sizeof(bt_addr_t))) {
            return false;
        } else {
            return true;
        }
