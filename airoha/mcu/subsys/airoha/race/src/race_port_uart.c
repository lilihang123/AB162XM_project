...
/* declare a weak symbol so that the app can override the default setting
 * by defining a strong symbol if needed.
 */
ATTR_WEAK_SYMBOL const mux_port_setting_t g_race_uart_default_setting = {
...
race_port_t race_uart_get_active_port(void)
{
    ...
    //LOG_WRN("[race_uart]active uart port is NULL.");
