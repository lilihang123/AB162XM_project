...
// 1: only permit to access FOTA region
uint32_t reace_storage_address_check_enable;
                                             ...
                                             uint32_t length)
{
...
#ifdef RACE_FOTA_CMD_ENABLE
    if (reace_storage_address_check_enable == 0) {
        if (RACE_ERRCODE_SUCCESS == race_storage_enable_fota_partition_accessibility()) {
            reace_storage_address_check_enable = 1;
        } else {
            return RACE_ERRCODE_PARAMETER_ERROR;
        }
    }
    ...
    LOG_WRN("Not accessible. storage_type:%d address:%x length:%x",
...
RACE_ERRCODE race_storage_enable_fota_partition_accessibility(void)
{
    ...
    ret = fota_flash_get_slot_info(&storage_type, &fota_address, &length);
