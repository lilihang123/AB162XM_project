...
RACE_ERRCODE race_event_register(int32_t *register_id, race_event_callback callback, void *user_data)
{
    ...
    LOG_INF("race event register_id:%x callback:%x", register_id, callback);
    ...
    if (reg_info) {
        if (reg_info->user_data != user_data) {
            //LOG_INF("Register failed. register_id:%d ret:%d", *register_id, RACE_ERRCODE_CONFLICT);
            return RACE_ERRCODE_CONFLICT;
        }

        //LOG_INF("Register succeed(duplicate). register_id:%d", *register_id);
...
RACE_ERRCODE race_event_register(race_event_callback callback, void *user_data)
{
    ...
    LOG_ERR("race event register callback:%x", callback);
...
RACE_ERRCODE race_send_event_notify_msg(race_event_type_enum event_type, void *param)
{
    ...
    msg_queue_item.dev_t = (race_port_t)event_type;
    msg_queue_item.msg_data = (uint8_t *)param;
    ret = race_send_msg(&msg_queue_item);
    LOG_INF("event_type:%d, param:%x, ret:%d", event_type, param, ret);
    ...
    if (param) {
        ...
        if (address && address_length) {
            ...
            if (RACE_ERRCODE_SUCCESS == ret) {
                ...
                if (address) {
                    param->address_length = RACE_EVENT_REMOTE_DEVICE_ADDRESS_LENGTH > sizeof(bt_addr_t) ? sizeof(bt_addr_t) : RACE_EVENT_REMOTE_DEVICE_ADDRESS_LENGTH;
                    memcpy(param->address, address, param->address_length);
                }
            }
        }
...
RACE_ERRCODE race_event_send_find_me_event(uint8_t is_blink, uint8_t is_tone)
{
    ...
    param->is_blink = is_blink;
    param->is_tone = is_tone;
    LOG_INF("[FIND_ME]send_find_me_event:blink:%x, is_tone:%x, param_b: %x, param_t : %x", is_blink, is_tone, param->is_blink, param->is_tone);
