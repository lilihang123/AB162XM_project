...
static const race_port_t g_race_dongle_valid_relay_port[] = {MUX_BT_BLE, MUX_BT_BLE_1, MUX_BT_BLE_2};
...
{
#if 1//defined(MTK_USB_DEMO_ENABLED) && defined(MTK_USB_AUDIO_HID_ENABLE)
...
#if defined(AIR_PURE_GAMING_DONGLE_ENABLE) || defined(AIR_BLE_ULTRA_LOW_LATENCY_ENABLE)
#ifdef AIR_BLE_ULTRA_LOW_LATENCY_ENABLE
        if (BT_ULL_ROLE_SERVER == bt_ull_le_hid_conn_srv_get_role()) { //dongle
#endif
...
void race_dongle_relay_conn_callback(race_port_t port, bt_addr_t *bd_addr)
{
    if (NULL != bd_addr) {
        ...
        if (NULL != pinfo) {
            pinfo->dst_port = port;
            memcpy(&pinfo->bd_addr, bd_addr, sizeof(bt_addr_t));
            LOG_INF("relay port connected: port:%d, pinfo:0x%x", port, (uint32_t)pinfo);
        }
    }
}
...
void race_dongle_relay_disconn_callback(race_port_t port)
{
    if (race_dongle_relay_is_valid_port(port)) {
        ...
        if (pinfo) {
            ...
            LOG_INF("relay port disconnected: port:%d, pinfo:0x%x", port, (uint32_t)pinfo);
        }
    }
}
...
void race_usb_relay_disable(void)
{
    race_unregister_relay_check_handler(race_dongle_relay_check);
    LOG_INF("[DISABLE]race_usb_relay");
}

void race_usb_relay_enable(void)
{
    race_register_relay_check_handler(race_dongle_relay_check);
    LOG_INF("[ENABLE]race_usb_relay");
}
