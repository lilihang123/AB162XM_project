...
{
    printk("lw_mux_uart0_tx, len:%d\r\n", len);
    return hal_uart_send_polling(HAL_UART_2, data, len);
}
...
uint32_t lw_mux_uart0_rx(uint8_t *data, uint32_t len)
{
    ...
    uint32_t rev_len = 0;
    while (rev_len < len) {
        res = hal_uart_get_char_unblocking(HAL_UART_2);
        if (res & 0xffffff00) {
#if 0  /* for debug */
            LOG_INF(LOG_DEBUG, "#");
#endif
    ...
    if (rev_len) {
        printk("lw_mux_uart0_rx, rev_len:%d\r\n", rev_len);
