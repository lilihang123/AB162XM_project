...
extern int query_current_syslog_port(void);
#endif
...
static void race_init_default_port(void)
{
    ...
    int port = query_current_syslog_port();
    LOG_INF("race_init_default_port, port:%d", port);
    if (-1 != port) {
        race_port = port;
    } else {
        race_port = MUX_UART_0;//CONFIG_RACE_PORT;
    }
#ifdef AIR_UART1_EARLY_LOG_ENABLE
    LOG_INF("race_init, AIR_UART1_EARLY_LOG_ENABLE == 1, config race port to uart1");
...
static void race_init_default_port(void)
{
    ...
    /* do not init race if disable all log flag set */
    if (!exception_get_advanced_power_save_flag()) {
        mux_open_save_to_nvdm(race_port, "RACE_CMD");
        race_open_default_port(race_port, race_get_port_type(race_port));
    }

#if defined (AIR_KEYBOARD_M607)
    if (race_port != MUX_UART_2) {
        race_port = MUX_UART_2;
        mux_open_save_to_nvdm(race_port, "RACE_CMD");
        race_open_default_port(race_port, race_get_port_type(race_port));
    }
#endif
...
#if defined( RACE_STORAGE_CMD_ENABLE) && defined(RACE_FOTA_CMD_ENABLE)
static void race_storage_nb_sha256_generate_continue_handler(race_general_msg_t *p_msg)
{
    race_fota_hash_nb_generate_continue_msg_process((void *)p_msg->msg_data);
}
#endif
...
#ifdef RACE_FOTA_CMD_ENABLE
    race_register_general_msg_hdl(MSG_ID_RACE_LOCAL_HASH_NB_GENERATE_CONTINUE_IND,
...
#endif
#if CONFIG_AIR_RACE_RELAY_CMD_ENABLE
    race_register_general_msg_hdl(MSG_ID_RACE_LOCAL_RELAY_BT_RACE_CMD, (race_general_msg_hdl_t)race_cmd_relay_bt_relay_msg_process);
#endif
...
race_status_t race_flush_packet(uint8_t *ptr, uint8_t channel_id)
{
    ...
    LOG_INF("race_flush_packet, ptr[0x%X] channel_id[%d]", ptr, channel_id);
    ...
    if (ptr != NULL) {
...
#elif (CONFIG_AIR_RACE_RELAY_CMD_ENABLE)
        if (RACE_CHANNEL_ID_IS_RELAY_CMD_FLAG_SET(channel_id)) {
            bool ret_val = false;

            channel_id = RACE_CHANNEL_ID_CLEAR_RELAY_CMD_FLAG(channel_id);
            ret_val = race_relay_send_bt(&pData->race_data, pData->length, channel_id, RACE_CMD_RSP_FROM_PARTNER);
            race_mem_free(pData);
            return (ret_val == true) ? RACE_STATUS_OK : RACE_STATUS_ERROR;
        }
#endif
        ...
        if (data == NULL) {
            LOG_ERR("race_flush_packet, alloc fail");
...
uint32_t race_port_send_data(uint32_t port_handle, uint8_t *buf, uint32_t buf_len)
{
...
#endif
    {
        ...
        race_send_output_cmd_msg_t *data = (race_send_output_cmd_msg_t *)msg_queue_item.msg_data;
        if (data == NULL) {
            LOG_ERR("race_port_send_data alloc fail");
            return 0;
        }
...
*/
#if 0
uint8_t race_get_device_id_by_conn_address(bt_addr_t *peer_address)
{
...
race_status_t race_serial_port_open(race_port_t port)
{
...
uint32_t race_get_serial_port_handle(race_port_t port)
{
    return race_get_tx_handle_by_port(port);
}
...
race_status_t race_init_port_service(race_port_t port)
{
    race_port_type_t type = race_get_port_type(port);
    if (race_open_default_port(port, type) == RACE_ERRCODE_SUCCESS) {
        return RACE_STATUS_OK;
...
int race_init(void)
{
    ...
    race_usb_relay_init();
#endif
#ifdef RACE_REMOTE_DEVICE_CONTROL_ENABLE
    race_rdc_init();
#endif
    return 0;
}
