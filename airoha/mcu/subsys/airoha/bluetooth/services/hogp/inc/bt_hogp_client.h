...
#include "bt_hogp_client_discovery.h"

/**
 * @brief The HID service characteristic max number.
 */
#define BT_HOGP_CLIENT_MAX_LINK_NUM                     5
...
#define BT_HID_SERVICE_REPORT_MAP_MAX_LENGTH            300
/**
...
#define HOGP_DISCOVEY_SERVICE_DISCOVERY_HIDS             (9)
#define HOGP_DISCOVEY_SERVICE_DISCOVERY_HID_INFO         (10)
#define HOGP_DISCOVEY_SERVICE_DISCOVERY_HID_REPORT_MAP      (11)
#define HOGP_DISCOVEY_SERVICE_DISCOVERY_HID_PROTOCOL_MODE   (12)
#define HOGP_DISCOVEY_SERVICE_DISCOVERY_HID_REPORTS         (13)
#define HOGP_DISCOVEY_SERVICE_DISCOVERY_HID_CONTROL_POINT   (14)
#define HOGP_DISCOVEY_SERVICE_DISCOVERY_BOOT_MOUSE_INPUT_REPORT   (15)
#define HOGP_DISCOVEY_SERVICE_DISCOVERY_BOOT_KB_INPUT_REPORT   (16)
#define HOGP_DISCOVEY_SERVICE_DISCOVERY_BOOT_KB_OUTPUT_REPORT  (17)
...
/**
 * @brief The HID service: Protocol Mode.
 */
enum bt_hids_pm {
    /** Boot protocol. */
    BT_HIDS_PM_BOOT = 0x00,
    /** Report protocol, Default Protocol Mode of all HID Devices. */
    BT_HIDS_PM_REPORT = 0x01
};

/** 
 * @brief The HID Service information.
 */
struct bt_hids_info {
    /** the version number of base USB HID Specification implemented by HID Device. */
uint16_t bcd_hid;

    /** country HID Device hardware is localized for.
     * Most hardware is not localized (value 0x00).
     */
    uint8_t b_country_code;

    /** Bit 0: RemoteWake, Bit 1: NormallyConnectable. */
    uint8_t flags;
};
...
enum bt_hids_control_point {
    ...
    /** Exiting the Suspend State.*/
BT_HIDS_CONTROL_POINT_EXIT_SUSPEND = 0x01
};

/**
 * @brief The HOGP Client handle.
 */
typedef struct{
...
typedef struct{
    ...
    uint8_t cccd_num;
    uint8_t ref_num;
    uint16_t value_handle[BT_HID_SERVICE_REPORT_REF_MAX_NUMBER];
...
typedef struct {
    ...
    struct bt_conn      *conn;                            /**< The connection handle. */
} bt_hogp_client_para_t;


typedef void (*bt_hogp_client_event_callback)(bt_hogp_client_event_t event, bt_hogp_client_para_t *para, void *buffer, uint16_t length);
/**
 * @brief The api used for init HOGP client.
 */
...
void bt_hogp_client_save_character(struct bt_conn *conn, const struct bt_gatt_attr *attr, uint8_t type);
