...
#define GROUP_BT_DRIVER  "BT_Dri"
#define GROUP_ATCI  "atci"
#define GROUP_RELAY  "Relay"
...
#define ATTR_HB_ASSERT_STR     static const char
#else
...
#define BT_ASSERT(assert_enable) if ((assert_enable) == false) {\
    __ASSERT(assert_enable, #assert_enable, __FILE__, __LINE__);   \
}
#endif
...
#define BT_LOGHEX(module_name, _str , _data, _length )\
    {\
        uint8_t i = 0; \
        printk("[H]["module_name"]"_str); \
        do { \
        printk("%02X ",*(const uint8_t *)(_data+i)); \
            i++; \
        } while (i < _length);\
        printk(" \n"); \
    }
...
#define BT_LOGI(module_name, format, ...)   \
    MDM_TM_PRINTF(format, ## __VA_ARGS__)
#else

#if defined (CONFIG_LOG)
#define BT_LOGI(module_name, format, ...)   \
    LOG_INF("[I][" module_name"]" format, ## __VA_ARGS__)
