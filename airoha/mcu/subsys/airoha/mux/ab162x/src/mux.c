...
#else
volatile mux_port_config_t *g_mux_port_configs = (volatile mux_port_config_t *)CROSS_CORE_MEM_VARIABLE(mux);
#endif/* MTK_SINGLE_CPU_ENV */

SIZE_STATIC_CHECK_LE(MUX_PRIVATE_LEN / sizeof(mux_port_config_t), MUX_PORT_END);
...
void mux_clear_sw_wptr(uint8_t port_index)
{
    g_mux_port_configs[port_index].sw_wptr = 0;
}
...
ATTR_TEXT_IN_TCM static uint32_t parse_port_type(mux_port_t port, mux_port_type_t *port_type)
{
    if (port <= MUX_UART_END) {
        ...
        *port_type = MUX_PORT_TYPE_BT;
        return port - MUX_BT_BLE;
    }
#endif /* AIR_BT_AIR_SERVICE_ENABLE */
...
{
    bool buf_full;
    mux_port_type_t port_type = MUX_PORT_TYPE_MAX;
...
void mux_pop_one_package_data_from_rx_buff_fifo_internal(uint32_t user_id)
{
...
bool mux_pop_one_package_data_from_rx_buff_fifo(uint32_t user_id, uint8_t *read_buf, uint32_t read_len, uint32_t *real_read_len)
{
    ...
    if (!((p_user_config->is_empty == true) || (read_len < p_user_config->data_size[p_user_config->rptr]))) {
        src_addr = p_user_config->p_data[p_user_config->rptr];
        src_len = p_user_config->data_size[p_user_config->rptr];
        mux_pop_one_package_data_from_rx_buff_fifo_internal(user_id);
    } else {
...
void mux_clear_package_data_from_rx_buff_fifo(uint32_t uid)
{
    volatile mux_user_config_t *p_user_config = &g_mux_user_configs[uid];
    uint32_t per_cpu_irq_mask = 0;
    uint32_t clear_counter = 0;
    uint8_t *src_addr;

    for (;clear_counter <= MAX_MUX_USER_RX_DATA_CNT;) {
        port_mux_local_cpu_enter_critical(&per_cpu_irq_mask);
        if (p_user_config->is_empty == false) {
            src_addr = p_user_config->p_data[p_user_config->rptr];
            mux_pop_one_package_data_from_rx_buff_fifo_internal(uid);
        } else {
            port_mux_local_cpu_exit_critical(per_cpu_irq_mask);
            break;
        }
    ...
    MUX_PORT_MSGID_I("port[%d] uid[%d] clear %d packets data\r\n", 3, p_user_config->port, uid, clear_counter);
}

void mux_clear_all_package_data_from_rx_buff_fifo(mux_port_t port)
{
    uint32_t uid;

    for (uid = 0; uid < MAX_MUX_USER_COUNT; uid++) {
        if (g_mux_user_configs[uid].port == port) {
            mux_clear_package_data_from_rx_buff_fifo(uid);
        }
    }
}
...
static void irq_mux_handler(mux_port_t port, mux_event_t type, void *irq_data)
{
    ...
    uint32_t rx_src_address_new, rx_src_address = 0, rx_src_length = 0;             /*for transparent mechanism*/
    mux_port_type_t port_type = MUX_PORT_TYPE_MAX;
    ...
    case MUX_EVENT_READY_TO_READ: {
        ...
        while (total_rx_left_data_size > 0) {
            hw_rx_read_ptr = port_mux_device_get_hw_rptr((uint8_t)port_type, port_index, true);
            ...
            if ((handle_to_port(handle) != port) && (new_pkg_len != 0)) {
                ...
                port_mux_device_set_rx_hw_rptr((uint8_t)port_type, port_index, total_rx_left_data_size);
            ...
            if (consume_len > 0) {
                /* Move the read pointer for corrupt data and protocol header */
                port_mux_local_cpu_enter_critical(&irq_mask);
                port_mux_device_set_rx_hw_rptr((uint8_t)port_type, port_index, consume_len);
            ...
            if (user_id >= MAX_MUX_USER_COUNT) {
                MUX_PORT_MSGID_E("Mux error: user_id %ld", 1, user_id);
                return;
            }
            ...
            if (mux_push_one_package_data_to_rx_buff_fifo(user_id, hw_rx_read_ptr, new_pkg_len, p_rx->rx_buf_addr, p_rx->rx_buf_size) == true) {
                port_mux_local_cpu_enter_critical(&irq_mask);
                port_mux_device_set_rx_hw_rptr((uint8_t)port_type, port_index, new_pkg_len);
                port_mux_local_cpu_exit_critical(irq_mask);
    ...
    case MUX_EVENT_TRANSPARENT_READ: {
        ...
        port_mux_device_get_rx_src_address((uint8_t)port_type, port_index, (uint32_t)&rx_src_address, (uint32_t)&rx_src_length);
...
extern volatile uint32_t g_syslog_port;
mux_status_t mux_init(mux_port_t port, mux_port_setting_t *setting, mux_protocol_t *protocol_callback)
{
    ...
    if ((sizeof(mux_port_config_t) * MUX_PORT_END) > MUX_PRIVATE_LEN) {
    ...
    if(g_syslog_port == port)
    {
        g_mux_port_configs[port].tx_buf_addr = setting->tx_buffer;
    }else{
        g_mux_port_configs[port].tx_buf_addr = (uint32_t)k_malloc(g_mux_port_configs[port].tx_buf_size);
    }
    if(!g_mux_port_configs[port].tx_buf_addr) {
        assert(0 && "MUX Init malloc txbuff fail");
    }
    g_mux_port_configs[port].rx_buf_size = setting->rx_buffer_size;
    if(g_syslog_port == port)
    {
    g_mux_port_configs[port].rx_buf_addr = setting->rx_buffer;
    }else{
        g_mux_port_configs[port].rx_buf_addr = (uint32_t)k_malloc(g_mux_port_configs[port].rx_buf_size);
    }
    if(!g_mux_port_configs[port].rx_buf_addr) {
        assert(0 && "MUX Init malloc rxbuff fail");
    }
    ...
    if (g_mux_device_ops_array[port_type] == NULL) {
        ...
        k_free((uint32_t *)g_mux_port_configs[port].tx_buf_addr);
        g_mux_port_configs[port].rx_buf_size = 0;
        k_free((uint32_t *)g_mux_port_configs[port].rx_buf_addr);
        }
...
{
    uint8_t port_type = MUX_PORT_TYPE_MAX, port_index, user_id;
    uint8_t cpu_id;
    ...
    if (MUX_STATUS_OK != port_mux_device_deinit((uint8_t)port_type, port_index)) {
...
mux_status_t mux_exception_init(mux_handle_t handle)
{
    ...
    port_index = parse_port_type(port, &port_type);
    port_mux_device_exception_init((uint8_t)port_type, port_index);
...
mux_status_t mux_exception_send(mux_handle_t handle, const mux_buffer_t *buffers, uint32_t buffers_counter)
{
    ...
    for (i = 0; i < buffers_counter; i++) {
        port_mux_device_exception_send((uint8_t)port_type, port_index, buffers[i].p_buf, buffers[i].buf_size);
    }
    return MUX_STATUS_OK;
}
...
{
    //mux_handle_t handle;
    uint32_t user_id, per_cpu_irq_mask = 0;
    volatile mux_user_config_t *p_user_config;
    *p_handle = 0xdeadbeef;
...
mux_status_t mux_query_user_port_setting(const char *user_name, mux_port_config_t *setting)
{
    ...
    for (i = 0; i < MAX_MUX_USER_COUNT; i++) {
        if (g_mux_user_configs[i].user_name != NULL) {
...
mux_status_t mux_query_user_name(mux_handle_t handle, const char **user_name)
{
    uint8_t user_id;
    user_id = handle_to_user_id(handle);
    if ((handle_is_valid(handle) == false) || (user_id >= MAX_MUX_USER_COUNT)) {
        return MUX_STATUS_ERROR_PARAMETER;
    }
    if (g_mux_user_configs[user_id].user_name == NULL) {
        *user_name = NULL;
    ...
    for (user_index = 0; user_index < 32; user_index++) {
        if ((p_buffer[port] & (0x1 << user_index)) != 0) {
            if (mux_query_name_by_user_index(user_index, &p_name) == MUX_STATUS_OK) {
                strncpy(port_conf[count].name, p_name, sizeof(port_conf[count].name) - 1);
                port_conf[count].name[sizeof(port_conf[count].name) - 1] = '\0';
                count++;
            }
        }
    }
...
mux_status_t mux_query_user_port_numbers_from_nvdm(const char *user_name, uint32_t *port_count)
{
    uint32_t *p_buffer;
    uint8_t nvkey_buffer[4 * MUX_PORT_NUMBER];
    uint8_t index = 0, count = 0, port_index = 0;
...
{
    uint32_t p_log_hw_wptr, per_cpu_irq_mask = 0;
    ...
    if (g_mux_port_configs[port].tx_buf_size != 0) {
        ...
        if (g_mux_port_configs[port].init_status == MUX_INIT_EXCEPTION) {
            if (head_buf_info.p_buf != NULL) {
                port_mux_device_exception_send((uint8_t)port_type, port_index, head_buf_info.p_buf, head_buf_info.buf_size);
            }
            for (i = 0; i < buffers_counter; i++) {
                port_mux_device_exception_send((uint8_t)port_type, port_index, buffers[i].p_buf, buffers[i].buf_size);
            }
            if (tail_buf_info.p_buf != NULL) {
                port_mux_device_exception_send((uint8_t)port_type, port_index, tail_buf_info.p_buf, tail_buf_info.buf_size);
            }
            return payload_size;
        }
#endif
        ...
        mux_status = port_mux_device_phase1_send((uint8_t)port_type, port_index);
        ...
        mux_status = port_mux_device_phase2_send((uint8_t)port_type, port_index);
        if (mux_status != MUX_STATUS_OK) {
            return mux_status;
        }
    } else {
        ...
        */
        ...
        if (buffers_counter == 1) {
            ...
            port_mux_device_set_tx_src_address((uint8_t)port_type, port_index, (uint32_t)buffers[0].p_buf, buffers[0].buf_size);
        } else {
...
mux_status_t mux_control(mux_port_t port, mux_ctrl_cmd_t command, mux_ctrl_para_t *para)
{
    ...
    switch (command) {
        ...
        case MUX_CMD_CHANGE_UART_PARAM: {
            port_index = parse_port_type(port, &port_type);
            status = port_mux_device_normal_control(port_type, (uint8_t)port_index, command, para);
        ...
        case MUX_CMD_CLEAN: {
            ...
            //mux_clear_all_package_data_from_rx_buff_fifo(port);
        } break;
