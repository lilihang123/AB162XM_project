...
void mux_common_device_buf_clear(virtual_read_write_point_t *p, bool is_rx)
{
    if (is_rx) {
...
bool mux_common_device_buf_is_empty(virtual_read_write_point_t *p, bool is_rx)
{
    if (is_rx) {
        return p->rx_buff_available_len == 0 ? TRUE : FALSE;
    } else {
        return p->tx_buff_available_len == 0 ? TRUE : FALSE;
    }
}
...
void mux_common_device_set_rx_hw_rptr(virtual_read_write_point_t *p, uint32_t move_bytes)
{
    ...
    MOVE_RX_RING_BUFF_RPTR(p, move_bytes);
...
void mux_common_device_set_rx_hw_wptr_internal_use(virtual_read_write_point_t *p, uint32_t move_bytes)
{
    ...
    MOVE_RX_RING_BUFF_WPTR(p, move_bytes);
...
void mux_common_device_set_tx_hw_rptr_internal_use(virtual_read_write_point_t *p, uint32_t move_bytes)
{
    ...
    MOVE_TX_RING_BUFF_RPTR(p, move_bytes);
...
void mux_common_device_set_tx_hw_wptr(virtual_read_write_point_t *p, uint32_t move_bytes)
{
    ...
    ************************/
    MOVE_TX_RING_BUFF_WPTR(p, move_bytes);
    /************************
