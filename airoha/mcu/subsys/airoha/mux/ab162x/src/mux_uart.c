...
static volatile mux_port_config_t *g_uart_port_configs = (volatile mux_port_config_t *)CROSS_CORE_MEM_VARIABLE(mux); //80 * N
#endif


#ifdef AIR_BYPASS_SYSLOG_WITH_LOG_FLOW_ENABLE
extern volatile uint32_t g_syslog_port;
#define MUX_LOG_PORT g_syslog_port
static virtual_read_write_point_t mux_uart_log_vrwp = {0};
#endif
extern uint8_t query_syslog_init_phase_states(void);
...
{
#ifdef AIR_BYPASS_SYSLOG_WITH_LOG_FLOW_ENABLE
    if ((port_index == MUX_LOG_PORT) && (exception_get_disable_all_log_flag() != 0) && (query_syslog_init_phase_states() != 0)) {
        return mux_common_device_get_hw_rptr(&mux_uart_log_vrwp, is_rx);
    } else {
...
#ifdef AIR_BYPASS_SYSLOG_WITH_LOG_FLOW_ENABLE
    if ((port_index == MUX_LOG_PORT) && (exception_get_disable_all_log_flag() != 0)&&(query_syslog_init_phase_states() != 0 )) {
        return mux_common_device_get_hw_wptr(&mux_uart_log_vrwp, is_rx);
    } else {
...
#ifdef AIR_BYPASS_SYSLOG_WITH_LOG_FLOW_ENABLE
    if ((port_index == MUX_LOG_PORT) && (exception_get_disable_all_log_flag() != 0)&& (query_syslog_init_phase_states() != 0 )) {
        mux_common_device_set_rx_hw_rptr(&mux_uart_log_vrwp, move_bytes);
    } else {
...
ATTR_TEXT_IN_TCM void port_mux_uart_set_tx_hw_wptr(uint8_t port_index, uint32_t move_bytes)
{
    ...
    if (g_uart_on_off_status[port_index] == true) {
        if ((port_index == MUX_LOG_PORT) && (exception_get_disable_all_log_flag() != 0)&& (query_syslog_init_phase_states() != 0 )) {
            mux_common_device_set_tx_hw_wptr(&mux_uart_log_vrwp, move_bytes);
            mux_common_device_set_tx_hw_rptr_internal_use(&mux_uart_log_vrwp, move_bytes);
        } else {
...
mux_status_t port_mux_uart_normal_init(uint8_t port_index, mux_port_config_t *p_setting, mux_irq_handler_t irq_handler)
{
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
    mux_port_t syslog_port_is_runing = MUX_UART_0;
    ...
    if (p_setting->p_user_setting->dev_setting.uart.flowcontrol_type == MUX_UART_NONE_FLOWCONTROL) {
        ...
        uart_send_xon_xoff(port_index, 1, 0); //send xon
...
#ifdef AIR_BYPASS_SYSLOG_WITH_LOG_FLOW_ENABLE
    if (port_index == MUX_LOG_PORT) {
        mux_common_device_r_w_point_init(&mux_uart_log_vrwp, p_setting);
    }
#endif
...
void port_mux_uart_clear_fifo(uint8_t port_index, mux_ctrl_cmd_t cmd)
{
    ...
    if(cmd == MUX_CMD_CLEAN_TX_VIRUTUAL) {
        if ((port_index == MUX_LOG_PORT) && (exception_get_disable_all_log_flag() != 0)) {
            mux_common_device_buf_clear(&mux_uart_log_vrwp, 0);
        } else {
            status = uart_clear_vfifo_and_fifo(port_index, 0);   //tx
        }
    } else if(cmd == MUX_CMD_CLEAN_RX_VIRUTUAL) {
        if ((port_index == MUX_LOG_PORT) && (exception_get_disable_all_log_flag() != 0)) {
            mux_common_device_buf_clear(&mux_uart_log_vrwp, 1);
        } else {
...
mux_status_t port_mux_uart_control(uint8_t port_index, mux_ctrl_cmd_t command, mux_ctrl_para_t *para)
{
    ...
    switch (command) {
...
#ifdef AIR_BYPASS_SYSLOG_WITH_LOG_FLOW_ENABLE
            if ((port_index == MUX_LOG_PORT) && (exception_get_disable_all_log_flag() != 0)) {
                uart_status = HAL_UART_STATUS_OK;
            } else {
...
#ifdef AIR_BYPASS_SYSLOG_WITH_LOG_FLOW_ENABLE
            if ((port_index == MUX_LOG_PORT) && (exception_get_disable_all_log_flag() != 0)) {
                isRxIdle = mux_common_device_buf_is_empty(&mux_uart_log_vrwp, 1);
                rxLength = mux_uart_log_vrwp.rx_buff_available_len;
            } else {
        ...
        case MUX_CMD_GET_TRX_PORT_IDLE: {
            ...
            if ((port_index == MUX_LOG_PORT) && (exception_get_disable_all_log_flag() != 0)) {
                isRxIdle = mux_common_device_buf_is_empty(&mux_uart_log_vrwp, 1);
                isTxIdle = mux_common_device_buf_is_empty(&mux_uart_log_vrwp, 0);
                rxLength = mux_uart_log_vrwp.rx_buff_available_len;
            } else {
                isRxIdle = uart_query_rx_empty(port_index);
                isTxIdle = uart_query_tx_empty(port_index);
