...
#define MUX_PORT_MSGID_I(fmt,cnt,arg...)           LOG_INF(fmt, ##arg)
#define MUX_PORT_MSGID_W(fmt,cnt,arg...)           LOG_WRN(fmt, ##arg)
#define MUX_PORT_MSGID_E(fmt,cnt,arg...)           LOG_ERR(fmt, ##arg)
#else
#define MUX_PORT_MSGID_I(fmt,cnt,arg...)
#define MUX_PORT_MSGID_W(fmt,cnt,arg...)
#define MUX_PORT_MSGID_E(fmt,cnt,arg...)
#endif
...
#define MUX_PRIVATE_LEN (sizeof(mux_port_config_t) * MUX_PORT_END)
...
typedef struct {
    ...
    mux_status_t (*phase1_send)(uint8_t port_index);
    mux_status_t (*phase2_send)(uint8_t port_index);
...
#define MOVE_RING_BUFF(start, current, move, end, new_point) \
    if ((current + move) >= end) {\
        new_point = start + (current + move - end);\
    } else {\
        new_point = current + move;\
    }

#define MOVE_TX_RING_BUFF_RPTR(p, move_bytes) \
do {\
...
#define MOVE_RX_RING_BUFF_WPTR(p, move_bytes) \
do {\
...
extern void mux_common_device_buf_clear(virtual_read_write_point_t *p, bool is_rx);
extern bool mux_common_device_buf_is_empty(virtual_read_write_point_t *p, bool is_rx);
typedef enum {
