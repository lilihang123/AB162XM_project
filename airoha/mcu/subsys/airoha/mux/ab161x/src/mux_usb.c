                                     ...
                                     uint16_t len, uint8_t *buf)
{
    ...
    if (id == AIR_USB_HID_AIR_IN_REPORT_ID && len == AIR_USB_HID_AIR_IN_REPORT_LEN) {
        ...
        buf[0] = AIR_USB_HID_AIR_IN_REPORT_ID;
        buf[1] = tx_size;
                                     ...
                                     uint16_t len, const uint8_t *buf)
{
    ...
    if (id == AIR_USB_HID_AIR_OUT_REPORT_ID && len == AIR_USB_HID_AIR_OUT_REPORT_LEN) {
        port_index = MUX_USB_PORT_HID_2;

        if (buf[0] != AIR_USB_HID_AIR_OUT_REPORT_ID) {
