...
#define MUX_BT_INVAILD_HANDLE    0xffff //0x00000000
...
static mux_status_t mux_bt_air_init(mux_bt_port_group_context_t *group_context)
{
    ...
    uint8_t i = 0;
#ifdef AIR_PURE_GAMING_DONGLE_ENABLE
    for (i = 0; i < (MUX_BT_BLE_1 - MUX_BT_BLE + 1); i ++)
#else
    for (i = 0; i < (MUX_BT_BLE - MUX_BT_BLE + 1); i ++)
#endif
    {
        mux_port_context[i].connection_handle = MUX_BT_INVAILD_HANDLE;
    }
