...
#if defined(CONFIG_AIR_FOTA_UPGRADE_ENABLE) || (MOD_CFG_FOTA_BL_RESERVED)
#include "hal_wdt.h"
#endif
	...
	lzma_alloc_t *lzma_alloc)
{
...
#ifndef MTK_FOTA_AES_ENCRYPT_ENABLE
    ...
    {
        printk("destination error: must at begin address of one block");
        return LZMA_ERROR_MEM;
    }
    ...
    {
        printk("ERROR: decompressed size over reserverd size!!!\r\n");
        return LZMA_ERROR_MEM;
    }

    LzmaDec_Construct(&state);
    LzmaDec_Allocate(&state, lzma_header, LZMA_PROPS_SIZE, g_alloc);
    printk("LZMA decode begin: \r\n");
    printk("destination = %x\r\n", (unsigned int)destination);
    printk("source = %x\r\n", (unsigned int)(source + sizeof(lzma_header)));
    printk("unpacked_size = %d\r\n", unpackSize);
