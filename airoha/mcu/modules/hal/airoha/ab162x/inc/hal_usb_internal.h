...
// #define    BIT(bit)                        ((uint32_t)0x1 << bit)
...
/* USB_INTRTX */
#define    USB_INTRTX_EPn(ep_num)    (0x01 << ep_num)    /* RO */
...
/* TQCSR */
#define    USB_TQCSRn(ep_num)            (((ep_num) - 1) * 0x10)
#define    USB_TXQ_START                     BIT(0)
#define    USB_TXQ_RESUME                    BIT(1)
#define    USB_TXQ_STOP                      BIT(2)
#define    USB_TXQ_ACTIVE                   BIT(15)

/* RQCSR */
#define    USB_RQCSRn(ep_num)            (((ep_num) - 1) * 0x10)
#define    USB_RXQ_START                     BIT(0)
#define    USB_RXQ_RESUME                    BIT(1)
#define    USB_RXQ_STOP                      BIT(2)
#define    USB_RXQ_ACTIVE                   BIT(15)

/* TQSAR */
#define    USB_TQSARn(ep_num)            (((ep_num) - 1) * 0x10)

/* RQSAR */
#define    USB_RQSARn(ep_num)            (((ep_num) - 1) * 0x10)

/* USBGCSR */
#define    USB_QUE_EN_TXn(ep_num)          (BIT(0) << (ep_num-1))
#define    USB_QUE_EN_RXn(ep_num)         (BIT(16) << (ep_num-1))

/* QCR2 */
#define    USB_QMU_TX_ZLPn(ep_num)        (BIT(8) << (ep_num-1))

/* QCR3 */
#define    USB_CFG_RX_COZn(ep_num)         (BIT(8) << (ep_num-1))
#define    USB_CFG_RX_ZLPn(ep_num)        (BIT(24) << (ep_num-1))

/* GPZCR */
#define    USB_QMU_TX_GPZCRn(ep_num)     (BIT(0) << (ep_num-1))
#define    USB_CFG_RnQ_PURE_ZLP(ep_num)     (BIT(0) << (ep_num-1))

/* IOCDISR */
//#define    USB_RX0_IOCDISR

/* TQCPR */
#define    USB_TXQ_CURR_PTRn(ep_num)     (((ep_num) - 1) * 0x10)

/* RXCPR */
#define    USB_RXQ_CURR_PTRn(ep_num)     (((ep_num) - 1) * 0x10)

/* 0xA08 TQCPR */ // unused
#define    USB_TQCPRn(ep_num)            (((ep_num) - 1) * 0x10)

/* 0xC00 QISAR */
#define    USB_QMU_TXn_DONE(ep_num)         (BIT(0) << (ep_num))
#define    USB_QMU_RXn_DONE(ep_num)         (BIT(8) << (ep_num))
#define    USB_QMU_TX_EMPTY                 BIT(16)
#define    USB_QMU_RX_EMPTY                 BIT(17)

/* 0xC08 QIMCR */
#define    USB_TnQ_DONE_INTMC(ep_num)       (BIT(0) << (ep_num-1))
#define    USB_RnQ_DONE_INTMC(ep_num)       (BIT(8) << (ep_num-1))
#define    USB_RXBEQ_ERR_INTMC              BIT(24)
#define    USB_RXQ_ERR_INTMC                BIT(25)
#define    USB_RXEP_ERR_INTMC               BIT(26)
#define    USB_TXQ_ERR_INTMC                BIT(27)
#define    USB_TXEP_ERR_INTMC               BIT(28)

/* 0xCB4 TQEIMCR */
/* 0xCB8 TEPEIMCR */
#define    USB_TnQ_LENERR_IMCR(ep_num)     (BIT(16) << (ep_num-1))

/* 0xCC8 TEPEIMCR */
#define    USB_TnEP_ERRIMCR(ep_num)         (BIT(0) << (ep_num-1))

/* 0xCC0 IOCDISR */
#define    USB_TnQ_IOC_DIS(ep_num)          (BIT(0) << (ep_num-1))
#define    USB_RnQ_IOC_DIS(ep_num)          (BIT(8) << (ep_num-1))

/* 0x0670 USB_PKTDONE_INTR_STS */
#define USB_PKTDONE_INTR_STS_SET0            (BIT(0))
#define USB_PKTDONE_INTR_STS_SET1            (BIT(1))

/* 0x0671 USB_PKTDONE_INTR_MSK */
#define USB_PKTDONE_INTR_MSK_SET0            (BIT(0))
#define USB_PKTDONE_INTR_MSK_SET1            (BIT(1))

/* 0x0672 USB_PKTDONE_INTR_CLR */
#define USB_PKTDONE_INTR_CLR_SET0            (BIT(0))
#define USB_PKTDONE_INTR_CLR_SET1            (BIT(1))

/* 0x0674 USB_PKTDONE_SET0 */
#define USB_PKTDONE_SET0_EP_OFFSET           0
#define USB_PKTDONE_SET0_DIR_OFFSET          4
#define USB_PKTDONE_SET0_EN_OFFSET           7

/* 0x0675 USB_PKTDONE_SET1 */
#define USB_PKTDONE_SET1_EP_OFFSET           0
#define USB_PKTDONE_SET1_DIR_OFFSET          4
#define USB_PKTDONE_SET1_EN_OFFSET           7

//========================definition of global value===================//
#define    USB_MAX_FIFO_SIZE                     (4096 + 64)
...
typedef struct {
    ...
    hal_usb_driver_interrupt_handler_t    sof_hdlr;
...
#ifdef HAL_USB_QMU_MODULE_ENABLED
    hal_usb_qmu_gpd_ring_t *ring;   // note 4 HAL_USB_MAX_NUMBER_DMA
    hal_usb_qmu_gpd_t *qmu_tx_buf;
#endif
} USB_Drv_Info;
...
typedef struct {
   ...
   uint8_t  revered18[100];
   /*0x670*/
   uint8_t  pktdone_intr_sts;
   uint8_t  pktdone_intr_msk;
   uint8_t  pktdone_intr_clr;
   uint8_t  reserved_pktdone_1;
   uint8_t  pktdone_set0;
   uint8_t  pktdone_set1;
   uint8_t  reserved_pktdone_2[14];
   /*0x684*/
   ...
   /*0x700*/
   uint32_t resreg;                     /* Reserved Register */
   uint16_t hwtpr;                      /* HW TxPktRdy Set Register */
   uint8_t  revered21[2];
   uint16_t hwtpr_en;                   /* HW TxPktRdy Enable Register */
   uint8_t  revered22[2];
   uint16_t hwtpr_err;                  /* HW TX Direct Path Error Detection Register */
   uint8_t  revered23[2];
   /*0x710*/
   uint8_t  lpm_csr;                    /* LPM CSR Register */
   uint8_t  lpm_interrupt;              /* LPM Interrupt Status Register */
   uint8_t  lpm_interrupt_en;           /* LPM Interrupt Enable Register */
   uint8_t  lpm_besl;                   /* LPM BESL Register */
   uint32_t lpm_timing_param;           /* LPM Timing Parameter Register */
   uint32_t lpm_exit_ep_chk;            /* LPM Exit Endpoint Check Register */
   uint32_t lpm_exit_ep_fc_chk;         /* LPM Exit Endpoint Flow Check Register */
   /*0x720*/
   uint8_t  lpm_accept_inactive_chk;    /* LPM Accept Inactive Check Register */
   uint8_t  revered24[15];
   /*0x730*/
   uint8_t  otg20_csrl;                 /* OTG20 Related Control Register L */
   uint8_t  otg20_csrh;                 /* OTG20 Related Control Register H */
   uint8_t  revered25[46];
   /*0x760*/
   uint32_t spintrtxrx;                 /* Split Tx/Rx Interrupt Status Register */
   uint32_t spintrtxrxe;                /* Split Tx/Rx Interrupt Enable Register */
   uint8_t  spintrusb;                  /* Split Common USB Interrupt Register */
   uint8_t  revered26[3];
   uint8_t  spintrusbe;                 /* Split Common USB Interrupt Enable Register */
   uint8_t  revered27[3];
   /*0x770*/
   uint32_t spusb_l1ints;               /* Split USB Level 1 Interrupt Status Register */
   uint32_t spusb_l1intm;               /* Split USB Level 1 Interrupt Mask Register */
   uint8_t  revered28[136];
   /*0x800*/
   uint32_t qcr0;                       /* Queue Control Register 0 */
   uint32_t qcr1;                       /* Queue Control Register 1 (wimax) */
   uint32_t qcr2;                       /* Queue Control Register 2 */
   uint32_t qcr3;                       /* Queue Control Register 3 */
   /*0x810*/
   uint32_t rqcsr0;                     /* RX Queue Command and Status Register 0 */
   uint32_t rqsar0;                     /* RX Queue Starting Address Register 0 */
   uint32_t rqcpr0;                     /* RX Queue Current Pointer Register 0 */
   uint8_t  revered29[4];
   /*0x820*/
   uint32_t rqcsr1;                     /* RX Queue Command and Status Register 1 */
   uint32_t rqsar1;                     /* RX Queue Starting Address Register 1 */
   uint32_t rqcpr1;                     /* RX Queue Current Pointer Register 1 */
   uint8_t  revered30[4];
   /*0x830*/
   uint32_t rqcsr2;                     /* RX Queue Command and Status Register 2 */
   uint32_t rqsar2;                     /* RX Queue Starting Address Register 2 */
   uint32_t rqcpr2;                     /* RX Queue Current Pointer Register 2 */
   uint8_t  revered31[84];
   /*0x890*/
   uint32_t rqtr0;                      /* RX Timeout Register 0 */
   uint32_t rqtr1;                      /* RX Timeout Register 1 */
   uint32_t rqtr2;                      /* RX Timeout Register 2 */
   uint8_t  revered32[100];
   /*0x900*/
   uint32_t rqldpr0;                    /* RX Queue Last Done Pointer Register 0 */
   uint32_t rqldpr1;                    /* RX Queue Last Done Pointer Register 1 */
   uint32_t rqldpr2;                    /* RX Queue Last Done Pointer Register 2 */
   uint8_t  revered33[244];
   /*0xA00*/
   uint32_t tqcsr0;                     /* TX Queue Command and Status Register 0 */
   uint32_t tqsar0;                     /* TX Queue Starting Address Register 0 */
   uint32_t tqcpr0;                     /* TX Queue Current Pointer Register 0 */
   uint8_t  revered34[4];
   /*0xA10*/
   uint32_t tqcsr1;                     /* TX Queue Command and Status Register 1 */
   uint32_t tqsar1;                     /* TX Queue Starting Address Register 1 */
   uint32_t tqcpr1;                     /* TX Queue Current Pointer Register 1 */
   uint8_t  revered35[4];
   /*0xA20*/
   uint32_t tqcsr2;                     /* TX Queue Command and Status Register 2 */
   uint32_t tqsar2;                     /* TX Queue Starting Address Register 2 */
   uint32_t tqcpr2;                     /* TX Queue Current Pointer Register 2 */
   uint8_t  revered36[4];
   /*0xA30*/
   uint32_t tqcsr3;                     /* TX Queue Command and Status Register 3 */
   uint32_t tqsar3;                     /* TX Queue Starting Address Register 3 */
   uint32_t tqcpr3;                     /* TX Queue Current Pointer Register 3 */
   uint8_t  revered37[196];
   /*0xB00*/
   uint32_t usbgcsr;                    /* USB General Control and Status Register */
   uint32_t usb_fw1;                    /* USB Firmware Register 1 (wimax) */
   uint32_t usb_fw2;                    /* USB Firmware Register 2 (wimax) */
   uint8_t  revered38[116];
   /*0xB80*/
   uint32_t t0qusbsc;                   /* T0Q USB Stream Constraints (wimax) */
   uint32_t t1qusbsc;                   /* T1Q USB Stream Constraints (wimax) */
   uint32_t t2qusbsc;                   /* T2Q USB Stream Constraints (wimax) */
   uint32_t t3qusbsc;                   /* T3Q USB Stream Constraints (wimax) */
   uint8_t  revered39[112];
   /*0xC00*/
   uint32_t qisar;                      /* QMU Interrupt Status and Acknowledgement Register */
   uint32_t qimr;                       /* QMU Interrupt Mask Register */
   uint32_t qimcr;                      /* QMU Interrupt Mask Clear Register */
   uint8_t  revered40[20];
   /*0xC20*/
   uint32_t dsicr;                      /* Device Software Interrupt Command Register (wimax) */
   uint8_t  revered41[12];
   /*0xC30*/
   uint32_t iocdisr;                    /* GPD Done Interrupt on GPD IOC bit Disable Register */
   uint32_t gpzcr;                      /* GPD Pure ZLP Control Register */
   uint8_t  revered42[8];
   /*0xC40*/
   uint32_t qmu_hwver;                  /* QMU Hardware Version Register */
   uint8_t  revered43[108];
   /*0xCB0*/
   uint8_t  revered44[8];
   uint32_t tqeimcr;                    /* TX Queue Error Indication Mask Clear Register */
   uint8_t  revered465[4];
   /*0xCC0*/
   uint8_t  revered46[8];               /* TX Endpoint Error Indication Mask Clear Register */
   uint32_t tepeimcr;
   /**/
} USB_REGISTER_T;
...
#define USBPHYACR5_RG_32K_RESUME_EN_SFT            (17)
#define USBPHYACR5_RG_K_STATE_LENGTH_SFT           (18)
#define USBPHYACR5_RG_IS_MODE_SHT                  (31)

#define USBPHYACR5_RG_32K_RESUME_EN_MSK            (1 << 17)
#define USBPHYACR5_RG_K_STATE_LENGTH_MSK           (0x0FFF << 18)
#define USBPHYACR5_RG_IS_MODE_MSK                  (1 << 31)
...
/**
 * @brief Get USB EP FIFO empty.
 * @param[in] ep_num is the endpoint number.
 * @param[in] dir_in    is the USB endpoint direction.
 * @return False, if there is data in FIFO pending to send.
 */
bool hal_usb_ep_empty(uint32_t ep_num, bool dir_in);

/**
 * @brief Get USB EP0 packet ready state
 * @return true, if tx packet ready is set.
 */
bool hal_usb_check_ep0_tx_packet_ready(void);

/**
 * @brief Clear EP0 queue event.
 *
 * @return void.
 */
...
/**
 * @brief Get USB EP interrupt enable status.
 * @param[in] ep_num is the endpoint number.
 * @return False, if related interrupt disable
 */
bool hal_usb_get_tx_interrupt_enable_status(uint32_t ep_num);
