...
typedef union {
    struct {
        uint32_t MCEN               :1;         /**< CACHE enable, bit 0 */
        uint32_t __reserved_00      :1;
        uint32_t CNTEN0             :1;         /**< Hit counter0 enable, bit 2 */
        uint32_t CNTEN1             :1;         /**< Hit counter1 enable, bit 3 */
        uint32_t __reserved_01      :3;
        uint32_t MDRF               :1;         /**< Early restart, bit 7 */
        uint32_t __reserved_02      :2;         /**< CACHE size, bit fixed 16KB */
        uint32_t TWO_WAY_CACHE_EN   :1;         /**< Enable 2-way cache look-up */
        uint32_t __reserved_03      :21;
    } b;
    uint32_t w;
} CACHE_CON_Type;
...
typedef union {
    struct {
        uint32_t __reserved_00      :8;
        uint32_t EN                 :1;         /**< Cacheable bit */
        uint32_t __reserved_01      :3;
...
extern CACHE_NCREMAP_ENTRY_Type g_cache_remap_entry[4];

/**< CACHE region size must be unit aligned */
#define CACHE_REGION_SIZE_UNIT  (CONFIG_CACHEABLE_REGION_ALIGNMENT)
...
#define CACHE_BACKUP_ONE_REMAP_REGION(num) { \
        g_cache_remap_entry[num].high.w = CACHE->remap[num].high.w; \
        g_cache_remap_entry[num].low.w = CACHE->remap[num].low.w; \
    }

#define CACHE_RESTORE_ONE_REMAP_REGION(num) { \
        CACHE->remap[num].high.w = g_cache_remap_entry[num].high.w; \
        CACHE->remap[num].low.w = g_cache_remap_entry[num].low.w; \
    }
 ...
 */
void CACHE_STATUS_BACKUP(void);


#define CACHE_RESTORE_ONE_CACHE_REGION(num) \
