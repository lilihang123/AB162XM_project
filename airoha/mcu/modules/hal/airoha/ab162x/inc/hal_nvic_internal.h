/* Copyright Statement:
 *
 * (C) 2017  Airoha Technology Corp. All rights reserved.
 *
 * This software/firmware and related documentation ("Airoha Software") are
 * protected under relevant copyright laws. The information contained herein
 * is confidential and proprietary to Airoha Technology Corp. ("Airoha") and/or its licensors.
 * Without the prior written permission of Airoha and/or its licensors,
 * any reproduction, modification, use or disclosure of Airoha Software,
 * and information contained herein, in whole or in part, shall be strictly prohibited.
 * You may only use, reproduce, modify, or distribute (as applicable) Airoha Software
 * if you have agreed to and been bound by the applicable license agreement with
 * Airoha ("License Agreement") and been granted explicit permission to do so within
 * the License Agreement ("Permitted User").  If you are not a Permitted User,
 * please cease any access or use of Airoha Software immediately.
 * BY OPENING THIS FILE, RECEIVER HEREBY UNEQUIVOCALLY ACKNOWLEDGES AND AGREES
 * THAT AIROHA SOFTWARE RECEIVED FROM AIROHA AND/OR ITS REPRESENTATIVES
 * ARE PROVIDED TO RECEIVER ON AN "AS-IS" BASIS ONLY. AIROHA EXPRESSLY DISCLAIMS ANY AND ALL
 * WARRANTIES, EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE IMPLIED WARRANTIES OF
 * MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE OR NONINFRINGEMENT.
 * NEITHER DOES AIROHA PROVIDE ANY WARRANTY WHATSOEVER WITH RESPECT TO THE
 * SOFTWARE OF ANY THIRD PARTY WHICH MAY BE USED BY, INCORPORATED IN, OR
 * SUPPLIED WITH AIROHA SOFTWARE, AND RECEIVER AGREES TO LOOK ONLY TO SUCH
 * THIRD PARTY FOR ANY WARRANTY CLAIM RELATING THERETO. RECEIVER EXPRESSLY ACKNOWLEDGES
 * THAT IT IS RECEIVER'S SOLE RESPONSIBILITY TO OBTAIN FROM ANY THIRD PARTY ALL PROPER LICENSES
 * CONTAINED IN AIROHA SOFTWARE. AIROHA SHALL ALSO NOT BE RESPONSIBLE FOR ANY AIROHA
 * SOFTWARE RELEASES MADE TO RECEIVER'S SPECIFICATION OR TO CONFORM TO A PARTICULAR
 * STANDARD OR OPEN FORUM. RECEIVER'S SOLE AND EXCLUSIVE REMEDY AND AIROHA'S ENTIRE AND
 * CUMULATIVE LIABILITY WITH RESPECT TO AIROHA SOFTWARE RELEASED HEREUNDER WILL BE,
 * AT AIROHA'S OPTION, TO REVISE OR REPLACE AIROHA SOFTWARE AT ISSUE,
 * OR REFUND ANY SOFTWARE LICENSE FEES OR SERVICE CHARGE PAID BY RECEIVER TO
 * AIROHA FOR SUCH AIROHA SOFTWARE AT ISSUE.
 */

#ifndef _HAL_NVIC_INTERNAL_H_
#define _HAL_NVIC_INTERNAL_H_
#include "hal_nvic.h"

#ifdef HAL_NVIC_MODULE_ENABLED
/* Please refer to configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY= 1 in freertos_config.h */
/* DEFAULT_IRQ_PRIORITY is from configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY  to  configKERNEL_INTERRUPT_PRIORITY -1 = (0xFF - 1)
    as pend_sv should be lowest irq priority to avoid unneccessary freert_os schedule */
#define DEFAULT_IRQ_PRIORITY                2//(FREERTOS_ISR_API_PRIORITY + 1)     /* please keep the setting */

#define RESERVED_IRQ_PRIORITY       (DEFAULT_IRQ_PRIORITY + 13)

#ifdef CONFIG_AIR_PRODUCT_TYPE_GAME_CONTROLLER
#define OS_GPT_IRQ_PRIORITY                (DEFAULT_IRQ_PRIORITY + 3)
#else
#define OS_GPT_IRQ_PRIORITY                (DEFAULT_IRQ_PRIORITY + 2)
#endif
#define MCU_DMA0_IRQ_PRIORITY              (DEFAULT_IRQ_PRIORITY + 3)
#define MCU_DMA1_IRQ_PRIORITY              (DEFAULT_IRQ_PRIORITY + 3)
#define UART_DMA0_IRQ_PRIORITY             (DEFAULT_IRQ_PRIORITY + 3)
#define UART_DMA1_IRQ_PRIORITY             (DEFAULT_IRQ_PRIORITY + 3)
#define UART_DMA2_IRQ_PRIORITY             (DEFAULT_IRQ_PRIORITY + 3)
#define PWM_DMA0_IRQ_PRIORITY              (DEFAULT_IRQ_PRIORITY + 13)
#define PWM_DMA1_IRQ_PRIORITY              (DEFAULT_IRQ_PRIORITY + 13)
#define I3C_DMA0_IRQ_PRIORITY              (DEFAULT_IRQ_PRIORITY + 3)
#define I3C_DMA1_IRQ_PRIORITY              (DEFAULT_IRQ_PRIORITY + 3)
#define I3C0_IRQ_PRIORITY                  (DEFAULT_IRQ_PRIORITY + 11)
#define I3C1_IRQ_PRIORITY                  (DEFAULT_IRQ_PRIORITY + 11)
#define SPI_MST0_IRQ_PRIORITY              (DEFAULT_IRQ_PRIORITY + 3)
#define SPI_MST1_IRQ_PRIORITY              (DEFAULT_IRQ_PRIORITY + 11)
#define SPI_MST2_IRQ_PRIORITY              (DEFAULT_IRQ_PRIORITY + 11)
#define SPI_SLV_IRQ_PRIORITY               (DEFAULT_IRQ_PRIORITY + 8)
#define UART0_IRQ_PRIORITY                 (DEFAULT_IRQ_PRIORITY + 11)
#define UART1_IRQ_PRIORITY                 (DEFAULT_IRQ_PRIORITY + 11)
#define UART2_IRQ_PRIORITY                 (DEFAULT_IRQ_PRIORITY + 11)
#define CRYPTO_IRQ_PRIORITY                (DEFAULT_IRQ_PRIORITY + 12)
#define TRNG_IRQ_PRIORITY                  (DEFAULT_IRQ_PRIORITY + 9)
#define RTC_IRQ_PRIORITY                   (DEFAULT_IRQ_PRIORITY + 12)
#define GPT_IRQ_PRIORITY                   (DEFAULT_IRQ_PRIORITY + 4)
#define GPT_SEC_IRQ_PRIORITY               (DEFAULT_IRQ_PRIORITY + 4)
#define SPM_IRQ_PRIORITY                   (DEFAULT_IRQ_PRIORITY + 6)
#define WDT_IRQ_PRIORITY                   (DEFAULT_IRQ_PRIORITY + 13)
#define EINT_SEC_IRQ_PRIORITY              (DEFAULT_IRQ_PRIORITY + 5)
#define EINT_IRQ_PRIORITY                  (DEFAULT_IRQ_PRIORITY + 5)
#define SFC_IRQ_PRIORITY                   (DEFAULT_IRQ_PRIORITY + 13)
#define ESC_IRQ_PRIORITY                   (DEFAULT_IRQ_PRIORITY + 13)
#define USB_IRQ_PRIORITY                   (DEFAULT_IRQ_PRIORITY + 1)
#define RISCV_IRQ_GEN_IRQ_PRIORITY          (DEFAULT_IRQ_PRIORITY + 2)
#define AUDIOSYS_IRQ_PRIORITY              (DEFAULT_IRQ_PRIORITY + 10)
#define BT_IRQ_PRIORITY                    (DEFAULT_IRQ_PRIORITY + 0)
#define BT_AURX_IRQ_PRIORITY               (DEFAULT_IRQ_PRIORITY + 10)
#define BT_AUTX_IRQ_PRIORITY               (DEFAULT_IRQ_PRIORITY + 10)
#define BT_TIMER_IRQ_PRIORITY              (DEFAULT_IRQ_PRIORITY + 3)
#define BT_PLAY_EN_PRIORITY                (DEFAULT_IRQ_PRIORITY + 0)
#define ZIGBEE_IRQ_PRIORITY                (DEFAULT_IRQ_PRIORITY + 0)
#define SEC_VIOLATION_IRQ_PRIORITY         (DEFAULT_IRQ_PRIORITY + 0)
#define BUS_ERR_IRQ_PRIORITY               (DEFAULT_IRQ_PRIORITY + 0)
#define MEM_ILLEGAL_IRQ_PRIORITY           (DEFAULT_IRQ_PRIORITY + 0)
#define PMU_IRQ_PRIORITY                   (DEFAULT_IRQ_PRIORITY + 6)
#define USB_32K_IRQ_PRIORITY               (DEFAULT_IRQ_PRIORITY + 11)
#define ARGB_IRQ_PRIORITY                  (DEFAULT_IRQ_PRIORITY + 13)
#define KEYSCAN_IRQ_PRIORITY               (DEFAULT_IRQ_PRIORITY + 5)
#define QDEC_IRQ_PRIORITY                  (DEFAULT_IRQ_PRIORITY + 5)
#define RAMBOZ_IRQ_PRIORITY                (DEFAULT_IRQ_PRIORITY + 5)
#define LED_IRQ_PRIORITY                   (DEFAULT_IRQ_PRIORITY + 13)
#define AIO_IRQ_PRIORITY                   (DEFAULT_IRQ_PRIORITY + 13)
#define IRTX_IRQ_PRIORITY                  (DEFAULT_IRQ_PRIORITY + 13)
#define IRRX_IRQ_PRIORITY                  (DEFAULT_IRQ_PRIORITY + 13)
#define CM33_reserved0_IRQ_PRIORITY        (DEFAULT_IRQ_PRIORITY + 3)

typedef struct {
    uint8_t index;
    uint8_t priority;
    const char *name;
} __attribute__((packed)) IRQ_INFO_t;

extern const IRQ_INFO_t irq_info[IRQ_NUMBER_MAX];

uint32_t save_and_set_interrupt_mask(void);
void restore_interrupt_mask(uint32_t mask);
hal_nvic_status_t nvic_irq_software_trigger(hal_nvic_irq_t irq_number);
int32_t hal_nvic_query_exception_number(void);
hal_nvic_status_t hal_nvic_save_and_set_interrupt_mask_special(uint32_t *mask);
hal_nvic_status_t hal_nvic_restore_interrupt_mask_special(uint32_t mask);
bool hal_nvic_is_irq_pending();
void hal_nvic_get_pending_irq_by_mask(uint32_t *irq_mask1,uint32_t *irq_mask2);
void hal_nvic_enable_irq_by_mask(uint32_t irq_mask1,uint32_t irq_mask2);
void hal_nvic_disable_irq_by_mask(uint32_t irq_mask1,uint32_t irq_mask2);
#endif //HAL_NVIC_MODULE_ENABLED
#endif //_HAL_NVIC_INTERNAL_H_

