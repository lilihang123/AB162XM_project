...
#ifdef HAL_SLEEP_MANAGER_ENABLED
#define  SPM_CFG_UART_OFFSET (8)
#define  SPM_CFG_UART_FLAG (0x1 << SPM_CFG_UART_OFFSET)
...
extern int exception_get_disable_all_log_flag(void);
#endif
...
/* Only triggered by UART break signal interrupt */
...
static const hal_eint_number_t g_uart_eint_num[HAL_UART_MAX] = {HAL_EINT_NUMBER_2, HAL_EINT_NUMBER_18, HAL_EINT_NUMBER_47};
static bool g_uart_config_sleep_eint[HAL_UART_MAX] = {false, false, false};
static uint8_t g_uart_config_pinmux_function[HAL_UART_MAX] = {HAL_GPIO_2_UART0_RXD, HAL_GPIO_18_UART1_RXD, HAL_GPIO_47_UART2_RXD};
...
static void eint_uart_handler(void *parameter)
{
    ...
    */
    ...
    // log_hal_msgid_info("[UART]  eint_uart_handler wakeup sleep\r\n", 0);
    ...
    */
    for (uart_port = HAL_UART_0; uart_port < HAL_UART_MAX; uart_port++) {
        if(g_uart_config_sleep_eint[uart_port] == true){
            hal_eint_deinit(g_uart_eint_num[uart_port]);
            hal_gpio_wakeup_extend_sleep_clear((hal_gpio_pin_t)g_uart_eint_num[uart_port]);
            hal_pinmux_set_function((hal_gpio_pin_t)g_uart_eint_num[uart_port], g_uart_config_pinmux_function[uart_port]);
            g_uart_config_sleep_eint[uart_port] = false;
        }
    }
}


void uart_enter_sleep_config_eint(void)
{
    hal_uart_port_t uart_port;

    if(hal_gpio_get_pinmux_function(HAL_GPIO_2) == HAL_GPIO_2_UART0_RXD){
        hal_pinmux_set_function(HAL_GPIO_2, HAL_GPIO_2_EINT2);
        g_uart_config_sleep_eint[0] = true;
    }
    if(hal_gpio_get_pinmux_function(HAL_GPIO_18) == HAL_GPIO_18_UART1_RXD){
        hal_pinmux_set_function(HAL_GPIO_18, HAL_GPIO_18_EINT18);
        g_uart_config_sleep_eint[1] = true;
    }
    if(hal_gpio_get_pinmux_function(HAL_GPIO_47) == HAL_GPIO_47_UART2_RXD){
        hal_pinmux_set_function(HAL_GPIO_47, HAL_GPIO_47_EINT47);
        g_uart_config_sleep_eint[2] = true;
    }
    for (uart_port = HAL_UART_0; uart_port < HAL_UART_MAX; uart_port++) {
        if ((g_uart_hwstatus[uart_port] != UART_HWSTATUS_UNINITIALIZED) && (g_uart_config_sleep_eint[uart_port] == true)) {
...
#ifdef AIR_BYPASS_SYSLOG_WITH_LOG_FLOW_ENABLE
    if (exception_get_disable_all_log_flag()!=0) {
        return;
    }
#endif

    log_para = (uart_backup_para_for_log_t *)CROSS_CORE_MEM_VARIABLE(syslog_uart_para);
    g_uart_port_for_logging = log_para->uart_log_port;
#ifdef UART_SLEEP_TRIGGER_ENABLED
    if(hal_gpio_get_pinmux_function(HAL_GPIO_2) == HAL_GPIO_2_UART0_RXD) {
        hal_gpio_wakeup_extend_sleep_config(HAL_GPIO_2);
        g_uart_config_sleep_eint[0] = true;
    } 
    if(hal_gpio_get_pinmux_function(HAL_GPIO_18) == HAL_GPIO_18_UART1_RXD) {
        hal_gpio_wakeup_extend_sleep_config(HAL_GPIO_18);
        g_uart_config_sleep_eint[1] = true;
    } 
    if(hal_gpio_get_pinmux_function(HAL_GPIO_47) == HAL_GPIO_47_UART2_RXD) {
        hal_gpio_wakeup_extend_sleep_config(HAL_GPIO_47);
        g_uart_config_sleep_eint[2] = true;
    } 
#endif
    for (uart_port = HAL_UART_0; uart_port < HAL_UART_MAX; uart_port++) {
...
void uart_restore_all_registers(void)
{
    ...
    vdma_channel_t tx_channel,rx_channel;
    for (hal_uart_port_t uart_port = HAL_UART_0; uart_port < HAL_UART_MAX; uart_port++) {
        tx_channel = uart_port_to_dma_channel(uart_port,0);
        rx_channel = uart_port_to_dma_channel(uart_port,1);
        if (uart_port == g_uart_port_for_logging) {
            ...
            internal_vdma_clock_control(tx_channel, true); // UART0 TX VDMA_CLOCK enable
            internal_vdma_clock_control(rx_channel, true); // UART0 RX VDMA_CLOCK enable
            internal_vdma_irq_control(tx_channel, true);  // UART0 TX VDMA IRQ enable
            internal_vdma_irq_control(rx_channel, true);  // UART0 RX VDMA IRQ enable
            ...
            */
            if ((*SPM_CFG_SPM_CFG_DUMMY_REG_A & SPM_CFG_UART_FLAG) != 0) {
                /*set to zero to avoid dsp to modify register when dsp wakeups after cm4.*/
                (*SPM_CFG_SPM_CFG_DUMMY_REG_A) &= ~SPM_CFG_UART_FLAG;
            ...
            if (g_uart_hwstatus[uart_port] != UART_HWSTATUS_UNINITIALIZED) {
                if(g_uart_hwstatus[uart_port] == UART_HWSTATUS_DMA_INITIALIZED){
                    internal_vdma_clock_control(tx_channel, true); // UART TX VDMA_CLOCK enable
                    internal_vdma_clock_control(rx_channel, true); // UART RX VDMA_CLOCK enable
                    internal_vdma_irq_control(tx_channel, true);   // UART TX VDMA IRQ enable
                    internal_vdma_irq_control(rx_channel, true);   // UART RX VDMA IRQ enable
                }
...
#endif /* IS_ENABLED(CONFIG_MCUBOOT) */
#endif /*HAL_SLEEP_MANAGER_ENABLED*/
...
hal_uart_status_t hal_uart_set_baudrate(hal_uart_port_t uart_port, hal_uart_baudrate_t baudrate)
{
    ...
    // if (baudrate == HAL_UART_BAUDRATE_12000000) {
    //     uart_switch_clock(uart_port, MUX_UART_IDX_UPLL_D5);      //62.4M source clock
    //     uart_input_clock[uart_port] = UART_INPUT_CLOCK_UPLL;
    // }

    uartx = g_uart_regbase[uart_port];
    actual_baudrate = g_uart_baudrate_map[baudrate];
    // uart_clock = uart_input_clock[uart_port];
...
hal_uart_status_t hal_uart_init(hal_uart_port_t uart_port, hal_uart_config_t *uart_config)
{
    ...
    uart_set_fifo(uartx);
#if defined(HAL_SLEEP_MANAGER_ENABLED) && (!IS_ENABLED(CONFIG_MCUBOOT)) && !defined(AIR_DOWNLOAD_AGENT)
...
#endif
    ao_backup_format_table_t uart0, uart1, uart2;
...
#endif /* defined(HAL_SLEEP_MANAGER_ENABLED) && IS_ENABLED(CONFIG_MCUBOOT) */
    return HAL_UART_STATUS_OK;
}
...
ATTR_TEXT_IN_TCM uint32_t uart_get_hw_rptr_addr(hal_uart_port_t uart_port, int32_t is_rx)
{
    vdma_channel_t dma_channel;
