...
/* check if transfer address is in flash code region, destination address can't be in flash code region.
    true: address is in flash code region
    false: address is not in flash code region
*/
#ifdef HAL_CACHE_MODULE_ENABLED
static bool sw_dma_address_is_flash_code_region(uint32_t address, uint32_t length)
{
    if (address < CODE_PARTITION_BASE || (address + length) > CODE_PARTITION_END) {
        return false;
    }
...
static hal_sw_gdma_status_t sw_gdma_parameter_check(hal_sw_dma_config_info_t *info)
{
    ...
    if ((true == hal_cache_is_cacheable(info->source_address)) && (false == sw_dma_address_is_flash_code_region(info->source_address, info->length))) {
        return HAL_SW_DMA_STATUS_INVALID_PARAMETER;
    }
#endif
...
void sw_dma_irq_handler(hal_nvic_irq_t irq_number)
{
    ...
    if (room_available >= 1) {
        ...
        if (g_sw_dma_is_ongoing == false) {
            ...
            config_info.source_address = hal_memview_mcu_to_infrasys(config_info.source_address);
            config_info.destination_address = hal_memview_mcu_to_infrasys(config_info.destination_address);
