...
#ifdef AIR_FOTA_DELAY_ERASE_FLASH_ENABLE
#include "race_fota_util.h"
//It is should be NG device if flash erase 4KB time is over 1 seconds
#define FLASH_ERASE_MAX_TIME (1000000)
uint32_t sf_erase_duration_previous;
#endif
...
uint32_t sf_erase_resume_to_suspend = 0xFFFFFFFF;
...
//record the flash P/E start or resume time
uint32_t flash_p_e_start_time;
#define RESUME_WAIT_TIME      (2000)       //wait (2000)us
#define RESUME_INTERVAL_TIME  10000       //Resume one time every 10ms
#define PROGRAM_BYTE          16          //Program 32byte every time
uint32_t flash_start_test;
uint32_t SF_DAL_TERS_set = RESUME_WAIT_TIME;
uint32_t SF_DAL_skip_resume = RESUME_INTERVAL_TIME;
uint32_t SF_DAL_write_length = PROGRAM_BYTE;
// update flash_p_e_start_time evey 100000ms = 1 * 5ms
#define TRIGGER_UPDATE_P_E_COUNT      (1)
#define WAIT_ERASE_SECTOR_TIME        (20000)
#define ERASE_SECTOR_STEP_TIME        (5000)

ATTR_TEXT_IN_TCM_RETENTION  uint32_t SaveAndSetIRQMask(void)
{
    uint32_t mask = 0;
...
ATTR_TEXT_IN_TCM_RETENTION  void RestoreIRQMask(uint32_t mask)
{
...
extern bool race_fota_is_running(bool is_race_fota);
#endif
...
/*!
  ...
  @remarks
    ...
    the device lock is prohibited.
*/
ATTR_TEXT_IN_TCM_RETENTION void Flash_ReturnReady(void)
{
    SF_MTD_Data         *D = NULL;
    SF_Status            status = 0xFF;
    uint32_t      savedMask = 0;

    #if defined(CONFIG_MCUBOOT) || defined(AIR_DOWNLOAD_AGENT)
        return;
    #endif

    /* NOR_FLASH_BUSY is 0 when flash is not in P/E and only changed to 1 in task level.
       It is impossible that NOR_FLASH_BUSY change from 0 to 1 at here, so it is not necessary to disable IRQ*/
    if (!NOR_FLASH_BUSY) {
        return;
    }
...
*/
ATTR_TEXT_IN_TCM_RETENTION int32_t SF_DAL_MountDevice(void *MTDData, void *Info)
{
...
*/
ATTR_TEXT_IN_TCM_RETENTION int32_t SF_DAL_ShutDown(void *MTDData)
{
    SF_MTD_Data            *D = MTDData;
    SF_Status               status;
    uint32_t              saved_mask;
...
*/
ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_FLAG_BUSY_SET(void)
{
    NOR_FLASH_BUSY = true;
}
ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_FLAG_BUSY_CLR(void)
{
    NOR_FLASH_BUSY = false;
    SFI_MaskAhbChannel(0);
}
ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_FLAG_SUS_SET(void)
{
    NOR_FLASH_SUSPENDED = true;
    SFI_MaskAhbChannel(0);
}
ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_FLAG_SUS_CLR(void)
{
    NOR_FLASH_SUSPENDED = false;
}

ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_check_config(void)
{
#ifdef RACE_FOTA_CMD_ENABLE
    if (true == race_fota_is_running(ture)) {
        SF_DAL_TERS_set = 0;
        SF_DAL_skip_resume = 0;
        SF_DAL_write_length = 64;
    } else {
        SF_DAL_TERS_set = RESUME_WAIT_TIME;
        SF_DAL_skip_resume = RESUME_INTERVAL_TIME;
        SF_DAL_write_length = PROGRAM_BYTE;
    }
#endif
}

ATTR_TEXT_IN_TCM_RETENTION int32_t SF_DAL_ProgramData(void *MTDData, void *Address, void *Data, uint32_t Length)
{
    SF_MTD_Data     *D = MTDData;
    uint32_t      savedMask = 0;
...
retry:
        ...
        hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &flash_p_e_start_time);
        hal_nvic_restore_interrupt_mask_special(savedMask);

        do {
            sf_program_flag = 0x4;
            result = SF_DAL_CheckReadyAndResume(D, (uint32_t)address + written - 1, *(p_data_first + written - 1));
            #if !defined(__FUE__) && !defined(__UBL__) && !defined(__EXT_DA__)
            /* don't keep polling SR to avoid toggling interrupts around 20us during flash program process.*/
            {
...
ATTR_TEXT_IN_TCM_RETENTION int32_t SF_DAL_EraseBlock(void *MTDData, uint32_t BlockIndex)
{
    int32_t            result = 0;
    uint32_t           blocksize;
    ...
    uint32_t           erase_use_time, erase_use_time_pre;
...
retry:
        ...
        sf_erase_flag = 0x3;
#ifdef AIR_FOTA_DELAY_ERASE_FLASH_ENABLE
        sf_erase_duration_previous = sf_erase_duration;
#endif
        flash_p_e_start_time = sf_erase_running;
        erase_use_time_pre = flash_p_e_start_time;
        hal_nvic_restore_interrupt_mask_special(savedMask);

        // 3. wait ready
        while (1) {
            hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &erase_use_time);
            /* sector erase time is about 20ms when IRQ disable,so wait 20ms before check device status
               check device ready status every 5ms after erase time over 20ms  */
            if ((erase_use_time - erase_use_time_pre) < (WAIT_ERASE_SECTOR_TIME + ERASE_SECTOR_STEP_TIME * erase_step)) {
                if (!SF_DAL_FLAG_SUS()) {
                    continue;
                }
            } else {
                erase_step++;
                if ((erase_step % TRIGGER_UPDATE_P_E_COUNT) == 0) {
                    // update flash_p_e_start_time every 5ms 
                    flash_p_e_start_time = erase_use_time;
                }
            }
            result = SF_DAL_CheckDeviceReady(D, BlockIndex);
            ...
            //if (sf_erase_duration >= FLASH_ERASE_MAX_TIME) {
            if (race_fota_is_race_fota_running() && sf_erase_duration_previous > 0 && (sf_erase_duration + sf_erase_duration_previous >= (FLASH_ERASE_MAX_TIME * 2))) {
                result = RAW_DISK_ERR_ERASE_OVER_TIME;
                sf_erase_duration = 0;
                sf_erase_duration_previous = 0;
            }

            hal_nvic_restore_interrupt_mask(savedMask);
            if (result == RAW_DISK_ERR_ERASE_OVER_TIME) {
                break;
            }
#endif
...
ATTR_TEXT_IN_TCM_RETENTION int32_t SF_DAL_Erase_chip(void *MTDData, const uint16_t CS)
{
    uint32_t savedMask;
    uint8_t cmd;
    int32_t result;
...
*/
ATTR_TEXT_IN_TCM_RETENTION int32_t SF_DAL_NonBlockEraseBlock(void *MTDData, uint32_t BlockIndex)
{
...
ATTR_TEXT_IN_TCM_RETENTION int32_t SF_DAL_CheckReadyAndResume(void *MTDData, uint32_t addr, uint8_t data)
{
    uint32_t          savedMask;
    SF_MTD_Data        *D               = MTDData;
    ...
    if (SF_DAL_FLAG_SUS()) {
        ...
        if (dsp_request_flag == 0) {
            ...
            if (D->DriverStatus == SFDRV_ERASING && 
                (start_count - sf_erase_resume_to_suspend) < SF_DAL_skip_resume) {
                // skip to issue resume command
            } else {
...
/*!
  ...
  @remarks
    GPRAM utilization is 1+1B: A device lock is not necessary.
*/
ATTR_TEXT_IN_TCM_RETENTION int32_t SF_DAL_CheckDeviceReady(void *MTDData, uint32_t BlockIndex)
{
...
#if 0
ATTR_TEXT_IN_TCM_RETENTION int32_t SF_DAL_SuspendErase(void *MTDData, uint32_t BlockIndex)
{
    SF_MTD_Data        *D  = MTDData;
    SF_Status           status;
    uint32_t          savedMask;
...
ATTR_TEXT_IN_TCM_RETENTION int32_t SF_DAL_ResumeErase(void *MTDData, uint32_t BlockIndex)
{
    SF_MTD_Data        *D = MTDData;
    SF_DAL_Dev_Resume(D);
    return FS_NO_ERROR;
...
*/
ATTR_TEXT_IN_TCM_RETENTION int SF_DAL_FailCheck(void *MTDData)
{
...
*/
ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_Dev_ReadID(SF_MTD_Data *D, uint8_t *id)
{
    uint8_t cmd;
    if (SFI_DevMode != 0)  {
...
*/
ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_Dev_Suspend(SF_MTD_Data *D)
{
    //uint32_t currtime, difftime;
    if (D->DriverStatus == SFDRV_ERASING) {
        DelayAWhile_UST(SF_DAL_TERS_set);
    } else {
        //DelayAWhile_UST(SF_DAL_GIGADEVICE_TERS_debug/2);
    ...
    if (sf_erase_flag == 3) {
        ...
        if (sf_erase_first_sus == 1) {
            ...
            sf_erase_resume_to_suspend = sf_erase_run_to_suspend;
        } else {
...
/*!
  ...
  @remarks
      Caller:  it should diable IRQ then call SF_DAL_Dev_Resume
*/
ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_Dev_Resume(SF_MTD_Data *D)
{
    SFI_Dev_Command(D->CS, D->CMD->Resume);
    hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &sf_dal_resume_time);
    sf_erase_resume_to_suspend = sf_dal_resume_time;

    //wait more than 20us after resume from vendor suggestion.
    hal_gpt_delay_us(30);
    SF_DAL_FLAG_SUS_CLR();
}
...
*/
ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_Dev_WriteEnable(SF_MTD_Data *D)
{
    SF_DAL_Dev_Command(D, D->CMD->WriteEnable, 1);
}

ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_Dev_EraseBlock(SF_MTD_Data *D, uint32_t addr)
{
    SF_DAL_FLAG_BUSY_SET();
...
ATTR_TEXT_IN_TCM_RETENTION uint32_t SF_DAL_Dev_ReadStatus(SF_MTD_Data *D, uint8_t cmd)
{
    uint32_t savedMask = 0;
    uint32_t result;
...
/*!
  ...
  @remarks
      Caller:  it should diable IRQ then call SF_DAL_Dev_Resume
*/
ATTR_TEXT_IN_TCM_RETENTION uint32_t ReadStatus_internal(SF_MTD_Data *D, uint8_t cmd)
{
    uint32_t result;
...
/*!
  ...
  @remarks
    ...
    Caller:  it should diable IRQ then call SF_DAL_Dev_Resume
*/
ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_Dev_Command(SF_MTD_Data *D, const uint32_t cmd, uint8_t outlen)
{
    SFI_MacEnable(D->CS);
    SFC_GPRAM->RW_SF_GPRAM_DATA = cmd;
...
*/
ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_Dev_Command_Ext(const uint16_t CS, uint8_t *cmd, uint8_t *data, const uint16_t outl, const uint16_t inl)
{
    uint32_t savedMask = 0;
...
*/
ATTR_TEXT_IN_TCM_RETENTION int SF_DAL_OTPLock_MXIC(void *MTDData)
{
    uint32_t           savedMask;
    int32_t            result;
    SF_MTD_Data         *D = (SF_MTD_Data *)MTDData;
...
*/
ATTR_TEXT_IN_TCM_RETENTION int SF_DAL_OTPWrite_MXIC(void *MTDData, uint32_t Offset, void *BufferPtr, uint32_t Length)
{
    uint32_t           savedMask;
    uint32_t           i;
    uint32_t           cmd;
...
*/
ATTR_TEXT_IN_TCM_RETENTION int SF_DAL_OTPRead_MXIC(void *MTDData, uint32_t Offset, void *BufferPtr, uint32_t Length)
{
    uint32_t           savedMask;
    SF_MTD_Data         *D = (SF_MTD_Data *)MTDData;
...
/*!
  ...
  @retval
    FS_NO_ERROR: no error
*/
ATTR_TEXT_IN_TCM_RETENTION int SF_DAL_OTPLock_GIGADEVICE(void *MTDData)
{
...
*/
ATTR_TEXT_IN_TCM_RETENTION int SF_DAL_OTPWrite_GIGADEVICE(void *MTDData, uint32_t Offset, void *BufferPtr, uint32_t Length)
{
    uint32_t          savedMask;
    uint32_t          i;
    uint32_t          cmd;
...
/*!
  ...
  @remarks
    The physical discontinuous address is covered by the funciton. It's continuous to upper layer users.
*/
ATTR_TEXT_IN_TCM_RETENTION int SF_DAL_OTPRead_GIGADEVICE(void *MTDData, uint32_t Offset, void *BufferPtr, uint32_t Length)
{
    uint32_t          savedMask;
    SF_MTD_Data         *D = (SF_MTD_Data *)MTDData;
    int32_t           i;
...
#if defined(SF_DAL_GIGADEVICE) || defined(SF_DAL_WINBOND)
ATTR_TEXT_IN_TCM_RETENTION uint32_t SF_DAL_OTP_Size_WINBOND(void *MTDData)
{
    SF_MTD_Data         *D = (SF_MTD_Data *)MTDData;
...
ATTR_TEXT_IN_TCM_RETENTION OTP_BANK_LOCK_enum SF_DAL_OTP_LOCK_Status_WINBOND(void *MTDData)
{
    SF_MTD_Data *D = (SF_MTD_Data *)MTDData;
    uint32_t status;
...
ATTR_TEXT_IN_TCM_RETENTION int SF_DAL_OTPLock_blank(void *MTDData, OTP_BANK_enum bank)
{
    uint32_t          data_SR1, data_SR2, savedMask;
    int32_t           result;
    uint32_t          cmd;
...
ATTR_TEXT_IN_TCM_RETENTION int SF_DAL_OTP_ERASE_WINBOND(void *MTDData, uint32_t block)
{
...
ATTR_TEXT_IN_TCM_RETENTION int SF_DAL_Flash_Protect(void *MTDData, uint32_t start, uint32_t length)
{
    uint32_t          cmd;
    SF_MTD_Data       *D = (SF_MTD_Data *)MTDData;
    uint32_t          status = FS_NO_ERROR;
    uint8_t           sr1, sr2;
    uint32_t          savedMask;
    ...
    if (start != 0 && length != 0x20000) {
        return FLASH_WRONG_PROTECT_REGION;
    }
...
ATTR_TEXT_IN_TCM_RETENTION int SF_DAL_Flash_UnProtect(void *MTDData)
{
    uint32_t          cmd;
    SF_MTD_Data       *D = (SF_MTD_Data *)MTDData;
    uint32_t          status;
...
#else
ATTR_TEXT_IN_TCM_RETENTION int SF_DAL_OTPLock_blank(void *MTDData)
{
...
/*!
  ...
  @retval
    Always FS_NO_ERROR
*/
ATTR_TEXT_IN_TCM_RETENTION int32_t SF_DAL_OTPQueryLength(void *MTDData, uint32_t *LengthPtr)
{
    SF_MTD_Data         *D = (SF_MTD_Data *)MTDData;
...
*/
ATTR_TEXT_IN_TCM_RETENTION int32_t SF_DAL_OTPAccess(void *MTDData, int32_t accesstype, uint32_t Offset, void *BufferPtr, uint32_t Length)
{
    SF_MTD_Data         *D = (SF_MTD_Data *)MTDData;
...
/*!
  ...
  @param[in] MTDData Pointer to the MTD driver data
*/
ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_Dev_WaitReady_EMIINIT(SF_MTD_Data *D)
{
    SF_Status    status;
    do   {
...
/*!
  ...
  @remarks
    ...
    GPRAM utilization is 1+1B. A device lock is not necessary for GPRAM.
*/
ATTR_TEXT_IN_TCM_RETENTION int SF_DAL_UnlockBlock_MXIC(void *MTDData)
{
    SF_MTD_Data  *D  = MTDData;
    uint32_t    unlock;
    uint8_t     cmd;
