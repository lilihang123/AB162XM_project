...
#define ZEPHYR_USB_PHY_EARLY_PORTING

#ifdef ZEPHYR_USB_PHY_EARLY_PORTING
...
#ifndef ZEPHYR_USB_PHY_EARLY_PORTING
#include "assert.h"
#include "hal_clock.h"
#include "hal_clock_internal.h"
...
#endif /* USE_DUMMY_FUNCTION_IN_EARLY_STAGE */
...
#define ISR_STATE_BUFFER_SIZE 256
ATTR_ZIDATA_IN_TCM_NONE_RETENTION static struct isr_state g_usb_isr_state[ISR_STATE_BUFFER_SIZE];
ATTR_ZIDATA_IN_TCM_NONE_RETENTION static uint32_t g_usb_write_index;
ATTR_ZIDATA_IN_TCM_NONE_RETENTION static uint32_t g_usb_read_index;
...
#include "hal_gpio.h"
#define GPIO_HISR HAL_GPIO_36
#define GPIO_LISR HAL_GPIO_37
static void hal_usb_dbg_io_init(void)
{
    hal_gpio_init(GPIO_HISR);
    hal_pinmux_set_function(GPIO_HISR, 0);
    hal_gpio_set_direction(GPIO_HISR, HAL_GPIO_DIRECTION_OUTPUT);
    hal_gpio_set_output(GPIO_HISR, HAL_GPIO_DATA_LOW);
    hal_gpio_init(GPIO_LISR);
    hal_pinmux_set_function(GPIO_LISR, 0);
    hal_gpio_set_direction(GPIO_LISR, HAL_GPIO_DIRECTION_OUTPUT);
    hal_gpio_set_output(GPIO_LISR, HAL_GPIO_DATA_LOW);
}
#endif /* USB_DBG_GPIO */

static uint32_t get_queue_remain_data(uint32_t write, uint32_t read)
{
    return (write >= read) ? (write - read) : (write + (ISR_STATE_BUFFER_SIZE - read));
}

static void queue_data_log(uint32_t queue_remain_data, uint32_t l_write_index, uint32_t l_read_index)
{
    static bool queue_log_3_4 = false, queue_log_2_4 = false, queue_log_1_4 = false;
    bool need_log = false;

    if (queue_remain_data >= (ISR_STATE_BUFFER_SIZE / 4 * 3)) {
        queue_log_2_4 = false;
        queue_log_1_4 = false;
        if (!queue_log_3_4) {
            queue_log_3_4 = true;
    ...
    } else if (queue_remain_data >= (ISR_STATE_BUFFER_SIZE / 2)) {
        queue_log_3_4 = false;
        queue_log_1_4 = false;
        if (!queue_log_2_4) {
            queue_log_2_4 = true;
    ...
    } else if (queue_remain_data >= (ISR_STATE_BUFFER_SIZE / 4)) {
        queue_log_3_4 = false;
        queue_log_2_4 = false;
        if (!queue_log_1_4) {
            queue_log_1_4 = true;
    ...
    if (need_log) {
        log_hal_msgid_info("usb_hisr: q_data[%d], w[%d], r[%d] USB[0x%x], Tx[0x%x], Rx[0x%x], q_usb[0x%x], q_tx[0x%x], q_rx[0x%x]", 9,
            queue_remain_data, l_write_index, l_read_index, musb->intrusb, musb->intrtx, musb->intrrx, g_usb_isr_state[l_read_index].IntrUSB, g_usb_isr_state[l_read_index].IntrTx, g_usb_isr_state[l_read_index].IntrRx);
    }
}
/************************************************************
    USB PDN
...
*************************************************************/
#if 1
static void hal_dump_regs(void) __unused;
static void hal_dump_regs(void)
{
// #define HAL_USB_DUMP_MAC
// #define HAL_USB_DUMP_PHY

#ifdef HAL_USB_DUMP_MAC
    volatile USB_REGISTER_T *mac = (USB_REGISTER_T *)USB_BASE;
#endif
#ifdef HAL_USB_DUMP_PHY
    volatile USBPHY_REGISTER_T *phy = (USBPHY_REGISTER_T *)USB_SIFSLV_BASE;
#endif

#if defined(HAL_USB_DUMP_MAC) || defined(HAL_USB_DUMP_PHY)
    bool clock_is_enabled = false;

    clock_is_enabled = (hal_clock_is_enabled(HAL_CLOCK_CG_USB) && hal_clock_is_enabled(HAL_CLOCK_CG_USB_DMA) && hal_clock_is_enabled(HAL_CLOCK_CG_USB_BUS));
    log_hal_msgid_info("RTC_DCXO_CON4__DBL_64M_EN  (0x%08x) : 0x%08x", 2, (uint32_t)RTC_DCXO_CON4__DBL_64M_EN, *RTC_DCXO_CON4__DBL_64M_EN);
    log_hal_msgid_info("USB clock : %d", 1, clock_is_enabled);
    if (!clock_is_enabled) {
        usb_pdn_disable();
    }
#endif
...
#ifdef HAL_USB_DUMP_MAC
    log_hal_msgid_info("mac->faddr                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->faddr,          mac->faddr);
    log_hal_msgid_info("mac->power                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->power,          mac->power);
    log_hal_msgid_info("mac->intrtx                (0x%08X) : 0x%08x", 2, (uint32_t)&mac->intrtx,         mac->intrtx);
    log_hal_msgid_info("mac->intrrx                (0x%08X) : 0x%08x", 2, (uint32_t)&mac->intrrx,         mac->intrrx);
    log_hal_msgid_info("mac->intrtxe               (0x%08X) : 0x%08x", 2, (uint32_t)&mac->intrtxe,        mac->intrtxe);
    log_hal_msgid_info("mac->intrrxe               (0x%08X) : 0x%08x", 2, (uint32_t)&mac->intrrxe,        mac->intrrxe);
    log_hal_msgid_info("mac->intrusb               (0x%08X) : 0x%08x", 2, (uint32_t)&mac->intrusb,        mac->intrusb);
    log_hal_msgid_info("mac->intrusbe              (0x%08X) : 0x%08x", 2, (uint32_t)&mac->intrusbe,       mac->intrusbe);
    log_hal_msgid_info("mac->frame                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->frame,          mac->frame);
    log_hal_msgid_info("mac->index                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->index,          mac->index);
    log_hal_msgid_info("mac->testmode              (0x%08X) : 0x%08x", 2, (uint32_t)&mac->testmode,       mac->testmode);
    log_hal_msgid_info("mac->txmap                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->txmap,          mac->txmap);
    log_hal_msgid_info("mac->txcsr                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->txcsr,          mac->txcsr);
    log_hal_msgid_info("mac->rxmap                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->rxmap,          mac->rxmap);
    log_hal_msgid_info("mac->rxcsr                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->rxcsr,          mac->rxcsr);
    log_hal_msgid_info("mac->rxcount               (0x%08X) : 0x%08x", 2, (uint32_t)&mac->rxcount,        mac->rxcount);
    log_hal_msgid_info("mac->txtype                (0x%08X) : 0x%08x", 2, (uint32_t)&mac->txtype,         mac->txtype);
    log_hal_msgid_info("mac->txinterval            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->txinterval,     mac->txinterval);
    log_hal_msgid_info("mac->rxtype                (0x%08X) : 0x%08x", 2, (uint32_t)&mac->rxtype,         mac->rxtype);
    log_hal_msgid_info("mac->rxinterval            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->rxinterval,     mac->rxinterval);
    log_hal_msgid_info("mac->fifosize              (0x%08X) : 0x%08x", 2, (uint32_t)&mac->fifosize,       mac->fifosize);
    log_hal_msgid_info("mac->fifo0                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->fifo0,          mac->fifo0);
    log_hal_msgid_info("mac->fifo1                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->fifo1,          mac->fifo1);
    log_hal_msgid_info("mac->devctl                (0x%08X) : 0x%08x", 2, (uint32_t)&mac->devctl,         mac->devctl);
    log_hal_msgid_info("mac->pwrupcnt              (0x%08X) : 0x%08x", 2, (uint32_t)&mac->pwrupcnt,       mac->pwrupcnt);
    log_hal_msgid_info("mac->txfifosz              (0x%08X) : 0x%08x", 2, (uint32_t)&mac->txfifosz,       mac->txfifosz);
    log_hal_msgid_info("mac->rxfifosz              (0x%08X) : 0x%08x", 2, (uint32_t)&mac->rxfifosz,       mac->rxfifosz);
    log_hal_msgid_info("mac->txfifoadd             (0x%08X) : 0x%08x", 2, (uint32_t)&mac->txfifoadd,      mac->txfifoadd);
    log_hal_msgid_info("mac->rxfifoadd             (0x%08X) : 0x%08x", 2, (uint32_t)&mac->rxfifoadd,      mac->rxfifoadd);
    log_hal_msgid_info("mac->hwcaps                (0x%08X) : 0x%08x", 2, (uint32_t)&mac->hwcaps,         mac->hwcaps);
    log_hal_msgid_info("mac->hwsvers               (0x%08X) : 0x%08x", 2, (uint32_t)&mac->hwsvers,        mac->hwsvers);
    log_hal_msgid_info("mac->busperf1              (0x%08X) : 0x%08x", 2, (uint32_t)&mac->busperf1,       mac->busperf1);
    log_hal_msgid_info("mac->busperf2              (0x%08X) : 0x%08x", 2, (uint32_t)&mac->busperf2,       mac->busperf2);
    log_hal_msgid_info("mac->busperf3              (0x%08X) : 0x%08x", 2, (uint32_t)&mac->busperf3,       mac->busperf3);
    log_hal_msgid_info("mac->epinfo                (0x%08X) : 0x%08x", 2, (uint32_t)&mac->epinfo,         mac->epinfo);
    log_hal_msgid_info("mac->raminfo               (0x%08X) : 0x%08x", 2, (uint32_t)&mac->raminfo,        mac->raminfo);
    log_hal_msgid_info("mac->linkinfo              (0x%08X) : 0x%08x", 2, (uint32_t)&mac->linkinfo,       mac->linkinfo);
    log_hal_msgid_info("mac->vplen                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->vplen,          mac->vplen);
    log_hal_msgid_info("mac->hs_eof1               (0x%08X) : 0x%08x", 2, (uint32_t)&mac->hs_eof1,        mac->hs_eof1);
    log_hal_msgid_info("mac->fs_eof1               (0x%08X) : 0x%08x", 2, (uint32_t)&mac->fs_eof1,        mac->fs_eof1);
    log_hal_msgid_info("mac->ls_eof1               (0x%08X) : 0x%08x", 2, (uint32_t)&mac->ls_eof1,        mac->ls_eof1);
    log_hal_msgid_info("mac->rst_info              (0x%08X) : 0x%08x", 2, (uint32_t)&mac->rst_info,       mac->rst_info);
    log_hal_msgid_info("mac->rxtog                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->rxtog,          mac->rxtog);
    log_hal_msgid_info("mac->rxtogen               (0x%08X) : 0x%08x", 2, (uint32_t)&mac->rxtogen,        mac->rxtogen);
    log_hal_msgid_info("mac->txtog                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->txtog,          mac->txtog);
    log_hal_msgid_info("mac->txtogen               (0x%08X) : 0x%08x", 2, (uint32_t)&mac->txtogen,        mac->txtogen);
    log_hal_msgid_info("mac->usb_l1ints            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->usb_l1ints,     mac->usb_l1ints);
    log_hal_msgid_info("mac->usb_l1intm            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->usb_l1intm,     mac->usb_l1intm);
    log_hal_msgid_info("mac->usb_l1intp            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->usb_l1intp,     mac->usb_l1intp);
    log_hal_msgid_info("mac->usb_l1intc            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->usb_l1intc,     mac->usb_l1intc);
    log_hal_msgid_info("mac->csr0                  (0x%08X) : 0x%08x", 2, (uint32_t)&mac->csr0,           mac->csr0);
    log_hal_msgid_info("mac->count0                (0x%08X) : 0x%08x", 2, (uint32_t)&mac->count0,         mac->count0);
    log_hal_msgid_info("mac->type0                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->type0,          mac->type0);
    log_hal_msgid_info("mac->naklimt0              (0x%08X) : 0x%08x", 2, (uint32_t)&mac->naklimt0,       mac->naklimt0);
    log_hal_msgid_info("mac->sramconfigsize        (0x%08X) : 0x%08x", 2, (uint32_t)&mac->sramconfigsize, mac->sramconfigsize);
    log_hal_msgid_info("mac->hbconfigdata          (0x%08X) : 0x%08x", 2, (uint32_t)&mac->hbconfigdata,   mac->hbconfigdata);
    log_hal_msgid_info("mac->configdata            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->configdata,     mac->configdata);
    log_hal_msgid_info("mac->tx1map                (0x%08X) : 0x%08x", 2, (uint32_t)&mac->tx1map,         mac->tx1map);
    log_hal_msgid_info("mac->tx1csr                (0x%08X) : 0x%08x", 2, (uint32_t)&mac->tx1csr,         mac->tx1csr);
    log_hal_msgid_info("mac->rx1map                (0x%08X) : 0x%08x", 2, (uint32_t)&mac->rx1map,         mac->rx1map);
    log_hal_msgid_info("mac->rx1csr                (0x%08X) : 0x%08x", 2, (uint32_t)&mac->rx1csr,         mac->rx1csr);
    log_hal_msgid_info("mac->rx1count              (0x%08X) : 0x%08x", 2, (uint32_t)&mac->rx1count,       mac->rx1count);
    log_hal_msgid_info("mac->tx1type               (0x%08X) : 0x%08x", 2, (uint32_t)&mac->tx1type,        mac->tx1type);
    log_hal_msgid_info("mac->tx1interval           (0x%08X) : 0x%08x", 2, (uint32_t)&mac->tx1interval,    mac->tx1interval);
    log_hal_msgid_info("mac->rx1type               (0x%08X) : 0x%08x", 2, (uint32_t)&mac->rx1type,        mac->rx1type);
    log_hal_msgid_info("mac->rx1interval           (0x%08X) : 0x%08x", 2, (uint32_t)&mac->rx1interval,    mac->rx1interval);
    log_hal_msgid_info("mac->fifosize1             (0x%08X) : 0x%08x", 2, (uint32_t)&mac->fifosize1,      mac->fifosize1);
    log_hal_msgid_info("mac->dma_intr_status       (0x%08X) : 0x%08x", 2, (uint32_t)&mac->dma_intr_status,       mac->dma_intr_status);
    log_hal_msgid_info("mac->dma_intr_unmask       (0x%08X) : 0x%08x", 2, (uint32_t)&mac->dma_intr_unmask,       mac->dma_intr_unmask);
    log_hal_msgid_info("mac->dma_intr_unmask_clear (0x%08X) : 0x%08x", 2, (uint32_t)&mac->dma_intr_unmask_clear, mac->dma_intr_unmask_clear);
    log_hal_msgid_info("mac->dma_intr_unmask_set   (0x%08X) : 0x%08x", 2, (uint32_t)&mac->dma_intr_unmask_set,   mac->dma_intr_unmask_set);
    log_hal_msgid_info("mac->dma_cntl_0            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->dma_cntl_0,     mac->dma_cntl_0);
    log_hal_msgid_info("mac->dma_addr_0            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->dma_addr_0,     mac->dma_addr_0);
    log_hal_msgid_info("mac->dma_count_0           (0x%08X) : 0x%08x", 2, (uint32_t)&mac->dma_count_0,    mac->dma_count_0);
    log_hal_msgid_info("mac->dma_limiter           (0x%08X) : 0x%08x", 2, (uint32_t)&mac->dma_limiter,    mac->dma_limiter);
    log_hal_msgid_info("mac->dma_config            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->dma_config,     mac->dma_config);
    log_hal_msgid_info("mac->ep1rxpktcount         (0x%08X) : 0x%08x", 2, (uint32_t)&mac->ep1rxpktcount,  mac->ep1rxpktcount);
    log_hal_msgid_info("mac->t0funcaddr            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->t0funcaddr,     mac->t0funcaddr);
    log_hal_msgid_info("mac->t0hubaddr             (0x%08X) : 0x%08x", 2, (uint32_t)&mac->t0hubaddr,      mac->t0hubaddr);
    log_hal_msgid_info("mac->t1funcaddr            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->t1funcaddr,     mac->t1funcaddr);
    log_hal_msgid_info("mac->t1hubaddr             (0x%08X) : 0x%08x", 2, (uint32_t)&mac->t1hubaddr,      mac->t1hubaddr);
    log_hal_msgid_info("mac->r1funcaddr            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->r1funcaddr,     mac->r1funcaddr);
    log_hal_msgid_info("mac->r1hubaddr             (0x%08X) : 0x%08x", 2, (uint32_t)&mac->r1hubaddr,      mac->r1hubaddr);
    log_hal_msgid_info("mac->tm1                   (0x%08X) : 0x%08x", 2, (uint32_t)&mac->tm1,            mac->tm1);
    log_hal_msgid_info("mac->hwver_date            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->hwver_date,     mac->hwver_date);
    log_hal_msgid_info("mac->srama                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->srama,          mac->srama);
    log_hal_msgid_info("mac->sramd                 (0x%08X) : 0x%08x", 2, (uint32_t)&mac->sramd,          mac->sramd);
    log_hal_msgid_info("mac->risc_size             (0x%08X) : 0x%08x", 2, (uint32_t)&mac->risc_size,      mac->risc_size);
    log_hal_msgid_info("mac->resreg                (0x%08X) : 0x%08x", 2, (uint32_t)&mac->resreg,         mac->resreg);
    log_hal_msgid_info("mac->otg20_csrl            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->otg20_csrl,     mac->otg20_csrl);
    log_hal_msgid_info("mac->otg20_csrh            (0x%08X) : 0x%08x", 2, (uint32_t)&mac->otg20_csrh,     mac->otg20_csrh);
#endif
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
/************************************************************
    lock/unlock sleep utilities
*************************************************************/
void hal_usb_lock_sleep(void)
{
    if (!hal_sleep_manager_is_sleep_handle_alive(SLEEP_LOCK_USB)) {
        hal_sleep_manager_lock_sleep(SLEEP_LOCK_USB);
        log_hal_msgid_info("usb lock sleep", 0);
    } else {
        log_hal_msgid_info("usb already lock sleep", 0);
    }
}

void hal_usb_unlock_sleep(void)
{
    if (hal_sleep_manager_is_sleep_handle_alive(SLEEP_LOCK_USB)) {
        hal_sleep_manager_unlock_sleep(SLEEP_LOCK_USB);
        log_hal_msgid_info("usb unlock sleep", 0);
    } else {
        log_hal_msgid_info("usb already unlock sleep", 0);
    }
}

/************************************************************
    lock/unlock ext-sleep utilities
*************************************************************/
void hal_usb_lock_extslp(void)
{
    if (!(hal_sleep_manager_get_extslp_lock_status() & (1 << SLEEP_LOCK_EXTEND_USB))) {
        hal_sleep_manager_lock_extslp(SLEEP_LOCK_EXTEND_USB);
        log_hal_msgid_info("usb lock extslp", 0);
    } else {
        log_hal_msgid_info("usb already lock extslp", 0);
    }
}

void hal_usb_unlock_extslp(void)
{
    if (hal_sleep_manager_get_extslp_lock_status() & (1 << SLEEP_LOCK_EXTEND_USB)) {
        hal_sleep_manager_unlock_extslp(SLEEP_LOCK_EXTEND_USB);
        log_hal_msgid_info("usb unlock extslp", 0);
    } else {
        log_hal_msgid_info("usb already unlock extslp", 0);
    }
}
#endif
/************************************************************
    DMA utilities
*************************************************************/
ATTR_TEXT_IN_TCM static uint8_t usb_get_dma_channel_num(uint8_t ep_num, hal_usb_endpoint_direction_t direction)
{
    uint8_t dma_chan;

    if (ep_num == 0) {
        log_hal_msgid_error("usb_get_dma_channel_num ep_num[%x] ERROR !!", 1, ep_num);
        return 0;
    }
    dma_chan = g_UsbDrvInfo->dma_port[direction][ep_num - 1];

    if ((dma_chan == 0) || (dma_chan > HAL_USB_MAX_NUMBER_DMA)) {
...
/* Stop DMA channel */
ATTR_TEXT_IN_TCM static void usb_hw_stop_dma_channel(uint32_t ep_num, hal_usb_endpoint_direction_t direction)
{
    uint8_t dma_chan;
    uint32_t savedMask;
    ...
    if (dma_chan < 1) {
        log_hal_msgid_error("usb_hw_stop_dma_channel : dma_chan < 1 Fail", 0);
        return;
    }
...
*************************************************************/
ATTR_TEXT_IN_TCM static void usb_ep_check(uint32_t ep_num, hal_usb_endpoint_direction_t direction, uint32_t line)
{
    if ((ep_num == 0) ||
...
ATTR_TEXT_IN_TCM static void usb_ep_dma_running_check(uint32_t ep_num, hal_usb_endpoint_direction_t direction, uint32_t line)
{
    uint8_t dma_chan;
...
ATTR_TEXT_IN_TCM hal_usb_status_t hal_usb_disable_endpoint0(void)
{
    uint32_t savedMask;
...
void hal_usb_clear_ep0_queue_event(void)
{
    hal_nvic_disable_irq(USB_IRQn);
    uint32_t start_index = g_usb_read_index;

    while (start_index != g_usb_write_index) {
        g_usb_isr_state[start_index].IntrTx &= (~USB_INTRTX_EP0);
        start_index = ((start_index + 1) % ISR_STATE_BUFFER_SIZE);
    }
...
ATTR_TEXT_IN_TCM hal_usb_status_t hal_usb_clear_endpoint0_fifo(void)
{
    uint32_t savedMask;
    hal_nvic_save_and_set_interrupt_mask(&savedMask);
    USB_DRV_SetBits(&musb->csr0, USB_CSR0_FLUSHFIFO);
    hal_nvic_restore_interrupt_mask(savedMask);
...
uint32_t hal_usb_get_sof_frame_number(void)
{
    return musb->frame;
}
...
void usb_ext_hisr(hal_nvic_irq_t irq_number)
{
    /* Enable clock source */
    hal_clock_enable(HAL_CLOCK_CG_USB_BUS);
    hal_clock_enable(HAL_CLOCK_CG_USB_DMA);
    ...
    /* Disable 32K wakeup module */
    USB_DRV_SetData32(USBPHYACR5, USBPHYACR5_RG_32K_RESUME_EN_MSK, 0 << USBPHYACR5_RG_32K_RESUME_EN_SFT);

    hal_usb_phy_leave_suspend();
    hal_nvic_enable_irq(USB_IRQn);
#ifdef HAL_SLEEP_MANAGER_ENABLED
    hal_usb_lock_sleep();
#endif
    log_hal_msgid_info("usb_ext_hisr: 32K wakeup", 0);
}

ATTR_TEXT_IN_TCM bool hal_usb_need_lisr(uint8_t IntrUSB, uint16_t IntrTx, uint16_t IntrRx)
{
    return ((IntrUSB & (~USB_INTRUSB_SOF)) || (IntrTx & USB_INTRTX_EP0) || IntrRx);
}

ATTR_TEXT_IN_TCM void usb_hisr(hal_nvic_irq_t irq_number)
{
#if defined(USB_DBG_GPIO)
    hal_gpio_set_output(GPIO_HISR, HAL_GPIO_DATA_HIGH);
#endif
    uint8_t IntrUSB;
    uint8_t IntrUSBE;
    uint16_t IntrTx;
    ...
    if((IntrTxE & USB_INTRTX_EP0) != USB_INTRTX_EP0){
        if(IntrTx & USB_INTRTX_EP0){
            //  log_hal_msgid_info("usb_hisr: EP0 Int was disabled", 0);
            IntrTx &= 0xFFFE;
        }
    }

    need_lisr = hal_usb_need_lisr(IntrUSB & IntrUSBE, IntrTx, IntrRx);
    if (need_lisr) {
        if (((g_usb_write_index + 1) % ISR_STATE_BUFFER_SIZE) != g_usb_read_index) {
            // Write 1 clear
            g_usb_isr_state[g_usb_write_index].IntrUSB = IntrUSB;
            g_usb_isr_state[g_usb_write_index].IntrUSBE = IntrUSBE;
            g_usb_isr_state[g_usb_write_index].IntrTx = IntrTx;
            g_usb_isr_state[g_usb_write_index].IntrTxE = IntrTxE;
            g_usb_isr_state[g_usb_write_index].IntrRx = IntrRx;
            g_usb_isr_state[g_usb_write_index].IntrRxE = IntrRxE;

            g_usb_write_index = ((g_usb_write_index + 1) % ISR_STATE_BUFFER_SIZE);
            // log_hal_msgid_info("usb_hisr: write[%d]", 1, g_usb_write_index);
        } else {
            log_hal_msgid_info("usb_hisr: over write!", 0);
#if defined(USB_DBG_GPIO)
            hal_gpio_set_output(GPIO_HISR, HAL_GPIO_DATA_LOW);
#endif
            return;
    ...
    if (IntrUSB & USB_INTRUSB_SOF) {
        if (g_UsbDrvInfo->sof_hdlr) {
            g_UsbDrvInfo->sof_hdlr();
        }
    }
...
#endif
    if (need_lisr) {
        hal_nvic_set_pending_irq(CM33_reserved0_IRQn);
    }
}

void usb_lisr(hal_nvic_irq_t irq_number)
{
#if defined(USB_DBG_GPIO)
    hal_gpio_set_output(GPIO_LISR, HAL_GPIO_DATA_HIGH);
#endif
    uint8_t   IntrUSB;
    uint8_t   IntrUSBE;
    uint16_t  IntrTx;
    ...
    uint32_t queue_remain_data;
    uint32_t local_write_index;
    uint32_t local_read_index;

    hal_nvic_disable_irq(USB_IRQn);
    // Get IRQ state
    if (g_usb_read_index != g_usb_write_index) {
        IntrUSB = g_usb_isr_state[g_usb_read_index].IntrUSB;
        IntrUSBE = g_usb_isr_state[g_usb_read_index].IntrUSBE;
        IntrTx = g_usb_isr_state[g_usb_read_index].IntrTx;
        IntrTxE =  g_usb_isr_state[g_usb_read_index].IntrTxE;
        IntrRx = g_usb_isr_state[g_usb_read_index].IntrRx;
        IntrRxE =  g_usb_isr_state[g_usb_read_index].IntrRxE;
        g_usb_read_index = ((g_usb_read_index + 1) % ISR_STATE_BUFFER_SIZE);
        // log_hal_msgid_info("usb_lisr: read[%d]", 1, g_usb_read_index);
    } else {
        log_hal_msgid_info("usb_hisr: over read", 0);
#if defined(USB_DBG_GPIO)
        hal_gpio_set_output(GPIO_LISR, HAL_GPIO_DATA_LOW);
#endif
        return;
    ...
    hal_nvic_enable_irq(USB_IRQn);
    // log_hal_msgid_info("usb_hisr: IntrUSB:0x%02x , frame:0x%04x", 2, IntrUSB, musb->frame);
    ...
    if (IntrUSB & USB_INTRUSB_SUSPEND) {
        ...
        /*initialize 32K wakeup module */
        USB_DRV_SetData32(USBPHYACR5, USBPHYACR5_RG_K_STATE_LENGTH_MSK, 0x60 << USBPHYACR5_RG_K_STATE_LENGTH_SFT);
        USB_DRV_SetData32(USBPHYACR5, USBPHYACR5_RG_32K_RESUME_EN_MSK, 1 << USBPHYACR5_RG_32K_RESUME_EN_SFT);

        hal_usb_phy_enter_suspend();
        /* disable USB clocks */
        hal_clock_disable(HAL_CLOCK_CG_USB_DMA);
        hal_clock_disable(HAL_CLOCK_CG_USB_BUS);
        hal_clock_disable(HAL_CLOCK_CG_USB);
...
#ifdef HAL_USB_QMU_MODULE_ENABLED
    // KS
    // 1. qmu and normal mode -- flow   not to watch:intrusb
    // 2. qmu interrupt --> QiSAR !!!
    hal_usb_qmu_isr();

    /*
    if(IntrTx & 0x02) {
        log_hal_msgid_error("usb_hisr: TX EP1 interrupt LOG, IntrTx value:0x%x", 1, IntrTx);
        hal_usb_qmu_isr();
    }
    if(IntrRx & 0x01) {
        log_hal_msgid_error("usb_hisr: RX EP1 interrupt LOG, IntrRx value:0x%x", 1, IntrRx);
        hal_usb_qmu_isr();
    }
    */
#endif
    ...
    if (!(IntrUSB & USB_INTRUSB_SUSPEND)) {
        hal_nvic_disable_irq(USB_IRQn);
        if (g_usb_write_index != g_usb_read_index) {
            hal_nvic_set_pending_irq(CM33_reserved0_IRQn);
        }
        local_write_index = g_usb_write_index;
        local_read_index = g_usb_read_index;
        queue_remain_data = get_queue_remain_data(local_write_index, local_read_index);
#if defined(USB_DBG_GPIO)
        hal_gpio_set_output(GPIO_LISR, HAL_GPIO_DATA_LOW);
#endif
        hal_nvic_enable_irq(USB_IRQn);
        queue_data_log(queue_remain_data, local_write_index, local_read_index);
    } else {
        log_hal_msgid_info("usb_hisr: w[%d], r[%d]", 2, g_usb_write_index, g_usb_read_index);
#if defined(USB_DBG_GPIO)
        hal_gpio_set_output(GPIO_LISR, HAL_GPIO_DATA_LOW);
#endif
    }
...
hal_usb_status_t hal_usb_drv_create_isr(void)
{
    ...
    hal_nvic_enable_irq(USB_IRQn);
    hal_nvic_register_isr_handler((hal_nvic_irq_t)CM33_reserved0_IRQn, usb_lisr);
    hal_nvic_enable_irq(CM33_reserved0_IRQn);

    /* 32K wakeup interrupt */
    hal_nvic_register_isr_handler((hal_nvic_irq_t)USB_32K_IRQn, usb_ext_hisr);
    hal_nvic_enable_irq(USB_32K_IRQn);
...
ATTR_TEXT_IN_TCM static bool usb_hw_is_ep_tx_empty(uint32_t ep_num)
{
    uint16_t CSR;
    uint32_t savedMask;
...
static void usb_dma_callback_func(uint8_t dma_chan)
{
    ...
    callback = g_UsbDrvInfo->dma_callback[dma_chan - 1];
...
ATTR_TEXT_IN_TCM bool hal_usb_get_tx_interrupt_enable_status(uint32_t ep_num)
{
    return (musb->intrtxe & USB_INTRTX_EPn(ep_num));
}

hal_usb_status_t hal_usb_enable_driver_intr(hal_usb_driver_handler_type_t type, uint32_t ep_num, uint8_t enable)
{
    if (enable != 1 && enable != 0) {
        return HAL_USB_STATUS_INVALID_PARAMETER;
    }

    switch (type) {
        case HAL_USB_DRV_HDLR_RESET:
            if (enable == 1) {
                USB_DRV_SetBits8(&musb->intrusbe, USB_INTRUSBE_RESET);
            }
            if (enable == 0) {
                USB_DRV_ClearBits8(&musb->intrusbe, USB_INTRUSBE_RESET);
            }
            break;
        case HAL_USB_DRV_HDLR_SUSPEND:
            if (enable == 1) {
                USB_DRV_SetBits8(&musb->intrusbe, USB_INTRUSBE_SUSPEND);
            }
            if (enable == 0) {
                USB_DRV_ClearBits8(&musb->intrusbe, USB_INTRUSBE_SUSPEND);
            }
            break;
        case HAL_USB_DRV_HDLR_RESUME:
            if (enable == 1) {
                USB_DRV_SetBits8(&musb->intrusbe, USB_INTRUSBE_RESUME);
            }
            if (enable == 0) {
                USB_DRV_ClearBits8(&musb->intrusbe, USB_INTRUSBE_RESUME);
            }
            break;
        case HAL_USB_DRV_HDLR_SOF:
            if (enable == 1) {
                USB_DRV_SetBits8(&musb->intrusbe, USB_INTRUSBE_SOF);
            }
            if (enable == 0) {
                USB_DRV_ClearBits8(&musb->intrusbe, USB_INTRUSBE_SOF);
            }
            break;
        ...
        case HAL_USB_DRV_HDLR_EP_TX:
            if (enable == 0) {
                USB_DRV_ClearBits8(&musb->intrtxe, USB_INTRTX_EPn(ep_num));
            } else {
                USB_DRV_SetBits8(&musb->intrtxe, USB_INTRTX_EPn(ep_num));
            }
            break;
...
hal_usb_status_t hal_usb_update_endpoint_0_state(hal_usb_endpoint_0_driver_state_t state, bool stall, bool end)
{
    ...
    } else if (state == HAL_USB_EP0_DRV_STATE_WRITE_RDY) {
        reg_state = USB_CSR0_TXPKTRDY;
    } else if (state == HAL_USB_EP0_DRV_STATE_NO_UPDATE) {
        reg_state = 0x00;
    } else {
        return HAL_USB_STATUS_INVALID_PARAMETER;
    }
    ...
    // printk("hal_usb_update_endpoint_0_state csr0:0x%02X new_state:%02X\n\r", musb->csr0, reg_state);
...
ATTR_TEXT_IN_TCM bool hal_usb_is_endpoint_tx_empty(uint32_t ep_num)
{
    return usb_hw_is_ep_tx_empty(ep_num);
}

ATTR_TEXT_IN_TCM bool hal_usb_is_endpoint_rx_empty(uint32_t ep_num)
{
    bool result;
    uint16_t CSR;
    uint32_t savedMask;
...
ATTR_TEXT_IN_TCM bool hal_usb_ep_empty(uint32_t ep_num, bool dir_in)
{
    return (dir_in ? hal_usb_is_endpoint_tx_empty(ep_num) : hal_usb_is_endpoint_rx_empty(ep_num));
}

ATTR_TEXT_IN_TCM bool hal_usb_check_ep0_tx_packet_ready(void)
{
    uint16_t CSR0;
    ...
    return !!(CSR0 & USB_CSR0_TXPKTRDY);
}

ATTR_TEXT_IN_TCM hal_usb_status_t hal_usb_clear_tx_endpoint_fifo(uint32_t ep_num, hal_usb_endpoint_transfer_type_t ep_type, bool b_reset_toggle)
{
    uint32_t savedMask;
...
ATTR_TEXT_IN_TCM hal_usb_status_t hal_usb_clear_rx_endpoint_fifo(uint32_t ep_num, hal_usb_endpoint_transfer_type_t ep_type, bool b_reset_toggle)
{
    uint32_t savedMask;
...
hal_usb_status_t hal_usb_stop_dma_channel(uint32_t ep_num, hal_usb_endpoint_direction_t direction)
{
    ...
    if (dma_chan < 1) {
        log_hal_msgid_error("hal_usb_stop_dma_channel : dma_chan < 1 Fail", 0);
        return HAL_USB_STATUS_ERROR;
    }
                                           ...
                                           hal_usb_dma_handler_t callback, bool callback_upd_run, hal_usb_dma_type_t dma_type)
{
    ...
    if (dma_chan == 0) {
        log_hal_msgid_error("hal_usb_start_dma_channel : dma_chan[%d], error", 1, dma_chan);
        return HAL_USB_STATUS_INVALID_PARAMETER;
    }
...
bool hal_usb_is_dma_running(uint32_t ep_num, hal_usb_endpoint_direction_t direction)
{
    ...
    if (dma_chan < 1) {
        log_hal_msgid_error("hal_usb_is_dma_running : dma_chan < 1 Fail", 0);
        return false;
    }
...
hal_usb_status_t hal_usb_is_dma_ready(uint32_t ep_num, hal_usb_endpoint_direction_t direction)
{
    ...
    if (dma_chan < 1) {
        log_hal_msgid_error("hal_usb_is_dma_ready : dma_chan < 1 Fail", 0);
        return HAL_USB_STATUS_ERROR;
    }
...
static void hal_usb_detect_linestate(void* p)
{
    ...
    hal_usb_linestate_t ls = hal_usb_phy_detect_linestate();
...
uint8_t hal_usb_pktdone_cfg(uint8_t set, uint8_t enable, uint8_t ep, uint8_t dir)
{
    if (set >= 2) {
        return 1;
    }
    ...
    if (set == 0) {
        USB_DRV_WriteReg8(&musb->pktdone_set0, (enable << USB_PKTDONE_SET0_EN_OFFSET) | (ep << USB_PKTDONE_SET0_EP_OFFSET) | (dir << USB_PKTDONE_SET0_DIR_OFFSET));
        if (enable == 1) {
            USB_DRV_SetBits8(&musb->pktdone_intr_msk, USB_PKTDONE_INTR_MSK_SET0);
        }
        else {
            USB_DRV_ClearBits8(&musb->pktdone_intr_msk, USB_PKTDONE_INTR_MSK_SET0);
        }
    }
    else if (set == 1) {
        USB_DRV_WriteReg8(&musb->pktdone_set1, (enable << USB_PKTDONE_SET1_EN_OFFSET) | (ep << USB_PKTDONE_SET1_EP_OFFSET) | (dir << USB_PKTDONE_SET1_DIR_OFFSET));
        if (enable == 1) {
            USB_DRV_SetBits8(&musb->pktdone_intr_msk, USB_PKTDONE_INTR_MSK_SET1);
        }
        else {
            USB_DRV_ClearBits8(&musb->pktdone_intr_msk, USB_PKTDONE_INTR_MSK_SET1);
        }
    }

#if 0
    log_hal_msgid_info("mac->pktdone_set0     (0x%08X) : 0x%02x", 2, (uint32_t)&musb->pktdone_set0, musb->pktdone_set0);
    log_hal_msgid_info("mac->pktdone_intr_msk (0x%08X) : 0x%02x", 2, (uint32_t)&musb->pktdone_intr_msk, musb->pktdone_intr_msk);
    log_hal_msgid_info("mac->pktdone_intr_sts (0x%08X) : 0x%02x", 2, (uint32_t)&musb->pktdone_intr_sts, musb->pktdone_intr_sts);
#endif

    return 0;
}

uint8_t hal_usb_pktdone_clear(uint8_t set)
{
    if (set >= 2) {
        return 1;
    }

    USB_DRV_WriteReg8(&musb->pktdone_intr_clr, 1 << set);

    return 0;
}

uint8_t hal_usb_pktdone_get_status(uint8_t set)
{
    uint8_t status = 0;
    if (set >= 2) {
        return status;
    ...
    status = (USB_DRV_Reg8(&musb->pktdone_intr_sts) & (1 << set)) ? 1 : 0;
