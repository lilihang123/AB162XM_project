...
#include "hal_gpt.h"
#include "hal_log.h"
#if !(defined(__EXT_DA__) || defined(__UBL__))
...
extern uint32_t sf_erase_duration_previous;
#endif
...
#define log_hal_flash_msgid_info_mask(_message, arg_cnt, ...)
#else
 ...
 **********************************************************************/
ATTR_TEXT_IN_TCM_RETENTION int32_t readRawDiskData(NOR_FLASH_DISK_Data *D, uint32_t addr, uint8_t *data_ptr, uint32_t len)
{
    uint32_t phyAddr;
 ...
 **********************************************************************/
ATTR_TEXT_IN_TCM int32_t writeRawDiskData(NOR_FLASH_DISK_Data *D, uint32_t addr, uint8_t *data_ptr, uint32_t len)
{
    uint32_t dest, src;
    uint32_t diskSize = D->DiskSize;
        ...
        if (((uint32_t)dest & (sizeof(uint16_t) - 1)) || ((len - DoneLength) == 1)) {
            ...
            if (programBytes == sizeof(uint16_t) && (src % sizeof(uint16_t))) {
                ...
                if (programBytes & 0x1) { //must be Sibley flash
                    log_hal_msgid_error("writeRawDiskData error programBytes = %d", 1, programBytes);
                } else {
...
ATTR_TEXT_IN_TCM_RETENTION hal_flash_status_t get_NOR_init_status(void)
{
...
ATTR_TEXT_IN_TCM_RETENTION int32_t NOR_EraseChip()
{
    NOR_FLASH_DISK_Data *D = ENTIRE_DISK_DRIVER_DATA;
    NOR_Flash_MTD_Data *mtdData = (NOR_Flash_MTD_Data *)D->MTDData;
...
hal_flash_status_t get_rawdisk_error_code(int32_t ori_err_code)
{
    if (ori_err_code == RAW_DISK_ERR_WRONG_ADDRESS) {
        ...
        log_hal_msgid_warning("address:0x%x, write:0x%x, readback:0x%x, SR = 0x%x", 4, error_addr, org_data, readback_data, error_status);
    ...
    } else if (ori_err_code == HAL_FLASH_STATUS_ERROR_LOCKED) {
        return HAL_FLASH_STATUS_ERROR_ERASE_FAIL;
    }
#ifdef AIR_FOTA_DELAY_ERASE_FLASH_ENABLE
    else if (ori_err_code == RAW_DISK_ERR_ERASE_OVER_TIME) {
        return HAL_FLASH_STATUS_ERROR_ERASE_TIME_OUT;
    }
#endif
    else {
        return HAL_FLASH_STATUS_ERROR_ERASE_FAIL;
...
ATTR_TEXT_IN_TCM_RETENTION void sfc_isr(hal_nvic_irq_t irq_number)
{
    uint32_t mac_ctl = 0;
    uint32_t count;
    uint32_t mask;
...
#ifdef FPGA_ENV
ATTR_TEXT_IN_TCM_RETENTION void flash_init_spiq(void)
{
    uint8_t sr;
    uint8_t cmd;
    uint32_t savedMask;
    ...
    SFC->RW_SF_MAC_INL = 1;
    SFI_MacWaitReady(0);
    sr = (unsigned char)((SFC_GPRAM->RW_SF_GPRAM_DATA) >> 8);

    if ((sr & 0x02) == 0x02) {
    } else {
        cmd = 0x1;
        SFC_GPRAM->RW_SF_GPRAM_DATA = 0x50;  //volatile WEN
        SFC->RW_SF_MAC_OUTL = 1;
        SFC->RW_SF_MAC_INL = 0;
        SFI_MacEnable(0);
        SFI_MacWaitReady(0);
        hal_gpt_delay_ms(1);
        //this is only for GD Flash
        cmd = 0x1;
        sr = 0x2;
        SFC_GPRAM->RW_SF_GPRAM_DATA = (((sr<<16) & 0xFF0000) | cmd);
        SFC->RW_SF_MAC_OUTL = 3;
        SFC->RW_SF_MAC_INL = 0;
        SFI_MacEnable(0);
        SFI_MacWaitReady(0);
        hal_gpt_delay_ms(1);
    ...
    //QE bit set
    *(volatile uint32_t*)(SFC_BASE +0X4) = 0xEB0B55F0;
    /* should not enable clock inverse */
    *(volatile uint32_t*)(SFC_BASE +0X8) = 0x52F80300;
    *(volatile uint32_t*)(SFC_BASE+0x28) = 0x1A00001A;
    SFI_MaskAhbChannel(0);   //umask DSP
...
hal_flash_status_t hal_flash_erase(uint32_t start_address, hal_flash_block_t block)
{
    ...
    partition_is_readonly(start_address, length, &is_readonly);
    #ifndef AIR_SLT_TEST
...
#ifdef AIR_FOTA_DELAY_ERASE_FLASH_ENABLE
    log_hal_msgid_info("[hal_flash_erase]: flash erase previous use time = %d, erase use time = %d", 2, sf_erase_duration_previous, sf_erase_duration);
#endif
...
ATTR_TEXT_IN_TCM hal_flash_status_t hal_flash_write(uint32_t address, const uint8_t *data, uint32_t length)
{
    NOR_FLASH_DISK_Data *D = ENTIRE_DISK_DRIVER_DATA;
    int32_t result;
    ...
    if ((address & (~SFC_GENERIC_FLASH_BANK_MASK)) > D->DiskSize) {
        return HAL_FLASH_STATUS_ERROR_WRONG_ADDRESS;
    }
    ...
    partition_is_readonly(address, length, &is_readonly);
    #ifndef AIR_SLT_TEST
    assert(is_readonly == false);
    #endif
#endif
    ...
    if (result != RAW_DISK_ERR_NONE) {
        log_hal_msgid_warning("[hal_flash_write] error=%d, address=0x%x, len=%d \r\n", 3, result, address, length);
        return get_rawdisk_error_code(result);
    }
...
{
    int32_t result = RAW_DISK_ERR_NONE;

    if (otp_addr >= ((SF_MTD_Data *)D->MTDData)->OTPLength
       || (otp_addr + len) > (((SF_MTD_Data *)D->MTDData)->OTPLength + ((SF_MTD_Data *)D->MTDData)->OTPBase)) {
        return RAW_DISK_ERR_WRONG_ADDRESS;
    }
...
extern int SF_DAL_OTPLock_blank(void *MTDData, OTP_BANK_enum bank);
