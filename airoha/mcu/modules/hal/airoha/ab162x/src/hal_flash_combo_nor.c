...
{
    if (CS == 0) {
        cmem_nor_index = CMEM_EMIINIT_Index();
        if (cmem_nor_index < 0) {
...
/* defines in OTP table of 162x start */
#define AB162X_OTP_CHIP_ID_ADDRESS    (0x0)
#define AB162X_OTP_FLASH_ID_ADDRESS   (0x2)
#define AB1620D           (0x1)
#define AB1623            (0x3)
#define VENDER_XM25LU16C  (6)
/* defines in OTP table of 162x start */


#define XMC_1MB_DEVICE    (0x100000)
extern uint8_t nor_id[4];
extern uint32_t protect_flash_last_1mb();

uint8_t chip_id_otp_addr_0;
uint8_t flash_id_otp_addr_2;

int32_t check_xmc_device()
{
    hal_flash_status_t status;
    status = hal_flash_otp_read(AB162X_OTP_CHIP_ID_ADDRESS, &chip_id_otp_addr_0, 1);
    if (status != HAL_FLASH_STATUS_OK) {
        //log_hal_msgid_error("[hal_flash_init] read chip id failed status = %d result ok\r\n", 1, status);
        return HAL_FLASH_STATUS_ERROR;
    }

    status = hal_flash_otp_read(AB162X_OTP_FLASH_ID_ADDRESS, &flash_id_otp_addr_2, 1);
    if (status != HAL_FLASH_STATUS_OK) {
        //log_hal_msgid_error("[hal_flash_init] read chip id failed status = %d result ok\r\n", 1, status);
        return HAL_FLASH_STATUS_ERROR;
    }

    //if ((chip_id_otp_addr_0 == AB1620D || chip_id_otp_addr_0 == AB1623 || chip_id_otp_addr_0 == 4) &&
    if ((chip_id_otp_addr_0 == AB1620D || chip_id_otp_addr_0 == AB1623) &&
        (flash_id_otp_addr_2 == VENDER_XM25LU16C) &&
        (nor_id[0] == 0x20 && nor_id[1] == 0x50 && nor_id[2] == 0x15)) {
             EntireDiskDriveData.DiskSize = XMC_1MB_DEVICE;
             uint32_t mask = 0;
             uint32_t sr = 0;
             hal_nvic_save_and_set_interrupt_mask(&mask);
             sr = protect_flash_last_1mb();
             hal_nvic_restore_interrupt_mask(mask);
             printk("check xmc device sr = 0x%x!!!! \r\n", sr);
    }

    printk("chip id = %d, flash_vendor_id = %d, flash size = 0x%x\r\n", chip_id_otp_addr_0, flash_id_otp_addr_2, EntireDiskDriveData.DiskSize);
    return 0;
}
...
int32_t CMEM_Init_FullDriver(void)
{
    ...
    //LayoutInfo.BlkSize = NOR_BLOCK_SIZE;
    //LayoutInfo.PageSize = NOR_PAGE_SIZE;
    //LayoutInfo.TotalLSMT = NOR_TOTAL_LSMT;dd
