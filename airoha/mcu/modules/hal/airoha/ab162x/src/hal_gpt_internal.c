...
ATTR_TEXT_IN_TCM_RETENTION uint32_t gpt_current_count(GPT_REGISTER_T *gpt)
{
...
ATTR_TEXT_IN_TCM_RETENTION void  gpt_start_free_run_timer(GPT_REGISTER_T *gpt, uint32_t clock_source, uint32_t divide)
{

    // gpt_open_clock_source();

    /* if free run timer is not open, open it */
    if (gpt->GPT_CON_UNION.GPT_CON_CELLS.EN == 0) {
        gpt->GPT_CLK = clock_source | divide;
        gpt->GPT_CLR = GPT_COUNT_CLEAR;
...
ATTR_TEXT_IN_TCM_RETENTION void gpt_delay_time(GPT_REGISTER_T *gpt, const uint32_t count)
{
    uint32_t current = 0;
    uint32_t  temp0 = 0;
    uint32_t  temp1 = 0;
...
ATTR_TEXT_IN_TCM void clear_gpt_and_restart_hw_gpt(uint32_t next_expired_count, hal_gpt_port_t port)
{
     ...
     */
    GPT(port)->GPT_CLK = GPT_CLOCK_16MHZ | (uint32_t) GPT_DIVIDE_1;
    ...
    if (port == HAL_GPT_SW_US_PORT) {
        GPT(port)->GPT_CLK = GPT_CLOCK_16MHZ | (uint32_t)GPT_DIVIDE_16;
    }
