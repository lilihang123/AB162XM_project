...
void reset_esc_register(void)
{
    ...
    esc_register->ESC_MISC_CTL1  = 0x02420103 | 0x10;  //default: delay 0.5T
    ...
    esc_register->ESC_DLY_CTL3   = 0x00000000;
    esc_register->ESC_DMY_BYTE   = 0x000000F0;
...
*/
ATTR_TEXT_IN_TCM void ESC_Dev_Command(const uint32_t cmd)
{
    uint32_t cmdi, cmdo;
    uint8_t  len;
...
/*!
  ...
  @remarks This function shall only be invoked in bootloader.
           Other callers must be aware of interrupts during the MAC mode
*/
ATTR_TEXT_IN_TCM void ESC_Dev_Command_Ext(const uint8_t *cmd, uint8_t *data, const uint16_t outl, const uint16_t inl)
{
    uint32_t tmp;
    uint32_t i, j;
...
const esc_psram_setting g_setting = {
...
#ifndef FPGA_ENV
    0x02420113,          //ESC_MISC_CTL1
#else
    0x02420103,          //ESC_MISC_CTL1
#endif
    0x00000000,          //ESC_MISC_CTL2
    { 0x00000000, 0x00000000, 0x0A00000A, 0x00000000 },    //ESC_DELAY_CTL_52MHZ
    { 0x00000000, 0x00000000, 0x0A00000A, 0x00000000 },    //ESC_DELAY_CTL_26MHZ
    { 0x00000000, 0x00000000, 0x0A00000A, 0x00000000 },    //ESC_DELAY_CTL_104MHZ
#elif defined(APS_CHIP_1604M_SQ_PSRAM)
#if 1
    ESC_QPI_MODE,        //device mode
    0x00000000,          //ESC_MAC_CTL
    ...
    { 0x00000000, 0x00000000, 0x0A00000A, 0x00000000 },    //ESC_DELAY_CTL_26MHZ
    { 0x00000000, 0x00000000, 0x0A00000A, 0x00000000 },    //ESC_DELAY_CTL_104MHZ
    //ESC_DLY_CTL1  ESC_DLY_CTL2  ESC_DLY_CTL3  Reserved
#else
    ...
    { 0x00000000, 0x00000000, 0x0A00000A, 0x00000000 },    //ESC_DELAY_CTL_104MHZ
#else
    #error "No SRAM device model specified"
#endif
};
...
{
#if defined(MICRO_CHIP_23A1024)
    ESC_Dev_Command(ESC_CMD_EXIT_QUAD_MODE);
#elif defined(APS_CHIP_1604M_SQ_PSRAM)
    ESC_Dev_Command(ESC_CMD_RESET_ENABLE);
    ESC_Dev_Command(ESC_CMD_RESET);
#elif defined(ESC_ASP6404L_SQ_PSRAM)
    ESC_Dev_Command(ESC_CMD_RESET_ENABLE);
    ESC_Dev_Command(ESC_CMD_RESET);
#endif
...
void esc_internal_init(void)
{
    ...
    if (clock == ESC_CLOCK_26MHz) {
        ...
        printk("Please check ESC clock setting!\r\n");
    }
    printk("esc_internal_init: ESC freq = %d Hz, clock is %d\r\n", hal_clock_get_freq_meter(f_fesc_ck, 100), clock);
#endif /* HAL_CLOCK_MODULE_ENABLED */

    esc_device_reset();

    if (g_setting.DEVICE_MODE == ESC_QPI_MODE) {
        ESC_Dev_Command(ESC_CMD_ENTER_QUAD_MODE);
    }
    sleep_management_register_infra_pd_suspend_callback(INFRA_PD_BACKUP_RESTORE_ESC, (sleep_management_suspend_callback_t)esc_backup_register_callback, NULL);
    sleep_management_register_infra_pd_resume_callback(INFRA_PD_BACKUP_RESTORE_ESC, (sleep_management_suspend_callback_t)esc_restore_register_callback, NULL);
}
...
const esc_device_info_t g_esc_support_flash_list[] = {
    {
        "G25WQ40E", 0x00C86513, 256, 2048, 0x0B, 0xEB, 0x02, 0x7A, 0x75, 1, 0, 7,
        { 0x20, 0x52, 0xD8, 0xC7 },    //erase command
        { 0x00000000, 0x00000000, 0x0A00000A, 0x00000006 },    //ESC_DELAY_CTL_52MHZ
        { 0x00000000, 0x00000000, 0x0A00000A, 0x00000006 },    //ESC_DELAY_CTL_26MHZ
        { 0x00000000, 0x00000000, 0x0A00000A, 0x00000003 }     //ESC_DELAY_CTL_104MHZ
    },
    {
        "G25WQ80E", 0x00C86514, 256, 4096, 0x0B, 0xEB, 0x02, 0x7A, 0x75, 1, 0, 7,
        { 0x20, 0x52, 0xD8, 0xC7 },    //erase command
        { 0x00000000, 0x00000000, 0x0A00000A, 0x00000006 },    //ESC_DELAY_CTL_52MHZ
        { 0x00000000, 0x00000000, 0x0A00000A, 0x00000006 },    //ESC_DELAY_CTL_26MHZ
        { 0x00000000, 0x00000000, 0x0A00000A, 0x00000003 }     //ESC_DELAY_CTL_104MHZ
...
hal_esc_flash_status_t esc_flash_parameter_check(uint32_t addr, void *ptr, uint32_t len)
{
    ...
    if (get_initial_state() == false) {
        printk("ESC flash not initial !!!");
        return HAL_ESC_FLASH_STATUS_ERROR_NO_INIT;
    }

    if ((NULL == ptr) || (0 == len) || (len > capacity) || (addr > capacity) || ((addr + len) > capacity)) {
        printk("ESC flash api was called with ptr(0x%08X) and len(0x%08X)", (unsigned int)ptr, len);
    ...
    if ((addr >= esc_addr_range[0])  && (addr < esc_addr_range[1]) && (addr < end_addr) && (end_addr < esc_addr_range[1])) {
        ...
        printk("ESC flash api was called with invalid address.");
        printk("input address range: 0x%08X, 0x%08X", addr, end_addr);
        printk("ESC allow address range: 0x%08X, 0x%08X", esc_addr_range[0], esc_addr_range[1]);
...
void esc_internal_init(void)
{
    ...
    printk("esc_internal_init: ESC freq = %d Hz, g_esc_ctl.clock is %d\r\n", hal_clock_get_freq_meter(f_fesc_ck, 100), g_esc_ctl.clock);
#else
    g_esc_ctl.clock = ESC_CLOCK_26MHz;
#endif

    esc_register->ESC_MISC_CTL1 |= 0x10;   //set o.5T at default
    ...
    if (ESC_PARSE_MANU_ID(jedec_id) == WB_JEDEC_ID || ESC_PARSE_MANU_ID(jedec_id) == GD_JEDEC_ID) {
        if (0xEF6018 == jedec_id || 0xEF6017 == jedec_id || 0xEF6016 == jedec_id ||
            0xC86018 == jedec_id || 0xC86017 == jedec_id || 0xC86016 == jedec_id ||  0xC86014 == jedec_id || 0xC86013 == jedec_id ||
            0xC86514 == jedec_id || 0xC86513 == jedec_id) {
            ...
            printk("esc_internal_init direct_ctl is 0x%08X\r\n", direct_ctl);
            write_qe_bit(1);
        } else {
            ...
            ESC_Dev_Command_Ext(&cmd_enter_4_byte_address_mode, NULL, 1, 0);
            esc_register->ESC_MISC_CTL2 |= ((uint32_t)0x1 << ESC_ADDRESS_MODE_BIT_OFFSET);
            printk("function %s line %d\r\n", __FUNCTION__, __LINE__);
        }
        esc_register->ESC_DIRECT_CTL = direct_ctl;
    }
    sleep_management_register_infra_pd_suspend_callback(SLEEP_BACKUP_RESTORE_ESC, (sleep_management_suspend_callback_t)esc_backup_register_callback, NULL);
    sleep_management_register_infra_pd_resume_callback(SLEEP_BACKUP_RESTORE_ESC, (sleep_management_suspend_callback_t)esc_restore_register_callback, NULL);
}
...
uint8_t read_device_reg(esc_device_reg_type_t type)
{
    ...
    if (ESC_PARSE_MANU_ID(jedec_id) == WB_JEDEC_ID || ESC_PARSE_MANU_ID(jedec_id) == GD_JEDEC_ID) {
        ...
        ESC_Dev_Command_Ext(&read_cmd, &reg_data, 1, 1);
    }
    return reg_data;
}


void adjust_device_iodriving()
{
    //set device driving for APS_CHIP_1604M_SQ_PSRAM @QPI mode
    uint8_t esc_sr;
    uint8_t cmd[8];
    cmd[0] = 0xB5;
    cmd[1] = 0x0;
    ...
    cmd[5] = 0x0;
    cmd[6] = 0x0;
    esc_register->ESC_MAC_CTL |= 0x10;
    ESC_Dev_Command_Ext((const uint8_t *)&cmd[0], &esc_sr, 7, 1);
    printk("[ESC] sr is 0x%x \r\n", esc_sr);
    cmd[0] = 0xB1;
    cmd[1] = 0x0;
    cmd[2] = 0x0;
    cmd[3] = 0x0;
    cmd[4] = 0x62;
    ESC_Dev_Command_Ext((const uint8_t *)&cmd[0], NULL, 5, 0);
...
bool write_qe_bit(uint8_t bit_value)
{
    ...
    if (ESC_PARSE_MANU_ID(jedec_id) == WB_JEDEC_ID) {
        ...
        cmd[1] = curr;
        ESC_Dev_Command_Ext(&cmd[0], NULL, 2, 0);
        ...
        printk("origin: 0x%02X, curr: 0x%02X, read_back: 0x%02X\r\n", origin, curr, read_back);
    }
    ...
    if (ESC_PARSE_MANU_ID(jedec_id) == GD_JEDEC_ID) {
        ...
        cmd[1] = 0x2;
        cmd[2] = curr;  //GD 0x2
        ESC_Dev_Command_Ext(&cmd[0], NULL, 3, 0);
        ...
        read_back = read_device_reg(REG_QE);
        printk("[write_qe_bit] origin: 0x%02X, curr: 0x%02X, read_back: 0x%02X\r\n", origin, curr, read_back);
    }
    return result;
}
...
ATTR_TEXT_IN_TCM bool esc_judge_device(void)
{
    ...
    ESC_Dev_Command_Ext(&read_id_cmd, id, 1, 3);
    jedec_id = ((uint32_t)(id[0]) << 16) | ((uint32_t)(id[1]) << 8) | (id[2]);
    printk("ESC flash id: 0x%02X, 0x%02X, 0x%02X, and JEDEC id: 0x%08X\r\n", id[0], id[1], id[2], jedec_id);
...
ATTR_TEXT_IN_TCM void esc_flash_suspend(void)
{
    ...
    if ((g_esc_ctl.suspended == false) && (g_esc_ctl.busy == true)) {
        ...
        if (device_is_busy(device_status)) {
            ...
            ESC_Dev_Command_Ext(&suspend_cmd, NULL, 1, 0);
...
ATTR_TEXT_IN_TCM int32_t esc_check_ready_and_resume(void)
{
    ...
    if (g_esc_ctl.ahb_req_while_mac == true) {
        ...
        if ((status & FLASH_BUSY_STATUS) == false) {
            ...
            if (g_esc_ctl.suspended == true) {
                ...
                ESC_Dev_Command_Ext(&resume_cmd, NULL, 1, 0);
    ...
    if (ahb_req_flag == true) {
        ...
        //printk("DMA or DSP direct read during ESC in MAC mode at %d.\r\n", gpt_tick);
        hal_gpt_delay_us(50);
        hal_nvic_save_and_set_interrupt_mask(&irq_mask);
...
hal_esc_flash_status_t hal_esc_erase_flash_block(uint32_t address, hal_esc_erase_type_t type)
{
    ...
    if (type == HAL_ESC_FLASH_ERASE_4K) {
        ...
        printk("hal_esc_erase_flash_block was called with invalid type %d\r\n", type);
        return HAL_ESC_FLASH_WRONG_PARAMETER;
    }
    ...
    if ((type != HAL_ESC_FLASH_ERASE_CHIP) && (address & (len - 1))) {
        printk("hal_esc_erase_flash_block was called with invalid address 0x%08X and type %d\r\n", address, type);
        return HAL_ESC_FLASH_STATUS_ERROR_WRONG_ADDRESS;
    }
...
ATTR_TEXT_IN_TCM void check_mac_en(const char *called_func, uint32_t error_value)
{
    ...
    if (mac_en == error_value) {
        ...
        for (;;) {
            printk("%s mac_en is %d, busy is %d, suspend is %d\r\n", called_func, error_value, g_esc_ctl.busy, g_esc_ctl.suspended);
...
uint32_t esc_flash_length(void)
{
    ...
    return (g_esc_ctl.device->page_size * g_esc_ctl.device->page_number);
#else
    return 0xFFFFFFFF;
#endif
}
