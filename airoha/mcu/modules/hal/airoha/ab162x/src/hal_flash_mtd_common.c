...
*/
ATTR_TEXT_IN_TCM_RETENTION uint32_t SFI_ReverseByteOrder(uint32_t num)
{
    unsigned int ret = 0;
...
*/
ATTR_TEXT_IN_TCM_RETENTION void SFI_MacEnable(const uint16_t CS)
{
    uint32_t val;
...
/*!
  ...
  @remarks
*/
ATTR_TEXT_IN_TCM_RETENTION void SFI_MacLeave(const uint16_t CS)
{
    uint32_t val;
...
*/
ATTR_TEXT_IN_TCM_RETENTION uint32_t SFI_MacTrigger(const uint16_t CS)
{
    uint32_t  val = 0;
    uint32_t  misc_ctl = 0;
...
*/
ATTR_TEXT_IN_TCM_RETENTION void SFI_MacWaitReady(const uint16_t CS)
{
    SFI_MacTrigger(CS);
    SFI_MacLeave(CS);
}
...
*/
ATTR_TEXT_IN_TCM_RETENTION void SFI_Dev_Command(const uint16_t CS, const uint32_t cmd)
{
    uint32_t cmdi, cmdo;
    uint8_t  len;
...
/*!
  ...
  @remarks This function is only called by bootloader.
           Other callers must be aware of interrupts during the MAC mode
*/
ATTR_TEXT_IN_TCM_RETENTION void SFI_Dev_CommandAddress(const uint16_t CS, const uint8_t cmd, const uint32_t address, const uint32_t address_bytes)
{
    uint32_t cmd1, cmd2;
...
*/
ATTR_TEXT_IN_TCM_RETENTION uint32_t SFI_GPRAM_Write_C1A3(const uint32_t cmd, const uint32_t address)
{
...
ATTR_TEXT_IN_TCM_RETENTION uint32_t SFI_GPRAM_Write_C1A4(const uint32_t cmd, const uint32_t address, uint8_t **p_data, uint32_t *length)
{
    sf_uint    tmp;
    uint32_t cmd1, cmd2;
    uint32_t i;
...
*/
ATTR_TEXT_IN_TCM_RETENTION void SFI_GPRAM_Write(uint32_t gpram_offset, void *buff, int32_t length)
{
    uint32_t i;
    uint8_t *p_data = (uint8_t *)buff;
    sf_uint tmp;

    gpram_offset += SFC_GPRAM_ADDR;
...
/*!
  ...
  @remarks This function shall only be invoked in bootloader.
           Other callers must be aware of interrupts during the MAC mode
*/
ATTR_TEXT_IN_TCM_RETENTION void SFI_Dev_Command_Ext(const uint16_t CS, const uint8_t *cmd, uint8_t *data, const uint16_t outl, const uint16_t inl)
{
    uint32_t      tmp;
    uint32_t      i, j;
    ...
    for (i = 0, p_data = ((uint8_t *)SFC_GPRAM_ADDR); i < outl; p_data += 4)   {
    ...
    for (i = 0, p_data = ((uint8_t *)SFC_GPRAM_ADDR + outl); i < inl; ++i, ++data, ++p_data) {
...
*/
ATTR_TEXT_IN_TCM_RETENTION static void write_en(const uint16_t CS, const uint32_t cmd, uint8_t outlen)
{
...
/*!
  @brief  it is used in  BL: custom_setSFIExt --> SFI_Dev_Command_List 
*/
ATTR_TEXT_IN_TCM_RETENTION static uint32_t ReadStatus(const uint16_t CS, uint8_t cmd)
{
    uint32_t savedMask = 0;
    uint32_t result;
...
*/
ATTR_TEXT_IN_TCM_RETENTION void SFI_Dev_Command_List(const uint16_t CS, const uint8_t *cmdlist)
{
    uint32_t i = 0;
    uint32_t val;
...
*/
ATTR_TEXT_IN_TCM_RETENTION bool SFI_Dev_GetUniqueID(uint8_t *buffer)
{
    int i = 0;
    for (i = 0; i < 8; i++) {
        buffer[i] = 0xFF;
...
ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_DEV_Backup(void)
{
...
ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_DEV_Enter_DPD(void)
{
    SF_DAL_DEV_Backup();

    /* set IES bit[13] as 0 */
    if (*PMU_DIG_CTRL10__F_EXTSLP_VIO18_OFF == 1) {
        //EXTSLP RET_VIO18 off
        *SFIO_CFG0_DRV = (*SFIO_CFG0_DRV) & 0xFFFDFFFF;  //set bit13 as 0
        *SFIO_CFG1_DRV = (*SFIO_CFG1_DRV) & 0xFFFDFFFF;  //set bit13 as 0
        *SFIO_CFG2_DRV = (*SFIO_CFG2_DRV) & 0xFFFDFFFF;  //set bit13 as 0
        *SFIO_CFG3_DRV = (*SFIO_CFG3_DRV) & 0xFFFDFFFF;  //set bit13 as 0
        *SFIO_CFG4_DRV = (*SFIO_CFG4_DRV) & 0xFFFDFFFF;  //set bit13 as 0
        *SFIO_CFG5_DRV = (*SFIO_CFG5_DRV) & 0xFFFDFFFF;  //set bit13 as 0
    }

    SFI_Dev_Command(0, SF_CMD_ENTER_DPD);
    hal_gpt_delay_us(DPDReadyTime);
}

ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_DEV_Restore(void)
{
...
ATTR_TEXT_IN_TCM_RETENTION void SF_DAL_DEV_Leave_DPD(void)
{
    /* set IES bit[13] as 1 */
    if (*PMU_DIG_CTRL10__F_EXTSLP_VIO18_OFF == 1) {
        //EXTSLP RET_VIO18 off
        *SFIO_CFG0_DRV = (*SFIO_CFG0_DRV) | 0x00020000;  //set bit13 as 1
        *SFIO_CFG1_DRV = (*SFIO_CFG1_DRV) | 0x00020000;  //set bit13 as 1
        *SFIO_CFG2_DRV = (*SFIO_CFG2_DRV) | 0x00020000;  //set bit13 as 1
        *SFIO_CFG3_DRV = (*SFIO_CFG3_DRV) | 0x00020000;  //set bit13 as 1
        *SFIO_CFG4_DRV = (*SFIO_CFG4_DRV) | 0x00020000;  //set bit13 as 1
        *SFIO_CFG5_DRV = (*SFIO_CFG5_DRV) | 0x00020000;  //set bit13 as 1
        //flash reset
        SFI_Dev_Command(0, 0x66);
        SFI_Dev_Command(0, 0x99);
        hal_gpt_delay_us(5000);

        /* Issue leave DPD Command */
        SFI_Dev_Command(0, SF_CMD_LEAVE_DPD);
        SF_DAL_DEV_Restore();

        uint8_t cmd[3];
        uint8_t flash_sr2;
        flash_sr2 = ReadStatus(0, 0x35);
        if ((flash_sr2 & 0x2) == 0) {
            // set QEbit
            if ((nor_id[0] == 0xC8) && (nor_id[1] == 0x60)) {
                cmd[0] = 0x01;
                cmd[1] = ReadStatus(0, SF_CMD_READ_SR);
                cmd[2] = flash_sr2 | 0x2; //enable QE bit
                SFI_Dev_Command(0, 0x50);
                SFI_Dev_Command_Ext(0, &cmd[0], NULL, 3, 0);
            } else if (((nor_id[0] == 0xEF) && (nor_id[1] == 0x60)) || 
                       ((nor_id[0] == 0x20) && (nor_id[1] == 0x50))) {
                cmd[0] = 0x31;
                cmd[1] = ReadStatus(0, 0x35) | 0x2;   //eanble QE bit
                SFI_Dev_Command(0, 0x50);
                SFI_Dev_Command_Ext(0, &cmd[0], NULL, 2, 0);
            }
        }
    } else {
        /* Issue leave DPD Command */
