...
***********************************************************************************************/
static volatile I3C_MASTER_REGISTER_T *s_i3c_base_register[HAL_I3C_MASTER_MAX] = { (I3C_MASTER_REGISTER_T *)(I2C_0_BASE),     (I3C_MASTER_REGISTER_T *)(I2C_1_BASE)};
...
hal_i3c_master_status_t i3c_config_i3c_speed(hal_i3c_master_port_t  i3c_port, uint32_t i3c_speed, uint32_t i3c_hs_speed)
{
    ...
    if (i3c_speed != 0) {
        ...
        /* config scl low */
        res = _i3c_calc_speed_with_time(i3c_speed, false, (clk_cyc_ns - I3C_SCL_HIGH_TM_NS), &clock_div, &smp_cnt_div, &stp_cnt_div);
        if (res == false) {
        ...
        /* config scl high */
        res = _i3c_calc_speed_with_time(i3c_speed, false, I3C_SCL_HIGH_TM_NS, &clock_div, &smp_cnt_div, &stp_cnt_div);
        if (res == false) {
...
void    i3c_config_io(hal_i3c_master_port_t  i3c_port,hal_i3c_io_config_t config)
{
    ...
    base_reg->IO_CONFIG &= ~(I3C_IO_CONFIG_AED_DIV_MASK | I3C_IO_CONFIG_AED_EN_MASK);
