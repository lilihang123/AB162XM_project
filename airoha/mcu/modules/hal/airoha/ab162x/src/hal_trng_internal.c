...
trng_result_t trng_config_timeout_limit(uint32_t timeout_value)
{
...
trng_result_t trng_enable_mode(bool H_FIRO, bool H_RO, bool H_GARO)
{
...
void trng_enable_mode_combination(uint32_t combination_value)
{
    trng->TRNG_CONF &= ~(0x7F << 8);
    trng->TRNG_CONF |= (combination_value << 8);
}

void  trng_start_freerun(void)
{
    volatile uint32_t temp_random = 0;
    temp_random = trng->TRNG_DATA;
    temp_random += 1;
    trng->TRNG_CTRL |= (TRNG_START_MASK | 0x00000001);
}
...
uint32_t  trng_get_random_data(void)
{
    ...
    data_ready = trng->TRNG_INT_SET;
    while ((data_ready & TRNG_RDY_MASK) == 0)  {
        if ((data_ready & TRNG_TIMEOUT_MASK) != 0) {
            trng->TRNG_INT_CLR = TRNG_INT_CLR_MASK;
            printk("[TRNG] TRNG_TIMEOUT");
            //return 0;
            break;
        }
...
void von_neumann_extractor(bool enable)
{
    if(enable){
        trng->TRNG_CONF |= TRNG_VON_EN_MASK;
    } else {
        trng->TRNG_CONF &= ~(TRNG_VON_EN_MASK);
    }
}
