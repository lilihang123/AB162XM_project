...
ATTR_TEXT_IN_TCM hal_mpu_status_t hal_mpu_enable(void)
{
    __DMB();
    /* Enable MPU */
...
ATTR_TEXT_IN_TCM hal_mpu_status_t hal_mpu_disable(void)
{
    __DMB();
    /* Disable MPU */
    MPU->CTRL &= ~MPU_CTRL_ENABLE_Msk;
...
ATTR_TEXT_IN_TCM hal_mpu_status_t hal_mpu_region_enable(hal_mpu_region_t region)
{
    /* Region is invalid */
...
ATTR_TEXT_IN_TCM hal_mpu_status_t hal_mpu_region_disable(hal_mpu_region_t region)
{
    /* Region is invalid */
    if (region >= HAL_MPU_REGION_MAX) {
        return HAL_MPU_STATUS_ERROR_REGION;
...
ATTR_TEXT_IN_TCM bool hal_mpu_region_is_enable(hal_mpu_region_t region)
{
    return (g_mpu_region_en & (1 << region)) != 0;
}

ATTR_TEXT_IN_TCM static uint8_t mpu_judge_memory_type(uint32_t start_addr, uint32_t end_addr)
{
...
ATTR_TEXT_IN_TCM hal_mpu_status_t hal_mpu_region_configure(hal_mpu_region_t region, const hal_mpu_region_config_t *region_config)
{
    uint32_t reg;
