...
hal_spi_slave_status_t spi_slave_send(hal_spi_slave_port_t spi_port, const uint8_t *data, uint32_t size)
{
    ...
    if (config_size != size) {
        ...
        g_spi_slave_register[spi_port]->spislv_buffer_base_addr.word = hal_memview_mcu_to_infrasys((uint32_t)data);
...
uint32_t spi_slave_send_vfifo(hal_spi_slave_port_t spi_port, const uint8_t *data, uint32_t size)
{
    ...
    write_address = hal_memview_infrasys_to_mcu(g_spi_slave_register[spi_port]->spislv_txdma_start_addr.word) + g_spi_slave_register[spi_port]->spislv_txdma_ptr.field.SPIS_TXDMA_WPTR;
    ...
    if (write_length > bytes_to_boundary) {
        ...
        write_address = hal_memview_infrasys_to_mcu(g_spi_slave_register[spi_port]->spislv_txdma_start_addr.word);
...
uint32_t spi_slave_receive_vfifo(hal_spi_slave_port_t spi_port, uint8_t *data, uint32_t size)
{
    ...
    read_address = hal_memview_infrasys_to_mcu(g_spi_slave_register[spi_port]->spislv_rxdma_start_addr.word) + g_spi_slave_register[spi_port]->spislv_rxdma_ptr.field.SPIS_RXDMA_RPTR;
    ...
    if (read_length > bytes_to_boundary) {
        ...
        read_address = hal_memview_infrasys_to_mcu(g_spi_slave_register[spi_port]->spislv_rxdma_start_addr.word);
...
void spi_slave_set_vfifo(hal_spi_slave_port_t spi_port, hal_spi_slave_vfifo_config_t *config)
{
    ...
    if (config->tx_buffer || config->rx_buffer) {
        if (config->tx_buffer) {
            g_spi_slave_register[spi_port]->spislv_txdma_start_addr.word = hal_memview_mcu_to_infrasys((uint32_t)config->tx_buffer);
        ...
        if (config->rx_buffer) {
            g_spi_slave_register[spi_port]->spislv_rxdma_start_addr.word = hal_memview_mcu_to_infrasys((uint32_t)config->rx_buffer);
...
hal_spi_slave_status_t spi_slave_receive(hal_spi_slave_port_t spi_port, uint8_t *buffer, uint32_t size)
{
    ...
    if (config_size != size) {
        ...
        g_spi_slave_register[spi_port]->spislv_buffer_base_addr.field.SPIS_BUFFER_BASE_ADDR= hal_memview_mcu_to_infrasys((uint32_t)buffer);
