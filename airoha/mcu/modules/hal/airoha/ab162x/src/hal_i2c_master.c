...
static i2c_transfer_config_t       * const s_i2c_q_buf[HAL_I2C_MASTER_MAX] = {s_i2c0_q_buf, s_i2c1_q_buf};
static const uint8_t                s_i2c_q_szb[HAL_I2C_MASTER_MAX] = {HAL_I2C_MASTER_0_FIFO_DEPTH, HAL_I2C_MASTER_1_FIFO_DEPTH};
...
static void                 _hal_i2c_master_gpio_timing(hal_i2c_port_t i2c_port, hal_i2c_frequency_t freq, bool is_i2c_mode);
...
static  void    hal_i2c_backup_all_register()
{
    ...
    for (i2c_port = 1; i2c_port < HAL_I2C_MASTER_MAX; i2c_port++) {
...
hal_i2c_status_t    hal_i2c_master_init(hal_i2c_port_t i2c_port, hal_i2c_config_t *i2c_config)
{
    ...
    _hal_i2c_master_gpio_timing(i2c_port,i2c_config->frequency,true);

#ifdef HAL_SLEEP_MANAGER_ENABLED
    if (i2c_port == 0) {
        ao_backup_format_table_t i3c0;
        i3c0.module_base_addr = 0x42210000;
        i3c0.module_end_addr = 0x42210048;
        i3c0.module_backup_enable = 0x1;
        sleep_management_register_infra_ao_backup_restore(SLEEP_MANAGEMENT_AO_BACKUP_I3C0, &i3c0);
    } else {
        sleep_management_register_infra_pd_suspend_callback(INFRA_PD_BACKUP_RESTORE_I3C1, (sleep_management_suspend_callback_t)hal_i2c_backup_all_register, NULL);
        sleep_management_register_infra_pd_resume_callback(INFRA_PD_BACKUP_RESTORE_I3C1, (sleep_management_resume_callback_t) hal_i2c_restore_all_register, NULL);
    }
#endif
...
static hal_i2c_status_t _hal_i2c_master_send_and_receive_dma_with_nb(hal_i2c_port_t i2c_port, hal_i2c_send_to_receive_config_ex_no_busy_t *config, bool is_non_block)
{
 ...
 /*step 1: find free item */
    ...
    _hal_i2c_master_gpio_timing(i2c_port, pconfig->speed,true);
 ...
 static void    _hal_i2c_master_gpio_timing(hal_i2c_port_t i2c_port, hal_i2c_frequency_t freq,bool is_i2c_mode)
{
    ...
    //     { HAL_I2C_MASTER_1_SCL_PIN, HAL_I2C_MASTER_1_SDA_PIN },
    //     { HAL_I2C_MASTER_2_SCL_PIN, HAL_I2C_MASTER_2_SDA_PIN },
    //     {                     0xFF,                      0xFF},
    // };
    // if (i2c_port >= HAL_I2C_MASTER_AO) {
    //     return;
    // }
    // if (is_i2c_mode && (freq < HAL_I2C_FREQUENCY_1M)) {
    ...
    //         hal_gpio_set_slew_rate(s_i2c_gpio[i2c_port][1]);
    //     #endif
    // }
    // else {
    //     #ifdef HAL_GPIO_FEATURE_SET_CAPACITANCE
    ...
    //         hal_gpio_clear_slew_rate(s_i2c_gpio[i2c_port][1]);
    //     #endif
    // }
#endif /* BL & DA */
}
