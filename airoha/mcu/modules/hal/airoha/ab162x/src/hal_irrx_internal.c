...
#if 0
#define log_irrx_info(fmt, cnt, ...)     log_hal_msgid_info("[hal][rtc] " fmt, cnt, ##__VA_ARGS__)
#define log_irrx_warning(fmt, cnt, ...)  log_hal_msgid_warning("[hal][rtc] "fmt, cnt, ##__VA_ARGS__)
#define log_irrx_error(fmt, cnt, ...)    log_hal_msgid_error("[hal][rtc] "fmt, cnt, ##__VA_ARGS__)
#else
#define log_irrx_info(fmt, cnt, ...)     printk(fmt, ##__VA_ARGS__)
#define log_irrx_warning(fmt, cnt, ...)  printk(fmt, ##__VA_ARGS__)
#define log_irrx_error(fmt, cnt, ...)    printk(fmt, ##__VA_ARGS__)
#endif
...
void irrx_nvic_set(bool enable)
{
    ...
    IRRX_REGISTER_T *irrx_register_base = (IRRX_REGISTER_T *)IRRX_BASE;
...
#ifdef IRRX_DEBUG
    log_irrx_info("irrx wakeup event\r\n", 0);
#endif
...
#ifdef IRRX_DEBUG
            log_irrx_info("irrx_isr:invalid data\r\n", 0);
#endif
            irrx_nvic_set(true);
...
#ifdef IRRX_DEBUG
            log_irrx_info("irrx_isr:invalid cnt\r\n", 0);
#endif
            irrx_nvic_set(true);
...
void irrx_read_hardware_mode_data(uint8_t *data, uint8_t length)
{
    ...
    if ((!data) || (length == 0)) {
        log_irrx_info("irrx_hardware_read_data param err!\r\n", 0);
        return;
    }
...
void irrx_decode_sirc_12bit_data(uint8_t *data, uint8_t length, irrx_sirc_bit12_protocol_t *protocol)
{
    ...
    protocol->addr = (irrx_byte_bit_shift((*(data + 1)) & 0xF0) << 1)|((*data) & 0x01); //5bit address

    log_irrx_info("SIRC 12bit decode:cmd = 0x%02x\r\n", 1, protocol->cmd);
    log_irrx_info("SIRC 12bit decode:addr = 0x%02x\r\n", 1, protocol->addr);
}
...
void irrx_decode_sirc_15bit_data(uint8_t *data, uint8_t length, irrx_sirc_bit15_protocol_t *protocol)
{
    /*decode potocol*/
    protocol->cmd  = irrx_byte_bit_shift((*data)) & 0x7F;  //7bit command
    protocol->addr = (irrx_byte_bit_shift((*(data + 1)) & 0xFE) << 1)|((*data) & 0x01); //8bit address

    log_irrx_info("SIRC 15bit decode:cmd = 0x%02x\r\n", 1, protocol->cmd);
    log_irrx_info("SIRC 15bit decode:addr = 0x%02x\r\n", 1, protocol->addr);
}
...
void irrx_decode_sirc_20bit_data(uint8_t *data, uint8_t length, irrx_sirc_bit20_protocol_t *protocol)
{
    ...
    protocol->cmd  = irrx_byte_bit_shift((*data)) & 0x7F;  //7bit command
    protocol->addr = (irrx_byte_bit_shift((*(data + 1)) & 0xF0) << 1)|((*data) & 0x01);      //5bit address
    protocol->extend = (irrx_byte_bit_shift((*(data + 1)) & 0x0F)>>4)|(irrx_byte_bit_shift((*(data + 2)) & 0xF0)<<4);  //8bit extend

    log_irrx_info("SIRC 20bit decode:cmd = 0x%02x\r\n",    1, protocol->cmd);
    log_irrx_info("SIRC 20bit decode:addr = 0x%02x\r\n",   1, protocol->addr);
    log_irrx_info("SIRC 20bit decode:extend = 0x%02x\r\n", 1, protocol->extend);
}
...
void irrx_decode_nec_data(uint8_t *data, uint8_t length, irrx_nec_protocol_t *protocol)
{
    ...
    log_irrx_info("NEC decode:addr = 0x%02x\r\n", 1, protocol->addr);
    log_irrx_info("NEC decode:addr_inverse = 0x%02x\r\n", 1, protocol->addr_inverse);
    log_irrx_info("NEC decode:cmd = 0x%02x\r\n", 1, protocol->cmd);
...
void irrx_decode_rc5_data(uint8_t *data, uint8_t length, irrx_rc5_protocol_t *protocol)
{
    ...
    log_irrx_info("RC5 decode:toggle bit = 0x%02x\r\n", 1, protocol->toggle_bit);
    log_irrx_info("RC5 decode:addr = 0x%02x\r\n", 1, protocol->addr);
    log_irrx_info("RC5 decode:cmd = 0x%02x\r\n",  1, protocol->cmd);
}
...
void irrx_decode_rc6_data(uint8_t *data, uint8_t length, irrx_rc6_mode0_protocol_t *protocol)
{
    ...
    log_irrx_info("RC6 decode:header bit = 0x%02x\r\n", 1, protocol->header);
    log_irrx_info("RC6 decode:ctrl = 0x%02x\r\n",      1, protocol->ctrl);
    log_irrx_info("RC6 decode:info = 0x%02x\r\n",      1, protocol->info);
}
...
void irrx_decode_rcmm_data(uint8_t *data, uint8_t length, irrx_rcmm_protocol_t *protocol)
{
    ...
    log_irrx_info("RCMM decode:length = 0x%02x\r\n", 1, protocol->length);
    for (i = 0; i < length; i++) {
        log_irrx_info("RCMM decode:data[%d] = 0x%02x\r\n", 2, i, (protocol->data)[i]);
    }
}

void irrx_register_dump(void)
{
    IRRX_REGISTER_T *irrx_register_base = (IRRX_REGISTER_T *)(IRRX_REGISTER_T *)IRRX_BASE;
    (void)irrx_register_base;
    log_irrx_info("irrx register dump:\r\n",   0);
    log_irrx_info("PDREG_IRH = 0x%08x\r\n",    1, irrx_register_base->PDREG_IRH);
    log_irrx_info("PDREG_IRM = 0x%08x\r\n",    1, irrx_register_base->PDREG_IRM);
    log_irrx_info("PDREG_IRL = 0x%08x\r\n",    1, irrx_register_base->PDREG_IRL);
    log_irrx_info("PDREG_IRCFGH = 0x%08x\r\n",     1, irrx_register_base->PDREG_IRCFGH);
    log_irrx_info("PDREG_IRCFGL = 0x%08x\r\n",     1, irrx_register_base->PDREG_IRCFGL);
    log_irrx_info("PDREG_IRTHD = 0x%08x\r\n",      1, irrx_register_base->PDREG_IRTHD);
    log_irrx_info("PDREG_IRRCM_THD = 0x%08x\r\n",  1, irrx_register_base->PDREG_IRRCM_THD);
    log_irrx_info("PDREG_IRRCM_THD_0 = 0x%08x\r\n", 1, irrx_register_base->PDREG_IRRCM_THD_0);
    log_irrx_info("PDREG_IRCLR = 0x%08x\r\n",      1, irrx_register_base->PDREG_IRCLR);
    log_irrx_info("PDREG_IREXP_EN = 0x%08x\r\n",   1, irrx_register_base->PDREG_IREXP_EN);
    log_irrx_info("PDREG_EXP_BCNT = 0x%08x\r\n",   1, irrx_register_base->PDREG_EXP_BCNT);
    log_irrx_info("PDREG_ENEXP_IRM = 0x%08x\r\n",  1, irrx_register_base->PDREG_ENEXP_IRM);
    log_irrx_info("PDREG_ENEXP_IRL = 0x%08x\r\n",  1, irrx_register_base->PDREG_ENEXP_IRL);
    log_irrx_info("PDREG_EXP_IRL0 = 0x%08x\r\n",   1, irrx_register_base->PDREG_EXP_IRL0);
    log_irrx_info("PDREG_EXP_IRM0 = 0x%08x\r\n",   1, irrx_register_base->PDREG_EXP_IRM0);
    log_irrx_info("PDREG_IRINT_EN = 0x%08x\r\n",   1, irrx_register_base->PDREG_IRINT_EN);
    log_irrx_info("PDREG_IR_INTCLR = 0x%08x\r\n",  1, irrx_register_base->PDREG_IR_INTCLR);
    log_irrx_info("PDREG_WAKEEN = 0x%08x\r\n",     1, irrx_register_base->PDREG_WAKEEN);
    log_irrx_info("PDREG_SELECT = 0x%08x\r\n",     1, irrx_register_base->PDREG_SELECT);
}
