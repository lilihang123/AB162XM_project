...
hal_wdt_status_t hal_wdt_init(hal_wdt_config_t *wdt_config)
{
    ...
    printk("[hal][wdt] init mod(%d),tm(%ds)", (int) wdt_config->mode, (int) wdt_config->seconds);
#endif
...
#if defined(AIR_BOOTREASON_CHECK_ENABLE) && (!defined(__EXT_BOOTLOADER__)) && (!defined(__EXT_DA__))
    {
...
#ifndef __EXT_BOOTLOADER__
    printk("[hal][wdt] sw rst, caller:0x%x", (unsigned int)lr_reg);
#endif
    hal_gpt_delay_ms(10);
...
void hal_wdt_enter_sleep(uint32_t seconds, bool enable)
{
    ...
    if (hal_wdt_get_enable_status() == true) {
        ...
        s_hal_wdt_config_backup[0].seconds = ((RGU->wdt_length.word) >> 16);
        s_hal_wdt_config_backup[0].seconds = s_hal_wdt_config_backup[0].seconds * WDT_TICK_UNIT / 1000000;
        if (s_hal_wdt_config_backup[0].seconds == 0) {
            s_hal_wdt_config_backup[0].seconds++;
        }
        if (enable) {
...
{
#ifndef CONFIG_WDT_FAST_MODE_ENABLE
    wdt_set_enable(0, false);
#endif
    /* wdt always set cold reset */
    wdt_set_pmu_mask(false);
    /* disable wdt1 */
    // wdt_set_enable(1, false);
    *(volatile uint32_t *)(WDT_BASE + 0x30) = WDT1_EN_KEY << WDT1_EN_KEY0_OFFSET;
...
void hal_wdt_enter_sleep_ext(uint32_t seconds)
{
    ...
    if (hal_wdt_get_enable_status_ext() == true) {
        ...
        s_hal_wdt_config_backup[1].seconds = ((RGU_1->wdt_length.word) >> 16);
        s_hal_wdt_config_backup[1].seconds = s_hal_wdt_config_backup[1].seconds * WDT_TICK_UNIT / 1000000;
        if (s_hal_wdt_config_backup[1].seconds == 0) {
            s_hal_wdt_config_backup[1].seconds++;
        }
        /* disable wdt */
