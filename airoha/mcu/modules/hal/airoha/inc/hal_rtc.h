...
*  - Step 1. Call #hal_rtc_enable_time() to initialize the RTC hardware, if it hasn't been called after the power went on.
*  - Step 2. Call #hal_rtc_set_time() to set the RTC current time.
*  - sample code:
*    @code
*       set_current_time(uint16_t year, uint8_t mon, uint8_t day, uint8_t hour, uint8_t min, uint8_t sec,uint16_t milli_sec) {
*           hal_rtc_time_t time;
*
*           if(HAL_RTC_STATUS_OK != hal_rtc_enable_time()) {
*               // Error handler
*           }
*
*           // as an offset. For example.
*           time.rtc_year = year;
*           time.rtc_mon = mon;
*           time.rtc_day = day;
*           time.rtc_hour = hour;
...
*  - sample code:
*    @code
*       get_current_time(uint8_t *year, uint8_t *mon, uint8_t *day, uint8_t *hour, uint8_t *min, uint8_t *sec, uint16_t *milli_sec) {
*           hal_rtc_time_t time;
*
*           if(HAL_RTC_STATUS_OK != hal_rtc_enable_time()) {
*               // Error handler
...
*  - sample code:
*    @code
*       set_alarm_time(uint16_t year, uint8_t mon, uint8_t day, uint8_t hour, uint8_t min, uint8_t sec,uint16_t milli_sec) {
...
*               // Error handler
*           }
*           if(HAL_RTC_STATUS_OK !=hal_rtc_alarm_get_handle(&handle)){
*              // Error handler
*           }
*           // The user has to define the base year and the RTC year is defined
*           alarm.rtc_year = year;
...
*           alarm.rtc_min = min;
*           alarm.rtc_sec = sec;
*           alarm.rtc_milli_sec = milli_sec;
*           // Set the RTC alarm time.
*           if(HAL_RTC_STATUS_OK != hal_rtc_set_alarm(handle,&alarm,time_change_cb,&g_user_data)) {
*               // Error handler
*           }
*       }
...
*            hal_rtc_alarm_free_handle(handle);
*       }
*    @endcode
*
* @}
...
*  - sample code:
*    @code
*       get_alarm_time(uint8_t *year, uint8_t *mon, uint8_t *day, uint8_t *hour, uint8_t *min, uint8_t *sec,uint16_t *milli_sec) {
*           hal_rtc_time_t alarm;
*
*           if(HAL_RTC_STATUS_OK != hal_rtc_enable_time()) {
*               // Error handler
*           }
*
*           if(HAL_RTC_STATUS_OK != hal_rtc_get_alarm(handle,&alarm)) {
*               // Error handler
*           }
*
*           // The user has to define the base year and the RTC year is defined
...
/** @brief RTC status */
typedef enum { 
    HAL_RTC_STATUS_ERROR_PORT_USE_FULL=-4,                    /**< RTC port use full. */
    HAL_RTC_STATUS_ERROR_PORT = -3,                          /**< RTC error port. */
...
/** @brief This enum defines the RTC running status. */
typedef enum {
    HAL_RTC_STOPPED = 0,                            /**< The RTC has stopped. */
    HAL_RTC_RUNNING = 1                             /**< The RTC is running. */
...
/**
 * @brief This function enable the RTC time.
 * @return #HAL_RTC_STATUS_OK, the operation completed successfully.
 * @par Example
...
/**
 * @brief This function disables an time. Call this function if the alarm notification is no longer required.
 * @return #HAL_RTC_STATUS_OK, the operation completed successfully.
 */
 ...
 * @return     #HAL_RTC_STATUS_OK, if the operation is successful.\n
 *             #HAL_RTC_STATUS_ERROR, if a timer cannot be allocated.
 */
 ...
 * @return     #HAL_RTC_STATUS_OK, if the operation is successful.\n
 *             #HAL_RTC_STATUS_INVALID_PARAMETER, if the handle is invalid. \n
 *             #HAL_RTC_STATUS_ERROR, for all other errors.
 */
 ...
 * @return     #HAL_RTC_STATUS_OK, if the operation is successful.\n
 *             #HAL_RTC_STATUS_INVALID_PARAMETER, if the parameter is invalid. \n
 *             #HAL_RTC_STATUS_ERROR, for all other errors.
 */
 ...
 * @param[out] time is a pointer to the hal_rtc_time_t structure that contains the date and time settings to configure the RTC alarm time.
 * @return     #HAL_RTC_STATUS_OK, if the operation is successful.\n
 *             #HAL_RTC_STATUS_INVALID_PARAMETER, if the parameter is invalid. \n
 *             #HAL_RTC_STATUS_ERROR, for all other errors.
 */
 ...
 * @brief      This function starts the software timer.
 * @param[in]  handle is the handle of the timer to be started.
 * @param[in]  timeout_time_tick is the timeout value in tick.
 * @param[in]  callback is the callback to be registered with this timer.
 *             This callback will be called when the timer times out.
 * @param[in]  user_data is the pointer to the user defined data to pass to the callback function.
 * @return     #HAL_RTC_STATUS_OK, if the operation is successful.\n
 ...
 *             #HAL_RTC_STATUS_ERROR, for all other errors.
 */
hal_rtc_status_t hal_rtc_set_alarm_tick(uint32_t handle, uint64_t timeout_time_tick, hal_rtc_time_callback_t callback, void *user_data);
/**
 * @brief      This function get timer count.
...
uint32_t hal_rtc_latch_get_current(hal_rtc_timer_port_t port);
void hal_rtc_latch_config(hal_rtc_timer_port_t port,uint8_t intervel_sel,bool enable);
#ifdef HAL_RTC_FEATURE_TIME_CALLBACK
/**
