...
typedef enum {
    ...
    HAL_USB_EP0_DRV_STATE_NO_UPDATE          /**< Do not change control pipe state. Used for endpoint 0 stall. */
...
typedef enum {
    ...
    HAL_USB_DRV_HDLR_SOF,        /**< USB bus sof callback function. */
...
/**
 * @brief This function enables or disables the interrupt for the specified endpoint.
 * @param[in] type is the interrupt handler type. Please refer to #hal_usb_driver_handler_type_t.
 * @param[in] ep_num is the endpoint number. The interrupt handler must be binding to a specific endpoint number if it uses #HAL_USB_DRV_HDLR_EP_TX and #HAL_USB_DRV_HDLR_EP_RX.
 * @param[in] enable is the enable flag. If enable is 1, the interrupt is enabled. If enable is 0, the interrupt is disabled.
 * @return #HAL_USB_STATUS_OK, if the operation completed successfully.
 * @par Example
 * Please refer to "Initialize the USB hardware" in @ref HAL_USB_Driver_Usage_Chapter.
 */
...
/**
 * @brief This function enables the USB endpoint 0 interrupt.
 * @return #HAL_USB_STATUS_OK, if the operation completed successfully.
 * @par Example
...
/**
 * @brief This function disables the USB endpoint 0 interrupt.
 * @return #HAL_USB_STATUS_OK, if the operation completed successfully.
 * @par Example
 * Please refer to "USB endpoint setup" in @ref HAL_USB_Driver_Usage_Chapter.
 */
...
/**
 * @brief This function clears the USB endpoint 0 interrupt.
 * @return #HAL_USB_STATUS_OK, if the operation completed successfully.
 * @par Example
 * Please refer to "USB endpoint setup" in @ref HAL_USB_Driver_Usage_Chapter.
 */
...
hal_usb_status_t hal_usb_clear_endpoint0_fifo(void);

/**
 * @brief Trigger USB HW pulled up DM line for remote-wakeup.
 *
 * @return uint8_t 0 on sucess.
 */
uint8_t hal_usb_rmwk_trigger(void);

/**
 * @brief Check USB HW is pulled up DM line or not.
 *
 * @return uint8_t 0 on sucess.
 */
...
/**
 * @brief Release the DM line for remote-wakeup.
 *
 * @return uint8_t 0 on sucess.
 */
