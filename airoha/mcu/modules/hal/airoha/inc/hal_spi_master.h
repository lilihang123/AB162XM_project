...
typedef enum {
    ...
    HAL_SPI_MASTER_CFG_CONFIG_ERROR         = 3,              /**< SPI master transsimission config error in no busy API. */
    HAL_SPI_MASTER_AHB_BUS_ERROR            = 4,              /**< SPI master transsimission ahb error in no busy API. */
} hal_spi_master_callback_event_t;
#endif

typedef enum {
    HAL_SPI_MASTER_UNKOWN_ERROR         = -3,              /**< SPI master transsimission unkown error in no busy API. */    
    HAL_SPI_MASTER_CFG_ERROR            = -2,              /**< SPI master transsimission config error in no busy API. */
    HAL_SPI_MASTER_AHB_ERROR            = -1,              /**< SPI master transsimission ahb error in no busy API. */
    HAL_SPI_MASTER_SUCCESS              = 0,              /**< SPI master send operation completed. */
} spi_master_irq_status_t;

typedef enum{
    HAL_SPI_WRITE_TO_WRITE_DELAY = 0,                     /**< SPI master write to write delay. */    
    HAL_SPI_WRITE_TO_READ_DELAY  = 1,                     /**< SPI master write to read delay. */
    HAL_SPI_READ_TO_READ_DELAY   = 2,                     /**< SPI master read to read delay. */
}spi_master_delay_status_t;

/** @brief SPI master status. */
typedef enum {
    HAL_SPI_MASTER_BY_OTHER_MODE                 = -9,            /**< SPI master in other mode. */
    HAL_SPI_STATUS_BT_TRIGGER_BUSY               = -8,            /**< SPI master in bt trigger. */
 ...
 * @param[in] master_port is the SPI master port number, the value is defined in #hal_spi_master_port_t.
 * @param[in] spi_send_and_receive_config is the structure that contains data buffer and data size, please refer to #hal_spi_master_send_and_receive_config_t
 * @param[in] start determine whether this transfer should be started
 *            , for more details. Check the SPI device's datasheet to determine if the data is sent or received.
 *            For example, if <a href="http://www.analog.com/media/en/technical-documentation/data-sheets/ADIS16375.pdf"> ADIS16375 </a>
 *            is used in full duplex communication mode, the data is received, while send command is valid.
 *            If <a href="http://www.thaieasyelec.com/downloads/EFDV423/PAH8001EI-2G.pdf"> PAH8001EI-2G </a> sensor is used,
...
hal_spi_master_status_t hal_spi_master_send_and_receive_dma_advanced(hal_spi_master_port_t master_port,
                                                            hal_spi_master_send_and_receive_config_t *spi_send_and_receive_config,bool start);
...
/**
 * @brief     SPI master set cmd data delay, only valid when SPI master is configured in single mode.
 * @param[in] master_port is the SPI master port number, the value is defined in #hal_spi_master_port_t.
 * @param[in] delay_cnt is the parameter for delay cnt.
 * @param[in] one_location_cnt is the parameter for one location cnt.
 * @param[in] is_multy is the parameter for multy delay or single delay.
 * @return    #HAL_SPI_MASTER_STATUS_ERROR_PORT, master_port parameter is an invalid port number; \n
 *            #HAL_SPI_MASTER_STATUS_ERROR_BUSY, the port of SPI master is busy and not available for use; \n
 *            #HAL_SPI_MASTER_STATUS_INVALID_PARAMETER, an invalid macro selection parameter is given by the user; \n
 *            #HAL_SPI_MASTER_STATUS_OK, the operation completed successfully.
...
/**
 * @brief     SPI master set mosi high or low level , only valid when SPI master is configured in single mode.
 * @param[in] master_port is the SPI master port number, the value is defined in #hal_spi_master_port_t.
 * @param[in] cmd_cnt is the parameter for cmd cnt.
 * @param[in] is_high is the parameter for high or low.
 * @param[in] enable is the parameter for enable or disable.
 * @return    #HAL_SPI_MASTER_STATUS_ERROR_PORT, master_port parameter is an invalid port number; \n
 *            #HAL_SPI_MASTER_STATUS_ERROR_BUSY, the port of SPI master is busy and not available for use; \n
 *            #HAL_SPI_MASTER_STATUS_INVALID_PARAMETER, an invalid macro selection parameter is given by the user; \n
 *            #HAL_SPI_MASTER_STATUS_OK, the operation completed successfully.
...
/**
 * @brief     SPI master whether enable BT trigger.
 * @param[in] master_port is the SPI master port number, the value is defined in #hal_spi_master_port_t.
 * @param[in] enable is the parameter for enable or disable.
 * @return    #HAL_SPI_MASTER_STATUS_ERROR_PORT, master_port parameter is an invalid port number; \n
 *            #HAL_SPI_MASTER_STATUS_ERROR_BUSY, the port of SPI master is busy and not available for use; \n
...
hal_spi_master_status_t hal_spi_master_set_wr_delay(hal_spi_master_port_t master_port, spi_master_delay_status_t delay_status,uint8_t delay_us);
#endif

/**
 * @brief     SPI master whether trigger start.
 * @param[in] master_port is the SPI master port number, the value is defined in #hal_spi_master_port_t.
 * @param[in] start is the parameter for start.
 * @return    #HAL_SPI_MASTER_STATUS_ERROR_PORT, master_port parameter is an invalid port number; \n
 *            #HAL_SPI_MASTER_STATUS_ERROR_BUSY, the port of SPI master is busy and not available for use; \n
 *            #HAL_SPI_MASTER_STATUS_INVALID_PARAMETER, an invalid macro selection parameter is given by the user; \n
 *            #HAL_SPI_MASTER_STATUS_OK, the operation completed successfully.
...
/**
 * @brief     SPI master whether enable nvic irq.
 * @param[in] master_port is the SPI master port number, the value is defined in #hal_spi_master_port_t.
 * @param[in] enable is the parameter for enable or disable.
 * @return    #HAL_SPI_MASTER_STATUS_ERROR_PORT, master_port parameter is an invalid port number; \n
 *            #HAL_SPI_MASTER_STATUS_ERROR_BUSY, the port of SPI master is busy and not available for use; \n
...
hal_spi_master_status_t hal_spi_master_send_and_receive_polling_is_fullmode(hal_spi_master_port_t master_port,
                                                                hal_spi_master_send_and_receive_config_t *spi_send_and_receive_config,bool full_mode);
hal_spi_master_status_t hal_spi_enable_flash_write(hal_spi_master_port_t master_port,bool enable);
hal_spi_master_status_t hal_spi_repeat_mode(hal_spi_master_port_t master_port, uint8_t repeat_type ,bool enable);
hal_spi_master_status_t hal_spi_cmd_set_config(hal_spi_master_port_t master_port,uint8_t *data,uint8_t size,uint8_t dummy_size);
hal_spi_master_status_t hal_spi_set_io_skew_config(hal_spi_master_port_t master_port,hal_spi_master_skew_t skew,uint8_t delay);
hal_spi_master_status_t hal_spi_set_full_mode(hal_spi_master_port_t port, bool full);
#endif
hal_spi_master_status_t hal_spi_master_send_and_receive_fifo_advanced(hal_spi_master_port_t master_port, hal_spi_master_send_and_receive_config_t *transfer_config, bool start);
hal_spi_master_status_t hal_spi_master_fifo_access(hal_spi_master_port_t master_port, bool is_write, uint8_t *data, uint32_t size);
#ifdef __cplusplus
}
#endif
