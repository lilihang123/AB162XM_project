...
typedef enum {
    HAL_PWM_STATUS_OTHER_IS_RUN         = -7,         /**< pwm channel is run*/
    HAL_PWM_STATUS_NOT_INIT             = -6,         /**<  The pwm not init */
    HAL_PWM_STATUS_ERROR_BUSY           = -5,         /**<  The pwm bus is busy, an error occurred. */
...
typedef struct {
    bool dma_8bit;                         /**<  set this bit,memory is 8bit transfer,else is 16 bit.*/
    uint8_t dma_channel_number;            /**<  set dma channel number,max number is 4.*/
    bool dma_nonaligin;                    /**<  set this bit, memory is not aligned tranfer*/
    bool dma_repeat_en;                    /**<  set this bit, memory is repeat tranfer*/
    uint8_t dma_data_repeat;               /**<  set this bit, memory data is repeat tranfer*/
    bool dma_sign;                         /**<  set this bit,if memory is 16bit,DMA calculated data by GAIN and SCALE*/
    uint8_t dma_gain;                      /**<  set this bit, if memory is 16bit and DMA_DATA_SIGN is enable,MEMORY DATA IS DATA*2^GAIN */
    uint8_t dma_scale;                     /**<  set this bit, if memory is 16bit and DMA_DATA_SIGN is enable,MEMORY DATA IS DATA/2^SCALE */
}hal_pwm_dma_config;

typedef struct {
    bool bufa_done_irq;                        /**<  set this bit,buffer A done generate IRQ.*/
    bool bufb_done_irq;                        /**<  set this bit,buffer B done generate IRQ.*/
    bool done_irq;                             /**<  set this bit,ALL done generate IRQ.*/
}hal_pwm_irq_enable;
...
***************/
typedef enum {
    HAL_PWM_ALL_DONE_EVENT_SUCCESS =  0b00001,               /**<  The transaction ALL completed wihtout any error. */
    HAL_PWM_BUFA_DONE_EVENT_SUCCESS = 0b00010,               /**<  The transaction BUFA completed wihtout any error.*/
    HAL_PWM_BUFB_DONE_EVENT_SUCCESS = 0b00100,               /**<  The transaction BUFB completed wihtout any error.*/
    HAL_PWM_BUFA_OV_EVENT_ERROR = 0b01000,                         /**<  The transaction BUFA overflow with an error.*/
    HAL_PWM_BUFB_OV_EVENT_ERROR = 0b10000,                         /**<  The transaction BUFB overflow with an error.*/
} hal_pwm_dma_callback_event_t;
/**
  * @}
  */
...
/**
 * @brief   This function sets the PWM polarity.
 * @param[in]  pwm_channel is the PWM channel number. For more details about the parameter, please refer to #hal_pwm_channel_t.
 * @param[in]  polarity  is PWM polarity, if polarity is ture,the initial level is high level, if polarity is false,the initial level is low level.
 * @return    To indicate whether this function call is successful or not.
 *            If the return value is #HAL_PWM_STATUS_OK, the operation completed successfully.
 *            If the return value is #HAL_PWM_STATUS_INVALID_PARAMETER, a wrong parameter is given. The parameter needs to be verified.
 * @par       Example
...
typedef void (*hal_pwm_callback_t)(hal_pwm_dma_callback_event_t event, void *user_data);


/**
 * @brief   This function deinitializes the PWM DMA hardware.
 * @param[in]  pwm_dma_port is the PWM DMA port. For more details about the parameter, please refer to #hal_pwm_dma_port_t.
 * @return    To indicate whether this function call is successful or not.
 *            If the return value is #HAL_PWM_STATUS_OK, the operation completed successfully.
 *            If the return value is #HAL_PWM_STATUS_INVALID_PARAMETER, a wrong parameter is given. The parameter needs to be verified.
 * @par       Example
 * For a sample code, please refer to @ref HAL_PWM_Driver_Usage_Chapter.
 * @sa  hal_pwm_dma_deinit()
 */
...
/**
 * @brief This function sets the PWM DMA frequency and retrieves total count of the PWM DMA hardware at the specified frequency.
 * @param[in]  pwm_dma_port is the PWM DMA port. For more details about the parameter, please refer to #hal_pwm_dma_port_t.
 * @param[in]  frequency is the PWM output frequency.
 * @param[out]   total_count is PWM hardware total count, the value of this parameter varies based on the given PWM frequency.
 * @return    To indicate whether this function call is successful or not.
...
/**
 * @brief This function sets the PWM DMA config.
 * @param[in]  pwm_dma_port is the PWM DMA port. For more details about the parameter, please refer to #hal_pwm_dma_port_t.
 * @param[in]  config is the PWM DMA config.For more details about the parameter, please refer to #hal_pwm_dma_config.
 * @return    To indicate whether this function call is successful or not.
 *            If the return value is #HAL_PWM_STATUS_OK, the operation completed successfully.
 *            If the return value is #HAL_PWM_STATUS_INVALID_PARAMETER, a wrong parameter is given. The parameter needs to be verified.
 * @par       Example
...
/**
 * @brief This function register the PWM DMA callback.
 * @param[in]  pwm_dma_port is the PWM DMA port. For more details about the parameter, please refer to #hal_pwm_dma_port_t.
 * @param[in]  irq_enable is the setting PWM DMA irq.For more details about the parameter, please refer to #hal_pwm_irq_enable.
 * @param[in]  pwm_callback is the user-defined callback function called at pwm ISR routine.
 * @param[in]  user_data is a user-defined input data returned during the callback function's call. See the last parameter of #hal_pwm_callback_t.
 * @return    To indicate whether this function call is successful or not.
 *            If the return value is #HAL_PWM_STATUS_OK, the operation completed successfully.
 *            If the return value is #HAL_PWM_STATUS_INVALID_PARAMETER, a wrong parameter is given. The parameter needs to be verified.
 * @par       Example
...
/**
 * @brief This function setting the PWM DMA bufa.
 * @param[in]  pwm_dma_port is the PWM DMA port. For more details about the parameter, please refer to #hal_pwm_dma_port_t.
 * @param[in]  bufaddr is the setting PWM DMA address.this address is must 4-byte align.
 * @param[in]  sample_cnt is the setting PWM DMA tranfer cnt.
 * @return    To indicate whether this function call is successful or not.
 *            If the return value is #HAL_PWM_STATUS_OK, the operation completed successfully.
 *            If the return value is #HAL_PWM_STATUS_INVALID_PARAMETER, a wrong parameter is given. The parameter needs to be verified.
 * @par       Example
...
/**
 * @brief This function setting the PWM DMA bufb.
 * @param[in]  pwm_dma_port is the PWM DMA port. For more details about the parameter, please refer to #hal_pwm_dma_port_t.
 * @param[in]  bufaddr is the setting PWM DMA address.this address is must 4-byte align.
 * @param[in]  sample_cnt is the setting PWM DMA tranfer cnt.
 * @return    To indicate whether this function call is successful or not.
...
/**
 * @brief This function starts the PWM dma execution.
 * @param[in]  pwm_dma_port is the PWM DMA port. For more details about the parameter, please refer to #hal_pwm_dma_port_t.
 * @return    To indicate whether this function call is successful or not.
 *            If the return value is #HAL_PWM_STATUS_OK, the operation completed successfully.
 *            If the return value is #HAL_PWM_STATUS_INVALID_PARAMETER, a wrong parameter is given. The parameter needs to be verified.
...
/**
 * @brief This function stop the PWM DMA execution.
 * @param[in]  pwm_dma_port is the PWM DMA port. For more details about the parameter, please refer to #hal_pwm_dma_port_t.
 * @return    To indicate whether this function call is successful or not.
 *            If the return value is #HAL_PWM_STATUS_OK, the operation completed successfully.
 *            If the return value is #HAL_PWM_STATUS_INVALID_PARAMETER, a wrong parameter is given. The parameter needs to be verified.
...
hal_pwm_status_t    hal_pwm_rgb_set_polarity(hal_pwm_port_t pwm_port,bool r_polarity,bool g_polarity,bool b_polarity);
