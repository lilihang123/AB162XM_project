...
class Image:
    ...
    def create(self, key, public_key_format, enckey, sign_by_aes, dependencies=None,
               sw_type=None, custom_tlvs=None, encrypt_keylen=128, clear=False,
               fixed_sig=None, pub_key=None, vector_to_sign=None):
        self.enckey = enckey
        # Check what hashing algorithm should be used
        #raise click.UsageError("{}, {}".format(public_key_format, sign_by_aes))
        ...
        if sign_by_aes == 'True':
            with open(os.path.dirname(__file__) + '/../../aes_key.txt', 'rb')as f:
                key_hex = f.read()
            key_hex = binascii.a2b_hex(key_hex)
            c = cmac.CMAC(algorithms.AES(key_hex))
            c.update(digest)
            output = c.finalize()
            tlv.add('AES', output)   #ASCII MAGIC for AES
