 ...
 */
 ...
 /* Modifications:
 2026-02-26 by Airoha - Added bootloader USB/UART DFU and FOTA support.
 */
...
ATTR_ROM_PARAMETER volatile rom_parameter_t g_rom_parameter = {
    {'R', 'O', 'P', 'S'},
    BL_USB_TIMEOUT,
    {'S', 'K', 'I', 'P'},
    {'R', 'O', 'P', 'E'} };
...
int main(void)
{
    ...
    printk("Starting bootloader");
#else
    printk("Starting Direct-XIP bootloader");
#endif
...
#ifdef CONFIG_AIR_BL_DFU_ENABLE
    if(fota_bl_dfu_flag_is_set() || bl_dfu_customize_entry()){
        mcuboot_status_change(MCUBOOT_STATUS_USB_DFU_ENTERED);
        printk("Waiting for USB DFU");
        bl_dfu_process();
        printk("USB DFU wait time elapsed");
    }
#endif
    ...
    printk("Bootloader chainload address offset: 0x%x",
                 rsp.br_image_off);

#if defined(MCUBOOT_DIRECT_XIP)
    printk("Jumping to the image slot");
#else
    printk("Jumping to the first image slot");
#endif
