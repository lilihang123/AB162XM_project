...
#if defined(MCUBOOT_SIGN_AES)
    #define EXPECTED_AES_SIG IMAGE_TLV_AES
#endif
...
int boot_retrieve_public_key_hash(uint8_t image_index, uint8_t *public_key_hash, size_t *key_hash_size)
{
    int i;
    volatile uint8_t *addr_start = 0x420A0770; //TODO: use MACRO for this address when porting done
    for(i = 0; i < *key_hash_size; i++){
        *(public_key_hash + i) = *(addr_start + i);
    }
    return 0;
}
static int
 ...
 */
static int sboot_cmac_verify_image(uint8_t *p_image, uint32_t img_len, uint8_t *cmac_hdr)
{
    hal_aes_buffer_t plain_text;
    hal_aes_buffer_t key;
    hal_aes_status_t ret = HAL_AES_STATUS_OK;

    uint8_t cmac_value[16] = {  0x0, 0x0, 0x0, 0x0,
                                0x0, 0x0, 0x0, 0x0,
                                0x0, 0x0, 0x0, 0x0,
                                0x0, 0x0, 0x0, 0x0
                             };
    plain_text.buffer = p_image;
    plain_text.length = img_len;
    key.length = 16;


    ret = hal_aes_cmac_message_hw(cmac_value,
                                  &plain_text,
                                  img_len,
                                  &key,
                                  HAL_AES_KEY_SOURCE_EFUSE1);


    if (ret != HAL_AES_STATUS_OK) {
        printk(" AES-CMAC fail ret = %x", ret);
        //return SBOOT_STATUS_FAIL;
        return -1; //FIH_FAILURE
    }
...
#endif
    if (memcmp(cmac_value, cmac_hdr, 16) != 0) {
        //return SBOOT_STATUS_INVALID_HDR;
        return -1; //FIH_FAILURE
    } else {
#ifdef SB_DEBUG_HIGH
    ...
    //return SBOOT_STATUS_OK;
    return 0; //FIH_SUCCESS
}
fih_ret
                      ...
                      int seed_len, uint8_t *out_hash)
{
...
#if defined (EXPECTED_SIG_TLV) || defined (EXPECTED_AES_SIG)
    ...
    while (true) {
        ...
        printk("tlv off = 0x%x, len = 0x%x, type = 0x%x", off, len, type);
        if (rc < 0) {
            goto out;
        } else if (rc > 0) {
            break;
        ...
        if (type == EXPECTED_HASH_TLV) {
            ...
            key_id = -1;
#endif /* EXPECTED_SIG_TLV */
#ifdef MCUBOOT_SIGN_AES
        } else if (type == EXPECTED_AES_SIG) {
            //verify cmac
            rc = LOAD_IMAGE_DATA(hdr, fap, off, aes_buf, 16);
            valid_signature = sboot_cmac_verify_image(hash, sizeof(hash), aes_buf);
            printk("Is AES CMAC valid? %d", valid_signature);
#endif
#ifdef MCUBOOT_HW_ROLLBACK_PROT
