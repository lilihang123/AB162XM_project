    ...
    #endif
    {
        ...
        if (event){
            ...
            switch(race_cmd_id)
            {
                ...
                {
                    event->JOYSTICK_PARAMETER.number = *(uint8_t*)para;
                    memcpy(&(event->JOYSTICK_PARAMETER.min_voltage), (void *)(para+1), sizeof(joystick_nvkey_parameter_t));
                }
                break;

                case RACE_SET_TRIGGER_PARAMETER:
                {
                    event->TRIGGER_PARAMETER.number = *(uint8_t*)para;
                    memcpy(&(event->TRIGGER_PARAMETER.full_release_voltage), (void *)(para+1), sizeof(uint8_t)+sizeof(trigger_nvkey_parameter_t));
                }
                break;
                ...
                {
                    event->LT_RT_REMAP.threshold = *(uint32_t*)para;
                }
                break;
                #endif
...
static bool app_race_cmd_evt_rsp(const struct af_evt_header *evt_header)
{
    ...
    switch(event->cmd)
    {
        ...
        case RACE_CMD_SET_LT_RT_THRESHOLD:
        {
            hid_race_cmd_set_lt_rt_threshold_rsp(event->OTHERS.status);
        }
        break;

        case RACE_CMD_GET_LT_RT_THRESHOLD:
        {
            hid_race_cmd_get_lt_rt_threshold_rsp(event->LT_RT_REMAP.threshold);
        }
        break;
        #endif
...
{
    hid_race_cmd_init(app_race_cmd_req_handler);
}
