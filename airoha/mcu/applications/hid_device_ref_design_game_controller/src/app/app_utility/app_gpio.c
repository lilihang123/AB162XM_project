...
uint8_t external_buck_gpio = 0;
#endif/*CONFIG_AIR_EXTERNAL_BUCK_GPIO_MODE*/
...
#ifdef CONFIG_AIR_EXTERNAL_BUCK_GPIO_MODE
void external_buck_gpio_init(hal_gpio_pin_t gpio_pin)
{
    APP_LOGI(thisMOD, "external_buck_gpio_init ->gpio = %d" ,gpio_pin);
    external_buck_gpio = gpio_pin;
    hal_gpio_init(external_buck_gpio);
    hal_pinmux_set_function(external_buck_gpio,0);
    hal_gpio_set_direction(external_buck_gpio,HAL_GPIO_DIRECTION_OUTPUT);
    hal_gpio_set_output(external_buck_gpio,HAL_GPIO_DATA_HIGH);
    pmu_ext_buck_enable(PMU_ON);
    pmu_ext_buck_volt_ctrl();
}

void external_buck_suspend_set_gpio(void)
{
    APP_LOGI(thisMOD, "external_buck_suspend_set_gpio");
    hal_gpio_init(external_buck_gpio);
    hal_pinmux_set_function(external_buck_gpio,0);
    hal_gpio_set_direction(external_buck_gpio,HAL_GPIO_DIRECTION_OUTPUT);
    pmu_ext_buck_enable(PMU_OFF);
    pmu_ext_buck_volt_ctrl();
    hal_gpio_set_output(external_buck_gpio,HAL_GPIO_DATA_LOW);
}

void external_buck_resume_set_gpio(void)
{
    APP_LOGI(thisMOD, "external_buck_resume_set_gpio");
    hal_gpio_init(external_buck_gpio);
    hal_pinmux_set_function(external_buck_gpio,0);
    hal_gpio_set_direction(external_buck_gpio,HAL_GPIO_DIRECTION_OUTPUT);
    hal_gpio_set_output(external_buck_gpio,HAL_GPIO_DATA_HIGH);
