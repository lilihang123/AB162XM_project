...
/* variables                                                                  */
/******************************************************************************/
static uint8_t led_flag = true;  //true means interrupt LED
static uint8_t flash_flag = false;
static uint8_t realtime_flag = false;
static uint16_t saved_flash = RGB_DEFAULT;

#if !defined (CONFIG_AIR_HID_DEBUG_PIN)
static uint8_t cmd_retry = false;
static uint8_t curr_speed = 0;
static uint8_t state_stay = APP_STATE_NONE;
    ...
    switch(state)
    {
        ...
        case APP_STATE_STANDBY:
        {
            ...
            if(app_usb_port__get_usb_plug_status() != APP_USB_PLUG_OUT)
            {
                ...
                // Here is an Example
                rgb_id_0.style = STYLE_STANDBY_STAY;
                rgb_default_pattern_setting[rgb_id_0.style].rgb_r.onoff = RGB_ON;
                rgb_default_pattern_setting[rgb_id_0.style].rgb_g.onoff = RGB_ON;
                rgb_default_pattern_setting[rgb_id_0.style].rgb_b.onoff = RGB_ON;

                if(led_flag){
                    rgb_id_0.level = RGB_LEVEL_CHARGING;
                    app_rgb__enable_normal_rgb(&rgb_id_0);
                }
            }
            else
        ...
        case APP_RGB_CONTINUE:
        {
            ...
            if(cnt > 0)
            {
                ...
                for(int idx = 0; idx < cnt; idx++)
                {
                    ...
                    status = rgb_middle_control_enable_for_node(rgb_id_0.num, rgb_data, RGB_LEVEL_MAX, RGB_REALTIME);
                    itl += 3;
                    if(status < 0){
...
{
    T_ACL_PKT_BUFFER_S acl_payload ;
    static uint16_t count = 0;
    count++;

    acl_payload.channel = ACL_PKT_CHANNEL_DEFAULT ;
    acl_payload.data = 0xA1 | ((para & 0xFF) << 8) | (count<<16) ;
    ...
    T_ACLPKT_ERR_CODE_E err = hid_common_set_hid_para(RAPA_TYPE_ACL_RESPONSE_VIA_HID_REPORT, (uint8_t*)&acl_payload, sizeof(acl_payload));
    APP_LOGI(thisMOD,"hid_common_set_hid_para  ACL response channel = 0x%08X, data = 0x%08X, err = %d", acl_payload.channel, acl_payload.data, err);
}
#endif
...
static void app_rgb__check_race_cmd_realtime_speed(uint8_t status)
{
    ...
    APP_LOGI(thisMOD, "realtime, cmd_retry = %d, deep = %d, prev_speed = 0x%X",cmd_retry, status, prev_speed);
