...
#if defined(CONFIG_AIR_MATH_COLLABORATION)
#include "app_math_collaboration.h"
#endif 

#if defined(CONFIG_AIR_MATH_COLLABORATION) || defined (CONFIG_AIR_WAKE_UP_RECONNECT_AT_HIGHEST_CLOCK)
#include "hal_dvfs.h"
#endif
...
#if defined(CONFIG_AIR_MATH_COLLABORATION)
bool app_power_mgr_lock_sleepp_status_ex = false;
#endif

#if defined(CONFIG_AIR_MATH_COLLABORATION) || defined (CONFIG_AIR_WAKE_UP_RECONNECT_AT_HIGHEST_CLOCK)
bool app_power_mgr_highest_clock = false;
#endif
...
static bool app_power_mgr__evt_app_state(const struct af_evt_header *evt_header)
{
    ...
    switch (event->app_state)
    {
        ...
        {
            #if defined (CONFIG_AIR_WAKE_UP_RECONNECT_AT_HIGHEST_CLOCK)
            app_power_mgr_enable_highest_clock();
            #endif
        ...
        {
            #if defined (CONFIG_AIR_WAKE_UP_RECONNECT_AT_HIGHEST_CLOCK)
            if(app_usb_port__check_configured_in_init() == APP_USB_CONFIGURED)
            {
                // 156 MHz clock will be enabled in hid scenario open command
                // app power manager stops the highest clock controller here and let USB mode handles it latter
                app_power_mgr_disable_highest_clock();
            }
            #endif
...
#if defined(CONFIG_AIR_MATH_COLLABORATION)
/** Using lock source SLEEP_LOCK_USB can speed up cm33 IRQ  */
void app_power_mgr_unlock_sleep_ex()
{
    APP_LOGI(thisMOD,"app_power_mgr_unlock_sleep_ex, app_power_mgr_lock_sleepp_status_ex = %d ", app_power_mgr_lock_sleepp_status_ex);
    if(app_power_mgr_lock_sleepp_status_ex)
    {
        hal_sleep_manager_unlock_sleep(SLEEP_LOCK_USB);
        app_power_mgr_lock_sleepp_status_ex = false;
    }
}

/** Using lock source SLEEP_LOCK_USB can speed up cm33 IRQ  */
void app_power_mgr_lock_sleep_ex()
{
    APP_LOGI(thisMOD,"app_power_mgr_lock_sleep_ex, app_power_mgr_lock_sleepp_status_ex = %d", app_power_mgr_lock_sleepp_status_ex);
    if(app_power_mgr_lock_sleepp_status_ex == false)
    {
        hal_sleep_manager_lock_sleep(SLEEP_LOCK_USB);
        app_power_mgr_lock_sleepp_status_ex = true;
    }
}
#endif

#if defined(CONFIG_AIR_MATH_COLLABORATION) || defined (CONFIG_AIR_WAKE_UP_RECONNECT_AT_HIGHEST_CLOCK)
void app_power_mgr_disable_highest_clock()
{
    APP_LOGI(thisMOD,"app_power_mgr_disable_highest_clock, app_power_mgr_highest_clock = %d", app_power_mgr_highest_clock);
    if(app_power_mgr_highest_clock)
    {
        hal_dvfs_lock_control(HAL_DVFS_OPP_HIGH, HAL_DVFS_UNLOCK);
        app_power_mgr_highest_clock = false;
    }
}

void app_power_mgr_enable_highest_clock()
{
    APP_LOGI(thisMOD,"app_power_mgr_enable_highest_clock, app_power_mgr_highest_clock = %d", app_power_mgr_highest_clock);
    if(app_power_mgr_highest_clock == false)
    {
        hal_dvfs_lock_control(HAL_DVFS_OPP_HIGH, HAL_DVFS_LOCK);
        app_power_mgr_highest_clock = true;
    }
}
...
void app_power_mgr_exit_low_power_mode()
{
    if (app_power_mgr_state == APP_POWER_MGR_LOW_POWER_MODE){
        ...
        HCLA_RamLogging_V2(SWLA_TYPE_LABEL, label_app_exit_low_power, SWLA_DIRECTION_START);
        #endif
        ...
        #if defined (AIR_SWLA_ENABLE)
        HCLA_RamLogging_V2(SWLA_TYPE_LABEL, label_app_exit_low_power, SWLA_DIRECTION_LABEL);
        #endif
        ...
        #if defined (AIR_SWLA_ENABLE)
        HCLA_RamLogging_V2(SWLA_TYPE_LABEL, label_app_exit_low_power, SWLA_DIRECTION_STOP);
        #endif
    }
}
