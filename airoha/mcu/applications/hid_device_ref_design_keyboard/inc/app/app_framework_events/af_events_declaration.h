...
typedef enum{
    ...
    APP_RACE_CMD_ID_BEGIN = 0x0001,
    APP_RACE_CMD_ID_MODE_SWITCH = APP_RACE_CMD_ID_BEGIN,
    APP_RACE_CMD_ID_POWER_OFF_REQUEST = 0x0002,
    APP_RACE_CMD_ID_QUERY_READY = 0x0003,
    APP_RACE_CMD_ID_LOW_POWER_REQUEST = 0x0004,
    APP_RACE_CMD_ID_END = APP_RACE_CMD_ID_LOW_POWER_REQUEST,
    #endif
}T_RACE_CMD_E;
...
struct evt_gatt_ch_status{
    struct af_evt_header header;
    uint16_t cccd_status;
    uint8_t cccd_ch_id;
    uint8_t cccd_ch_status;
};

struct evt_hid_kb_std{
    struct af_evt_header header;
    uint8_t kb_id[5];
};

struct evt_hid_kb_consumer{
    struct af_evt_header header;
    uint16_t kb_consumer_id[2];
};

struct evt_hid_kb_nKey{
    struct af_evt_header header;
    uint8_t nKey_group;
    uint8_t nKey_combokey;
    uint8_t nKey_bitmap[5];
};


struct evt_hid_ms_key{
    struct af_evt_header header;
    uint8_t mouse_key;
};

struct evt_hid_ms_motion{
    struct af_evt_header header;
    int16_t motion_X;
    int16_t motion_Y;
};

struct evt_hid_ms_wheel{
    struct af_evt_header header;
    int8_t mouse_wheel;
};

struct evt_inactivity_timeout{
    struct af_evt_header header;
    uint8_t reserved;
};

struct evt_link_switch_req{
    struct af_evt_header header;
...
struct evt_mode_switch_status{
    struct af_evt_header header;
    T_SLIDE_SWITCH_STATUS_E position;
};

struct evt_race_mode_switch{
    struct af_evt_header header;
    T_RACE_CMD_E cmd;
    uint16_t len;
    uint8_t kb_mode;
...
AF_EVT_EXTERN(evt_button);
AF_EVT_EXTERN(evt_bt_status);

//AF_EVT_EXTERN(evt_conn_status);
AF_EVT_EXTERN(evt_conn_req);

AF_EVT_EXTERN(evt_fake_report_req);
AF_EVT_EXTERN(evt_fake_report_execute);

AF_EVT_EXTERN(evt_gatt_ch_status);

AF_EVT_EXTERN(evt_hid_kb_std);
AF_EVT_EXTERN(evt_hid_kb_consumer);
AF_EVT_EXTERN(evt_hid_kb_nKey);
AF_EVT_EXTERN(evt_hid_ms_key);
AF_EVT_EXTERN(evt_hid_ms_motion);
AF_EVT_EXTERN(evt_hid_ms_wheel);

AF_EVT_EXTERN(evt_link_switch_req);
...
AF_EVT_EXTERN(evt_mode_switch_status);
AF_EVT_EXTERN(evt_race_mode_switch);
#if defined (CONFIG_AIR_HID_DEVICE_SCENARIO_SERVICE_USB_MODE)
