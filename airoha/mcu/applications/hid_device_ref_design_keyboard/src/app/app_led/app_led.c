...
void app_led__check_close_led_status(struct k_timer *timer_id);
void app_led__setting_all_style(uint8_t *store_rgb, uint8_t style);

/* timers defined ------------------------------------------------------------*/
K_TIMER_DEFINE(check_close_led_timer, app_led__check_close_led_status, NULL);
#define CHECKING_CLOSE_TIMEOUT   400  //ms


/******************************************************************************/
/* variables                                                                  */
/******************************************************************************/
uint8_t app_state_stay = APP_STATE_NONE;
...
void app_led__check_close_led_status(struct k_timer *timer_id)
{
    if (app_state_stay == APP_STATE_DISCONNECTED || app_state_stay == APP_STATE_TEST ||
        app_state_stay == APP_STATE_STANDBY || app_state_stay == APP_STATE_USB_STOP)
    {
        app_led__disable_all_led();
    }
    else{
        APP_LOGI(thisMOD,"DO NOT close led!");
    }
}
...
static void app_led__app_state_handler(uint8_t state)
{
    ...
    switch(state)
    {
        ...
        {
            app_led__setting_all_style(style_rgb, STYLE_USB_ACTIVE);
        ...
        case APP_STATE_TEST:
        {
            k_timer_stop(&check_close_led_timer);
            app_led__check_close_led_status(NULL);
            break;
        }
        default:{
            break;
...
static bool app_led__evt_app_state(const struct af_evt_header *evt_header)
{
    ...
    app_state_stay = event->app_state;
...
static bool app_led__evt_rr_status(const struct af_evt_header *evt_header)
{
    ...
    APP_LOGI(thisMOD,"app_led__evt_rr_status, event->category = %d, app_state = %d", event->category, app_state_stay); // gaming = 1, usb rr = 0
...
static void app_led_timer_init()
{
    k_timer_init(&check_close_led_timer, app_led__check_close_led_status, NULL);
}
