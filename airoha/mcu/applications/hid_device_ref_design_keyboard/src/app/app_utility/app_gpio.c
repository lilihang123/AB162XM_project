...
{
    APP_LOGI(thisMOD, "app_gpio_configure_output pin_%02d = %d", pin_id, level);
...
void app_gpio_init(T_GPIO_PIN_USAGE_LIST_S * pin_usage_list)
{
    ...
    #endif
    uint8_t usb_vdd_en_pin = 46;
    uint8_t usb_vdd_en_pin_lv = 1;
    app_gpio_configure_output(usb_vdd_en_pin, usb_vdd_en_pin_lv );

    uint8_t MUX_en_pin = 5;
    uint8_t MUX_en_pin_lv = 0;

    app_gpio_configure_output(MUX_en_pin, MUX_en_pin_lv );

    uint8_t Sensor_en_pin = 44;
    uint8_t Sensor_en_pin_lv = 1;

    app_gpio_configure_output(Sensor_en_pin, Sensor_en_pin_lv );
}
