...
/******************************************************************************/
#if defined (AIR_BATTERY_MANAGEMENT_ENABLE)
int app_battery__bt_bas_set_battery_level(uint8_t level);
...
bool app_battery__check_vbat_change(uint32_t bm_vbat);
bool app_battery__check_soc_change(uint8_t bm_soc);
bool app_battery__check_chg_state_change(uint8_t bm_chg_state);
bool app_battery__check_chg_en_sta_change(bool bm_chg_en_sta);
bool app_battery__check_chg_exist_change(bool bm_chg_exist);

bool app_battery__check_low_battery();
void app_battery_update_var();
void app_battery_check_info();
...
/* variables                                                                  */
/******************************************************************************/
bool app_bat_low_battery_warn = false;
#endif /* _AIR_BATTERY_MANAGEMENT_ENABLE_ */

bool app_bat_init = true;
T_APP_BATTERY_INFO_S app_bat; //store APP battery var
...
static bool app_battery__evt_race_cmd_req(const struct af_evt_header *evt_header)
{
    ...
    switch(event->cmd)
    {
        ...
        {
            app_battery_update_var();
            struct evt_race_cmd_rsp* evt = create_evt_race_cmd_rsp();
            if (evt)
            {
                evt->cmd = RACE_CMD_GET_BATTERY;
                evt->BATTERY.charging = app_bat.chg_en_sta;
                evt->BATTERY.battery = app_bat.soc;
                AF_EVT_SUBMIT(evt);
                APP_LOGI(thisMOD,"RACE_CMD_GET_BATTERY, evt->charging = %d, evt->battery = %d", evt->BATTERY.charging, evt->BATTERY.battery);
            }
        }
        break;
...
static bool app_battery__evt_app_bat_timer(const struct af_evt_header *evt_header)
{
    ...
    switch(event->cmd)
    {
        ...
        case APP_BAT_SOC:  //1
        {
            app_battery__check_soc_change(bm_get_info(BM_SOC));
        ...
        {
            app_battery__check_chg_en_sta_change(bm_get_info(BM_CHG_EN_STA));
            break;
        }
...
bool app_battery__check_vbat_change(uint32_t bm_vbat)
{
    if((app_bat.vbat != bm_vbat) || app_bat_init)
    {
...
bool app_battery__check_soc_change(uint8_t bm_soc)
{
    if((app_bat.soc != bm_soc) || app_bat_init)
    {
        if(app_bat.chg_en_sta)
        {
...
void app_battery_test_soc()
{
    ...
    if (soc > 0)
    {
        APP_LOGI(thisMOD,"app_battery_test_soc, would be set to %d to %d!!!", soc, soc-2);
        soc -= 2;
        app_bat.soc = soc;
        app_battery__check_low_battery();
        app_battery_bat_event_announcement(BAT_EVT_PERCENT_CHANGE, app_bat.soc);
