...
typedef enum {
    ...
    EXTERNAL_FLASH_STATUS_NOT_INIT = -5,           /**< flash driver is not initialized. */
...
/** @brief external flash definition */
typedef struct {
    hal_spi_master_port_t spi_port;     /**< SPI port of this external flash device instance used. */
    uint8_t spi_freq_in_MHz;            /**< Frequency of this external flash device. */
    uint32_t logic_addr_cfg;            /**< Logic address of this external flash device. */
} bsp_external_flash_cfg_t;


#if defined(AIR_BSP_EXTERNAL_SERIAL_FLASH_ASYNC_ENABLE)
/** @brief flash operation definition */
typedef enum {
    FLASH_ASYNC_OPERATION_SPECIAL = 0x5A5A5A5A,     /**< special definition of flash async operation */
    FLASH_ASYNC_OPERATION_ERASE,                    /**< definition of flash async erase operation */
    FLASH_ASYNC_OPERATION_WRITE,                    /**< definition of flash async write operation */
    FLASH_ASYNC_OPERATION_READ,                     /**< definition of flash async read operation */
    FLASH_ASYNC_OPERATION_MAX,                      /**< definition of flash async MAX operation */
} bsp_external_flash_async_operation_t;


/**
 * @brief BSP external flash callback for asynchronous operation requests
 *
 * @param type flash asynchronous operation type from @ref bsp_external_flash_async_operation_t
 * @param address address of flash asynchronous operation
 * @param length length of flash asynchronous operation
 * @param op_status status of flash asynchronous operation
 * @param user_data user data of flash asynchronous operation callback
 */
typedef void (*bsp_external_flash_callback_t)(
    bsp_external_flash_async_operation_t type,
    uint32_t address,
    uint32_t length,
    bsp_external_flash_status_t op_status,
    void *user_data
);
...
/**
 * @brief This function does the initialization for the serial flash and SPI Master by using bsp_external_flash_cfg variable whose type is array of bsp_external_flash_cfg_t.
...
#if defined(AIR_BSP_EXTERNAL_SERIAL_FLASH_ASYNC_ENABLE)
/**
 * @brief This asynchronous function reads data from the serial flash.
 * @param[in] address:  read address
 * @param[in] buffer: buffer for read data(MUST live until callback completes)
 * @param[in] length: read data length
 * @param[in] flash_async_cb: callback of flash async operation
 * @param[in] user_data: user data to callback
 * @return    #EXTERNAL_FLASH_STATUS_BUSY means the device is busy.\n
 *            #EXTERNAL_FLASH_ERROR_NOT_FOUND_DEVICE means it did not find the device. \n
 *            #EXTERNAL_FLASH_WRONG_PARAMETER means an invalid parameter is given by the user.\n
 *            #EXTERNAL_FLASH_STATUS_NOT_INIT means the driver is not initialized.\n
 ...
 *            #EXTERNAL_FLASH_STATUS_OK means this function returns successfully.\n
 */
bsp_external_flash_status_t bsp_external_flash_read_async(uint32_t address, uint8_t *buffer, uint32_t length, bsp_external_flash_callback_t flash_async_cb, void *user_data);

/**
 * @brief This asynchronous function writes data to the serial flash.
 * @param[in] address:  write address
 * @param[in] data: write data(MUST live until callback completes)
 * @param[in] length: write data length
 * @param[in] flash_async_cb: callback of flash async operation
 * @param[in] user_data: user data to callback
 * @return    #EXTERNAL_FLASH_STATUS_BUSY means the device is busy.\n
 ...
 *            #EXTERNAL_FLASH_STATUS_OK means this function returns successfully.\n
 */
bsp_external_flash_status_t bsp_external_flash_write_async(uint32_t address, uint8_t *data, int32_t length, bsp_external_flash_callback_t flash_async_cb, void *user_data);


/**
 * @brief This asynchronous function erases the serial flash.
 * @param[in] address: erase start address
 * @param[in] block_size: erase block size
 * @param[in] flash_async_cb: callback of flash async operation
 * @param[in] user_data: user data to callback
 ...
 *            #EXTERNAL_FLASH_STATUS_OK means this function returns successfully.\n
 */
bsp_external_flash_status_t bsp_external_flash_erase_async(uint32_t address, block_size_type_t block_size, bsp_external_flash_callback_t flash_async_cb, void *user_data);
#endif /* defined(AIR_BSP_EXTERNAL_SERIAL_FLASH_ASYNC_ENABLE) */
