...
typedef enum {
    ...
    SYSTEM_ONLINE_DUMP_CLOCK = 1,
    SYSTEM_ONLINE_DUMP_MCPS  = 2,
    SYSTEM_ONLINE_DUMP_PROFILE  = 3,
...
// ################# HCLA task profilling struct info #################
typedef struct {
    uint8_t task_id;     // [AT Profilling] 1 ~ config_MAX_NUM_OF_TASK
    uint16_t percentage; // [AT Profilling] 0.0% ~ 100.0%
    uint16_t minstack;   // [AT Profilling] 0 ~ 64K
} __attribute__((packed)) hcla_task_info_t;

typedef struct {
    uint32_t timestamp;
    uint8_t task_number;
    hcla_task_info_t task_info[32];
} __attribute__((packed)) hcla_profile_task_info_t;
