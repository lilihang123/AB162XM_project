...
static struct oscore_init_params get_default_params(enum reverse_t is_reversed,
						    enum freshness_t is_fresh)
{
...
void t10_oscore_client_server_after_reboot(void)
{
	...
	const uint8_t OSCORE_REQ1[] = { 0x41, 0x02, 0x00, 0x00, 0x4A, 0x92,
					0x09, 0x14, 0xFF, 0x61, 0x27, 0x10,
					0x81, 0xAD, 0xF0, 0x0E, 0xEA, 0x55,
					0xF1, 0x52, 0x39, 0x6C, 0xA3, 0x72,
					0x46, 0x96, 0x73, 0xB2, 0x09, 0xF7 };

	const uint8_t OSCORE_RESP1[] = { 0x61, 0x44, 0x00, 0x00, 0x4A, 0x93,
					 0x09, 0x14, 0x01, 0xFF, 0xBC, 0xB9,
					 0x3C, 0xA6, 0x0E, 0xF7, 0x11, 0x52,
					 0x39, 0x45, 0x1B, 0xEA, 0x4C, 0x81,
					 0x90, 0xCA, 0x60, 0x38, 0x76, 0xD6,
					 0x09, 0xDE, 0x46 };

	const uint8_t OSCORE_RESP2[] = { 0x61, 0x44, 0x00, 0x00, 0x4A, 0x93,
					 0x09, 0x15, 0x01, 0xFF, 0x97, 0xA4,
					 0x25, 0x7B, 0x17, 0x39, 0xC0, 0x1D,
					 0x5E, 0x9D, 0xDA, 0x02, 0x74, 0xDB,
					 0xD4, 0xBF, 0xE6, 0x82, 0x18, 0x9B,
					 0x3B, 0xBF, 0x0E };

	const uint8_t OSCORE_REQ3[] = {
		0x41, 0x02, 0x00, 0x00, 0x4B, 0x92, 0x09, 0x16, 0xFF,
		0x8C, 0x2F, 0xED, 0xB3, 0xBB, 0xCD, 0xEE, 0x17, 0xB0,
		0x12, 0x03, 0x95, 0x74, 0xD8, 0x08, 0x55, 0x40, 0x53,
		0x82, 0x2B, 0x3C, 0x79, 0xA5, 0x1B, 0xCA, 0xFC, 0xC9,
		0xD0, 0xC6, 0x35, 0x50, 0xDC, 0x5D, 0x1D, 0x13
	};

	const uint8_t OSCORE_RESP3[] = { 0x61, 0x44, 0x00, 0x00, 0x4B, 0x93,
					 0x09, 0x16, 0x01, 0xFF, 0x9B, 0xE4,
					 0x04, 0x05, 0x9F, 0x1C, 0xCE, 0x6A,
					 0x43, 0x85, 0xD2, 0xD7, 0x12, 0x52,
					 0x49, 0xB2, 0x8A, 0xD7, 0xD5, 0x0A,
					 0x67, 0x09, 0x82 };

	const uint8_t OSCORE_REQ4[] = {
		0x41, 0x02, 0x00, 0x00, 0x4B, 0x92, 0x09, 0x17, 0xFF,
		0xCD, 0x4A, 0x87, 0x1E, 0xCD, 0xE0, 0x07, 0xD2, 0xCB,
		0xF5, 0xC4, 0x76, 0x84, 0x1E, 0x4C, 0x94, 0x39, 0xCE,
		0x82, 0x6F, 0x1A, 0xF6, 0xB4, 0xC1, 0x68, 0xE6, 0xB3,
		0xBB, 0xDB, 0x8B, 0x84, 0x4F, 0xDE, 0x95, 0x94
	};

	const uint8_t OSCORE_RESP4[] = { 0x61, 0x44, 0x00, 0x00, 0x4B, 0x90,
					 0xFF, 0x5E, 0x04, 0x3E, 0xD6, 0x11,
					 0x1F, 0xE7, 0xF7, 0x9D, 0x1E, 0x5F,
					 0x30, 0x27, 0x30 };
...
void t11_oscore_ssn_overflow_protection(void)
{
	...
	zassert_equal(result, oscore_ssn_overflow,
		      "SSN overflow not detected in coap2oscore");

	/* test SSN overflow protection in oscore2coap */
	result = oscore2coap((uint8_t *)T1__OSCORE_RESP, T1__OSCORE_RESP_LEN,
			     (uint8_t *)&buf_coap, &buf_coap_len,
			     &security_context);
	zassert_equal(result, oscore_ssn_overflow,
		      "SSN overflow not detected in oscore2coap");
}
