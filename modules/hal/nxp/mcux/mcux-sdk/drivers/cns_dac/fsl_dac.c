...
#define FSL_COMPONENT_ID "platform.drivers.cns_dac"
#endif
...
{
    /* Reset each channel. */
    DAC_ResetChannel(base, (uint32_t)kDAC_ChannelA | (uint32_t)kDAC_ChannelB);
...
void DAC_SetChannelConfig(DAC_Type *base, uint32_t channelMask, const dac_channel_config_t *channelConfig)
{
    ...
    if ((channelMask & (uint32_t)kDAC_ChannelA) == (uint32_t)kDAC_ChannelA)
    {
    ...
    if ((channelMask & (uint32_t)kDAC_ChannelB) == (uint32_t)kDAC_ChannelB)
    {
