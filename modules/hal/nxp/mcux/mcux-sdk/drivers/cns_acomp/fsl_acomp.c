...
#define FSL_COMPONENT_ID "platform.drivers.cns_acomp"
#endif
...
void ACOMP_Init(ACOMP_Type *base, const acomp_config_t *config)
{
    ...
    tmp32 |= ACOMP_CTRL0_GPIOINV((((uint32_t)(config->outPinMode)) & 0x2UL) >> 1UL);
    ...
    tmp32 |= ACOMP_ROUTE0_OUTSEL((uint32_t)(config->outPinMode) & 0x1UL) |
             ACOMP_ROUTE0_PE((~((uint32_t)(config->outPinMode) >> 2UL)) & 0x1UL);
...
void ACOMP_GetDefaultConfig(acomp_config_t *config)
{
    ...
    (void)memset(config, 0, sizeof(acomp_config_t));
                          ...
                          const acomp_negative_input_config_t *negInput)
{
    ...
    {
        tmp32 |= ACOMP_CTRL0_LEVEL_SEL(((uint32_t)negInput->channel & 0x3UL) << 4UL);
    }
