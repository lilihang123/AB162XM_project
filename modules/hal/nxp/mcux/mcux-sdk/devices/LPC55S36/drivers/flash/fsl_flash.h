...
/*!
 * @brief Non-blocking Erases the flash sectors encompassed by parameters passed into function.
 *
 * This is a non-blocking function, which returns right away.
 * This function erases the appropriate number of flash sectors based on the desired start
 * address and length, and get the command execute status from the "FLASH_GetCommandState".
 *
 * @param config The pointer to the storage for the driver runtime state.
 * @param start The start address of the desired flash memory to be erased.
 *              NOTE: The start address need be 4 Bytes-aligned.
 *
 * @param lengthInBytes The length, given in bytes need be 4 Bytes-aligned.
 *
 * @param key The value used to validate all flash erase APIs.
 ...
 * @retval #kStatus_FLASH_Success API was executed successfully.
 * @retval #kStatus_FLASH_InvalidArgument An invalid argument is provided.
 * @retval #kStatus_FLASH_ModifyProtectedAreaDisallowed Flash hiding read is not allowed.
 */
 ...
 * @param config A pointer to the storage for the driver runtime state.
 *
 * @retval #kStatus_FLASH_CommandOperationInProgress Indicate that a flash command in progress.
 * @retval #kStatus_FLASH_CommandFailure Run-time error during the command execution.
 * @retval #kStatus_FLASH_CommandNotSupported Flash API is not supported.
 * @retval #kStatus_FLASH_EccError A correctable or uncorrectable error during command execution.
 * @retval #kStatus_FLASH_RegulationLoss A loss of regulation during read.
