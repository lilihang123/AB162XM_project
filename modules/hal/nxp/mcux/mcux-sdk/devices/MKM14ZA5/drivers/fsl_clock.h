 ...
 *
 * The XTAL1/EXTAL1 (OSC1) clock frequency in Hz. When the clock is set up, use the
 * function CLOCK_SetXtal1Freq to set the value in the clock driver. For example,
 * if XTAL1 is 8 MHz:
 * @code
 * CLOCK_InitOsc1(...);
 * CLOCK_SetXtal1Freq(80000000);
 * @endcode
 *
 * This is important for the multicore platforms where only one core needs to set up the
 * OSC1 using the CLOCK_InitOsc1. All other cores need to call the CLOCK_SetXtal1Freq
 * to get a valid clock frequency.
 */
extern volatile uint32_t g_xtal1Freq;
...
#define SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY (50000000UL)
#endif
...
typedef enum _clock_ip_name
{
    ...
    kCLOCK_Mcg       = CLK_GATE_DEFINE(0x1034U, 4U),
    kCLOCK_Osc       = CLK_GATE_DEFINE(0x1034U, 6U),
    ...
    kCLOCK_Wdog   = CLK_GATE_DEFINE(0x1038U, 19U),
    ...
    kCLOCK_Dmamux1 = CLK_GATE_DEFINE(0x103CU, 2U),
    kCLOCK_Dmamux2 = CLK_GATE_DEFINE(0x103CU, 3U),
    kCLOCK_Dmamux3 = CLK_GATE_DEFINE(0x103CU, 4U),
    ...
    kCLOCK_SimLp   = CLK_GATE_DEFINE(0x103CU, 30U),
    kCLOCK_SimHp   = CLK_GATE_DEFINE(0x103CU, 31U),
...
{
    SIM->CTRL_REG = ((SIM->CTRL_REG & ~SIM_CTRL_REG_CLKOUTSEL_MASK) | SIM_CTRL_REG_CLKOUTSEL(src));
}
...
{
    SIM->CTRL_REG = ((SIM->CTRL_REG & ~SIM_CTRL_REG_SAR_TRG_CLK_SEL_MASK) | SIM_CTRL_REG_SAR_TRG_CLK_SEL(src));
}
 ...
 *
 * This function disables the PLL1 in FLL mode. It should be used together with the
...
#if (defined(FSL_FEATURE_SOC_OSC_COUNT) && (FSL_FEATURE_SOC_OSC_COUNT > 1))
/*!
 * @brief Sets the XTAL1 frequency based on board settings.
 *
 * @param freq The XTAL1/EXTAL1 input clock frequency in Hz.
 */
static inline void CLOCK_SetXtal1Freq(uint32_t freq)
{
    g_xtal1Freq = freq;
}
#endif /* (FSL_FEATURE_SOC_OSC_COUNT > 1) */
