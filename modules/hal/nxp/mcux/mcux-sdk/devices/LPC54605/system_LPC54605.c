 ...
 * @version 1.2
 * @date 2017-06-08
 * @brief Device specific configuration file for LPC54605 (implementation file)
 *
...
#define PLL_MDEC_VAL_P (0U)                                       /* MDEC is in bits  16:0 */
...
static uint32_t getWdtOscFreq(void)
{
    ...
    if ((SYSCON->PDRUNCFG[0] & SYSCON_PDRUNCFG_PDEN_WDT_OSC_MASK) == SYSCON_PDRUNCFG_PDEN_WDT_OSC_MASK)
    {
        return 0U;
    }
    else
