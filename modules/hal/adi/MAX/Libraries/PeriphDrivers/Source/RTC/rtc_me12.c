...
int MXC_RTC_TrimCrystal(mxc_tmr_regs_t *tmr)
{
    if (MXC_TMR_GET_IDX(tmr) < 0 ||
        MXC_TMR_GET_IDX(tmr) > 2) { // Timer must support ERFO as clock source
        return E_BAD_PARAM;
    }

    if (MXC_SYS_ClockSourceEnable(MXC_SYS_CLOCK_ERFO) != E_NO_ERROR) {
        return E_TIME_OUT;
    }
    ...
    tmr_cfg.pol = 0;
    MXC_TMR_Init(tmr, &tmr_cfg, false, MAP_A);
