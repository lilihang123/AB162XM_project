...
//******************************************************************************
int MXC_FLC_ME12_GetByAddress(mxc_flc_regs_t **flc, uint32_t addr)
{
...
//******************************************************************************
int MXC_FLC_ME12_GetPhysicalAddress(uint32_t addr, uint32_t *result)
{
    if ((addr >= MXC_FLASH_MEM_BASE) && (addr < (MXC_FLASH_MEM_BASE + MXC_FLASH_MEM_SIZE))) {
...
int MXC_FLC_PageErase(uint32_t address)
{
    ...
    // Get FLC Instance
    if ((err = MXC_FLC_ME12_GetByAddress(&flc, address)) != E_NO_ERROR) {
        return err;
    }

    if ((err = MXC_FLC_ME12_GetPhysicalAddress(address, &addr)) < E_NO_ERROR) {
        return err;
    }
...
int MXC_FLC_Write32(uint32_t address, uint32_t data)
{
    ...
    if ((err = MXC_FLC_ME12_GetPhysicalAddress(aligned, &addr)) < E_NO_ERROR) {
        return err;
    }
...
{
    /* User accessible area in MAX32662 infoblock does not need to be unlocked. */
