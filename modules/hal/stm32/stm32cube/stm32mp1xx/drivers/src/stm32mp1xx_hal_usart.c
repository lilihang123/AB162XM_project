...
HAL_StatusTypeDef HAL_USART_Transmit_DMA(USART_HandleTypeDef *husart, const uint8_t *pTxData, uint16_t Size)
{
  ...
  if (husart->State == HAL_USART_STATE_READY)
  {
    ...
    if (husart->hmdmatx != NULL)
    {
      ...
      tmp = (const uint32_t *)&pTxData;
      status = HAL_MDMA_Start_IT(husart->hmdmatx, *(const uint32_t *)tmp, (uint32_t)&husart->Instance->TDR, Size, 1);
    }
#endif
  ...
  if (HAL_IS_BIT_SET(husart->Instance->CR3, USART_CR3_DMAT))
  {
    ...
    if (husart->hmdmatx != NULL)
    {
      /* USART Tx MDMA Abort callback has already been initialised :
         will lead to call HAL_USART_AbortCpltCallback() at end of MDMA abort procedure */
    ...
    if (husart->ErrorCode != HAL_USART_ERROR_NONE)
    {
          ...
          (errorcode != 0U))
      {
        ...
        if (HAL_IS_BIT_SET(husart->Instance->CR3, USART_CR3_DMAR))
        {
          ...
          if (husart->hmdmatx != NULL)
          {
            /* Set the USART Tx MDMA Abort callback to NULL : no callback
               executed at end of MDMA abort procedure */
            husart->hmdmatx->XferAbortCallback = NULL;
          ...
          else if (husart->hmdmarx != NULL)
          {
            /* Set the USART Rx MDMA Abort callback :
               will lead to call HAL_USART_ErrorCallback() at end of MDMA abort procedure */
            husart->hmdmarx->XferAbortCallback = USART_MDMAAbortOnError;

            /* Abort MDMA RX */
            if (HAL_MDMA_Abort_IT(husart->hmdmarx) != HAL_OK)
            {
              /* Call Directly husart->hmdmarx->XferAbortCallback function in case of error */
  ...
  *  force OVER8 to 1 to allow to reach the maximum speed (Fclock/8) */
  ...
  switch (clocksource)
  {
    ...
    case USART_CLOCKSOURCE_PLL4Q:
      ...
      usartdiv = (uint32_t)(USART_DIV_SAMPLING8(pll4_clocks.PLL4_Q_Frequency, husart->Init.BaudRate, husart->Init.ClockPrescaler));
      break;
