 ...
 * |        |          |If user wonu2019t use the bus any more, clearing this bit will drop VBUS to save power
 ...
 * |        |          |When the USB frame acts as A-device, set this bit when A-device allows to process HNP protocolu2014A-device changes role from Host to Peripheral
 * |        |          |This bit will be cleared when OTG state changes from a_suspend to a_peripheral or goes back to a_idle state
 * |        |          |When the USB frame acts as B-device, set this bit after the OTG A-device successfully sends a SetFeature (b_hnp_enable) command to the OTG B-device to start role change u2014 B-device changes role from Peripheral to Host
 ...
 * |        |          |If device is configured as neither OTG device nor ID dependent, this bit is u201Cdonu2019t careu201D.
 * |        |          |0 = OTG PHY Disabled.
 * |        |          |1 = OTG PHY Enabled.
