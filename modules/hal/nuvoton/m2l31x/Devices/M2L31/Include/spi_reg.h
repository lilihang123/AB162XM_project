 ...
 * |        |          |If MCLKDIV >= 1, f_MCLK = f_i2s_clk_src/(2*MCLKDIV)
 * |        |          |If MCLKDIV = 0, f_MCLK = f_i2s_clk_src
 * |        |          |where
 * |        |          |f_i2s_clk_src is the frequency of I2S peripheral clock source, which is defined in the clock control register, CLK_CLKSEL2 or CLK_CLKSEL3
 ...
 * |        |          |    f_BCLK = f_i2s_clk_src/[2*(BCLKDIV+1)]
 * |        |          |where
 * |        |          |f_i2s_clk_src is the frequency of I2S peripheral clock source, which is defined in the clock control register, CLK_CLKSEL2 or CLK_CLKSEL3.
 * |        |          |In I2S Slave mode, this field is used to define the frequency of peripheral clock and it is determined by
 * |        |          |    f_i2s_clk_src/[(BCLKDIV/2)+1]
 ...
 */
    ...
    __IO uint32_t RX;                    /*!< [0x0030] SPI Data Receive Register                                        */
