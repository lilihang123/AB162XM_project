 ...
 * |        |          |The pulse counter may be initialized to a predetermined value when one of the following events occur:
 ...
 * |        |          |If the EQEI unit timer is enabled (EQEI_CTL2[8]) =1, and the unit timer counter value also matches UTCMP(EQEI_UTCMP[31:0]), then UTIEF (EQEI_STATUS[10]) will be set
 * |        |          |This register is software writable.
 * @var EQEI_T::STATUS
