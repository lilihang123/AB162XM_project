...
/* Modifications:
2025-11-21 by Airoha - Airoha performance optimization.
*/
                             ...
                             size_t input_length)
{
    ...
    if (X != NULL) {
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
        air_mbedtls_word_memset(X, 0, X_limbs * ciL);
#else
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
        air_mbedtls_word_memset(output + stored_bytes, 0, output_length - stored_bytes);
#else
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
        air_mbedtls_word_memset(output, 0, output_length - stored_bytes);
#else
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
        air_mbedtls_word_memset(X, 0, limbs * ciL);
#else
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
// b[AH, AL] * s[BH, BL] = [r5, 32'h0] + [r6, 16'h0] + [r7, 16'h0] + r4 = [r5, r4]  
#define MULADDC_CM0P \
        __asm(       \
            ".syntax unified                                                   \n"  \
            "PUSH   {r3-r7}                                                    \n"  \
       ...
       "L_mula_loop:                                                           \n"  \
            "PUSH   {r3, r4}                                                   \n"  \
            "LSRS   r5, r3, #16         /* r5 = b >> 16                     */ \n"  \
            "UXTH   r3, r3              /* r3 = b & 0xffff                  */ \n"  \
            "LDM    r0!, {r6}           /* r6 = *s++                        */ \n"  \
            "LSRS   r7, r6, #16         /* r7 = *s >> 16                    */ \n"  \
            "UXTH   r6, r6              /* r6 = *s & 0xffff                 */ \n"  \
            "MOV    r4, r3              /* r4 = b & 0xffff                  */ \n"  \
            "MULS   r4, r4, r6          /* r4 = AL * BL                     */ \n"  \
            "MULS   r6, r6, r5          /* r6 = AH * BL                     */ \n"  \
            "MULS   r5, r5, r7          /* r5 = AH * BH                     */ \n"  \
            "MULS   r7, r3, r7          /* r7 = AL * BH                     */ \n"  \
            "LSRS   r3, r6, #16         /* r3 = r6 >> 16                    */ \n"  \
            "ADD    r5, r5, r3          /* r5 = r5 + (r6 >> 16)             */ \n"  \
            "LSRS   r3, r7, #16         /* r3 = r7 >> 16                    */ \n"  \
            "ADDS   r5, r5, r3          /* r5 = r5 + (r6 >> 16) + (r7 >> 16)*/ \n"  \
            "ADCS   r4, r4, r2          /* r4 = r4 + r2(c)                  */ \n"  \
            "MOVS   r2, #0              /* update N,Z                       */ \n"  \
            "ADCS   r5, r5, r2          /*                                  */ \n"  \
            "LSLS   r3, r6, #16         /* r3 = r6 << 16                    */ \n"  \
            "ADDS   r4, r4, r3          /*                                  */ \n"  \
            "ADCS   r5, r5, r2          /*                                  */ \n"  \
            "LSLS   r3, r7, #16         /* r3 = r7 << 16                    */ \n"  \
            "ADDS   r4, r4, r3          /*                                  */ \n"  \
            "ADCS   r5, r5, r2          /*                                  */ \n"  \
            "LDR    r3, [r1]            /* r3 = *d                          */ \n"  \
            "ADDS   r3, r3, r4          /*                                  */ \n"  \
            "ADCS   r2, r2, r5          /* [r5, r4] + r3 -> [r2, r3]        */ \n"  \
            "STM    r1!, {r3}           /* *(d++) = r3                      */ \n"  \
            "POP    {r3, r4}                                                   \n"  \
            "SUBS   r4, r4, #1                                                 \n"  \
            "BNE    L_mula_loop                                                \n"  \
            ...
            "MOV    %2, r0              /* s = r0                           */ \n"  \
          : "=r" (c),  "=r" (d), "=r" (s)                                           \
          : "r" (s), "r" (d), "r" (c), "r" (b), "r" (s_len)                         \
          : "r0", "r1", "r2", "cc"                                                  \
            );         
#endif

#ifdef AIR_MBEDTLS_CODE_OPTIMIZE   
static inline mbedtls_mpi_uint inline_mbedtls_mpi_core_mla(mbedtls_mpi_uint* d, size_t d_len,
...
{
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
    air_mbedtls_word_memset(X, 0, (A_limbs + B_limbs) * ciL);

    for (size_t i = 0; i < B_limbs; i++) {
        (void)inline_mbedtls_mpi_core_mla(X + i, A_limbs + 1, A, A_limbs, B[i]);
    }
#else
...
{
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
    air_mbedtls_word_memset(T, 0, (2 * AN_limbs + 1) * ciL);
#else
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
    air_mbedtls_word_memset(X, 0, overhead);
    air_mbedtls_word_memset((unsigned char *) X + limbs * ciL, 0, (X_limbs - limbs) * ciL);
#else
    memset(X, 0, overhead);
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
    air_mbedtls_word_memset(Wtable, 0, AN_limbs * ciL);
#else
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
    air_mbedtls_word_memcpy(W1, A, AN_limbs * ciL);
#else
...
#ifdef AIR_MBEDTLS_CODE_OPTIMIZE
    air_mbedtls_word_memcpy(X, Wtable, AN_limbs * ciL);
#else
